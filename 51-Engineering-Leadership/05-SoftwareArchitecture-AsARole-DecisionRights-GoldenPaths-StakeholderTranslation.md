...
**Establish current state from telemetry before producing any target state.** *Why:* a target drawn from an unmeasured baseline is a picture, not a plan, and it will change nothing regardless of quality. *When NOT to:* a genuinely greenfield system — there is no current state to establish.

**Apply the externality test to every constraint.** *Why:* constraints that prevent variance imposing no external cost spend legitimacy for nothing, leaving none for the ones that matter. *When NOT to:* where the operability argument applies instead — the firm genuinely cannot afford three brokers even if each choice is locally fine. Say that is the argument rather than dressing it as technical merit.

**Prefer instruments low on the compliance-cost axis: templates, libraries, CI checks.** *Why:* durability is inversely related to compliance cost; anything expensive to honor is honored nominally. *When NOT to:* for constraints requiring genuine judgment, which cannot be templated — there, review is correct, and the scope should be explicitly small.

**Facilitate where all affected parties are present; decide where they are not.** *Why:* the architect is the only participant representing absent parties — other domains, future consumers, the audit position — and representing them is a decision-making act. *When NOT to:* never abdicate on absent-party interests; but be explicit in the room about which mode you are in.

**Derive diagrams; do not draw them.** *Why:* a derived diagram is always current and the effort is one-time; a drawn one is current for a week and is trusted for years. *When NOT to:* for a *proposed* future state, which necessarily must be drawn — but label it unambiguously as proposal, not description.

**Diff declared against observed continuously, and require an explicit disposition for every delta.** *Why:* silence lets drift accumulate invisibly; a delta with three legitimate dispositions (fix the system, update the declaration, accept with expiry) makes drift a managed queue. *When NOT to:* never — but tune the noise floor, or the report gets ignored.

**Write the one-page paper.** *Why:* banks run on papers; an architect who cannot produce one a non-technical committee can act on is limited to influencing engineers. *When NOT to:* when the audience is engineers, who want the detail and read the summary as evasion.

**Co-author every reference architecture with a practitioner from an affected team.** *Why:* it is the practical defence against designs that are correct in principle and unimplementable in practice, and it converts the most likely critic into an author. *When NOT to:* never, in a function with any distance from delivery.

**Retire standards actively.** *Why:* an accumulating rule set becomes a body of rules whose rationale nobody can state, satisfied nominally, and defended on process grounds. *When NOT to:* controls with regulatory backing — those get narrowed or made cheaper, not retired.

---

## 6. Anti-patterns

**The target state with no baseline.** *Looks like:* a beautiful future-state architecture with no accurate description of what exists now. *Why it fails:* the path cannot be planned from an unknown origin, so the artifact changes nothing and its authors conclude they need better communication. *Fix:* derive current state from telemetry first; treat it as the function's primary product.

**The principles document.** *Looks like:* 20–50 numbered principles, mostly unobjectionable, none testable. *Why it fails:* unobjectionable and untestable means unusable — engineers cannot tell whether they have complied, so they do not try. *Fix:* keep only what passes the externality test, restate as testable constraints, and enforce mechanically.

**The review board as the only control.** *Looks like:* designs approved at a gate, with no check on what was built. *Why it fails:* point-in-time approval verifies intent, never reality; drift is total within two years and invisible. *Fix:* move enforcement into the pipeline, narrow the board to an enumerated set.

**The ivory tower.** *Looks like:* architects who have not written production code or been on-call in this decade, producing designs practitioners can see are unimplementable. *Why it fails:* one such design destroys the function's credibility on everything else. *Fix:* depth maintenance and mandatory practitioner co-authorship.

**Constraining implementation.** *Looks like:* standards on project structure, frameworks, testing libraries. *Why it fails:* spends all available legitimacy on decisions with no external cost, leaving none for interfaces and controls. *Fix:* externality test, ruthlessly, including retirement of existing standards that fail it.

**Architecture as documentation production.** *Looks like:* a function whose KPIs are designs reviewed and standards published. *Why it fails:* it optimizes for artifact volume and is correctly ignored, because artifacts are not the point. *Fix:* measure conformance distribution, drift rate, and template-derived compliance instead.

**The diagram that is wrong and still trusted.** *Looks like:* a wiki architecture page used for migration planning, three years stale. *Why it fails:* worse than no diagram, because decisions are being made on it with confidence. *Fix:* derive, or mark the drawn one with a last-verified date and expire it.

**Approval without capability.** *Looks like:* an architecture function that can mandate but cannot build templates or CI checks. *Why it fails:* mandates without cheap compliance produce nominal conformance. *Fix:* organizational — merge with platform, or give the function delivery capacity.

**Inflating preference into risk.** *Looks like:* framing a wanted re-architecture as a resilience or compliance concern it is not. *Why it fails:* business stakeholders calibrate over time and one detected instance discounts everything after. *Fix:* say plainly when something is optionality rather than risk — that is a legitimate argument.

---

## 7. Performance Engineering

The quantities an architecture function should be measured on, none of which are artifact counts.

**Conformance distribution per constraint, over time.** Not the mean — the shape. A constraint at 94% with the remaining 6% concentrated in one legacy division tells a completely different story from 94% scattered randomly, and only the first has an obvious remediation.

**Drift rate.** Deltas per week between declared and observed architecture, and their disposition mix. A healthy steady state has a low, stable rate dominated by "declaration lagging intentional change." A rising rate dominated by "system is wrong" means constraints are being violated faster than they are being enforced.

**Template-derived compliance.** The proportion of new services that were compliant *by construction* rather than by remediation. This is the single best measure of whether the function's instruments are working, because it measures the mechanism rather than the outcome.

**Review latency and review volume.** Both should be falling. A function whose review volume is growing is expanding its scope, which demonstrates is the wrong direction — the queue's length is the architecture function's most visible property to engineering, and it is nearly always the property that determines its reputation.

**Constraint half-life.** How long a constraint remains adopted after enforcement is removed. Constraints embedded in templates and libraries survive; constraints held up by review decay within months. Measuring this occasionally — by sampling services created before and after a check was introduced — tells you which instruments are actually working in your organization rather than in principle.

**The one to be careful with: coupling metrics.** Fan-in, cyclomatic dependency, SCC count are genuine architectural health measures and they are also easily gamed and easily over-interpreted. A rising coupling metric is a prompt to investigate, never a finding on its own — the same discipline this course applies to any single-number performance metric.

**Baseline before intervening.** Before claiming an architecture intervention improved anything, capture the measure. the numbers were persuasive precisely because the before-state had been measured, and the function's previous 18 months of work had produced no such evidence for anything.

---

## 8. Security

**The architect owns security *properties* of the composed system**, which is distinct from any component's security and is not the sum of them.

**Trust boundaries are architectural artifacts and are frequently undocumented.** Where does authentication actually happen, and where is identity merely asserted by an upstream caller? In a multi-hop call chain, is the end-user's identity propagated, or is it silently replaced by a service identity at hop two — after which every downstream authorization decision is made against the wrong principal? This is the confused-deputy shape, it is extremely common in bank estates that grew by integration, and nobody owns the question because each team's boundary is individually correct. Documenting and verifying trust boundaries is one of the clearest, highest-value things this role does.

**Data classification propagates through the architecture and almost never does in practice.** If a system holds PII or cardholder data, everything it flows to inherits that classification and its controls. In practice classification is applied at the system of record and lost at the first hop, so a downstream analytics store holds regulated data with none of the corresponding controls — and discovers it during an audit. **Deriving data classification propagation from the actual data-flow graph** (the same graph built for dependencies) turns this from an assertion into a measurement, and it is one of the strongest arguments for the derived-current-state investment.

**Segmentation as a designed property.** Network and identity segmentation determines blast radius, and blast radius is architectural. The question the architect must be able to answer — and most cannot — is: if this service is fully compromised, what is reachable? Answering it requires the dependency graph plus the network policy plus the identity grants, which is exactly the composite view that only this role assembles.

**The security review's structural weakness, and the fix.** Point-in-time security review of a design has the same defect as point-in-time architecture review: it verifies intent, and the built system drifts. The fix is identical — encode what is checkable (workload identity present, no long-lived credentials, TLS enforced, data classification tags propagated) as continuous checks, and reserve human review for the judgment-requiring residue such as threat modelling a genuinely novel flow.

**And the honest limit:** the most consequential security properties — is this threat model complete, is this trust assumption sound — are not mechanically checkable. An architecture function that automates everything automatable and then *stops thinking* has replaced one failure mode with another. The checks handle drift; they do not handle novelty.

---

## 9. Scalability

**How the role changes with estate size:**

| Estate | Architect's shape | Binding constraint |
|---|---|---|
| **<20 systems** | No dedicated architect; a Staff engineer does it | Nothing — the estate fits in one head |
| **20–100** | 1–2 architects; personal knowledge still viable but degrading | The transition from knowing to measuring |
| **100–500** | Domain architects + an enterprise view; derived current state is mandatory | Keeping the derived model accurate as ingestion sources multiply |
| **500+** | Formal function, federated domain ownership, EA sets only cross-domain contracts | Coordination between domain architects; risk of conflicting domain target states |

**The transition that breaks the function is the second one**, and it is the same transition identified for Staff engineers: personal familiarity stops scaling, and the failure is continuing to rely on it while believing it still works. For an architecture function the symptom is specific and recognizable — architecture documents that are accurate about the systems the architects personally worked on and wrong about the rest, with no way to tell which is which from reading them.

**Scaling constraints without scaling bureaucracy.** The mechanism is's: enforcement cost must not grow with the number of teams. Review scales linearly and therefore fails; templates and CI checks are constant-cost per team and therefore scale. Every proposed constraint should be assessed on this at design time — *if we had twice as many teams, would enforcing this cost twice as much?* If yes, it will be abandoned at some scale, and knowing that now is better than discovering it later.

**Federating architecture without losing coherence.** Above ~500 systems, domain architects must own their domains' internal architecture and the enterprise function must own only cross-domain contracts — the same contract-versus-implementation distinction as everywhere else in this domain. The failure mode is an enterprise function that keeps opinions about domain internals, which recreates the bottleneck; the opposite failure is domains whose target states are mutually incompatible at the boundary, which is what happens when the enterprise function abdicates entirely rather than narrowing.

**Resilience.** An architecture function whose knowledge lives in its members' heads loses it at attrition. The derived current-state model is the mitigation and it is a genuine one — it is the only architecture artifact that survives the departure of everyone who built it, because it regenerates from the systems themselves.
