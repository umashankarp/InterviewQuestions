...
Entra ID Conditional Access policy evaluation (§2.2) adds a real, if typically small, latency cost at authentication time — for an extremely high-authentication-volume workload (a customer-facing application with very frequent token refresh), the specific set of Conditional Access policies applied should be reviewed for necessity and evaluated for their aggregate authentication-latency impact, the same "convenience/security capability isn't automatically free" discipline Module 63 §9's App Mesh sidecar-overhead discussion already established. Key Vault has its own per-vault throughput limits on secret/key retrieval operations (analogous to Module 58 §7's KMS rate-limit discussion) — a high-throughput workload retrieving secrets on every request rather than caching them (Module 58 §7's caching recommendation) risks hitting these limits, with the identical fix (cache retrieved secrets in memory for a bounded window) applying directly.

## 8. Security
Conditional Access (§2.2) provides an authentication-time policy layer that should be treated as a genuinely necessary complement to RBAC's resource-authorization-time layer, not a redundant one — requiring MFA and a compliant/trusted device for any access to production-scope role assignments (especially PIM-activated ones, §2.6) closes a gap RBAC alone doesn't address (RBAC governs what an *already-authenticated* identity can do; Conditional Access governs the *circumstances under which authentication itself succeeds*), directly the same two-independent-axes reasoning Module 57 §8 established for network-vs-identity control, now recurring as authentication-time-vs-authorization-time control within Azure's identity layer specifically. Key Vault's private-endpoint capability (restricting network reachability to the vault itself, independent of RBAC permissions) should be enabled as a default for any vault holding production secrets, directly Module 59 §8's S3-Block-Public-Access-style "genuinely non-bypassable network-level restriction, independent of any individual access-policy correctness" pattern, now applied to Key Vault specifically.

## 9. Scalability
RBAC role assignments have per-subscription count limits (a real, if generous, ceiling) that an organization with a very large number of fine-grained custom-role assignments (the natural consequence of rigorously applying §2.6's least-privilege discipline at scale) should proactively track, the same subscription-level-resource-count capacity-planning discipline Module 65 §9 already established generally for Azure. PIM's approval-workflow-based activation (§2.6), while a genuine security improvement, introduces a real operational-latency cost for legitimate, time-sensitive access needs (an on-call engineer needing emergency production access at 3 AM) — a Principal Engineer designing an organization's PIM policy must explicitly balance this security-vs.-operational-responsiveness trade-off, typically via a distinct, faster "break-glass" emergency-access path with its own compensating audit/alerting controls, rather than applying the same friction-heavy approval workflow uniformly regardless of urgency.
