...
Durable Functions orchestrator replay itself has a real performance cost proportional to the orchestration's history length — a very long-running orchestration with many awaited steps accumulates a correspondingly long history that must be replayed on every resume, meaning extremely long-lived, many-step orchestrations should consider Durable Functions' **eternal orchestration** pattern (periodically calling `ContinueAsNew` to reset history length) to avoid replay time growing unboundedly, a genuinely Durable-Functions-specific performance concern with no direct Step Functions equivalent (since Step Functions' declarative execution doesn't replay history the same way). Azure Functions Consumption plan cold starts are directly affected by the same package-size and language-runtime factors Module 61 §7 established for Lambda, with the additional Azure-specific nuance that .NET's "in-process" vs. "isolated worker process" execution models have measurably different cold-start characteristics worth explicitly benchmarking for a latency-sensitive Consumption-plan function.

## 8. Security
API Management's policy pipeline (§2.4) should be the primary enforcement point for authentication/authorization/rate-limiting before a request reaches a backend Function App — but, directly Module 61 §8's defense-in-depth discipline, the Function App itself should still independently validate security-relevant assumptions rather than assuming APIM's policy layer is the sole safeguard, particularly for any Function App reachable via a path that might bypass APIM (a direct Function App URL, an internal service-to-service call). Durable Functions' orchestration state (persisted in the configured storage backend, typically Azure Storage or Netherite/Durable Task Scheduler) can contain sensitive workflow data across its entire history — this storage backend should receive the same encryption-at-rest and access-control review (Module 66 §2.4, Module 67 §8) as any other data store, since orchestration history is itself a genuine, persistent data store, not merely transient in-memory execution state.

## 9. Scalability
Consumption-plan Azure Functions scale automatically similar to Lambda (Module 61 §2.2), but Durable Functions orchestrations specifically have their own scaling characteristics governed by the configured storage backend's partition/throughput characteristics (the default Azure Storage backend has documented practical limits on orchestration throughput per task hub) — a very high-volume Durable Functions workload should evaluate the Durable Task Scheduler or Netherite backend (alternative, higher-throughput storage providers specifically designed for higher-scale Durable Functions workloads) rather than assuming the default backend scales as transparently as Lambda/Step Functions' fully-managed scaling, directly the same "the specific backing implementation has its own capacity ceiling requiring explicit verification" discipline this domain has established repeatedly (Module 67 §9's storage-account throughput limits, Module 68 §9's Cosmos DB RU capacity planning).
