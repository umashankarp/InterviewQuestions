...
## 5. Best Practices
- Explicitly select a Cosmos DB consistency level per access pattern based on that pattern's actual requirement — never leave every access pattern on the Session default without deliberate consideration of session-boundary-crossing staleness risk.
- Choose Azure SQL Managed Instance (not Azure SQL Database) for any SQL Server workload with genuine instance-level feature dependencies (cross-database transactions, SQL Agent, linked servers) — verify this requirement explicitly rather than defaulting to the simpler PaaS option.
- Use Azure SQL Database Business Critical tier's readable secondaries for genuine read-scaling needs, but verify this against the workload's actual consistency requirement the same way established for RDS read replicas.
- Explicitly estimate RU cost per operation type during Cosmos DB schema/query design, treating RU-inefficient access patterns as a first-class design concern, not an afterthought discovered via throttling in production.
- Evaluate Elastic Pools for any multi-tenant, database-per-tenant Azure SQL architecture with non-simultaneous per-tenant peak usage, as a genuinely Azure-native cost-optimization opportunity.

## 6. Anti-patterns
- Leaving Cosmos DB's consistency level at its Session default for every access pattern without considering session-boundary-crossing staleness risk, based on a DynamoDB-derived "the default is usually fine" assumption.
...
- Defaulting to Last-Writer-Wins conflict resolution for a genuinely multi-writer, conflict-prone Cosmos DB workload without evaluating the custom-conflict-resolution-procedure option the DynamoDB Global Tables material had no equivalent for.

---

## 7. Performance Engineering

**CPU/Memory:** Azure SQL Database's **DTU (Database Transaction Unit)** model bundles CPU/memory/IO into a single blended metric per tier (Basic/Standard/Premium) — simple to reason about but opaque when a workload is genuinely bottlenecked on one dimension only (e.g., CPU-bound but IO-light); the **vCore** model exposes CPU and memory as independently observable and separately scalable dimensions, letting a Principal Engineer diagnose "we're CPU-bound at 40% memory utilization" precisely, something DTU's blended number obscures — vCore is the recommended model for any production workload where precise capacity diagnosis matters.

**Latency:** Cosmos DB point reads on a well-partitioned container return in single-digit milliseconds at the 99th percentile (Microsoft's documented SLA); a cross-partition query's latency is dominated by fan-out cost across physical partitions, directly the RU-cost discussion (§2.5) now expressed as a latency, not just a cost, concern.

**Throughput:** Azure SQL Hyperscale decouples compute from a distributed, log-structured storage layer capable of scaling to 100 TB, with **read scale-out** via up to four secondary replicas — a genuinely different scaling architecture from General Purpose/Business Critical's fixed-size, replicated-disk model (§2.1), purpose-built for workloads whose growth trajectory or backup/restore-time requirements exceed what standard tiers handle gracefully (a multi-terabyte trade-history table with a sub-hour restore-time requirement is the canonical Hyperscale case).

**Scalability:** Cosmos DB throughput is provisioned in **RU/s**, either per-container or shared across a database; **autoscale** throughput scales between 10% and 100% of a configured maximum automatically based on real-time usage, trading a higher per-RU unit cost for eliminating manual capacity planning — a Principal Engineer should model whether a workload's traffic pattern (spiky vs. steady) actually benefits from autoscale's premium, or whether manually-provisioned throughput sized against a known steady-state is more cost-effective.

**Benchmarking:** benchmark Cosmos DB query patterns against the container's *actual* partition-key cardinality and access-pattern shape, not a synthetic uniform-key-distribution test — the RU cost of a query is a direct, deterministic function of the query's actual execution plan (single-partition vs. cross-partition fan-out), and this is directly observable per-request via the `x-ms-request-charge` response header, giving a Principal Engineer a precise, per-query cost signal unavailable on most relational systems without separate profiling tooling.

**Caching:** Azure SQL Database supports Query Store-driven plan-caching diagnostics and, at the application layer, Azure Cache for Redis as a read-through cache in front of either Azure SQL or Cosmos DB for genuinely hot, read-heavy paths — reducing RU consumption directly, since a cache hit avoids the RU charge entirely.

---

## 8. Security

**Threats:** an Azure SQL Database exposed with a public endpoint and an overly-permissive firewall rule (`0.0.0.0`–`255.255.255.255`, sometimes left from initial development convenience) is the most common Azure SQL exposure vector; for Cosmos DB, an account-level primary/secondary key leaked into application logs or a repository grants full read/write access to every container in that account, with no per-database or per-container scoping possible via the legacy key model.

**Mitigations:** for Azure SQL, restrict firewall rules to specific, documented IP ranges or (preferably) use **Private Endpoints** to remove public internet exposure entirely, combined with Azure AD-only authentication (disabling SQL-authentication logins for production databases where feasible); for Cosmos DB, prefer **Azure RBAC via Managed Identity** (fine-grained, per-principal, individually revocable, and auditable via Entra ID logs) over primary/secondary key-based access, and configure the account's **IP firewall / VNet service endpoint / Private Link** to restrict network-level access regardless of credential validity.

**OWASP mapping:** broken access control (an overly-broad Cosmos DB key or an unrestricted Azure SQL firewall rule); injection (parameterized queries remain mandatory for Azure SQL; Cosmos DB's SQL API is likewise injectable if query text is string-concatenated from user input rather than parameterized via `QueryDefinition.WithParameter`, directly the same discipline applied to a different query language).

**AuthN/AuthZ:** Azure SQL Database supports Azure AD authentication (individual or group-based, integrating with the organization's existing identity governance) alongside legacy SQL authentication (username/password, a weaker, separately-managed credential store) — Azure AD authentication should be the default for any new production database; Cosmos DB's RBAC model supports built-in and custom role definitions scoped to specific data actions (read-only, read-write) at the account, database, or container level.

**Secrets:** Azure SQL connection strings and Cosmos DB account keys must live in Key Vault, retrieved at runtime via Managed Identity — never embedded in application configuration or environment variables directly, the same secrets-management discipline established for Storage Account keys, now applied to database credentials.

**Encryption:** Azure SQL Database enables **Transparent Data Encryption (TDE)** by default (encrypting data at rest, transparent to the application) and offers **Always Encrypted** for column-level encryption where even database administrators should never see plaintext (a genuine, distinct guarantee beyond TDE — TDE protects against disk/backup theft, Always Encrypted protects against a privileged-but-untrusted DBA or a compromised database-engine-level attacker, since the encryption/decryption happens client-side, with the database engine only ever handling ciphertext); Cosmos DB encrypts data at rest by default and supports customer-managed keys via Key Vault for accounts requiring independent key-lifecycle control.

---

## 9. Scalability

**Horizontal scaling:** Cosmos DB scales horizontally via **partitioning** — data is distributed across physical partitions keyed by the chosen partition key, with Azure automatically splitting a partition as it grows beyond the per-partition storage limit (20 GB logical partition size ceiling as of current service limits, though physical partitioning is Azure-managed) — a well-chosen partition key (high cardinality, even access distribution, aligned with the dominant query pattern) is the single most consequential Cosmos DB schema decision, directly the DynamoDB partition-key discipline this course established, now with Cosmos DB's own RU-cost lens (§2.5) making a poor choice immediately, quantifiably visible.

**Vertical scaling:** Azure SQL Database's DTU/vCore tier can be resized (online, with brief connection interruption for most tiers) — a coarser-grained lever than Cosmos DB's RU/s dial, and one requiring capacity planning against known peak load rather than continuous, automatic adjustment.

**Caching:** see §7 — Azure Cache for Redis as an external layer; Cosmos DB's own integrated cache (the "integrated cache" feature for the dedicated gateway) reduces RU consumption for repeated point-reads/queries directly within the service.

**Replication/Partitioning:** Cosmos DB's **multi-region write** capability (§2.4) is itself a replication-and-partitioning strategy operating at global scale — every region holds a full, independently-writable replica of every partition, with conflict resolution (last-writer-wins or custom) reconciling concurrent writes across regions; Azure SQL Database's replication is narrower in scope — synchronous within a Region (Business Critical's Always On group, §2.1) and, separately, asynchronous cross-Region via **Active Geo-Replication** or **Auto-Failover Groups** (up to four readable secondaries, with Auto-Failover Groups additionally providing an automatic, application-transparent failover endpoint).

**Load balancing:** Cosmos DB's SDK automatically routes requests to the nearest/preferred region per the account's configured region priority list; Azure SQL read-scale-out requires explicit `ApplicationIntent=ReadOnly` connection-string routing by the client, a manual, application-aware load-balancing decision rather than an automatic one.

**High Availability:** Azure SQL Database Business Critical provides sub-30-second automatic failover to a synchronous secondary within a Region (§2.1); Hyperscale provides similarly fast failover via its distributed storage layer's own redundancy; Cosmos DB's multi-region configuration with automatic failover provides continued availability even through a full Region loss, with the account's configured failover priority list determining the new write region automatically if automatic failover is enabled.

**Disaster Recovery:** Azure SQL Database Auto-Failover Groups provide a documented RPO of 5 seconds and RTO of 1 hour (for asynchronous geo-replication under standard conditions — the Principal Engineer's job is verifying these are the *actual* current SLA figures for the specific tier in use, not treating them as permanently fixed); Cosmos DB's multi-region-write configuration with Strong or Bounded Staleness consistency provides a materially tighter RPO than the Session-consistency default (§2.3), directly tying the consistency-level decision to the DR posture, not merely to read-path correctness.

**CAP theorem:** Azure SQL Database (a single-primary-writer relational system even in multi-region configurations) is architecturally CP — a Region-level partition during failover involves a brief unavailability window (RTO), not a silent availability-over-consistency trade; Cosmos DB explicitly exposes the CAP/PACELC trade-off as a first-class, per-container configuration choice via its five consistency levels — Strong consistency sacrifices availability/latency during a partition for linearizability, while Eventual consistency remains available at every region through a partition at the cost of staleness, making Cosmos DB one of the clearest production systems where the CAP trade-off is a deliberate dial, not an emergent property.
