...
Application Gateway's bundled WAF (§2.4) introduces request-processing overhead for rule evaluation, similar in kind to any WAF's inherent latency cost — a genuinely latency-critical path should benchmark this overhead explicitly rather than assuming a bundled capability is free, the same "convenience isn't automatically zero-cost" discipline Module 63 §9 already established for App Mesh's sidecar overhead. VNet peering (connecting two VNets, Azure's equivalent of AWS VPC peering) and any subnet-crossing traffic within a single VNet both have their own throughput characteristics worth capacity-planning for at genuinely high-throughput scale, directly the same NAT-Gateway-throughput-awareness discipline Module 57 §7 established for AWS.

## 8. Security
NSGs' dual subnet/NIC association model (§2.2) is a genuine additional defense-in-depth opportunity relative to AWS's single-layer model — a Principal Engineer can deliberately apply a broader, coarser-grained rule set at the subnet level (a baseline policy for an entire application tier) and a narrower, more specific rule set at the NIC level (per-VM refinement), a two-tier design AWS's model doesn't directly support in the same way — but this same dual-layer model also means a security audit must be more thorough (checking both layers, §6) to avoid a false sense of restrictiveness from checking only one. Azure additionally provides **Azure Policy** (organization-wide, enforceable configuration constraints, e.g., "no VM may be provisioned without Availability Zone configuration") — directly the Azure-native equivalent of AWS's Service Control Policies (Module 58 §8), and the correct mechanism for structurally preventing the exact §4 incident from recurring, rather than relying on runbook documentation alone.

## 9. Scalability
VM Scale Sets, like AWS ASGs (Module 57 §9), are bounded by subscription-level quota limits (VM core counts per Region, per VM family) that must be proactively verified and increased ahead of anticipated need — the same "silent capacity ceiling, independent of the scale set's own configured maximum" risk Module 57 §9 established for AWS applies directly here, with Azure's quota system being a distinct, separately-tracked set of limits from any AWS account's quotas for any organization operating in both clouds. Azure's per-subscription resource-count limits (the number of VNets, NSGs, or other resources allowed per subscription) are a genuinely Azure-specific capacity dimension with no precise AWS equivalent (AWS's per-account limits are structured differently), meaning a large-scale, many-resource-group Azure estate should explicitly capacity-plan against subscription-level resource-count limits, not just VM-core quotas.
