...
The `SettlementInstruction` migration is now executed at genuine production scale: **eleven million historical settlement instructions**, spanning six years, across four prior schema revisions, migrated from traditional persistence to full Event Sourcing with zero downtime and zero risk to live settlement processing — using the Parallel Run technique (the preview) as the actual, worked migration mechanism.
...
**Why:** the incidents both arose from *already-migrated* systems' ongoing operation; this capstone addresses the migration itself — the highest-risk, one-time event in this pattern's adoption lifecycle, where an error affects not just future events but the interpretation of the *entire pre-existing historical record*.

**When:** Once the calibration test (genuine, demonstrated audit/regulatory value) has justified adoption for a specific, already-live Aggregate — this module addresses *how*, safely, not *whether*.
...
During Phase 2, every new command writes to both the traditional store (still authoritative) and the new event store (being validated) — directly the dual-write migration pattern, now applied to a persistence-model migration specifically rather than a data-store technology migration.

### 2.3 Reconciliation — Detecting Divergence Before Cutover
A continuous, automated job (extending the reconciliation approach) compares event-store-derived state against the traditional store's actual current state for every active instruction, flagging any divergence — directly the reconciliation discipline, now the central go/no-go gate for cutover.

### 2.4 The Backfill-Derived-History Risk — a New, Distinct Category
Unlike ongoing dual-write divergence (detectable and fixable before cutover), an *inaccuracy in the backfilled historical events themselves* may not surface as a reconciliation discrepancy at all — since the traditional store's *current* state might still match correctly even if the *path* the backfill assumed to reach it was wrong — meaning reconciliation alone cannot fully validate historical-event accuracy, only current-state agreement.

### 2.5 Cutover — a Feature-Flagged, Reversible Transition
Mirroring the feature-flag-based cutover discipline: the event store becomes authoritative behind a flag, with the traditional store continuing to be dual-written for a defined post-cutover safety window, providing an instant rollback path if any post-cutover issue emerges.

### 2.6 Decommissioning — the "Old Systems Never Die" Risk, Recurring Again
Directly the already-established finding: the traditional store must have an explicit, tracked decommissioning deadline, or it lingers indefinitely as a redundant, increasingly-stale dual-write burden — precisely the structural, incentive-driven risk named, recurring here at the close of this specific migration.
...
**Problem:** Execute the `SettlementInstruction` migration at full production scale — 11M instructions, 6 years, 4 prior schema revisions — without any risk to live settlement processing.

**Architecture:** The five-phase migration (the How) with reconciliation as the central go/no-go gate, and a dedicated anomaly-review process specifically for backfill-derived history (the distinct risk).
...
**Lessons learned:** During the reconciliation phase, a genuine divergence was found affecting roughly 400 instructions where the backfill's derived event sequence produced a *current* state matching the traditional store (so it wouldn't have been caught by current-state reconciliation alone, the exact risk) but with an internal, intermediate `NettingCompleted` timestamp that was demonstrably wrong (derived from an ambiguous audit-log entry) — only caught because a separate, dedicated spot-check specifically compared a sample of backfilled *intermediate* states (not just final state) against an independent, older archival system used for a different purpose entirely. This directly validated the warning: reconciliation alone, checking only current state, would have silently missed this specific, real inaccuracy, motivating the addition of intermediate-state spot-checking as a permanent, additional verification step for any future backfill of this kind.

---

## 5. Best Practices
- Explicitly document and flag any backfilled records where full historical granularity couldn't be reconstructed — never silently present coarse, best-effort history as equivalent in quality to genuine, contemporaneously-recorded events.
- Add intermediate-state spot-checking (not just current-state reconciliation) specifically for backfilled history, given/the demonstrated blind spot.
- Use a feature-flag-based, reversible cutover with a defined safety-window dual-write period, never an irreversible, one-way cutover.
- Set an explicit, tracked decommissioning deadline for the retired traditional store from day one of the migration plan, not as an afterthought.
...
## 6. Anti-patterns
- Presenting backfilled, best-effort historical events as equivalent in integrity/completeness to genuine, contemporaneously-recorded ones without explicit flagging.
- Relying solely on current-state reconciliation to validate backfill accuracy, missing the intermediate-state-divergence risk.
- An irreversible, one-way cutover with no dual-write safety window or rollback path.
- Leaving the retired traditional store's decommissioning undated and untracked, letting it linger indefinitely.
...
**Latency:** Dual-write period adds a second write's latency to every live command during migration — measure and ensure this remains within acceptable command-processing latency budgets throughout the entire migration window, not just at its start.
...
**Scalability:** Backfill and reconciliation both parallelize naturally across instruction ID ranges/partitions, given each instruction's own independence (the partitioning pattern, reapplied here for a batch migration workload specifically).
...
**Secrets:** The backfill job's elevated, time-boxed credentials are managed and rotated with the same rigor as any other regulated-system secret, with an explicit, tracked expiration tied to the migration's own completion.

**Encryption:** Backfilled historical event payloads receive identical field-level encryption treatment as newly-created events — never a lesser standard applied to "just historical" data.
...
**Disaster Recovery:** The migration plan itself should have its own rollback runbook (per the reversible-cutover design), tested before the real migration, not improvised during an actual incident.

**CAP theorem:** During the dual-write period, the traditional store's own existing CP guarantee remains the system's actual correctness backbone; the event store's own consistency guarantee is being validated, not yet relied upon, until reconciliation and cutover confirm it's ready to bear that responsibility.
 ...
 **Common mistakes:** Assuming backfilled history has the same accuracy guarantee as genuinely contemporaneous, post-migration events (/).
 **Follow-ups:** "Why might backfilled history be less accurate than post-migration events?" (It's derived after the fact from whatever incomplete audit trail already existed, not recorded at the moment each original event actually occurred,.)

2. **Q: What is the dual-write period, and which store remains authoritative during it?**
 **A:** A period where every command writes to both stores; the traditional store remains authoritative until cutover.
 ...
 **Follow-ups:** "What happens if the event-store write fails during dual-write but the traditional-store write succeeds?" (The command still succeeds, per the traditional store's authority; the event-store gap is logged/retried and caught by reconciliation,.)

3. **Q: What does the reconciliation job check for during migration?**
 **A:** Whether event-store-derived state matches the traditional store's actual current state, for every instruction, continuously.
 **Why correct:** States the specific comparison being verified.
 **Common mistakes:** Assuming reconciliation alone validates full historical accuracy, missing the intermediate-state blind spot.
 **Follow-ups:** "What specific risk does current-state-only reconciliation miss?" (An inaccurate intermediate historical state that still happens to arrive at the correct final state,/.)

4. **Q: Why is cutover implemented via a feature flag rather than a one-time code deployment?**
 **A:** To provide an instant, reversible rollback path if any post-cutover issue emerges, without needing a slower code-revert-and-redeploy cycle.
 ...
 **Follow-ups:** "What additional safety measure accompanies the flagged cutover?" (A defined, continued dual-write safety window even after cutover,.)

5. **Q: Why must the traditional store's decommissioning have an explicit, tracked deadline?**
 **A:** Without one, it risks lingering indefinitely as a redundant burden — directly the "old systems never die" structural risk.
 ...
 **Follow-ups:** "What consequence follows from failing to decommission it?" (Ongoing, unnecessary dual-write/maintenance burden and confusion about which store is truly authoritative,.)

6. **Q: Approximately how many of the 11M instructions couldn't be backfilled with full historical granularity, and why?**
 **A:** About 30,000, from the system's earliest, pre-audit-logging era, which had no reconstructable intermediate-state detail beyond final known state.
 ...
 **Follow-ups:** "How were these specific instructions handled?" (A single, explicitly-flagged `LegacyStateImported` event carrying only final known state,.)

7. **Q: What specific, additional verification step did the incident motivate, beyond ordinary current-state reconciliation?**
 ...
 **Follow-ups:** "Why couldn't ordinary reconciliation alone catch this specific divergence?" (The final state matched correctly despite an inaccurate intermediate timestamp,/.)

8. **Q: Should the backfill job's elevated data-access credentials remain active after migration completes?**
 **A:** No — they should be time-boxed to the migration window and explicitly revoked afterward.
 ...
 **Follow-ups:** "What OWASP-relevant risk does an un-revoked elevated credential represent?" (An unnecessarily broad, long-lived privileged-access surface,.)

9. **Q: During the dual-write period, whose consistency guarantee does the system actually rely on for correctness?**
 **A:** The traditional store's already-established CP guarantee — the event store's own guarantee is being validated, not yet relied upon.
 ...
 **Follow-ups:** "When does this reliance shift to the event store?" (Only after cutover, following validated, sustained reconciliation success,.)

10. **Q: What determines when cutover is safe to proceed, per this module's own recommendation?**
 **A:** Sustained, demonstrated reconciliation success over a defined period — not a fixed, pre-chosen calendar date.
 ...
 **Follow-ups:** "What would you do if reconciliation still showed divergence on the planned cutover date?" (Delay cutover — the reconciliation gate takes priority over the calendar,.)
...
2. **Q: Design the specific data source and method for intermediate-state spot-checking, given the incident.**
 **A:** Compare a statistically-significant sample of backfilled intermediate states (e.g., netting-completion timestamps) against an independent archival system that recorded similar detail for a different original purpose (in the case, an older reporting archive) — providing cross-validation from a genuinely separate source of truth, not merely re-deriving the same audit-log data the backfill itself was based on.
...
3. **Q: Why does the dual-write period's failure-handling policy specifically favor the traditional store's write succeeding as the command's actual success criterion?**
 **A:** Because the traditional store remains authoritative throughout migration — a live command's real-world correctness must never depend on the not-yet-validated event store's write succeeding, which is precisely why the traditional write's outcome, not the event-store write's, gates command success during this phase.
 ...
 **Follow-ups:** "What happens to a failed event-store write in this design?" (Logged and retried, or flagged for reconciliation follow-up — never blocking the live command,.)

4. **Q: How would you determine, concretely, what "sustained reconciliation success" (/) should mean as a specific, measurable cutover gate?**
 ...
 **Follow-ups:** "Why specifically include peak-volume days in this validation window?" (Directly the discipline — validating under realistic peak conditions, not just steady-state or low-activity periods, since edge cases and race conditions are more likely to surface under genuine load.)
 ...
 **A:** This removes the entire validation mechanism (/) this migration's safety depends on — any backfill inaccuracy or event-store implementation bug would be discovered only *after* the event store is already authoritative for live processing, with no proven, already-running traditional-store fallback to catch or absorb the error, a direct violation of the own established migration-risk-minimization principle.
 **Why correct:** Identifies the specific safety mechanism (dual-write plus reconciliation as a pre-cutover validation gate) this shortcut removes, and connects it to the already-established migration discipline.
...
7. **Q: Design the specific rollback runbook entry for a post-cutover issue discovered during the safety-window dual-write period.**
 ...
 **Follow-ups:** "How would you decide when it's safe to end the safety-window dual-write and fully retire the traditional store?" (Only after a defined, additional period of confidence-building post-cutover with zero rollback triggers, per the tracked-decommissioning-deadline discipline.)

8. **Q: Why does backfilled event data require the identical field-level encryption treatment as newly-created events, rather than a simpler approach given it's "just historical data being migrated"?**
 **A:** The event store treats all events, regardless of age or origin, under one uniform security/compliance standard — applying a lesser standard to backfilled data would create an inconsistent, auditable gap in the system's own claimed security posture, undermining exactly the kind of uniform, defensible compliance evidence (/121's established theme) this entire migration is meant to strengthen, not weaken.
...
10. **Q: Synthesize this capstone's specific, new findings back into the own established toolkit — what does this module add that didn't already cover?**
 **A:** Earlier analysis established the ongoing, steady-state mechanics of an already-Event-Sourced Aggregate (reconstruction, snapshotting, upcasting); this capstone adds the specific, one-time migration mechanics required to *get there safely* from an existing, traditionally-persisted Aggregate — backfill-accuracy risk and its intermediate-state-blind-spot, the dual-write/reconciliation/feature-flagged-cutover sequence (the five phases), and the specific credential-lifecycle and decommissioning-deadline governance this one-time transition requires — genuinely new, additive content rather than a restatement of the own steady-state operational concerns.
 **Why correct:** Precisely distinguishes the steady-state operational content from this capstone's genuinely new, migration-specific content, rather than presenting this module as repetitive.
 **Common mistakes:** Assuming this capstone merely demonstrates the already-established mechanics at larger scale, missing the specific, new risk categories (backfill accuracy, migration-phase governance) unique to the one-time transition itself.
 **Follow-ups:** "Would a brand-new Aggregate, built Event-Sourced from day one, ever need any of this capstone's specific techniques?" (No — backfill, dual-write, and cutover are exclusively migration-specific concerns; a new Aggregate starts directly in the steady-state operational model with no prior history to migrate.)

### Advanced (10)

1. **Q: Diagnose the incident from first principles and design the complete, structural fix for this specific class of risk in any future backfill-based migration.**
 **A:** Root cause: reconciliation validated only current state, structurally blind to intermediate-state inaccuracy in derived historical events. Fix: mandate intermediate-state spot-checking against an independent data source (Intermediate Q2) as a standing, required step for any future backfill migration — not merely a one-off addition for this specific instance — codified as a governance checklist item any team executing a similar Aggregate migration must complete before their own cutover, directly extending this incident's specific lesson into an organizational, reusable requirement.
...
2. **Q: A team proposes running the reconciliation job only once, immediately before cutover, rather than continuously throughout the dual-write period. Evaluate this proposal.**
 ...
 **A:** This would silently misrepresent this system's own audit-completeness guarantee to any future regulator or internal auditor querying those specific 30,000 instructions' history — the correct approach (/Intermediate Q1) is ensuring every downstream consumer of historical event data (audit-reconstruction queries,-style read-model projections) can explicitly distinguish and appropriately caveat results touching `LegacyStateImported`-flagged instructions, never presenting them with the same completeness confidence as genuinely granular records.
 **Why correct:** Extends the "explicit flagging" principle (/Intermediate Q1) to its necessary downstream consequence — every consumer of this data must also respect and surface the distinction, not just the backfill process itself.
 ...
 **A:** Directly extending the peak-burst load-testing methodology: run the dual-write path under synthetic, realistic peak-volume order/settlement traffic (not steady-state averages) for a sustained period, measuring command-processing P99 latency with dual-write enabled versus the pre-migration baseline — confirming the added event-store write doesn't push latency past this system's already-established SLA (the own latency budget), specifically during the highest-load periods the real migration window will actually encounter, not just quiet, low-traffic testing conditions.
...
5. **Q: How would you decide whether the 400-instance intermediate-state divergence found in the incident was severe enough to delay cutover entirely, versus proceeding with a documented, accepted exception for those specific instances?**
 ...
 **A:** Yes — disclose the migration's occurrence, the backfill methodology and its known limitations (the 30,000 `LegacyStateImported` records and the 400-instance documented exception, per Advanced Q3/Q5), and the validation process (dual-write, reconciliation, feature-flagged cutover) as concrete, positive evidence of a rigorous, risk-managed transition — framing the honestly-disclosed limitations not as a weakness to hide, but as evidence of the exact kind of careful, transparent risk management (directly the own "honest, bounded characterization is stronger than overclaiming completeness" principle) a sophisticated regulator is specifically looking for.
 **Why correct:** Correctly reapplies the already-established honest-disclosure principle to this migration's own specific, genuine limitations, framing transparency as a credibility strength rather than a liability to minimize or conceal.
 ...
 **Common mistakes:** Treating "cutover succeeded" as equivalent to "migration is fully proven safe," missing that genuine confidence requires the additional, sustained post-cutover observation period the safety window specifically provides.
 ...
 **A:** Because the safety-window design specifically kept the traditional store dual-written throughout, it remains fully current and consistent with all three weeks of activity — rollback is *not* operationally disruptive precisely because this design anticipated exactly this scenario; the instant feature-flag flip reverts authority to the still-current traditional store with zero data loss or catch-up work required, which is the entire, deliberate point of maintaining dual-write through the full safety window rather than stopping it immediately at cutover.
 ...
 **Follow-ups:** "At what point would rollback genuinely become disruptive?" (Only after the traditional store's dual-writing has actually stopped, i.e., after full decommissioning — which is exactly why that decommissioning decision must be made only with high, sustained confidence, not prematurely.)
 ...
 **A:** (1) An explicit, calibrated justification that this specific Aggregate's migration is warranted before starting. (2) A backfill process with mandatory, explicit flagging of any historical period lacking full reconstructable granularity (/Advanced Q3), never silently presented as complete. (3) Mandatory intermediate-state spot-checking against an independent data source, not merely current-state reconciliation (Advanced Q1). (4) A continuous, representative-activity-calibrated reconciliation validation window as the actual cutover gate — never a fixed calendar date (Intermediate Q4/Advanced Q2). (5) A feature-flagged, reversible cutover with a representative-business-cycle-calibrated post-cutover safety window (Advanced Q7), never an immediate, irreversible transition. (6) An explicit, tracked decommissioning deadline for the retired traditional store, preventing indefinite lingering. (7) Proactive, honest regulatory disclosure of the migration's methodology and any known, documented limitations (Advanced Q6), framed as evidence of rigor, not a hidden weakness.
 ...
 **Follow-ups:** "Which single element of this program is most directly attributable to a lesson only this specific, at-scale capstone (not the more general treatment) could have taught?" (Mandatory intermediate-state spot-checking, Advanced Q1 — a risk category specific to backfilling genuinely pre-existing historical data at scale, never encountered in the steady-state, already-migrated operational scope.)

### Expert (FinTech Principal Panel)

**E1. Q: You're migrating an existing state-stored, regulated ledger aggregate (you have current balances, not a history of events) to event sourcing. You can't recover events that were never recorded. How do you seed the event store honestly, and what's the limit of the audit history you can claim?**
**A:** The hard truth: **event sourcing gives you audit history only from the moment you start recording events** — you cannot manufacture a real, event-level history for a period during which you only ever stored current state. So seed honestly: (1) for each aggregate, write a single **`Initialized`/`OpeningBalance` genesis event** at migration time that captures the *current authoritative state* (balance, positions) as the starting point — reconciled exactly against the legacy source (per-account and aggregate sums) so day-one state is provably correct; (2) from then on, **all changes are real events**, so forward history is genuine and reconstructable; (3) where a legacy transaction *log* exists, you may **backfill derived events from it** to give partial historical fidelity — but only if that log is authoritative and reconcilable, and with **intermediate-state spot-checks** (this capstone's finding) proving the replayed history matches known past states; (4) be explicit in documentation/audit that pre-migration detail comes from the legacy system of record, not the event store, and keep the legacy records as the historical audit source for that period. Don't fabricate synthetic per-transaction events for the pre-migration period and present them as a real audit trail — that's a worse audit posture than an honest "history begins at migration, prior history in the legacy system." The Principal framing: you can only event-source *forward* from migration, so seed each aggregate with a reconciled opening-balance genesis event (exactly matched to the legacy source), optionally backfill from an authoritative legacy transaction log with intermediate-state verification, and clearly bound the claim — genuine event-level audit history starts at migration, and the pre-migration period remains the legacy system's audit responsibility — because manufacturing fake history is both wrong and a compliance risk.
**Why correct:** Seeds with a reconciled opening-balance genesis event, only claims genuine history from migration forward (optionally backfilled from an authoritative log with spot-checks), and honestly bounds the audit claim rather than fabricating history.
**Common mistakes:** Fabricating synthetic per-transaction events for the pre-migration period and calling it audit history; unreconciled opening balances; backfilling from a non-authoritative log without verification; claiming full retroactive audit history the store can't actually support.
**Follow-ups:** "Why can't you present manufactured pre-migration events as a real audit trail?" / "What must the opening-balance genesis event be reconciled against, and to what tolerance?"

**E2. Q: Before committing a regulated aggregate to event sourcing at scale, how do you decide it's worth it, and what are the highest-risk correctness pitfalls of the migration specifically?**
**A:** Decide with the domain's calibrated discipline: adopt event sourcing where the aggregate's **history is itself a requirement** — auditability, point-in-time reconstruction, temporal/regulatory queries (a ledger, settlement, positions) — and *not* where current-state storage suffices, because you're taking on permanent versioning/upcasting, snapshotting, projection/CQRS, and operational cost. For a regulated aggregate the audit/reconstruction benefits often *do* justify it, but only if the team can sustain that operational burden. The highest-risk correctness pitfalls of the migration specifically: (1) **reconstruction correctness** — inline version-branching in `Apply` (the fundamentals' incident) or bad **upcasters** silently produce wrong historical state; keep events immutable and evolve via versioned event types + upcasters, not by editing history; (2) **opening-state/backfill correctness** — E1's reconciliation; a wrong genesis balance corrupts everything forward; (3) **snapshot correctness** — versioned, reproducible, events-win-on-disagreement; (4) **parallel-run and reconciliation** — run event-sourced and legacy in parallel and reconcile derived state **to the cent** before the event-sourced path is authoritative, with instant fallback (money migrations are correctness proofs, not cutovers); (5) **idempotency/exactly-once** across the migration boundary so in-flight changes aren't lost or double-applied. The Principal framing: commit a regulated aggregate to event sourcing when reconstructability/audit is a genuine requirement and the team can carry the ongoing cost — and de-risk the migration itself with reconciled opening state, versioned-upcaster (never edited) history, verified snapshots, and exact parallel-run reconciliation with fallback, because the migration's failure mode is silent, permanent balance/history corruption, not a recoverable bug.
**Why correct:** Ties adoption to genuine reconstructability/audit need + sustainable cost, and names the money-specific migration pitfalls (reconstruction/upcaster correctness, reconciled opening state, verified snapshots, exact parallel-run + fallback, idempotency).
**Common mistakes:** Event-sourcing a regulated aggregate for fashion without the operational commitment; editing history instead of upcasting; unreconciled opening state; no parallel-run reconciliation; no idempotency across the boundary.
**Follow-ups:** "What makes a regulated aggregate's history a genuine requirement vs. a nice-to-have?" / "Why is exact parallel-run reconciliation the gate before the event-sourced path becomes authoritative?"

---

## 11. Coding Exercises

### Easy — Backfill Event Derivation from Audit-Log Data
...
### Medium — Reconciliation Job Comparing Event-Derived vs. Traditional State
...
**Optimized solution:** Parallelize the per-instruction comparison across partitions (the pattern), and incrementally re-check only instructions modified since the last reconciliation run rather than the full population every time, once an initial full pass has established a baseline.

### Hard — Feature-Flagged Cutover with Safety-Window Dual-Write
...
### Expert — Intermediate-State Spot-Check Against an Independent Archive (Advanced Q1)
...
**Optimized solution:** Expand from a statistical sample to full-population coverage specifically for any historical era already flagged as lower-confidence (per the incident's own affected period), reserving sampling for eras with otherwise-strong, already-validated confidence.
...
**Architecture:** The five-phase migration; a backfill job; a continuous reconciliation job; a feature-flagged Repository implementation supporting both pre- and post-cutover write modes.
...
**Database selection:** No change from the own event-store/snapshot-store selection — this capstone concerns the migration process, not new persistence-technology choices.

**Caching:** Not a primary concern for this one-time migration workload.

**Messaging:** Unchanged /120's existing Outbox/projection architecture — this migration concerns write-side persistence, with downstream projections continuing to consume Domain Events unaffected by the underlying persistence-model transition.
...
**Failure handling:** Feature-flagged, instantly-reversible cutover with a representative-business-cycle safety window; traditional-store-write-as-command-success-gate during dual-write.
...
**Trade-offs:** Accepted, explicitly-flagged reduced historical granularity for the oldest 30,000 pre-audit-era instructions, in exchange for proceeding with the migration's substantial benefit for the remaining 10.97M+ — a deliberate, documented, risk-accepted decision, not a silent gap.
...
**Sequence diagram:** See the migration-runbook sequence diagram.

**Design patterns used:** Strategy (feature-flag-selected write behavior); Template Method (each `IMigrationPhase` following a common execute/rollback structure); Command (each migration phase as an explicit, independently-executable and independently-rollback-able step).
...
**Extensibility:** A future, similar migration for a different regulated Aggregate reuses the identical `IMigrationPhase` structure and reconciliation/spot-check tooling, requiring only Aggregate-specific backfill-derivation logic to be newly written.

**Concurrency/thread safety:** Backfill and reconciliation both process instructions independently and in parallel by partition; the dual-write path's two writes (traditional, event-store) within a single command execution must be sequenced correctly per the failure-handling policy (traditional write's outcome gates command success), not executed as an uncoordinated, purely parallel pair.
...
*Disadvantages:* the incident directly demonstrates that a genuinely rare, race-condition-driven divergence can emerge well after cutover, entirely undetectable without continued reconciliation running.
...
*Advantages:* Directly catches exactly the class of rare, post-cutover issue demonstrated, providing genuine, ongoing assurance beyond the initial migration-validation period.
...
*Cost:* Higher ongoing cost; directly, demonstrably justified by the own real incident.

**Recommendation:** **Option B**, directly reapplying the own already-established principle that reconciliation (there, for read-model drift; here, for write-side event-store correctness) is a permanent, ongoing correctness safeguard, not a one-time migration-validation tool — the own incident is concrete, first-hand evidence this recommendation isn't merely theoretical caution but a demonstrated, real necessity for this specific system.

---

## 17. Principal Engineer Perspective

**Business impact:** A successful, zero-downtime migration of this magnitude (11M instructions, zero risk to live processing) is itself a significant organizational achievement worth quantifying and communicating — both as direct evidence of engineering rigor to stakeholders and regulators, and as a reusable governance template reducing the cost and risk of any future, similar migration.
