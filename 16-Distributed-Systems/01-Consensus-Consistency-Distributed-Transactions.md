...
Consensus/quorum-based systems trade latency for durability guarantees in a precisely quantifiable way: a larger write quorum (W) increases durability confidence but adds latency (waiting for more nodes' acknowledgment) — directly the same trade-off Module 22 §2.3 (synchronous replication), Module 24 §2.1 (write concern), and Module 26 §2.5 (`WAIT`) each expressed as an engine-specific knob, now understood as one, unified `W`/`R`/`N` quorum-latency trade-off applicable to any quorum-based system, including ones this course hasn't directly covered. 2PC's two full round-trips (prepare, then commit) versus Saga's single, immediately-committing step per stage is a direct, quantifiable latency difference — Saga's eventual-consistency model typically achieves lower per-step latency at the cost of a temporarily-observable intermediate state, exactly Module 37 §Advanced Q8's trade-off, now with 2PC's concrete comparison point.

## 8. Security
A consensus system's leader-election mechanism is itself a potential attack surface — a malicious or compromised node participating in leader election (in a permissioned/private consensus cluster) could attempt to disrupt quorum formation (a denial-of-service against the cluster's ability to make progress) or, in a poorly-secured cluster, attempt to become leader illegitimately; production consensus deployments (etcd, Consul) require mutual TLS authentication between cluster members specifically to prevent an unauthorized node from participating in the consensus protocol at all — directly Module 37 §8's "TLS everywhere, including internal traffic" principle applied specifically to inter-node consensus communication, where the stakes (control over the entire cluster's agreed-upon state) are particularly high.

## 9. Scalability
Consensus algorithms (Raft/Paxos) have an inherent scalability ceiling in **node count** — every write must be replicated to and acknowledged by a majority, meaning throughput doesn't improve (and can even degrade) by simply adding more nodes to a single consensus group, unlike a stateless service's horizontal-scaling story (Module 37 §9) — this is precisely why large-scale distributed systems typically use consensus for a **small**, critical coordination layer (a handful of nodes agreeing on cluster metadata/leader assignment) while the actual, high-volume data/traffic is **sharded** across many independent groups/partitions (Module 23/27's partition-key discipline), each potentially running its own, smaller consensus group — recognizing this "consensus for coordination metadata, sharding for data-volume scale" pattern is precisely how systems like Kubernetes (etcd for cluster state) and many distributed databases (a consensus group per shard) actually scale to very large sizes despite consensus's inherent per-group node-count ceiling.
