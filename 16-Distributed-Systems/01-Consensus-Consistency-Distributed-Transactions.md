...
## 5. Best Practices
- Recognize that engine-specific consistency mechanisms (MongoDB's write concern, DynamoDB's read consistency, Redis Sentinel's quorum) are all applications of the same underlying quorum/consensus theory — use this unified understanding to reason about novel systems, not just memorized per-engine behavior.
...
## 6. Anti-patterns
- Hand-rolling an ad-hoc two-phase-commit-like protocol without accounting for its well-documented blocking failure mode (the incident) — either use a mature, battle-tested distributed-transaction coordinator, or prefer Saga.
...
- Using wall-clock timestamps for distributed event ordering, vulnerable to clock skew (directly the specific incident, now understood as a special case of this module's general logical-clock theory).
