...
- ✅ **Module 31**: Design Patterns → Creational & Structural Patterns — *file removed 2026-09-06; superseded by* [`11-Design-Patterns/00-Design-Patterns-Interview-Master-Guide-DotNet-TechLead.md`](../11-Design-Patterns/00-Design-Patterns-Interview-Master-Guide-DotNet-TechLead.md) (recoverable at commit `8deee37`)
- ✅ **Module 32**: Design Patterns → Behavioral Patterns — *file removed 2026-09-06; superseded by the same guide* — **`11-Design-Patterns` domain complete (Modules 31–32; extended by Module 186 for full 23-pattern GoF coverage), then narrowed on 2026-09-06 to the 15-pattern .NET Technical Lead guide**
...
- ✅ **Module 64**: AWS → Observability, Cost & the Well-Architected Framework — CloudWatch, X-Ray & Multi-Region DR — [`21-AWS/08-Observability-Cost-WellArchitectedFramework.md`](../21-AWS/08-Observability-Cost-WellArchitectedFramework.md) — **`21-AWS` domain complete (Modules 57–64) — full-content rewrite 2026-09-12**, on explicit request, against a 26-service/33-part complete-systems brief (how every service connects to every other, not isolated service descriptions): hop-by-hop request-journey and .NET-connectivity tracing in every module, EKS treated as its own major topic (Module 63), decision frameworks carrying an explicit weak-answer/demolition treatment for each forced scenario, and this capstone's three fully-worked System Design flagships (payment platform, insurance quote aggregation, multi-tenant SaaS) plus 12 scenario sketches and a bad-architecture review exercise. Same 8 filenames/module numbers (57–64) and prerequisite links deliberately preserved — see CLAUDE.md §B (2026-09-12) for why
...
- ✅ **Module 186**: Design Patterns → Completing the GoF Set — Bridge, Composite, Flyweight, Template Method, Iterator, Mediator, Memento, Visitor & Interpreter — *file removed 2026-09-06* (Mediator survives in [`11-Design-Patterns/00-Design-Patterns-Interview-Master-Guide-DotNet-TechLead.md`](../11-Design-Patterns/00-Design-Patterns-Interview-Master-Guide-DotNet-TechLead.md) §4.5; the other eight were dropped as out of scope for the .NET Technical Lead role — recoverable at commit `8deee37`)
...
- ✅ **Module 191**: System Design → Scaling Foundations: From a Single Server to Millions of Users, and the Building-Block Catalogue — [`14-System-Design/21-Scaling-Foundations-SingleServer-To-Millions-BuildingBlocks.md`](../14-System-Design/21-Scaling-Foundations-SingleServer-To-Millions-BuildingBlocks.md) (A9 variant-1 gap-fill. A term-frequency probe of all 20 System-Design files against the ByteByteGo "zero to millions" ladder and the GeeksforGeeks building-block taxonomy found the *foundations layer* missing under an otherwise strong case-study domain: **0 hits** folder-wide for the single-server→millions progression, leader-follower replication topology, read-through caching, forward-vs-reverse proxy, long-vs-short polling, block storage, and named partitioning strategies; CAP appeared in one file and quorum once. Recurring finding named here: **every rung on the scaling ladder converts a capacity problem into a consistency problem** — and the ladder is a history, not a design method, so a rung climbed without a measured bottleneck buys its failure modes for free. Second finding: **in a regulated firm the ladder is reordered** — multi-region arrives at rung 3 for DORA/RTO reasons, and residency law can make sharding illegal rather than merely expensive)
- ✅ **Module 192**: System Design → Designing an Insurance Platform — Policy Administration, Underwriting & Claims — [`14-System-Design/22-Designing-Insurance-Policy-Underwriting-Claims.md`](../14-System-Design/22-Designing-Insurance-Policy-Underwriting-Claims.md) (first module authored under the post-§10 template: §2 carries the full depth including the judgement questions and the module's own discriminating question. Central finding: **the product is a promise conditioned on a past state**, so bitemporality is mandatory rather than a refinement — a claim must be adjudicated against the policy version in force at the *date of loss*, under reference data *as it was*. Second finding, generalising beyond insurance: **when a system's output is an entitlement rather than a transaction, internal consistency proves nothing about correctness** — §4's incident settled 3,100 claims against the wrong policy version for fourteen months with a perfectly balanced ledger, and §14's £14m reserve shortfall hid inside an aggregate whose conservative bias on 97% of claims masked deterioration on the 3% that mattered)
...
### Role-targeted curation: `17-Microservices` Interview Master Guide (2026-09-06)

A review of the nine `17-Microservices` modules against a **14-year .NET Technical Lead / Architect** interview found the folder calibrated above the role: substantial content sits at Principal/Distinguished-Engineer and Head-of-Platform altitude (hyperscale cell sizing and migration, platform-as-a-product org design, LB billing and packet-level detail), while the .NET specifics that decide these interviews (Polly v8 pipelines, `IHttpClientFactory`/`SocketsHttpHandler`, Kestrel timers, `CancellationToken` deadline flow, graceful-shutdown ordering, YARP strangler routing) were thin. A single curated guide was added that keeps what fits, removes what does not with the reason stated, and supplies the missing .NET layer.

- ✅ [`17-Microservices/00-Microservices-Interview-Master-Guide-DotNet-TechLead-Architect.md`](../17-Microservices/00-Microservices-Interview-Master-Guide-DotNet-TechLead-Architect.md) — 18 sections: scope-and-exclusions table, the twelve-question spine, boundaries/communication/cross-boundary data/resilience/observability/contracts/deployment/runtime-infrastructure/blast-radius/Strangler-Fig/leadership, 40 role-calibrated Q&A, six war stories, a 45-minute design drill, a red-flag table, a .NET reference card of packages and defaults, and a two-week revision plan.

This is a **curation of existing modules, not a new module and not a retrofit** — Modules 49, 50, 51, 135–139 and 173 are unchanged and remain the reference behind the guide. It is therefore logged as an unnumbered dated block, following the `CLAUDE.md` §A9 convention for additive work that does not use the main module template. Its distinguishing property versus a coverage-gap closure is that it *narrows* an already-complete domain to one role rather than widening it.

---

### Role-targeted curation: `11-Design-Patterns` Interview Master Guide (2026-09-06)

Second application of the role-targeted curation pattern established for `17-Microservices` the same day. The folder covers all 23 GoF patterns across three modules; a **14-year .NET Technical Lead** interview asks about roughly fourteen of them, plus Repository/Unit of Work, which is not GoF and was not covered here at all. A single focused guide was added covering exactly 15 patterns in depth.

- ✅ [`11-Design-Patterns/00-Design-Patterns-Interview-Master-Guide-DotNet-TechLead.md`](../11-Design-Patterns/00-Design-Patterns-Interview-Master-Guide-DotNet-TechLead.md) — Factory Method, Abstract Factory, Builder, Singleton, Adapter, Decorator, Facade, Proxy, Strategy, Observer, Command, Chain of Responsibility, Mediator, State, Repository/Unit of Work. Each pattern carries a **star rating** for .NET interview frequency and runs a fixed teaching shape: Problem (framed as a concrete "suppose…" scenario) · ASCII diagram · Interface · Implementations · Wiring it up · When to use? · When NOT to use? · **Where .NET already does this** · Classic example (ASCII tree) · Interview Q&A · Don't confuse with. Plus a confusion matrix for the ten classic pattern pairs, a one-page framework-realisation table, three production incidents, a red-flag table and a rated cheat sheet.

**Dropped, with the reason recorded in the guide's §0:** Prototype, Bridge, Composite, Flyweight, Template Method, Iterator, Memento, Visitor, Interpreter (nine GoF patterns that are absorbed into the language, rare in .NET application code, or effectively never asked), plus the C#-vs-Java comparison tables from Modules 31 §2.9 and 32 §2.6 (written for a polyglot panel, not a .NET one).

**Authored fresh:** §5 Repository / Unit of Work — the generic-repository critique, the `DbContext`-is-a-Unit-of-Work / `DbSet<T>`-is-a-Repository argument, when an aggregate-scoped repository does earn its place, the `IQueryable<T>` leak and the Specification alternative, and the Outbox as the unit of work extended past the database.

**The three source modules were then deleted on explicit instruction** — `01-Creational-Structural-Patterns.md` (Module 31), `02-Behavioral-Patterns.md` (Module 32) and `03-RemainingGoF-...md` (Module 186). The folder now contains only the guide. This is the **first deliberate removal of authored modules** in the program, and therefore a precedent worth naming: it is not a template retrofit but a *scope* decision — the domain was narrowed to one role, and the superseded material was removed rather than left alongside. All three files remain recoverable at commit `8deee37` (`git checkout 8deee37 -- 11-Design-Patterns/`).

Inbound links were repaired in the same pass rather than left dangling: prerequisite pointers in `15-Low-Level-Design/01`, `15-Low-Level-Design/02` and `42-Angular/01` now resolve to the guide, and the Progress Log entries for Modules 31, 32 and 186 record the removal in place rather than being deleted (module numbers are never reused — see the 2026-09-03 precedent).

Logged as an unnumbered dated block per `CLAUDE.md` §A9.

---

### Role-targeted curation: `17-Microservices` unified guide (2026-09-06)

Third role-targeted curation of the day, and the first to use a new answer spine. A distributed-patterns guide was written alongside the earlier microservices guide, then **the two were merged into one** on the same day once the overlap became clear — the folder now holds a single 2,675-line guide plus its nine unchanged source modules.

- ✅ [`17-Microservices/00-Microservices-Interview-Master-Guide-DotNet-TechLead-Architect.md`](../17-Microservices/00-Microservices-Interview-Master-Guide-DotNet-TechLead-Architect.md) — 33 sections in four parts: **Part 0** scope, the twelve-question spine and the answer spine; **Part I** foundations (boundaries, communication, cross-boundary data); **Part II** the 14 distributed patterns as a dependency tree with **Saga at the root** (Saga → Outbox / Idempotency / CQRS; Outbox → Kafka → EDA) plus Resilience, Circuit Breaker, Retry, Bulkhead, API Gateway, Service Discovery, Strangler Fig, Sidecar; **Part III** delivery and operations (observability, contracts, deployment, load balancing and the Kestrel seam, blast radius, leadership); **Part IV** 40 questions, ten stories, the design drill, red flags, a .NET reference card and a revision plan.

**The answer spine (new convention).** Every Part II pattern is answered as **Problem → Pattern → Why → Implementation → Trade-off → Failure scenario → Real project example**, on the principle that at Architect level a pattern question tests whether the candidate has *operated* one. Steps 5 and 6 — the cost, and how it fails in production with the metric that detects it — are what the spine exists to force; a mechanism recital without them is a Senior-level answer. Part I and Part III keep the descriptive form, since they are reasoning and operational detail rather than patterns.

**What the merge removed.** The two guides overlapped substantially, and every duplicate was resolved to a single telling: resilience/circuit-breaker/retry/bulkhead now live only as Part II patterns (with the unique deadline-propagation, backpressure, retry-budget and thread-pool-starvation material folded in); Strangler Fig, Service Discovery and the Outbox keep only their 7-step versions; the composition correctness-ceiling reasoning stays in Part I while CQRS keeps the pattern depth; and each production incident is told **once** with every other section cross-referencing it — most notably the shared-ALB idle-timeout vs Kestrel keep-alive 502s, which is simultaneously a gateway-consolidation story and a timer-seam story and is told at §25.7 with §18 pointing to it.

**Grounding.** Failure scenarios and project examples are drawn from the incidents already recorded across the repo — the 502s nobody owned (§25.7), the unbounded-queue congestive collapse (§14.6), the silent cross-service sort under pagination (§11.7), 71% co-change (§4.4), the malformed-config partition-wide outage (§26.3), 94% adoption with zero currency where it mattered (§27.2), the circuit breaker that counted declines (§15.7), the regenerated idempotency key (§16.7), the shared connection pool (§17.7) and the non-sticky cutover (§20.6). Running domain is the same payment platform as the design-patterns guide, so the two compose.

Paired with a companion rewrite of [`11-Design-Patterns/00-Design-Patterns-Interview-Master-Guide-DotNet-TechLead.md`](../11-Design-Patterns/00-Design-Patterns-Interview-Master-Guide-DotNet-TechLead.md), which was re-based onto the same payment domain end to end (every one of the 15 patterns now draws its example from payment methods, gateways, the payment lifecycle or refunds) and cross-links to this guide.

---

### Template change: §10 Interview Questions retired, `14-System-Design/` converted (2026-09-11)

Direct instruction — *"Remove all Interview Questions including basic, intermediate etc. Rather than your explanations will cover everything so I cannot fail any interview"* — retires §10 from the module template (`CLAUDE.md` §A6a) and transfers what it provided into the explanatory prose. Executed across all **21 files** of `14-System-Design/`.

**What was done, per file:** the substance of the 40 questions (roughly 840 across the folder, ~4,500 lines) was absorbed into §2 Deep Dive as named subsections; §10 was then deleted; and every orphaned cross-reference — about 570 pointers of the form `§A5`, `§E7`, `"Advanced Q2"` — was rehomed to the subsection now carrying the material. §2 roughly doubled in most files and is now the centre of gravity of every module in the folder.

**What §2 gained that it did not have before:** interviewer push-backs answered inline rather than parked under "Common mistakes"; judgement material as prose (build-versus-buy, evaluating a colleague's proposal, what to tell a regulator or an executive, when to refuse a request); procedures written out (engine and algorithm migrations, live resharding, incident investigations, rollback sequencing); an honest statement per module of **what the design cannot do and which failures have no detector**; and, in each case study, **the module's own discriminating question** — the one that separates a Staff answer from a Senior one — with both answers worked.

**Defects found and fixed in the same pass**, since every file was read end to end:

- **Five broken cross-references** left by an earlier scripted edit (`to/41/42's`, `(/)`, `generalizing/28's`) in Modules 37, 38, 42 and 43.
- **A datastore contradiction in Module 177**: §3's AWS-mapped diagram specified DynamoDB as the primary store while §12 recommended PostgreSQL, and §2.4 asserted "per §12 the choice is DynamoDB." Resolved by making §3 explicitly the AWS-native rendering, adding §2.17 as the honest comparison, and correcting §2.4.
- **Module 177's §12 brought onto the A7 four-step standard** (scope dialogue, requirements, estimation ending in what the numbers say the hard problem is, component glossary, numbered walkthrough, API and schema tables with status lifecycle, failure-ordered deep dive, wrap-up, numbered references). With 01–14 and 20–21 already compliant, this leaves **Modules 178 and 179 as the folder's only pre-standard §12 sections**.

**Scope:** `14-System-Design/` only. Other folders keep their §10 until separately instructed, so cross-folder references into another domain's Q&A remain valid.

Logged as an unnumbered dated block per `CLAUDE.md` §A9 — no new module numbers were issued.
