...
**One context per unit of work — request, message, or batch chunk.** *Why:* the tracker is unbounded and the identity map goes stale; short lifetimes make both properties harmless. *When not to:* a genuinely long-running batch that must be one transaction — then chunk explicitly and accept the transaction duration as a deliberate decision.

**Default to `NoTracking`; opt in to tracking.** *Why:* the forgotten-`AsNoTracking` failure is silent overhead; the forgotten-`AsTracking` failure is loud and caught by any test. Choose the asymmetry that fails loudly. *When not to:* a codebase that is overwhelmingly write-path, where the opposite default is less error-prone.

**Project to DTOs for reads.** *Why:* it eliminates tracking, snapshots, over-fetching and over-exposure in one move, and it decouples the API contract from the persistence model. It is simultaneously a performance, security (§8) and design practice. *When not to:* when you need the entity to modify it, or when the projection would be a mechanical copy of every field and the entity is small.

**Configure the model with `IEntityTypeConfiguration<T>` classes, applied by assembly scan.** *Why:* `OnModelCreating` becomes a thousand-line method otherwise, and per-entity files match how people actually navigate a domain. *When not to:* trivial models where the indirection is pure ceremony.

**Put estate-wide rules in conventions, not in per-entity configuration.** Decimal precision, string max length, `DateTime` kind, tenant filters, soft-delete filters. *Why:* the enforceability argument from Module 174 §15 — one place, applied automatically to types that do not exist yet. *When not to:* when the rule genuinely has more exceptions than instances.

**Always specify decimal precision and scale for money.** *Why:* the default mapping is provider-dependent and has historically produced silent truncation or rounding on SQL Server (`decimal(18,2)` by convention only in some versions). In a payments or ledger context, a silently truncated fourth decimal place is a financial control failure. *When not to:* never — money always gets an explicit type.

**Use `rowversion`/`xmin` concurrency tokens on every entity where lost updates matter.** *Why:* §2.9 — it is the only lost-update protection that does not require locks, and its absence is an audit finding on limit, balance and position tables. *When not to:* immutable/append-only tables, where there is no update to lose.

**Read the generated SQL for every non-trivial query, in review.** `ToQueryString()` in a test, or provider logging in development. *Why:* the C# is the request; the SQL is what runs. This is the highest-yield practice in this entire module. *When not to:* n/a.

**Apply migrations as an explicit pipeline step, never `Migrate()` at startup.** *Why:* §2.10 — multi-instance races, no change control, no rollback story. *When not to:* single-instance internal tools and local development, where the convenience is worth it and the risk is nil — but keep the production path separate so nobody promotes the convenient one.

**Design every migration to be backward-compatible with the running app (expand → migrate → contract).** *Why:* rolling deploys mean old and new code run simultaneously; a destructive migration makes rollback impossible and turns a bad deploy into an incident. *When not to:* a maintenance window you actually have — rare, and shrinking.

**Keep `DbContext` free of your own mutable instance state if you pool it.** *Why:* §2.2 — a leaked tenant ID is a data breach. *When not to:* if you need per-request state, implement `IResettableService` deliberately or do not pool.

---

## 6. Anti-patterns

**1. `Migrate()` on application startup in a multi-instance deployment.** *Why it fails:* concurrent instances race; schema change is coupled to deploy; no audit trail; rollback of code does not roll back schema. *Fix:* idempotent scripts or migration bundles as a governed pipeline step.

**2. A `DbContext` registered as singleton, or captured by a singleton.** *Why it fails:* unbounded tracker growth, stale identity map, cross-request data bleed, and concurrent-use exceptions under load. *Fix:* scoped registration, `IDbContextFactory<T>` for background work, and leave DI scope validation on.

**3. `Task.WhenAll` over multiple queries on the same context.** *Why it fails:* `DbContext` permits exactly one active operation; the result is an `InvalidOperationException` at best and interleaved state corruption at worst. *Fix:* await sequentially, or create one context per parallel branch from a factory.

**4. Loading entities to update or delete them in bulk.** `foreach (var t in db.Trades.Where(...)) { t.X = y; }` over 100k rows. *Why it fails:* materializes and tracks everything, then `DetectChanges` scans it all, then emits 100k individual statements. *Fix:* `ExecuteUpdate`/`ExecuteDelete` (EF Core 7+), or set-based SQL. Module 176 §2 covers what those bypass.

**5. `SaveChanges` inside a loop.** *Why it fails:* one round trip and one transaction per iteration, plus a full `DetectChanges` scan per iteration over a tracker that keeps growing — quadratic. *Fix:* accumulate and save once per chunk, with a fresh context per chunk.

**6. `Include` everything "just in case."** *Why it fails:* cartesian explosion — including two collection navigations multiplies rows (10 legs × 20 fees = 200 rows to carry one trade), inflating transfer and materialization cost superlinearly. *Fix:* include only what is used; use `AsSplitQuery()` when multiple collections are genuinely needed; prefer projection.

**7. Lazy loading enabled by default.** *Why it fails:* it turns property access into I/O, invisibly, so an innocuous `foreach` over a navigation issues N queries. It also breaks the moment the entity is serialized (the serializer touches every navigation) or the context is disposed. *Fix:* disable it; make loading explicit with `Include` or projection. If you keep it, you have accepted that N+1 is now a property-access-level risk across the whole codebase.

**8. Mutable value objects behind a value converter with no comparer.** *Why it fails:* §2.7 — snapshot by reference means no change is ever detected and the save silently does nothing. *Fix:* immutable value objects, or an explicit `ValueComparer` with a deep-clone snapshot.

**9. `Update(entity)` on a disconnected DTO-mapped entity.** *Why it fails:* marks every property modified, so the `UPDATE` touches every column — overwriting fields the caller never intended to send (a classic mass-assignment data-loss bug when the DTO has fewer fields than the entity), inflating audit records, and firing triggers unnecessarily. *Fix:* load the entity, apply only the fields the operation owns, and save; or `Attach` and mark specific properties modified.

**10. Catching `DbUpdateException` and retrying `SaveChanges` unchanged.** *Why it fails:* entity states are not reset after a failed save, so the retry re-applies; if the original attempt actually committed and only the acknowledgement was lost, you have now double-applied. *Fix:* use the provider's execution strategy for transient faults, make the operation idempotent at the database level (a unique constraint on a business key), and never retry blindly across a transaction boundary.

**11. Using EF Core for reporting aggregations over large tables.** *Why it fails:* it is the wrong engine; translation limits force awkward LINQ, and the resulting SQL is rarely what a DBA would write for a 200-million-row aggregation. *Fix:* the governed escape hatch — a view, a stored procedure, or a columnar store. Module 174 §15.

**12. `EnableSensitiveDataLogging` in production.** *Why it fails:* parameter values land in logs — card numbers, national IDs, account balances — usually shipped to a lower-trust aggregator with broader access than the database. It is a reportable data-protection finding, and it is a one-line config difference from correct. *Fix:* enable in development only, gated by environment, and assert it is off in a production smoke test.

---

## 7. Performance Engineering

*(Module 176 §7 covers query-level tuning — N+1, split queries, indexes, compiled queries. This section covers the costs that come from the **internals** described above.)*

### 7.1 Where the time actually goes, by phase

| Phase | Cost driver | Typical magnitude | Lever |
|---|---|---|---|
| Model building | entity count × property count × conventions | 100 ms – 2 s, **once per process** | compiled models; fewer models per process |
| Query shape compilation | tree size, translation complexity | 0.1 – 5 ms, **once per shape** | stable shapes; avoid unbounded shape churn |
| Query execution | database work + network | dominant for most queries | indexes, projection, narrower results |
| Materialization | rows × columns × converters | ~µs/row; matters at 10k+ rows | project fewer columns; no-tracking |
| Tracking registration | entities × properties (snapshot) | roughly doubles materialization | no-tracking / DTOs |
| `DetectChanges` | **tracked entities × properties** | the §4 killer | track fewer entities |
| Command batching | commands, `MaxBatchSize` | round trips × RTT | batch size tuning; fewer saves |

The single most useful diagnostic instinct: **if the database says the query was fast and the client says the call was slow, the cost is in materialization, tracking, or `DetectChanges` — in that order of likelihood.**

### 7.2 The tracking overhead, quantified

A tracking query holds, per entity: the entity, an `InternalEntityEntry`, and an original-values snapshot array. As a rough rule, a tracked entity costs **2–3× the memory** of the same entity materialized no-tracking, and materialization is perhaps 20–40% slower. Those numbers are unremarkable in isolation; they become decisive because they multiply into `DetectChanges` at save time (§4).

A DTO projection avoids all three costs *and* reduces the columns the database reads and the bytes on the wire. It is the rare optimization that improves every layer simultaneously, which is why "project to a DTO" is the highest-leverage single change in most EF Core codebases.

### 7.3 Batching and round trips

`SaveChanges` batches, but batching is capped and is disabled in specific circumstances (§2.8). The arithmetic that matters: with a 1 ms RTT to the database, 10,000 individual `INSERT`s cost ≥ 10 s in latency alone, while batches of 42 cost ~240 round trips ≈ 0.24 s. That is a 40× difference produced entirely by round-trip count, and it is why "EF Core is slow for bulk inserts" is usually "batching was disabled" or "we called `SaveChanges` in a loop."

Tuning `MaxBatchSize` is genuinely two-sided: larger batches mean fewer round trips but bigger transactions, longer lock hold times, larger log records, and more work lost on a rollback. On a busy OLTP table, a very large batch can cause more contention harm than the round-trip saving is worth. Start at the default; change it only with a measurement and with lock-wait statistics in view.

For genuinely large loads — hundreds of thousands of rows — the answer is not a bigger batch but a different mechanism: `SqlBulkCopy`/`COPY`, or a staging table plus a set-based `MERGE`. Module 176 §2 covers the trade-offs, including what you give up (interceptors, concurrency tokens, generated keys).

### 7.4 Startup cost and cold start

Model building is paid on the first query after every process start. In a Kubernetes estate with frequent rollouts, or in Lambda/Functions with cold starts, this shows up as a P99.9 tail that correlates with deployment and scale-out events rather than with load. Two mitigations: **compiled models** (near-zero model build, at the cost of a regeneration step), and **warm-up** — issuing a trivial query during startup readiness so the first *user* request does not pay it. The warm-up is often the better first move: it costs nothing to implement and it moves the cost to a moment when the load balancer is not yet sending traffic (Module 173 §2.3's `slow_start` is the infrastructure half of the same idea).

### 7.5 Query cache pollution

The query cache is bounded. Unbounded *shape* churn — a distinct query structure per call, or literals inlined from user input — evicts useful entries and forces recompilation, so a system can degrade gradually as its cache hit rate falls. It also pollutes the **database's** plan cache, which is worse: SQL Server's plan cache is a shared resource, and thousands of single-use plans push out plans other applications depend on. Symptoms: rising CPU on the database with no change in query volume, and a falling plan-cache hit ratio. Diagnose with `sys.dm_exec_cached_plans` grouped by `usecounts = 1`.

### 7.6 Measuring EF Core honestly

- **`ToQueryString()`** — see the SQL without running it. First stop, always.
- **Provider logging / `LogTo`** with `DbLoggerCategory.Database.Command` — actual commands and their durations, including how many ran (the N+1 detector).
- **EF Core event counters** (`Microsoft.EntityFrameworkCore` provider in `dotnet-counters`) — active contexts, queries/sec, `SaveChanges`/sec, compiled-query cache hit rate. The cache hit rate is the direct measurement of §7.5.
- **Interceptors** — `IDbCommandInterceptor` to emit per-command telemetry with the calling context; `TagWith("…")` / `TagWithCallSite()` to stamp a comment into the SQL so a DBA looking at `sys.dm_exec_query_stats` can trace an expensive query back to a line of C#. **This one practice does more for cross-team debugging than any amount of documentation** — it converts "some ORM query is hammering us" into a file and line number.
- **BenchmarkDotNet against a real database** for save-path work; in-memory providers measure a different system entirely and will mislead you.

---

## 8. Security

**8.1 Parameterization is automatic — until you opt out.** Every closure-captured value becomes a `DbParameter` (§2.4), so ordinary LINQ is injection-proof by construction. The opt-outs are `FromSqlRaw` and `ExecuteSqlRaw` with a concatenated or interpolated string. `FromSql`/`FromSqlInterpolated`/`ExecuteSql` take a `FormattableString` and convert the holes to parameters — the names differ by one word and the security properties differ completely. Ban the `Raw` variants by analyzer and require a review exception for any use.

**8.2 Identifiers cannot be parameterized.** Table names, column names and sort directions are not parameterizable in SQL. Dynamic ordering or dynamic table selection must go through an **allow-list mapping** from user input to a value you wrote. This is the most common residual injection vector in ORM codebases precisely because the ORM handles everything else.

**8.3 Global query filters are a security control, and they have documented holes.** They apply to LINQ queries over the entity, and they do **not** apply to: `Find()` when it hits the identity map, raw SQL, explicit loading in some paths, and anywhere someone called `IgnoreQueryFilters()`. Treat every `IgnoreQueryFilters()` call site as a security exception requiring justification, enforce with an architecture test, and — for high-sensitivity data — add defence in depth at the database (row-level security, or physical separation per tenant). An application-layer filter fails **open**: if it is not applied, you get more rows, not an error.

**8.4 Over-fetching is a data-protection issue, not just a performance one.** `Select(e => e)` ships every mapped column to whatever serializes it. If the entity has `PasswordHash`, `Pan`, `NationalId` or an internal risk score, those are now in the API response, the log, and the client's cache. DTO projection is **data minimization**, which is an explicit GDPR principle and a PCI-DSS scope-reduction technique: a service that never materializes a PAN is out of scope for controls that would otherwise apply to it.

**8.5 `EnableSensitiveDataLogging` and exception detail.** Sensitive-data logging puts parameter values in logs. `EnableDetailedErrors` puts column and value detail into exception messages, which frequently reach clients through a poorly-configured error handler. Both are development affordances; both are reportable findings in production.

**8.6 Concurrency tokens are an integrity control.** Without them, two concurrent operations silently produce a lost update: operator A raises a credit limit to £1M, operator B lowers it to £100k, last writer wins, and there is no evidence anything went wrong. With a `rowversion`, the second save fails and the application decides. In a limits, balances or positions system this is the difference between a system that can be reconciled and one that cannot.

**8.7 Migrations are a privileged change path.** The identity that applies migrations needs DDL rights; the identity the application runs as should not have them. Using one high-privilege connection string for both means an application-level SQL injection escalates to schema modification and data destruction. Separate them — a migration identity used only by the pipeline, and a least-privilege runtime identity with DML only on the tables it needs. This is standard in banking estates and routinely missing elsewhere.

**8.8 Audit trails belong in the same transaction as the change.** An audit record written outside the `SaveChanges` transaction can be lost while the change commits, producing an unexplained state change — the exact thing the audit exists to prevent. A `ISaveChangesInterceptor` that appends audit entries to the same unit of work gives atomicity for free; the outbox pattern (Module 176 §2) does the same for events.

---

## 9. Scalability

**9.1 Connection pooling is the real limit, and it is smaller than people think.** ADO.NET pools connections per distinct connection string, with a default maximum around 100 on SQL Server. At 3,000 requests/second, each holding a connection for 20 ms, you need ~60 concurrent connections — so the default is adequate until a slow query stretches hold time, at which point the pool exhausts and requests queue for a connection with a timeout error that *looks* like a database problem but is a client-side queue. **`Timeout expired. The timeout period elapsed prior to obtaining a connection from the pool` is a pool-exhaustion message, not a database-slowness message**, and knowing the difference saves hours.

The scalability implication: **hold connections for as short a time as possible.** Do not open a context before doing HTTP work; do not keep a transaction open across a call to another service; do not `await` a message broker inside a database transaction. Every one of those converts a database connection into a resource held for the duration of somebody else's latency.

**9.2 Read scaling.** Route read-only work to replicas with a second `DbContext` (or a second connection string on the same context type via `IDbContextFactory`), and accept eventual consistency explicitly: after a write, the same user's immediate read may not see it. The standard mitigations are read-your-writes routing (send a user to the primary for a short window after their write) or returning the written state directly rather than re-reading. Silent replica reads after writes produce the "I saved it and it disappeared" class of bug reports.

**9.3 Write scaling has no easy lever, so reduce write work instead.** A single primary is the bottleneck by design. The levers, in order of preference: fewer round trips (batching), narrower updates (`Attach` + specific properties instead of `Update`), shorter transactions, set-based operations for bulk changes, and only then sharding. Sharding an EF Core estate means the tenant/partition key must be in every query — which, if you built the tenant filter as a convention (§2.3), you have already done.

**9.4 The tracker bounds your unit of work.** A batch job that loads a million entities into one context will exhaust memory and quadratic-scan itself to death (§2.6). Chunked processing with a **fresh context per chunk** is the standard shape, and it is also what makes the job restartable — each chunk is an independent, idempotent unit with a recorded high-water mark.

**9.5 Multi-tenancy shapes everything.** Three models, with different scaling properties: shared database with a tenant column (cheapest, best density, but noisy-neighbour risk and the filter is the only isolation); schema per tenant (better isolation, model cache key must be widened — §2.3's OOM trap); database per tenant (best isolation and per-tenant restore/DR, but connection-pool fragmentation, since pools are per connection string, and hundreds of tenants means hundreds of pools). Most estates land on shared-with-tenant-column, with database-per-tenant reserved for the largest or most regulated clients — and that hybrid must be designed in from the start, because retrofitting it means changing every connection acquisition.

**9.6 Caching.** EF Core has no second-level cache by design, and that is defensible: an invisible cache with unclear invalidation across a distributed estate causes worse problems than it solves. Cache deliberately, at the application layer, for data whose staleness you can reason about — reference data, FX rates with a known refresh cadence, permission sets — with an explicit TTL and an explicit invalidation path. Reference data cached in a `FrozenDictionary` snapshot (Module 174 §10 A9) removes far more database load than any generic query cache, because it removes the query entirely.
...
**Ideal answer:** Never hold a context — or worse, a transaction — across an external call. The context holds a pooled connection, and an external call's latency is unbounded and outside your control; a downstream slowdown becomes connection-pool exhaustion, which presents as database timeouts across every unrelated endpoint (§9.1). The pattern: open a context, read what you need, dispose or at least complete the unit of work, make the external call, then open a **new** context to write the result. If the read-modify-write must be consistent, use a concurrency token to detect that the world changed while you were away rather than holding a lock across the call. If the write must be atomic with a message publish, use the outbox pattern — write the message to the same transaction and publish asynchronously (Module 176 §2).
