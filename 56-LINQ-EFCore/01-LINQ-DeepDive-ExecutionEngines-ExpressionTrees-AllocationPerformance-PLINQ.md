...
**Filter before projecting, and project before ordering.** Every element removed early is an element the rest of the chain never allocates for, never dispatches a delegate for, and never buffers. *Why it matters:* it is the only optimization that compounds across every downstream operator. *When not to:* when the filter is far more expensive than the projection (a regex or a remote call in the predicate) and the projection is a cheap field read — measure rather than assume.

**Keep the concrete source type as long as possible.** Accept and pass `T[]`, `List<T>` or `Span<T>` in hot code rather than upcasting to `IEnumerable<T>` at the first opportunity. *Why:* §2.2(a) — the fast paths are runtime-type checks. *When not to:* public library APIs, where accepting `IEnumerable<T>` is the correct contract; do the type test internally instead.

**Materialize exactly once, at a deliberate line.** End provider queries with `ToListAsync()`/`AsEnumerable()` written on purpose; end in-memory chains with `ToList()` only where the result is genuinely consumed more than once. *Why:* it makes both the double-enumeration bug and the world-transition visible to a reviewer. *When not to:* when the result is consumed exactly once and streaming saves real memory — then `ToList()` is pure waste.

**Prefer `Any()` over `Count() > 0`, and `TryGetNonEnumeratedCount` when you need a count cheaply.** *Why:* `Any()` short-circuits after one element; `Count()` enumerates unless the source is an `ICollection`. Analyzers CA1827/CA1860 enforce this. *When not to:* never — there is no case where `Count() > 0` is preferable.

**Pass an explicit comparer to every hash-based operator over strings.** `ToDictionary`, `GroupBy`, `Distinct`, `Join`, `Contains`. *Why:* §2.6 — the default is ordinal, real-world feeds are not case-consistent, and the failure is silent. *When not to:* when the key is a genuinely canonical identifier you control end to end, and even then a comment beats an assumption.

**Cache every compiled expression.** A `Compile()` result should live in a static field, a `ConcurrentDictionary`, or a source-generated delegate — never inside a request path. *Why:* §2.7 — compilation is orders of magnitude more expensive than invocation. *When not to:* one-off startup or tooling code where clarity wins and the cost is paid once.

**Use `static` lambdas wherever nothing is captured.** *Why:* the compiler then *guarantees* the delegate is cached and no display class exists; it also makes accidental future captures a compile error rather than a silent allocation regression. *When not to:* when you legitimately need capture — then be deliberate about what is captured and how long the query object lives.

**Give queries a name when they encode a business rule.** `EligibleForNetting(trades)` beats an inline five-operator chain repeated in three services. *Why:* a named method is testable, reusable, and — for `IQueryable` — becomes a reusable `Expression` that composes. *When not to:* single-use, obvious chains; a wrapper per `Where` is its own anti-pattern.

**In `IQueryable` code, check the generated SQL, not the C#.** `ToQueryString()` (EF Core 5+) or a logged command. *Why:* the C# is a *request*; the SQL is what runs. Reviewing LINQ without reading its SQL is reviewing an intention. *When not to:* n/a — this one is unconditional for data-access code.

**Decide the LINQ-vs-loop boundary as a team, by call frequency, and write it down.** *Why:* the §4 incident's real fix. *When not to:* if you have no hot path, don't invent a policy for one.

---

## 6. Anti-patterns

**1. Multiple enumeration of a deferred query.** Passing `IEnumerable<T>` around and calling `.Count()` then `foreach`ing it re-runs the whole chain — twice the CPU, twice the database round trips, and *different results* if the source is volatile. *Why it fails:* the query is a recipe, not a result. *Fix:* materialize once at the boundary and pass `IReadOnlyList<T>`; enable the analyzer that flags possible multiple enumeration.

**2. `.ToList()` immediately followed by more filtering.** `db.Trades.ToList().Where(t => t.Desk == desk)` downloads the table and filters in memory. *Why it fails:* it forces the one-way door before the predicate. *Fix:* move every translatable operator above the materialization line. In review, treat any `.ToList()` that is not the last meaningful call as a defect until proven otherwise.

**3. `.Count() > 0`, `.Where(p).Count()`, `.OrderBy(k).First()` written as `.OrderBy(k).ToList()[0]`.** *Why they fail:* each discards a fast path the BCL already implements. *Fix:* `Any()`, `Count(p)`, `MinBy`/`First()`.

**4. `Compile()` inside a loop or request handler.** *Why it fails:* §2.7 — invoking a compiler per row. Symptom is CPU-bound latency with no obvious hotspot in your own code and heavy `System.Linq.Expressions` frames. *Fix:* cache; if the tree varies structurally, key the cache on the structural parts and parameterize the rest.

**5. Returning `IQueryable<T>` across an architectural boundary.** *Why it fails:* §2.8 — it exports a lifetime and a translatability constraint the caller cannot see. *Fix:* return materialized results or a `Specification<T>`; if you must return `IQueryable`, keep it inside the persistence assembly with an analyzer or architecture test enforcing it.

**6. Side effects in LINQ operators.** `Select(x => { Log(x); return Map(x); })` or a `Where` that mutates. *Why it fails:* deferred execution means side effects fire at an unpredictable time, an unpredictable number of times (zero, once, or n on repeated enumeration), and possibly on another thread under PLINQ; worse, cheap-count fast paths (§2.2c) can skip your selector entirely, so the log line silently disappears. *Fix:* `foreach` for effects, LINQ for values.

**7. `GroupBy` over a huge sequence "to stream" it.** *Why it fails:* §2.5 — it buffers everything before yielding the first group. *Fix:* pre-sort at the source (or read from an already-ordered file/table) and chunk manually, or aggregate into a dictionary in one pass.

**8. `AsParallel()` sprinkled on a slow query.** *Why it fails:* §7.4 — partitioning, merging and delegate overhead usually exceed the work per element, ordering requirements serialize the merge, and the thread pool is now competing with your request-handling threads. *Fix:* measure first; parallelize only CPU-bound work with large per-element cost and no ordering requirement.

**9. Assuming `OrderBy` on the database matches `OrderBy` in memory.** *Why it fails:* SQL Server's collation-based ordering, `NULL` placement, and case sensitivity differ from .NET's ordinal/culture comparisons. A test that sorts in memory and an implementation that sorts in SQL will disagree on `"a"` vs `"B"`, and on where nulls land. *Fix:* decide where ordering happens, keep it there, and assert against the real engine.

**10. Building predicates in a `for` loop without a loop-local copy.** *Why it fails:* §2.4 Trap 2 — shared display class, last value wins. *Fix:* `var localLimit = limits[i];` inside the loop body, then capture that. Compiler warnings do not catch this.

**11. Using LINQ to express what an index already answers.** Scanning a `List<Trade>` with `.First(t => t.Id == id)` inside a loop is O(n·m) where a `Dictionary` lookup is O(n+m). *Why it fails:* it hides an accidental quadratic behind readable code — the most common cause of "it was fine in UAT with 500 rows." *Fix:* build a lookup outside the loop, or use `ToLookup`/`ToDictionary` once.

---

## 7. Performance Engineering

### 7.1 The allocation budget of a LINQ query, itemized

For `list.Where(x => x.V > limit).Select(x => x.Id).ToList()` over a `List<Trade>`:

| Allocation | When | Size (approx.) | Avoidable? |
|---|---|---|---|
| Display class holding `limit` | Query construction | 24 B | Yes — make the bound a field/const, or use `static` lambda |
| `Func<Trade,bool>` delegate | Query construction | 64 B | Yes — cached if no capture |
| `Func<Trade,int>` delegate | Query construction | 64 B | Yes — no capture here, so already cached |
| Fused `WhereSelectListIterator` | Query construction | 48 B | No (one object for both stages) |
| Result `List<int>` + backing array | `ToList()` | 32 B + 4·n | No — it is the result |
| `IEnumerator` boxing | — | 0 | Already avoided by list specialization |

Total fixed overhead ≈ **200 bytes per query construction**, independent of n. That is irrelevant at 100 queries/second and material at 500k/second. **The correct question is never "how much does LINQ cost" but "how many times per second do I pay the fixed cost, and how many elements amortize it."**

### 7.2 Per-element costs, ranked

1. **Delegate invocation** — an indirect call per element per operator. Dynamic PGO (on by default since .NET 8) can guarantedly devirtualize and inline a *monomorphic* call site, which sometimes erases this entirely; a call site that sees several different lambdas stays polymorphic and does not benefit. This is why microbenchmarks of a single query overstate real-world LINQ performance in a codebase where the same generic operator instantiation is shared.
2. **Interface dispatch on `IEnumerator<T>.MoveNext()`/`Current`** — avoided on array/list fast paths, paid otherwise.
3. **Bounds checks** — the JIT elides them in simple indexed loops; through an enumerator abstraction it usually cannot.
4. **Boxing of struct enumerators** — `List<T>.Enumerator` is a struct used directly by `foreach`, but any path through `IEnumerable<T>` boxes it: one heap allocation plus a permanent loss of inlining.
5. **Cache behaviour** — a `Select` producing new reference-type objects turns a sequential array walk into pointer chasing. In §4 this was the difference between a 250k-element linear scan and 250k allocations plus 250k cache misses.

### 7.3 When LINQ genuinely beats a hand-written loop

Worth saying in an interview, because it inverts the expected answer and shows you have measured rather than absorbed folklore:

- **`int[].Sum()`/`Min()`/`Max()`/`Contains()`** — vectorized fast paths (§2.2e) beat a scalar `for` loop, often by 4–8×. Most engineers' hand-written loop is *slower* than the BCL.
- **`OrderBy(k).Take(n)`** — the partial-sort path beats "sort the whole thing then take", which is what most hand-rolled versions do.
- **`ToArray()` on an unknown-length source** — the segmented builder beats naive `List<T>` growth plus `ToArray()` because it avoids the final double-copy.
- **`Enumerable.Range(...).Sum()`** and friends — closed-form and O(1)-count fast paths.

The generalization: **the BCL has already implemented the optimization you were about to write, but only on paths it can recognize.** Your job is to not obscure the source type, not interpose a custom iterator, and measure.

### 7.4 PLINQ: the mechanics and the narrow window where it wins

`AsParallel()` partitions the source, runs the operator chain on thread-pool threads, and merges. The details that decide whether it helps:

- **Partitioning strategy is chosen for you**: *range* partitioning for indexable sources (arrays, `IList<T>`) — cheap and balanced only if per-element cost is uniform; *chunk* partitioning for opaque `IEnumerable<T>` — adaptive chunk sizes, handles skew better, costs synchronization; *hash* partitioning is **forced** by `GroupBy`, `Join`, `Distinct` and friends, because equal keys must land on the same worker — that is a full repartition and it is expensive; *striped* for `Take`/`Skip`.
- **Merging is buffered by default** (`AutoBuffered`). `WithMergeOptions(ParallelMergeOptions.NotBuffered)` reduces latency to first result; `FullyBuffered` maximizes throughput for `ToList()`-style consumption.
- **`AsOrdered()` is not free** — it forces the merge to reassemble original order, adding buffering and often erasing the parallel gain. If you need order, prefer to sort the parallel *result*.
- **`WithDegreeOfParallelism(n)`** matters in a server: PLINQ competes with ASP.NET Core's request handling for the same thread pool. A parallel report query on a busy API node can induce **thread-pool starvation** — the incident in §14.
- **Exceptions arrive as `AggregateException`**, and `WithCancellation(token)` is the only clean way to stop early. `ForAll` skips the merge but abandons ordering entirely.

**The rule of thumb worth quoting:** parallelize when *(elements × per-element cost)* is large — practically, tens of milliseconds of genuine CPU work minimum — the work is CPU-bound (never for I/O; use `Task`/`Channel`/`Parallel.ForEachAsync` instead), ordering is not required, and you are not on a request-handling thread pool that has other jobs. Outside that window PLINQ is reliably a pessimization, and interviewers know it.

### 7.5 Benchmarking LINQ honestly

BenchmarkDotNet with `[MemoryDiagnoser]`, and four traps to name:

1. **Dead-code elimination** — a benchmark that builds a query and never enumerates it measures nothing. Return or `Consume()` the result.
2. **Benchmarking construction instead of execution** — if the query is built inside `[Benchmark]` and enumerated once over 10 elements, you are measuring §7.1's fixed cost, not throughput.
3. **Unrepresentative source types** — benchmarking over `T[]` and shipping over `IEnumerable<T>` measures a code path you do not run.
4. **Tiered JIT / PGO warmup** — the first iterations run tier-0 code and un-profiled call sites; BenchmarkDotNet handles this, an ad-hoc `Stopwatch` loop does not, and the difference is routinely 3–10×.

### 7.6 Alternatives, and when to reach for them

| Approach | Use when | Cost |
|---|---|---|
| `foreach` + accumulator | Hot path, one pass, few operators | Verbosity |
| `Span<T>` / `CollectionsMarshal.AsSpan(list)` | Contiguous data, no allocation tolerated | No LINQ operators; manual code |
| `SearchValues<T>`, vectorized helpers | Membership tests over a fixed set | Setup cost, .NET 8+ |
| Struct-enumerator LINQ libraries | Allocation-free operator chains needed | Third-party dependency, generic bloat, longer builds |
| PLINQ | §7.4's narrow window | Thread-pool contention |
| Push the work to the database | Data is in the database | Round trip, translation limits |

---

## 8. Security

LINQ is not an attack surface by itself, but it sits directly on top of two that are: **query translation** and **user-supplied query shape**.

**8.1 Injection through dynamic LINQ.** Libraries that parse strings into expression trees (`System.Linq.Dynamic.Core`, home-grown OData-ish filters) accept user text and produce executable predicates. This is *code injection by design*, and the only safe posture is an allow-list: permitted property names, permitted operators, permitted value types, with a length cap on the expression. `"Notional > 0 && Password.StartsWith(\"a\")"` is a legitimate parse and a **blind data-exfiltration oracle** — a classic way to read a column the API never returns, one character at a time. Treat any dynamic-filter endpoint as a security-reviewed component, not a convenience feature.

**8.2 Parameterization is a security property, not just a plan-cache property.** In the `IQueryable` world, closure capture (§2.7) produces parameters and therefore immunity to SQL injection. String-interpolated raw SQL (`FromSqlRaw($"… WHERE Id = {id}")`) breaks it; `FromSqlInterpolated`/`FromSql` with a `FormattableString` re-establishes it by turning holes into `DbParameter`s. The distinction is one method name and it is the whole difference between parameterized and concatenated SQL.

**8.3 Untrusted paging and ordering parameters.** `OrderBy(userSuppliedColumnName)` implemented by building an expression from a string is the same injection class as 8.1 with a smaller blast radius. Map user input through a dictionary of allowed sort keys → expressions. Similarly, an unbounded `Take(pageSize)` from the query string is a **denial-of-service parameter**: clamp it server-side, always.

**8.4 Over-projection leaks data.** `Select(u => u)` on an entity ships every column — including `PasswordHash`, `NationalId`, internal risk scores — to whatever serializer sits downstream. Projecting to an explicit DTO is a data-minimization control that satisfies GDPR/DPA scrutiny and PCI-DSS scope reduction, not just a performance tip. In a regulated estate this is the difference between an in-scope and out-of-scope service.

**8.5 Regex and other expensive predicates in `Where`.** A user-supplied pattern evaluated per element is catastrophic backtracking multiplied by n. Use `RegexOptions.NonBacktracking` or a timeout, and never accept the pattern itself from a client.

**8.6 Exception messages leak schema.** A translation failure or a `FormatException` from a projection can surface column names, provider details and SQL fragments. `EnableSensitiveDataLogging` must never be on in production — it puts **parameter values**, meaning PII and card data, into logs that are typically shipped to a lower-trust log aggregator. This is a recurring audit finding at payments firms.

**8.7 Timing side channels.** `SequenceEqual` and `==` over secrets short-circuit on the first differing element. Comparing an API key or HMAC with LINQ is a timing oracle; use `CryptographicOperations.FixedTimeEquals`.

---

## 9. Scalability

**9.1 Scale the query shape before scaling the machine.** The §4 incident scaled from 180 ms to 41 ms by changing operator order — a 4× improvement no amount of vertical scaling would have delivered, at zero infrastructure cost. Query-shape work is almost always the cheapest scalability lever available and the first one an interviewer expects you to name.

**9.2 Memory is the scaling limit for in-memory LINQ, and buffering operators set it.** A streaming chain over a 40 GB file is O(1) memory and scales indefinitely; the same chain with an `OrderBy` in the middle requires 40 GB of RAM plus overhead. Scaling therefore means **moving buffering operators to a system built for them**: sort in the database, sort in the file system (external merge sort), or arrange for the source to arrive pre-ordered. This is the same lesson as Module 140's stream-processing state stores, arrived at from the single-process side.

**9.3 Partitioning: the same idea at three altitudes.** PLINQ partitions across cores; a consumer group partitions across processes (Module 139); a sharded database partitions across nodes. All three break on the same thing — **skew** — and all three are broken by the same operators, the ones requiring a global view (`GroupBy` across the whole key space, `Distinct`, `OrderBy`, `Join` on a non-partition key). If a LINQ query cannot be parallelized because it needs a global view, that same query will not shard either. Recognizing this makes "can we parallelize this report?" and "can we shard this table?" the same conversation.

**9.4 Backpressure.** `IEnumerable<T>` is a **pull** model and is therefore naturally backpressured: the consumer's `MoveNext()` sets the pace, so a slow consumer cannot be overwhelmed. `IObservable<T>` is push and is not. `IAsyncEnumerable<T>` is pull with awaits, which is why it is the right abstraction for streaming a large result set out of a database or an HTTP response without buffering — and why `await foreach` over an EF Core query is a genuinely different scalability profile from `ToListAsync()`. `System.Threading.Channels` is the right tool when producer and consumer must be decoupled with a bounded buffer.

**9.5 Streaming large HTTP responses.** Returning `IAsyncEnumerable<T>` from an ASP.NET Core action streams JSON as rows arrive: constant server memory, first byte in milliseconds. Returning `ToListAsync()` buffers the entire result in memory before serializing — and a 500k-row export then produces a Gen2/LOH allocation spike and a latency cliff. The trade-off is real, though: streaming means **the status code is already sent** when a mid-stream failure occurs, so a partial response cannot be turned into a 500. Choose deliberately; for exports, a pre-signed object-store URL is usually better than either.

**9.6 Multi-tenant fairness.** One tenant's unbounded query — no `Take`, an `OrderBy` over their largest table — consumes shared memory, thread-pool threads and database connections belonging to everyone. Per-tenant limits (max page size, max rows scanned, query timeout, a bulkhead on the report path) are a scalability control, not a nicety. Module 176 §9 revisits this with EF Core's actual knobs.
...
- **Operational overhead:** migration management needs a real process (Module 176 §5).
