...
Manual acknowledgment mode, while necessary for reliability (§5), introduces per-message acknowledgment network round-trip overhead compared to auto-ack — mitigated by acknowledging messages in **batches** (acknowledging multiple consumed messages with a single ack call, using AMQP's multiple-message ack capability) rather than individually, trading a small amount of at-least-once redelivery blast radius (a crash before a batched ack redelivers the whole unacknowledged batch, not just one message) for meaningfully reduced acknowledgment overhead — a direct, RabbitMQ-specific instance of Module 54 §7's general batching-trade-off pattern. Queue length itself has performance implications distinct from Kafka's partition-log model — an unbounded, rapidly-growing RabbitMQ queue (consumers falling behind producers) degrades broker memory/performance more directly than Kafka's disk-based log growth, making queue-length monitoring and alerting a more immediately performance-critical concern in RabbitMQ than in Kafka.

## 8. Security
RabbitMQ supports TLS for encryption-in-transit and a permission model scoped per virtual host (vhost) — a lightweight multi-tenancy mechanism allowing logically-isolated groups of exchanges/queues/bindings within a single broker cluster, with per-vhost user permissions — directly useful for isolating unrelated applications/teams sharing infrastructure without requiring entirely separate broker clusters, while still requiring the same "never assume internal traffic is trusted" discipline (Module 49 §8, Module 54 §8) applied to inter-service RabbitMQ connections specifically. Dead Letter Queue contents (§2.6) carry the same sensitive-data-handling requirement Module 53 §8 established generically — a DLQ containing a failed payment-notification message, as in §4's scenario, holds exactly the same sensitive payload as the original message and must be governed accordingly.

## 9. Scalability
RabbitMQ scales primarily via clustering (multiple broker nodes) combined with either queue mirroring/replication (for durability across node failures) or, in more modern deployments, quorum queues (a Raft-based replication mechanism, directly reusing Module 47's consensus concepts, providing stronger consistency guarantees than the older mirrored-queue model) — but a single queue's throughput ceiling is fundamentally different from Kafka's partition-based parallelism model (Module 54 §9): RabbitMQ queues don't have an inherent "partition count," so scaling consumer parallelism for a single logical stream of work typically means running multiple competing consumers against the **same** queue (RabbitMQ delivers each message to only one of them, round-robin by default) rather than Kafka's explicit, partition-bounded consumer-group model — a materially different mental model for reasoning about maximum achievable parallelism.
