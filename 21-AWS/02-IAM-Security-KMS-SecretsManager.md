...
STS `AssumeRole` calls and Secrets Manager retrieval calls both introduce a network round-trip and should be **cached** by the calling application for their credential/secret's valid lifetime rather than called on every single request — a naive implementation calling Secrets Manager on every incoming request (rather than caching the retrieved secret in memory until its rotation-driven invalidation) introduces unnecessary latency and can hit Secrets Manager's API rate limits under load, a concrete, easy-to-overlook performance cost of an otherwise-correct security pattern. KMS `Decrypt` calls similarly have a per-call cost and rate limit — for high-throughput encryption/decryption workloads, using **envelope encryption** (KMS encrypts a local data key once, which is then used to encrypt/decrypt data locally without a KMS API call per operation) avoids both the latency and the rate-limit exposure of calling KMS directly for every individual object.

## 8. Security
The explicit-deny-always-wins rule (§2.1) is the correct way to implement a genuinely non-bypassable restriction — e.g., a Service Control Policy (an organization-wide policy applied across all accounts) using an explicit Deny is the mechanism to enforce an absolute organizational rule (such as "no IAM User access keys may ever be created") that individual account administrators cannot override via their own more-permissive identity-based policies. KMS's two-factor access model (§2.5) should be treated as a deliberate defense-in-depth layer specifically for the most sensitive data classes — using separate, more-tightly-scoped KMS keys (rather than a single account-wide default key) for genuinely sensitive data (PII, payment data) ensures that even a broad IAM compromise on the underlying resource doesn't automatically grant decrypt capability, directly connecting to Module 57 §8's defense-in-depth discussion, now expressed as a data-encryption-layer control rather than a network-layer one.

## 9. Scalability
As an organization grows to many teams and many AWS accounts, a flat, ad hoc IAM structure (individually-crafted policies per role, no shared governance mechanism) becomes unmanageable and is exactly where the §4 incident pattern (permissive-policy accumulation with no enforcement) tends to recur at increasing scale — the scalable pattern is Permission Boundaries (§2.3) plus centrally-managed policy templates plus automated policy-linting/Access-Analyzer checks in the deployment pipeline, so that least-privilege is enforced structurally at the point a new role/policy is created, rather than relying on distributed, inconsistent manual review across many independent teams. Secrets Manager and KMS both have account-level API rate limits and resource-count quotas that, like Module 57 §9's ASG/subnet capacity mismatch, must be proactively tracked as the number of services, secrets, and encrypted resources grows — a sudden secrets-retrieval spike (many services scaling out simultaneously, each fetching a secret on cold start, §7) can hit these limits if not anticipated and mitigated with caching.
