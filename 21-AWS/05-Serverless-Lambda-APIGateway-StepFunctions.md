...
Cold-start latency is directly influenced by deployment package size and runtime choice — a Lambda function with a large deployment package (many bundled dependencies) or a runtime with inherently slower initialization (historically, JVM-based runtimes versus lighter interpreted/compiled runtimes) will have measurably longer cold starts, meaning minimizing package size (removing unused dependencies, using Lambda layers for shared code judiciously) is a legitimate, concrete performance lever, not just a code-hygiene concern. Provisioned concurrency (§2.2) directly trades continuous cost for eliminated cold-start latency — the correct sizing question is not "how much provisioned concurrency can we afford" but "what is our actual expected baseline concurrent traffic, with headroom for typical variance," since under-provisioning still leaves a residual cold-start-prone tail during traffic above the provisioned level, while over-provisioning pays for capacity that's rarely actually used.

## 8. Security
API Gateway's authentication/authorization integration (Cognito, IAM, Lambda authorizers) should be the primary enforcement point for request-level access control before a request ever reaches Lambda — but Lambda functions should still independently validate any security-relevant assumption about the request (never assume "API Gateway already validated this, so the Lambda function doesn't need to check anything," directly Module 50 §8's defense-in-depth-at-every-layer discipline, since a misconfigured API Gateway authorizer or an internal invocation path bypassing API Gateway entirely would otherwise leave the Lambda function with no independent protection). Lambda's execution role (an IAM role, per Module 58 §2.2) should be scoped per-function to exactly the specific resources/actions that function needs — a shared, overly broad execution role reused across many functions recreates Module 58 §4's exact incident pattern (a single function's vulnerability inheriting an entire shared role's permission set as its blast radius), now at the serverless-function granularity.

## 9. Scalability
Lambda's near-instantaneous concurrency scaling is a genuine strength for absorbing traffic spikes, but is simultaneously the mechanism by which a downstream, more-slowly-scaling dependency (a traditional RDS/Aurora database, a rate-limited third-party API) can be overwhelmed far faster than that same traffic spike would have overwhelmed a traditional ASG-fronted fleet (whose own scaling is comparatively gradual, per Module 57 §2.5) — reserved concurrency (§2.2) is the direct mitigation, deliberately capping a function's maximum concurrency to match what its downstream dependencies can actually absorb, a genuinely serverless-specific capacity-planning exercise with no direct equivalent in traditional compute. Step Functions' own execution-history limits and per-state-transition rate limits (an account-level quota) should be capacity-planned for any very-high-volume workflow the same way Module 57 §9 capacity-plans AWS service quotas generally — a high-throughput, per-request Step Functions workflow can hit these limits well before the underlying Lambda functions it orchestrates hit any limit of their own.
