...
- Configure load-balancer health checks to validate actual readiness (dependency connectivity, warm-up completion), not just process liveness, especially for any workload where new-instance warm-up time is non-trivial.
...
- Using a liveness-only health check where a readiness check is actually needed, routing live traffic to instances that are alive but not yet correctly able to serve requests.
...
### 7.1 Cross-AZ Data Transfer — the Hidden Latency and Cost Tax
Traffic between resources in **different** AZs within the same Region incurs both a small but real latency penalty (typically sub-millisecond to low-single-digit milliseconds versus same-AZ traffic, which is effectively negligible) and a per-GB data-transfer **cost** that same-AZ traffic does not — a chatty microservice architecture (Module 17's service-to-service call patterns) deployed naively across AZs without AZ-awareness can silently accumulate meaningful cross-AZ transfer costs and latency at high request volume. The mitigation is **AZ-aware routing**: an ALB's default behavior is to distribute evenly across all registered targets regardless of AZ, but enabling **cross-zone load balancing** deliberately trades a small latency/cost increase for better load distribution, while for latency-sensitive service-to-service calls, some architectures deliberately colocate a calling service and its most-frequently-called dependency in the same AZ (topology-aware routing) — the decision is a genuine trade-off between even load distribution and minimizing cross-AZ cost/latency, not a default either way.

### 7.2 Load Balancer Idle Timeout vs. Backend Keep-Alive — a Latency/Correctness Interaction
An ALB's **idle timeout** (default 60 seconds) closes a connection if no data is sent for that period; if a backend's own HTTP keep-alive timeout is **shorter** than the ALB's idle timeout, the backend can close a connection the ALB still considers open, and the next request the ALB attempts to send over that connection fails with a 502 — this is a latency-optimization feature (connection reuse avoids repeated TCP/TLS handshake overhead) that becomes a correctness bug when the two timeout values aren't reconciled. The fix is mechanical but easy to miss: the backend's keep-alive timeout must always be configured **longer** than the load balancer's idle timeout, never shorter or equal.

### 7.3 NAT Gateway Throughput and Connection-Tracking Limits
A NAT Gateway supports up to 55,000 simultaneous connections per unique destination IP/port, and roughly 5 Gbps of baseline throughput per gateway (bursting higher, but not unbounded) — a private-subnet workload making a very high volume of outbound connections to a small number of external endpoints (a payment-gateway integration hammering one third-party API host) can exhaust the per-destination connection limit well before it exhausts overall bandwidth, manifesting as intermittent outbound connection failures under load that look like a downstream provider issue but are actually a NAT Gateway connection-tracking ceiling. Mitigation: multiple NAT Gateways (one per AZ is already standard for resilience, but genuinely high-volume workloads may need additional NAT Gateways per AZ) or, for AWS-service-to-AWS-service traffic specifically, routing via a **VPC Endpoint** (§8.3) to bypass the NAT Gateway entirely for that traffic class.

### 7.4 Auto Scaling Responsiveness — Cooldown, Warm Pools, and Predictive Scaling
A naive target-tracking scaling policy reacts to a CloudWatch metric that itself lags real-time load by the metric's aggregation period (commonly 1–5 minutes), then launches an instance that itself takes the warm-up time discussed in §4's incident before it's genuinely useful — for a workload with sharp, fast-onset traffic spikes (a flash sale, a market-open trading surge), this reaction latency can be the dominant driver of the error-rate spike, not insufficient eventual capacity. Three levers address this, and a mature production setup layers more than one: (1) a **warm pool** — pre-initialized (but not traffic-serving) instances kept ready in a "Stopped" or "Running" state so scale-out latency drops from "boot + application warm-up" to just "application warm-up" or less; (2) **predictive scaling**, which uses historical load patterns (a Monday-morning volume pattern, a known market-open surge) to pre-scale ahead of an anticipated spike rather than reacting after the metric crosses a threshold; (3) tuning the metric's own evaluation period tighter for workloads that can tolerate more sensitive (noisier) scaling decisions in exchange for faster reaction.

### 7.5 Elastic Network Interface (ENI) and IP Address Exhaustion at Scale
Each EC2 instance consumes at least one private IP from its subnet's CIDR block (per §Advanced Q5/Q3 of §10), and instance types have a maximum number of ENIs/IPs they can attach — at very high instance density (large ASGs, or container platforms like ECS running in `awsvpc` networking mode where each **task**, not just each instance, consumes an IP), subnet IP exhaustion becomes a real, silent scaling ceiling well before compute or memory limits are reached. Production capacity planning for high-density compute must explicitly reconcile subnet CIDR sizing against the actual per-instance or per-task IP consumption rate, not just instance count.

---

## 8. Security

### 8.1 Security Groups vs. Network ACLs — Stateful vs. Stateless, and Why Both Exist
A **Network ACL (NACL)** operates at the **subnet** level (not per-instance) and is **stateless** — unlike a Security Group, a NACL requires an explicit rule permitting *both* the inbound request *and* the outbound response independently, and it supports explicit **Deny** rules evaluated in numbered order (first match wins), unlike a Security Group's allow-list-only model. NACLs are a coarser, subnet-wide layer typically used for broad, static rules (blocking a known-malicious CIDR range subnet-wide) while Security Groups provide the fine-grained, per-instance, application-aware layer — using a NACL to implement fine-grained per-service rules is an anti-pattern (its statelessness makes this error-prone and hard to reason about); using only Security Groups with no NACL-level blocking of known-bad traffic forfeits a genuinely independent, coarser defense-in-depth layer that survives even if every Security Group in the subnet is individually misconfigured.

### 8.2 Blast Radius of a Security Group Misconfiguration
Because a Security Group is often shared across every instance in an ASG (or even across multiple services, per §4's Production Example pattern from the sibling IAM module), a single overly-broad ingress rule doesn't just expose one instance — it exposes every instance the Security Group is attached to, simultaneously, for as long as the misconfiguration persists. The practical mitigation mirrors §Advanced Q6's IAM-side automated-audit pattern: an automated Security Group audit (AWS Security Hub, IAM Access Analyzer's network-reachability findings, or a third-party CSPM tool) that flags any rule permitting `0.0.0.0/0` on a non-load-balancer, non-genuinely-public resource, converting a manual-review-dependent control into a standing, non-bypassable gate — because, exactly as with IAM policy scope, "we'll tighten this later" is the pattern that lets an overly broad rule persist until an unrelated vulnerability turns it into an actual incident.

### 8.3 VPC Endpoints — Removing the Internet (and NAT Gateway) from the Trust Path Entirely
A **VPC Endpoint** (Gateway Endpoint for S3/DynamoDB, or Interface Endpoint — backed by AWS PrivateLink — for most other AWS services) allows a private-subnet resource to reach an AWS service **without traversing the internet or a NAT Gateway at all** — traffic stays entirely within the AWS network. This is both a security improvement (the traffic never has a path where it could be exposed to internet-level interception or requires an internet-reachable NAT Gateway to exist) and, per §7.3, a NAT Gateway throughput/connection-tracking relief valve for AWS-service-bound traffic specifically. Any private-subnet workload making heavy use of AWS APIs (S3, DynamoDB, Secrets Manager, KMS — directly the sibling IAM module's services) should use VPC Endpoints for that traffic rather than routing it out through a NAT Gateway to the public AWS API endpoint, both for the security posture and the throughput/cost benefit.

### 8.4 DDoS and Layer-7 Protection at the Load Balancer Boundary
The ALB (or CloudFront in front of it) is the natural enforcement point for **AWS Shield** (automatic, always-on DDoS protection at the network/transport layer, included at no extra cost for Shield Standard) and **AWS WAF** (Layer-7 rule-based filtering — rate-based rules, SQL-injection/XSS pattern matching, IP-reputation lists) — a production-facing ALB for any workload handling financial transactions should have WAF attached with rate-based rules scoped to the specific abuse patterns relevant to that endpoint (e.g., aggressive rate limiting on a login or payment-submission endpoint specifically, not a uniform rate limit across the whole application) — this is the network/infrastructure-layer complement to the later dedicated Security module's application-layer input validation and rate-limiting discussion, not a substitute for it.

### 8.5 Bastion Hosts vs. Session Manager — Eliminating the SSH Attack Surface Entirely
A traditional bastion host (a hardened, internet-facing EC2 instance used as an SSH jump box into private-subnet instances) is itself a standing, internet-exposed attack surface and a long-lived-credential (SSH key) management burden. **AWS Systems Manager Session Manager** eliminates this: it establishes a secure, IAM-authenticated, audit-logged shell session to a private-subnet instance with **no open inbound port at all** (no SSH port, no bastion host, no security-group rule permitting inbound SSH from anywhere) — the instance only needs outbound connectivity to the SSM service (via VPC Endpoint, §8.3, for a fully private-subnet setup with no NAT dependency either). This is a direct, concrete instance of §8.3's "remove the internet from the trust path" principle applied to operator access specifically, and is the AWS-native answer to "how do you access a private-subnet instance for troubleshooting" that a Principal Engineer should default to over a bastion host.

---

## 9. Scalability

### 9.1 Horizontal vs. Vertical Scaling at the Infrastructure Layer
Vertical scaling (moving to a larger EC2 instance type) has a hard ceiling (the largest available instance type) and requires a stop/resize/start cycle causing brief unavailability for that instance — horizontal scaling (more instances via the ASG) has no comparable hard ceiling (bounded only by account service quotas and subnet IP capacity, §7.5) and — because it's what an ASG already does automatically — is the default scaling dimension for stateless compute. Vertical scaling remains relevant specifically for workloads with a genuine single-instance bottleneck that horizontal scaling can't address (a single-writer database primary, discussed in the sibling SQL Server/PostgreSQL modules) — the two are not competing defaults but apply to different workload shapes.

### 9.2 Multi-Region Architecture — Beyond Multi-AZ
Per §Advanced Q4, multi-AZ protects against AZ-level failure but not Region-level failure. A genuine multi-region architecture requires: infrastructure replicated in a second Region (a second VPC, its own ASG/ALB stack), a data-replication strategy for anything stateful (the sibling database modules' replication discussion), and a traffic-routing mechanism to redirect users to a healthy Region — either **Route 53** with health-check-based failover routing (DNS-level, with a TTL-bounded propagation delay before clients pick up the change) or **AWS Global Accelerator** (anycast-IP-based, routing at the network layer with faster failover than DNS TTL expiry allows, and additionally providing consistent entry-point IPs and improved latency via AWS's global network backbone even in the non-failover case). The choice between them is a genuine trade-off: Route 53 failover is simpler and cheaper for workloads that can tolerate DNS-propagation-speed failover; Global Accelerator is the correct choice when failover speed genuinely matters (a trading or payment-processing workload where minutes of DNS propagation delay during a Region outage is unacceptable) or when the latency/throughput improvement from AWS's backbone routing itself is valuable independent of failover.

### 9.3 High Availability vs. Disaster Recovery — RTO/RPO as the Governing Metrics
Multi-AZ deployment is a **high-availability** mechanism (minutes-or-less automatic recovery from a common, comparatively frequent failure class); multi-region is a **disaster-recovery** mechanism (a lower-probability, higher-impact event, with recovery time and complexity that scales with how much state must be replicated and how fresh that replication is) — the correct DR posture is chosen by first defining **RTO** (Recovery Time Objective — how long can the system be down) and **RPO** (Recovery Point Objective — how much data loss, measured in time, is acceptable) for the specific workload, then choosing among the standard DR strategies these translate to: backup-and-restore (cheapest, slowest, largest RPO/RTO), pilot light (minimal always-on footprint in the DR Region, scaled up on failover), warm standby (a scaled-down but fully functional DR Region, promoted on failover), or active-active (both Regions serving live traffic continuously, fastest RTO/near-zero RPO, highest cost and complexity) — a Principal Engineer should never default to the most expensive option (active-active) without first establishing that the workload's actual, business-stated RTO/RPO genuinely requires it, since DR posture cost scales steeply toward the active-active end.

### 9.4 Spot Instances in an Auto Scaling Group — Cost-Optimized Elastic Capacity
An ASG can mix On-Demand and **Spot Instance** capacity (spare EC2 capacity offered at a significant discount, reclaimable by AWS with a two-minute interruption warning) via a **mixed instances policy** — appropriate for horizontally-scalable, stateless, interruption-tolerant workloads (batch processing, non-latency-critical background workers) where the cost saving is substantial and an individual instance's interruption is a non-event handled by the ASG simply replacing it; **not** appropriate for a workload's baseline, latency-critical serving capacity (a payment-processing checkout path) where an unpredictable, AWS-driven instance reclaim mid-request is an availability risk the business shouldn't accept for a cost saving — a mature ASG design for a mixed workload often uses a small, cost-optimized Spot proportion for burst/background capacity and a guaranteed On-Demand (or Reserved/Savings-Plan-covered) floor for baseline serving capacity, an explicit cost-vs-reliability trade-off decision, not a default to maximize either dimension alone.
