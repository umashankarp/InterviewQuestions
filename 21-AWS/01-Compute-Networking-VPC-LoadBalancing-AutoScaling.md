...
NAT Gateway throughput and per-hour cost scale with the volume of outbound traffic private-subnet resources send through it — a workload with unexpectedly high outbound-traffic volume (frequent, large calls to external APIs) can encounter both a cost surprise and, at extreme volume, a genuine bandwidth constraint, worth explicitly capacity-planning for rather than assuming NAT Gateway is a limitless pass-through. Auto Scaling Group cooldown periods (the time an ASG waits after a scaling action before considering another one) must be tuned against actual instance warm-up time (§4) and metric-evaluation latency — too short a cooldown risks over-aggressive, oscillating scaling actions reacting to the same still-stabilizing metric; too long a cooldown risks slow reaction to a genuine, sustained demand change.

## 8. Security
Security Groups' implicit-deny, explicit-allow model (§2.6) directly implements least-privilege at the network layer — every rule should be added deliberately for a specific, justified need, never broadly opened (`0.0.0.0/0` for all inbound traffic) without explicit justification, since an overly permissive security group directly expands the instance's exposed attack surface regardless of how well-segmented the surrounding VPC/subnet structure otherwise is. Private subnets (§2.1) are a foundational defense-in-depth layer specifically for data-tier and internal-application-tier resources — an attacker who compromises a public-facing resource (a load balancer or a bastion host) still cannot directly reach private-subnet resources without an additional, separate compromise of the network path between them, directly limiting lateral-movement blast radius, conceptually connecting to Module 49 §8's service-boundary-based blast-radius-limiting discussion, now expressed at the network-topology level.

## 9. Scalability
Auto Scaling Groups' maximum achievable scale is bounded by AWS account-level service quotas (a default limit on the number of EC2 instances of a given type, or VPC-level IP-address exhaustion within a subnet's CIDR block) — a Principal Engineer must proactively verify these quotas are sufficient for the workload's actual peak-demand ceiling, requesting quota increases well ahead of an anticipated need, rather than discovering a quota ceiling during an actual traffic spike when it's too late to react. Load Balancer target-group capacity and the underlying subnet's available IP address space must also scale in lockstep with the ASG's maximum instance count — an ASG configured to scale up to 200 instances but placed in a subnet with a CIDR block only supporting 100 usable IP addresses will hit a silent, hard scaling ceiling well before its configured maximum, a subtle, easy-to-overlook capacity-planning mismatch between two independently-configured settings (directly echoing this module's recurring "independently-configured settings that must be reasoned about together" pattern, §2.5's durability-style trap now expressed as an IP-address-capacity trap).
