...
S3 request rates scale automatically and near-limitlessly per prefix as of AWS's 2018 partitioning improvements, but extremely high-throughput workloads should still spread requests across enough key-prefix diversity to benefit from S3's internal partitioning, and should batch small objects where possible, since per-request overhead (not raw throughput) tends to dominate at very high object counts of very small objects. EBS volume performance (both IOPS and throughput) is provisioned per-volume and is **independent of the attached EC2 instance's own network/EBS-optimized bandwidth ceiling** — a workload can be constrained by either limit independently, meaning a Principal Engineer diagnosing an EBS performance ceiling must check both the volume's own provisioned IOPS/throughput *and* the instance type's EBS bandwidth limit, since either one alone being adequate doesn't guarantee the combined path is (the same "reconcile independently-configured capacity dimensions together" pattern recurring from Module 57 §9 and Module 58 §9).

## 8. Security
S3 bucket policies (resource-based, per Module 58 §2.1) and IAM identity-based policies both apply to S3 access — a bucket accidentally configured with public read access (whether via an overly permissive bucket policy or, historically, an ACL) is a genuinely common and severe real-world data-exposure incident category, which is why AWS's **S3 Block Public Access** account/bucket-level setting (a blanket override that prevents public access regardless of any individual bucket policy or ACL misconfiguration) should be enabled by default for any bucket not deliberately and explicitly intended to serve public content — directly Module 58 §8's Service-Control-Policy-style "genuinely non-bypassable" governance pattern, now applied at the S3-bucket-configuration layer specifically. EBS volume encryption (via KMS, per Module 58 §2.5) should be enabled by default for every volume — enabling it after a volume already contains unencrypted data requires creating a new encrypted volume and migrating data, not an in-place conversion, making "enable it by default at creation" a substantially cheaper policy than remediating later.

## 9. Scalability
S3 scales storage capacity and request throughput essentially without a practical account-level ceiling a Principal Engineer needs to actively plan around (in sharp contrast to Module 57 §9's ASG/subnet-IP-capacity concerns), which is precisely why offloading the authoritative copy of scaling data onto S3 (§4's fix) also sidesteps an entire category of capacity-planning burden that a self-managed, EBS/EFS-backed storage tier would otherwise require. EFS, by contrast, has meaningful account-level and per-filesystem throughput/IOPS limits (particularly in its default "Bursting" throughput mode, which ties available throughput to the filesystem's total stored size) that a genuinely high-throughput, latency-sensitive shared-filesystem workload can hit — such workloads should explicitly evaluate EFS's "Elastic" or "Provisioned" throughput modes and capacity-plan against them the same way Module 57 §9 capacity-plans ASG/subnet limits together, rather than assuming EFS scales as transparently and limitlessly as S3.
