...
## 5. Best Practices
- Treat S3 as the default choice for the authoritative copy of unstructured data (uploads, backups, generated artifacts) given its Region-spanning, snapshot-independent 11-nines durability — use EBS/local disk only for working copies or genuinely latency-sensitive local access.
...
## 6. Anti-patterns
- Storing the sole, authoritative copy of important data on a single-AZ EBS volume with only a periodic snapshot as the durability backstop, without explicitly reasoning about the resulting data-loss exposure window.
- Assuming S3 is still only eventually consistent for overwrites/deletes and building unnecessary application-level workarounds for a consistency gap that was closed in December 2020.
...
### 7.1 S3 request-rate scaling and the sequential-key hot-partition trap
S3 no longer requires the old "randomize your key prefix" advice as a blanket rule — S3 automatically partitions a bucket's key space based on observed request rate, and each partitioned prefix supports roughly **3,500 PUT/COPY/POST/DELETE and 5,500 GET/HEAD requests per second**, with S3 scaling that partitioning up automatically as sustained request rate grows. The trap that still exists in production is **the scaling lag, not the ceiling**: automatic partitioning reacts to *sustained* request-rate growth over minutes, not an instantaneous burst — so a workload with a genuinely sequential, monotonically increasing key prefix (a settlement-file writer using `settlements/2026-08-28/...` as its key pattern, writing thousands of objects in a tight loop at end-of-day batch close) can still throttle (`503 SlowDown`) during the first seconds of a sudden burst, before S3's partitioning has caught up to the new rate, even though the bucket would happily sustain that same rate a few minutes later. For a workload with a known, spiky burst pattern (an EOD batch, not steady organic traffic), the concrete mitigation is either to pre-warm the prefix with a synthetic ramp before the real burst, or — the more robust fix — to add high-cardinality entropy early in the key (a hash or reversed-timestamp prefix: `a3f9/settlements/2026-08-28/...`) so the burst is naturally spread across partitions from the first request rather than depending on reactive scaling to catch up mid-burst.

### 7.2 EBS IOPS and throughput — the two independent ceilings
Every EBS volume has two independently provisioned (and independently *capped*) dimensions: **IOPS** and **throughput (MiB/s)**. gp3 provides a 3,000 IOPS / 125 MiB/s baseline at no extra cost regardless of volume size, and both can be provisioned up further (up to 16,000 IOPS / 1,000 MiB/s on gp3) independently of each other and independently of volume size — a real improvement over gp2, where both IOPS and throughput scaled *only* with provisioned size (3 IOPS/GB), forcing teams to over-provision capacity purely to buy IOPS headroom. io2 Block Express reaches up to 256,000 IOPS / 4,000 MiB/s for the most demanding transactional workloads (a core ledger or order-book database under sustained high-frequency write load). The recurring diagnostic trap: **a well-provisioned volume can still hit a ceiling set by the EC2 instance's own EBS-optimized bandwidth**, which is a separate, per-instance-type limit independent of the volume's own provisioned IOPS/throughput — a `db.r6g.large`-equivalent instance's own EBS bandwidth ceiling can throttle a volume provisioned well above what the instance can actually drive, and CloudWatch's `VolumeQueueLength` climbing while `VolumeReadOps`/`VolumeWriteOps` plateau below the volume's provisioned IOPS is the concrete signal that the *instance*, not the volume, is the bottleneck — checking only the volume's own metrics in isolation misses this class of ceiling entirely.

### 7.3 EFS performance modes and the throughput-vs-size coupling
EFS's default **Bursting** throughput mode ties baseline throughput allowance directly to total filesystem size (50 KiB/s per GiB baseline, with burst credits accumulated when under baseline and spent when over) — a filesystem backing a shared reconciliation-output directory that stays consistently small (a few GB of daily break reports) but is read intensively by multiple downstream jobs simultaneously can exhaust burst credits and fall back to a baseline throughput measured in single-digit MiB/s, a genuine, measurable ceiling with no relationship to how "important" or "hot" the data actually is. Provisioned Throughput mode decouples throughput from size at a fixed cost; Elastic Throughput mode (billed per GB transferred) auto-scales to actual instantaneous demand and is the correct default for genuinely spiky, unpredictable access patterns — CloudWatch's `PermittedThroughput` versus `DataReadIOBytes`/`DataWriteIOBytes` is the pair to graph together to catch this before it becomes a customer-visible slowdown in a downstream batch job.

---

## 8. Security

### 8.1 S3 bucket-policy and public-access-block misconfiguration as the recurring, single highest-severity storage risk
The single most common, highest-severity S3 misconfiguration in real incident histories across the industry is an accidental public bucket policy or ACL — a policy drafted with `Principal: "*"` intended to be scoped down later, or a `PutBucketAcl` call left at `public-read` from an early prototype and never revisited once the bucket started holding real customer statements or trade confirmations. **S3 Block Public Access** (settable at the account level and per-bucket) is the non-bypassable backstop: even if every individual bucket policy and ACL is misconfigured, Block Public Access enabled at the account level structurally prevents public exposure regardless — this is the same "non-bypassable override beats policy-review diligence alone" pattern already established for IAM Service Control Policies, now applied at the storage layer, and it should be enabled by default account-wide with an explicit, reviewed, narrowly-scoped exception process for the rare genuinely-public bucket (public marketing assets, a public documentation site), never a default-open posture with per-bucket opt-out.

### 8.2 Encryption at rest and in transit
Every S3 bucket holding regulated financial data (trade confirmations, statements, PII) should have **default bucket encryption** (SSE-KMS, with a customer-managed key for auditable key-rotation and access-logging control, rather than SSE-S3's AWS-managed key, when the compliance framework requires demonstrable control over key lifecycle) enabled at bucket creation, plus a bucket policy that **denies any `PutObject` lacking the expected encryption header** — this closes the gap where default encryption alone doesn't stop a misconfigured client from explicitly requesting no encryption or a different key. TLS should be enforced in transit via a bucket policy `aws:SecureTransport` condition denying any non-HTTPS request. EBS volumes backing any database holding transactional or customer data should have encryption enabled at volume creation — as with S3, this cannot be added in place to an existing volume; it requires creating a new encrypted volume and migrating.

### 8.3 EFS access points — isolating multi-tenant access to a shared filesystem
When multiple applications or teams share one EFS filesystem (a common pattern for a shared reconciliation-artifact store consumed by several downstream reporting jobs), **EFS Access Points** provide an application-specific entry point into the filesystem, each enforcing its own POSIX user/group identity and a root directory scoped to a specific path — this means a reporting job for Team A can be granted an access point rooted at `/reconciliation/team-a/` with no visibility into `/reconciliation/team-b/`, without needing separate filesystems or relying purely on IAM policy to enforce the boundary. Skipping access points and instead granting broad, filesystem-root IAM/NFS access to every consumer is the recurring anti-pattern — it works until a misconfigured or compromised job in one tenant's context can read or, worse, write into another tenant's directory tree, an entirely avoidable blast-radius expansion.

### 8.4 Audit trail and least privilege
S3 Server Access Logging (or, more commonly today, CloudTrail data events for S3) and EBS/EFS API activity via CloudTrail management events together form the audit trail a regulator will ask for after any storage-related incident — "who read this trade confirmation, and when" must be answerable from logs that were enabled *before* the incident, not reconstructed after. IAM policies scoping S3 access down to specific prefixes (`s3:prefix` condition keys) rather than whole-bucket access is the least-privilege default for any multi-team bucket, directly mirroring the EFS access-point isolation principle at the S3 layer.

---

## 9. Scalability

### 9.1 S3 Cross-Region Replication and Multi-Region Access Points
For a workload with a genuine multi-region durability or latency requirement (a global custody platform needing trade confirmations readable with low latency from both a US and EU region, or a regulatory requirement that a copy of records be retained in a specific data-residency region), **S3 Cross-Region Replication (CRR)** asynchronously replicates objects to a bucket in a different Region — inheriting the same asynchronous-replication caution already established for RDS read replicas: a reader immediately following a write, if routed to the replica Region, can observe a stale or missing object during the replication lag window, so CRR is not a substitute for explicit consistency reasoning about which Region a given read must be served from. **S3 Multi-Region Access Points** layer a single global endpoint over multiple regional bucket replicas with automatic latency-based routing and failover, removing the need for application-level Region selection logic, but the underlying asynchronous-replication lag caveat still applies underneath that convenience.

### 9.2 EBS Multi-Attach — shared block access, with a sharp caveat
**EBS Multi-Attach** (io1/io2 only) allows a single EBS volume to be attached to multiple EC2 instances simultaneously within the same AZ — but critically, EBS Multi-Attach provides **no filesystem-level coordination**; it is purely a shared block device, meaning every attached instance must run a cluster-aware filesystem or application-level coordination (a clustered database engine designed for shared-disk access) to avoid silent data corruption from uncoordinated concurrent writes. This is a narrow, specialist tool for a small set of clustered-database architectures — reaching for it as a general "shared storage for a fleet" solution (where EFS is almost always the correct, POSIX-coordinated answer) is the same category of mismatch already flagged for EFS-as-default-shared-storage, just one layer lower and with a considerably sharper failure mode (silent corruption rather than merely suboptimal latency) if the coordination requirement is missed.

### 9.3 S3 as the horizontally-unbounded tier, and EBS/EFS's real ceilings
S3 has no meaningful account-level storage ceiling and scales its request-rate handling automatically (§7.1) — for a growing FinTech platform, S3 is therefore the tier that structurally never becomes the scaling bottleneck if the access pattern is respected. EBS and EFS both have real, finite scaling ceilings that must be explicitly planned for: a single EBS volume has a maximum size and IOPS/throughput ceiling per volume type (necessitating a genuine database-sharding or read-replica strategy, not a bigger EBS volume, once a single instance's ceiling is reached), and EFS's per-filesystem throughput (even in Elastic mode) has an upper bound that a sufficiently large, concurrent-access workload can reach, at which point splitting across multiple filesystems by tenant or workload becomes the scaling lever. Recognizing which tier a given workload's growth will actually bottleneck on — and validating that early rather than discovering it under real production load — is a direct extension of the storage-selection discipline established in §2.5.
