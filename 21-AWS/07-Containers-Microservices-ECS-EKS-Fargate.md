...
Fargate task startup time (provisioning a new task's compute, pulling the container image, running the container's own application-level warm-up) is directly analogous to Module 57 §4's EC2 instance warm-up and Module 61 §2.1's Lambda cold start — the same "don't route production traffic to a newly-started unit before it's genuinely ready" discipline applies, now via the orchestrator's own deployment-gating health-check mechanism (§2.6) rather than a load balancer's health check directly. Container image size directly affects both Fargate task startup latency and EC2-backed ECS/EKS's image-pull time during scaling events — minimizing image size (multi-stage builds, minimal base images) is a legitimate, concrete performance lever for scaling responsiveness, directly analogous to Module 61 §7's Lambda-package-size discussion.

## 8. Security
Task-level IAM roles (§2.5) are the primary, structural least-privilege mechanism for container platforms, but should be paired with network-level segmentation (security groups scoped per-service, and — for EKS specifically — Kubernetes NetworkPolicies restricting pod-to-pod traffic) as a defense-in-depth layer, directly Module 57 §8's principle that IAM/identity-layer control and network-layer control are independent, both-necessary axes, now recurring at the container-task granularity. App Mesh's mTLS capability (§2.4) should be enabled mesh-wide as a default for any service handling sensitive data or operating in a genuinely zero-trust internal network posture — centrally enforced mTLS at the sidecar layer means individual services don't need to correctly implement TLS certificate management themselves, removing an entire class of potential per-service misconfiguration.

## 9. Scalability
ECS/EKS cluster-level and task/pod-level scaling both have AWS account-level service quotas (analogous to Module 57 §9's ASG/VPC-IP-capacity discussion) that must be proactively verified for any workload anticipating significant scale — additionally, Fargate has its own per-Region concurrent-task-count considerations distinct from EC2-backed capacity limits, meaning a workload migrating from EC2-backed to Fargate-backed compute should explicitly re-verify its capacity assumptions rather than assuming quota parity between the two compute models. App Mesh's sidecar proxies introduce a small additional per-hop latency and consume additional compute resources per task (the sidecar itself runs as a container within the task) — a genuinely latency-critical, high-throughput service should explicitly capacity-plan for this overhead (both in latency budget and in the task's own CPU/memory allocation, which must now be shared between the application container and the sidecar) rather than assuming mesh adoption is a zero-cost architectural improvement.
