...
## 6. Anti-patterns
- Embedding a genuinely unbounded one-to-many relationship (comments, logs, any ever-growing list) directly in a parent document (the incident).
...
**Index selectivity and the equality-sort-range (ESR) rule.** A compound index's field order determines whether MongoDB can use it efficiently: put **equality** predicates first (`$match: { status: "completed" }`), then **sort** fields (so the index itself is already in the required order, avoiding an in-memory `SORT` stage), then **range** predicates last (`{ orderDate: { $gte: ... } }`). An index built in the wrong order (range before sort, for example) still gets used, but MongoDB can't leverage it for the sort, and falls back to `SORT_KEY_GENERATOR`/in-memory sort — which spills to disk past the default 100MB working-set limit for a blocking sort stage, producing a `QueryExceededMemoryLimitNoDiskUseAllowed` error in production if `allowDiskUse` isn't set, or a silently slow query if it is. `db.orders.explain("executionStats")` is the equivalent of a SQL Server execution plan — check `stage` for `COLLSCAN` (full collection scan, the MongoDB equivalent of a missing-index table scan) vs. `IXSCAN` (index used), and `totalDocsExamined` vs. `nReturned` — a large ratio between the two means the index isn't selective enough for the query, even if an index is technically being used.

**Working-set size and the WiredTiger cache.** MongoDB's default storage engine (WiredTiger) keeps a cache (default ~50% of RAM minus 1GB) of the most recently used pages. If a collection's **working set** — the actively-queried subset of data plus its indexes — exceeds available cache, every query pays a disk read instead of a cache hit, and latency degrades sharply and non-linearly once the working set crosses the cache boundary (not a gradual slowdown — a cliff). This is why an over-embedded document (§2.2's 16MB-approaching post) doesn't just risk the hard limit — every read of it evicts proportionally more of the cache to hold it, pushing other collections' working sets out and degrading unrelated queries.

**Query-plan cache.** MongoDB caches the winning query plan (chosen by running several candidate plans in parallel for a brief "trial period" and keeping the best) per query shape, so subsequent structurally-identical queries (same filter/sort/projection shape, different literal values) skip re-planning. A schema or data-distribution change (e.g., a new index, or a field's selectivity shifting dramatically after a large data migration) can leave a **stale cached plan** in place — `db.collection.getPlanCache().clear()` or `PlanCache.clearPlansByQuery` forces re-evaluation; monitor `planCacheTotalSizeEstimateBytes` and plan-cache-miss rate as a leading indicator of a workload whose query shapes are too varied to benefit from caching at all.

**Aggregation-pipeline cost specific to this module.** `$group`, `$sort`, and `$lookup` are the expensive stages: `$group` materializes intermediate accumulator state per group (bounded by `internalDocumentSourceGroupMaxMemoryBytes`, default 100MB, before spilling to disk), `$sort` without an index-supported order does the same, and `$lookup` performs, in effect, a nested-loop join per input document unless the foreign collection's join key is indexed — an un-indexed `$lookup` against a large foreign collection is the single most common cause of an aggregation pipeline timing out in production, directly reinforcing §2.4's "heavy `$lookup` reliance is a modeling smell," now with the concrete performance mechanism behind why.

## 8. Security

**Field-level encryption for regulated data.** MongoDB's **Client-Side Field Level Encryption (CSFLE)** / **Queryable Encryption** (7.0+) encrypts specific fields (SSNs, account numbers, KYC documents) *before* they leave the application, using keys the database server itself never sees in plaintext — meaningfully stronger than encryption-at-rest alone (which protects against disk theft but not against a compromised DBA credential or an over-privileged query), and directly relevant for PCI-DSS scope reduction: a field encrypted client-side with a key MongoDB never holds is not "cardholder data the database can read," narrowing the system's compliance boundary. The trade-off: encrypted fields lose most query capability (equality queries are supported for deterministically-encrypted fields; range/regex are not, or only with the newer Queryable Encryption's more limited support) — so field-level encryption is a targeted tool for specific sensitive fields, not a blanket schema strategy.

**RBAC and least privilege.** MongoDB's built-in roles (`read`, `readWrite`, `dbAdmin`, `clusterAdmin`, etc.) should be scoped per-database and per-collection where the driver/deployment supports it, never granting `root` or `dbOwner` to an application's service account — a compromised application credential with `readWrite` scoped only to its own database limits blast radius far more than one with cluster-wide privileges. Custom roles (`db.createRole`) let you grant exactly the actions a service needs (e.g., `find` and `insert` on `orders` only, no `remove`/`drop`) rather than reaching for a coarse built-in role that happens to be "close enough."

**Authentication mechanisms.** Production MongoDB should never run with `--noauth`; SCRAM-SHA-256 is the default credential mechanism, with x.509 client-certificate authentication, LDAP, and Kerberos available for enterprise integration — x.509 is generally preferred for service-to-service authentication in a FinTech deployment since it ties into the same certificate-based identity/rotation infrastructure already used for mTLS elsewhere in the architecture, rather than managing a separate credential store.

**Injection via unsanitized query operators.** MongoDB queries built from unsanitized user input are vulnerable to **operator injection** — if a login handler builds `{ username: userInput, password: passwordInput }` and `passwordInput` is attacker-controlled JSON like `{ "$ne": null }` rather than a string, and the application doesn't type-validate/sanitize before constructing the query, the resulting query becomes `{ password: { $ne: null } }` — "password is not null," which matches virtually every user, bypassing authentication entirely. This is MongoDB's structural analogue of SQL injection, and the fix is the same discipline: strictly type-validate all user input (reject non-string types where a string is expected) before it reaches query construction, and never pass a raw, un-typed request body directly into a filter object.

## 9. Scalability

**Sharding recap and zone sharding.** Beyond basic shard-key selection (§2.5), **zone sharding** lets you pin ranges of a shard key to specific shards/regions — directly useful for data-residency requirements (EU client data must physically reside in an EU-region shard, a GDPR/data-sovereignty control) or for tiering (recent, hot data on fast NVMe-backed shards; older, cold data on cheaper storage-optimized shards) — the shard key and zone configuration together become the mechanism enforcing both a technical scaling requirement and a regulatory constraint simultaneously.

**Replica-set read scaling.** Routing reads to secondaries (`readPreference: "secondaryPreferred"`) scales read throughput horizontally without adding shards — appropriate for read-heavy, staleness-tolerant workloads (catalog browsing, reporting) but not a substitute for sharding when the bottleneck is *write* throughput or *working-set size* exceeding a single primary's capacity, since every secondary still holds a full copy of the data and inherits the same write load via replication.

**HA/DR.** A replica set's automatic failover (§2, Module 24) provides HA within a region; **cross-region replica-set members** (a secondary in a second AWS region/Azure region) provide DR against a full regional outage, at the cost of higher replication latency to that member (and a corresponding risk of that member being excluded from a fast `w: "majority"` acknowledgment if latency makes it a laggard). A common FinTech pattern: majority-write-concern-eligible members concentrated in the primary region for low write latency, with one or more explicitly non-voting or lower-priority members in a DR region — trading a slower DR failover (since the DR member isn't preferentially elected) for not slowing down every write waiting on cross-region acknowledgment.

**Capacity planning.** Shard count and instance sizing should be planned against **projected working-set growth**, not just current data volume — since the WiredTiger cache cliff (§7) means a shard that's "fine today" can degrade sharply once its portion of the working set outgrows its cache, well before disk capacity becomes the binding constraint.
