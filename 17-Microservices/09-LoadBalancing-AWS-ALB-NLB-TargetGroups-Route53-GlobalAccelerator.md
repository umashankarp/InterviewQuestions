...
1. **Assert the timeout ordering as code, not as documentation.** `targetKeepAlive > lbIdleTimeout`, `stopTimeout > deregistrationDelay + drainTime`, `clientTimeout ≥ lbIdleTimeout` where that is intended. Encode these in the LB module and fail the plan. *Why:* they are cross-component invariants no single team can verify locally. *When not:* nowhere — this is unconditional.
2. **Set `deregistration_delay` from your measured P99.9 request duration.** *Why:* the default 300 s makes every deploy slow, and a hand-picked 5 s truncates in-flight work. *When not:* long-lived streaming connections need a different mechanism (max connection age), not a longer delay.
3. **Enable `slow_start` for services with warm-up cost** (JIT, connection pools, caches). *Why:* it is off by default and directly fixes the post-scale-out P99 spike. *When not:* for genuinely stateless, warm-on-arrival services it adds nothing.
4. **Prefer `least_outstanding_requests` over the round-robin default on ALB.** *Why:* Module 136 §2.2 — it self-corrects for slow targets. *When not:* when request costs are genuinely uniform and you want the simplest possible reasoning about distribution.
5. **Use `ip` target mode and pod readiness gates on EKS.** *Why:* `instance` mode inserts a `kube-proxy` hop that discards the LB's target choice; without readiness gates, rolling updates create registration gaps. *When not:* `instance` mode is acceptable for non-critical internal services where the extra hop is irrelevant.
6. **Keep target counts balanced across AZs — or enable cross-zone and pay for it.** *Why:* §2.5's skew is invisible in fleet averages. *When not:* single-AZ workloads (rare and usually a mistake) don't have the problem.
7. **Alert on LB-generated errors separately from target errors**, and on `RejectedConnectionCount`, `UnHealthyHostCount`, and per-AZ `TargetResponseTime`. *Why:* the whole class of failure in §4 is invisible to application metrics. *When not:* never; this is the cheapest detection available.
8. **Enable access logs and connection logs to S3 from day one.** *Why:* the `target_status_code = -` diagnosis is impossible without them, and you cannot retroactively enable logging for an incident that already happened. *When not:* n/a — budget for the storage, apply a lifecycle policy.
9. **Use Global Accelerator when the RTO is a commitment rather than an aspiration.** *Why:* DNS failover's tail is client-controlled and unbounded. *When not:* when you need Route 53's richer routing (geolocation, complex weighting) more than you need bounded failover, or when the fixed cost genuinely doesn't clear the bar.
10. **Terminate TLS at the ALB and re-encrypt to targets where the control framework requires end-to-end encryption**, using ACM for public certs and Private CA internally. *Why:* centralises certificate rotation while satisfying encryption-in-transit controls. *When not:* if the requirement is true end-to-end with no intermediate visibility, use NLB TLS passthrough and accept losing L7 features.
11. **Give one team clear ownership of each load balancer's LB-scoped attributes.** *Why:* §4. *When not:* n/a.
12. **Set `PooledConnectionLifetime` on .NET `HttpClient` (and equivalents elsewhere).** *Why:* §2.2 — without it, long-lived clients never see LB scaling or node replacement. *When not:* n/a; 2–5 minutes is a safe default.

---

## 6. Anti-patterns

1. **Putting gRPC or WebSocket services behind an NLB and expecting balancing.** *Why it fails:* NLB pins flows; a multiplexed HTTP/2 connection is one flow. *Fix:* ALB with target-group protocol version `GRPC`/`HTTP2`, or client-side balancing/mesh, plus server-side max connection age.
2. **Treating an ALB's resolved IPs as stable.** *Why it fails:* nodes are replaced and scaled; the allow-list breaks intermittently. *Fix:* NLB with per-AZ static IPs or EIPs, or PrivateLink.
3. **One health endpoint serving both liveness and readiness, and having it check downstream dependencies.** *Why it fails:* a downstream blip removes the whole fleet simultaneously (Module 136 §2.3). *Fix:* separate endpoints; readiness reflects *this* instance's ability to serve.
4. **Exiting immediately on SIGTERM.** *Why it fails:* the LB keeps routing for the deregistration delay; those requests 502. *Fix:* the §2.4 Pair-2 ordering, with `preStop` sleep and a sufficient `stopTimeout`.
5. **Sticky sessions used as a substitute for externalised session state.** *Why it fails:* defeats balancing and makes a single target's failure customer-visible. *Fix:* Redis/DynamoDB session store; reserve stickiness for genuinely session-affine legacy.
6. **Claiming a sub-minute RTO backed by DNS failover.** *Why it fails:* client-side caching is outside your control; a DR test will show a residual tail. *Fix:* Global Accelerator, or restate the RTO honestly.
7. **Leaving the ALB idle timeout at 60 s while a legitimate long-running endpoint sits behind it — or raising it without auditing every co-located service's keep-alive.** *Why it fails:* one direction causes truncated downloads; the other causes §4. *Fix:* separate the long-running endpoint onto its own LB.
8. **Health checks so lenient they only prove the process is running.** *Why it fails:* a degraded instance keeps serving broken responses. *Fix:* a readiness check that exercises the instance's own critical path.
9. **Health checks so aggressive they remove healthy targets during a GC pause.** *Why it fails:* removes capacity precisely when the fleet is under pressure, cascading. *Fix:* set the timeout above your P99 pause, tune threshold rather than interval alone.
10. **Unbounded listener-rule growth on a shared ALB.** *Why it fails:* hits the per-LB rule quota, makes rule-evaluation the billed LCU dimension, and turns rule ordering into an unreviewable shared artefact. *Fix:* partition by bounded context with a rule budget per LB.
11. **`instance` target mode on EKS for latency-sensitive traffic.** *Why it fails:* `kube-proxy` re-balances and discards the LB's decision, adding a hop and hiding per-pod load. *Fix:* `ip` mode.
12. **Reading `HTTPCode_ELB_5XX` as "the load balancer is broken" and raising an AWS ticket.** *Why it fails:* it almost always means the *contract between* the LB and your targets is broken, which is yours to fix. *Fix:* the §2.4 fork, then the access log.

---

## 7. Performance Engineering

**Latency budget.** An ALB adds meaningful per-request work: TLS termination, full HTTP parse, rule evaluation, target selection, and a separate backend connection. Typical added latency is low single-digit milliseconds when backend connections are reused, and materially more when they are not — a fresh TCP handshake plus TLS handshake to the target can add tens of milliseconds. **Connection reuse is therefore the single largest performance lever at this layer**, and it is also the largest cost lever (§2.9). NLB's flow forwarding adds far less, which is why ultra-low-latency paths (market data, FIX) use NLB or bypass LBs entirely.

**Where P99 actually comes from.** Three sources dominate, and only one of them is the LB itself:
- **Cold targets** taking full share immediately after becoming healthy — fixed by `slow_start`, not by more capacity.
- **Slow targets** retaining an equal share under round-robin — fixed by `least_outstanding_requests`.
- **Skewed targets** under cross-zone-disabled NLB — fixed by AZ balance (§2.5). Fleet-average metrics hide all three; you need per-target and per-AZ views.

**Metrics that matter, and what each one tells you.** `TargetResponseTime` (your service, excluding LB overhead), `RequestCount` per target group and per AZ, `ActiveConnectionCount` and `NewConnectionCount` (their ratio is your effective reuse rate — a low ratio is both a latency and a billing finding), `TargetConnectionErrorCount` (Pair 1), `RejectedConnectionCount` (the LB is the constraint), `ConsumedLCUs` (which dimension dominates), `ClientTLSNegotiationErrorCount` / `TargetTLSNegotiationErrorCount`, `UnHealthyHostCount` per AZ.

**Benchmarking discipline.** Load-test *through* the load balancer, not against a target directly, and ramp rather than step — a step test measures LB scaling behaviour rather than your service's, and will produce `RejectedConnectionCount` that a production traffic curve never would. Test the deploy path too: a large share of this topic's real-world latency and error impact occurs during deployments, not at steady state, and a steady-state load test will never reveal it.

**HTTP/2 and keep-alive.** Enabling HTTP/2 client-side on the ALB reduces handshake and header overhead for browser clients. Note that ALB historically converts to HTTP/1.1 toward targets unless the target group protocol version says otherwise — so an HTTP/2 client with an HTTP/1.1 target group gets edge benefits only.

**Caching.** The LB is not a cache. CloudFront in front of it is, and for read-heavy public endpoints it removes both latency and LCU-billed bytes. For authenticated APIs, the edge TLS termination alone is often worth it for geographically distant clients.

---

## 8. Security

**Threats and where the LB is the control point.** The load balancer is the outermost enforcement surface for HTTP-layer threats, and it is the natural place to put controls that must not be re-implemented per service.

- **OWASP-class input attacks (injection, XSS, path traversal, deserialisation payloads):** **AWS WAF** attaches to an ALB (and to CloudFront), with managed rule groups plus custom rules. This is defence in depth, not a substitute for application validation — a WAF that is your only input validation is a compliance artefact rather than a control.
- **Volumetric DDoS:** **Shield Standard** is automatic; **Shield Advanced** adds detection, response team access, and cost protection. NLB absorbs L4 floods far better than ALB.
- **Application-layer abuse (credential stuffing, scraping, enumeration):** WAF rate-based rules at the LB give you a coarse, always-on limit; per-consumer quotas belong at the API gateway layer (Module 38).
- **Request smuggling / desync:** ALB's **desync mitigation mode** (`monitor` / `defensive` / `strictest`) governs how ambiguous requests are handled. `defensive` is the sensible default; `strictest` for regulated internet-facing paths. This is a real, specific, frequently-unset control.
- **TLS posture:** ACM-managed certificates with automatic renewal, a modern security policy (TLS 1.2 minimum, TLS 1.3 preferred), SNI for multiple certificates on one listener, and `ClientTLSNegotiationErrorCount` monitored — a rise there is often a client attempting a deprecated cipher, which is exactly what you want to see *before* you tighten the policy.

**Authentication at the edge.** ALB can enforce OIDC or Cognito authentication on a listener rule, terminating the auth flow before traffic reaches your service. Useful for internal tools; for a customer API, token validation usually belongs at the gateway or service so that authorisation decisions stay with the code that understands the domain (Modules 40/41).

**mTLS.** ALB supports mutual TLS — either `verify` mode against a **trust store** of CAs you upload (the ALB validates the client certificate and passes its details in headers), or passthrough mode (the ALB forwards the certificate for the application to validate). For an acquirer or counterparty connection where mutual authentication is contractual, `verify` mode centralises certificate validation and revocation checking; passthrough is the choice when the application must make identity decisions from the certificate itself. NLB TLS passthrough gives true end-to-end.

**Network isolation.** Security groups on both ALB and NLB (NLB gained SG support in 2023 — before that, target SGs had to permit the VPC CIDR, a materially weaker posture worth knowing about for older estates). The target's SG should allow traffic **only** from the LB's SG, referenced by group ID rather than CIDR, so the rule stays correct as addresses change. Internal LBs in private subnets; public LBs in public subnets with targets private.

**Client IP for security decisions.** Fraud scoring, geo-blocking and IP allow-listing need the real client IP. Behind an ALB that means trusting `X-Forwarded-For` — and trusting it *correctly*: take the appropriate entry given your number of trusted proxies, never the leftmost blindly, since clients can inject the header. Behind an NLB with client IP preservation, the socket's peer address is genuine and cannot be forged. **If IP-based security decisions are load-bearing, that is an argument for NLB**, and it's a strong answer to give when asked "when would you choose NLB?"

**Audit and evidence.** Access logs and connection logs to S3 with object lock where retention is mandated; VPC Flow Logs for the network layer; CloudTrail for LB configuration changes — the last of which is how §4's timeline was reconstructed, and is the difference between an incident report that names a cause and one that speculates.

---

## 9. Scalability

**Horizontal scaling of targets** is the ordinary case and is bounded by three things people forget: the target group's registration limits, the LB's own scaling rate (§2.2), and the readiness/warm-up behaviour of new targets (§2.3's slow start). Adding 200 tasks in ten seconds does not add 200 tasks' worth of *useful* capacity in ten seconds.

**Scaling the load balancer itself.** ALB and NLB scale automatically; the observable failure mode is `RejectedConnectionCount` during a step load. NLB tolerates step loads far better. If your traffic has a genuine cliff — a market open, a settlement batch, a televised event — design for it explicitly (ramp synthetic traffic ahead, or choose NLB) rather than assuming elasticity is instantaneous.

**Multi-AZ.** Enable every AZ where you have targets, keep target counts balanced (§2.5), and know that an ALB with targets in three AZs survives a single-AZ failure with 2/3 capacity — which means your steady-state utilisation must leave room for that, or an AZ loss becomes a capacity incident on top of an availability one. This is the concrete arithmetic behind Module 137's cell sizing.

**Multi-region.** Active-active with Global Accelerator traffic dials or Route 53 weighted records; active-passive with health-check-driven failover. The load balancing layer is the easy half. The hard half is data (Aurora Global Database's replica lag and its manual-vs-managed failover semantics; DynamoDB Global Tables' last-writer-wins conflict resolution) — and in a ledger or settlement system the data layer, not the LB, sets your true RPO and therefore your real architecture.

**Partitioning and cells.** Load balancers are how cell routing is physically realised: a cell per LB (or per target group with weighted rules), with the router above choosing the cell. Module 137's warning applies directly here — cells contain only the failures that respect cell boundaries, and a *shared* load balancer across cells is exactly the kind of shared component that silently breaks the isolation you believe you have.

**CAP framing.** A load balancer's health checking is an availability mechanism with a consistency cost that is rarely named: **there is always a window in which the LB's view of target health is stale.** During that window, requests are routed to a dead target (unavailability you experience as errors) or withheld from a recovered one (capacity you paid for and aren't using). Tuning health checks is choosing where in that trade-off to sit; it cannot be eliminated, only moved.

**HA/DR.** The load balancing layer's own DR story is mostly "it is regional and AWS-managed" — which means your DR plan must cover the *configuration*, not the fleet: LB and target-group definitions in Terraform, deployable into the DR region, exercised. A DR region whose ALB exists but whose listener rules were hand-edited in the console over two years is a DR region that will not work, and this is one of the most common findings in a real DR audit.
