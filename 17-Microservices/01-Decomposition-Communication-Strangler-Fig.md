...
## 5. Best Practices
- Decompose services along business capabilities (aligned with independently-varying stakeholders/features, the SRP), never along technical layers.
...
## 6. Anti-patterns
- Decomposing along technical layers instead of business capabilities, producing the distributed-monolith anti-pattern (the incident) — all the complexity cost of microservices, none of the independent-deployability benefit.
- Sharing a database across service boundaries, reintroducing tight coupling and dual-write risks that database-per-service exists to prevent.
...
### 7.1 The Strangler Fig routing layer's added latency budget
Every request that traverses the Strangler Fig routing layer (§2.5) pays an extra network hop compared to the pre-migration monolith's direct, in-process call — the routing decision itself (typically a path/header inspection against `IMigrationConfig`, §11's Expert exercise) is cheap in CPU terms (low single-digit microseconds for a simple lookup), but the **hop's own connection/TLS overhead**, not the routing logic, dominates the added latency: a pooled-connection reuse to the target adds roughly 1-3ms; a fresh connection (no pooling, or a cold routing-layer instance) adds tens of milliseconds for TCP+TLS setup. At the scale of hundreds of migrated capabilities routed through one shared gateway, this per-request tax is a real, measurable line item against the pre-migration baseline, and it must be **budgeted explicitly** against the SLA rather than discovered as an unexplained regression after cutover — the same latency-budgeting discipline used for any additional synchronous hop (§2.3) applies identically here, since the routing layer is, mechanically, just one more synchronous hop inserted in front of the real target.

### 7.2 Decomposition granularity as a direct latency multiplier
A single monolith method call that used to complete in-process, in microseconds, becomes — after decomposition — a network round trip with serialization, deserialization, and (per §2.3) mandatory timeout/circuit-breaker overhead. The finer the decomposition granularity (Advanced Q9), the more of a single logical business operation gets split across network hops, and each additional hop compounds not just the availability risk (§2.3) but the **latency** cost multiplicatively for any synchronous chain — three sequential 20ms hops is 60ms+ of pure network overhead that didn't exist in the monolith, before any actual business-logic execution time is counted. This is a first-class reason (alongside the SRP-alignment reasoning) that decomposition granularity is a genuine engineering trade-off, not a "more services is always better" default: performance work on a synchronous call chain should always ask "could two of these hops collapse into one service, or become parallel/asynchronous instead" before reaching for caching or infrastructure scaling as the fix.

### 7.3 Async communication's throughput advantage, and its own hidden cost
Moving an interaction from synchronous to asynchronous (§2.4) removes it from the caller's critical-path latency budget entirely — the caller's response time no longer includes the subscriber's processing time at all, which is why this is usually the single biggest lever for improving a request's P99 once cascading-hop latency (7.2) has been identified as the bottleneck. The hidden cost is **queue/broker throughput and consumer-side backpressure**: a synchronous chain's throughput is naturally self-limiting (a slow downstream directly slows the caller, which slows the caller's own upstream), while an asynchronous producer can outpace its consumers indefinitely, silently building an ever-growing backlog (directly this course's recurring "decoupling availability doesn't decouple *capacity*" principle) unless consumer scaling and backpressure signals are explicitly designed for — performance testing an asynchronous migration must therefore measure sustained producer/consumer throughput ratios under load, not just single-request latency, since the failure mode here (unbounded queue growth) is invisible in a single-request benchmark.

### 7.4 Benchmarking discipline for a migration in progress
During an active Strangler Fig migration, the system is, at any given moment, a genuine hybrid: some traffic takes the new, shorter (or, per 7.1, marginally longer) path through the new microservice, and some still takes the old, direct monolith path. Load-testing "the system" as a single homogeneous target during this period produces a misleading, blended P99 that hides which path is actually degrading — tests must be run **per capability/route**, comparing the newly-migrated path's latency distribution explicitly against both the pre-migration monolith baseline and the target SLA, so that a regression on the *specific, newly-migrated* capability doesn't get diluted into a fleet-wide average that still looks acceptable.

---

## 8. Security

### 8.1 The routing layer as new, migration-specific attack surface
The Strangler Fig routing layer (§2.5) is new infrastructure that didn't exist before the migration began, and it now sits directly in the request path for both legacy and migrated traffic — it must therefore be held to the same authentication/authorization-enforcement standard as the API Gateway pattern it structurally is (Intermediate Q4's "never treat internal traffic as inherently trusted" applies with equal force here), not treated as a lightweight, temporary routing shim exempt from normal security review because it's "just scaffolding." A misconfigured routing rule (an overly broad path match intended to route one capability that accidentally also captures an unrelated, sensitive path) is a realistic, migration-specific vulnerability class that a permanent, stable architecture doesn't have to continuously guard against in the same way, precisely because the routing configuration is, by design, being actively and frequently changed throughout the migration.

### 8.2 The temporary shared-database window's security exposure
Advanced Q2 established that a Strangler Fig migration sometimes requires a temporary, time-boxed data-synchronization bridge between the legacy monolith's database and a new microservice's own database. This bridge is a genuine, if temporary, violation of database-per-service's blast-radius containment (Intermediate Q9) — for as long as it exists, a compromise of either side (the legacy monolith or the new microservice) has a plausible path to the other's data via the sync mechanism, which is exactly the coupling database-per-service exists to prevent. This is a strong reason the sync bridge needs an explicit, tracked removal date and its own access-control review (least-privilege credentials scoped only to the specific tables/fields the sync actually needs, not broad database-level access), rather than being waved through as "temporary, so it doesn't need the same rigor" — temporary infrastructure that outlives its intended lifespan (a realistic, common migration-scope-creep failure mode) becomes permanent, unreviewed infrastructure.

### 8.3 Service-to-service authentication as decomposition's new obligation
A monolith's internal method calls have no authentication boundary at all — decomposition introduces one at every service boundary, and every new synchronous or asynchronous inter-service call (§2.3, §2.4) is a new place where "is this caller actually authorized to invoke this operation" must be enforced, where previously it simply wasn't a question the code needed to ask. This is frequently under-scoped during an initial migration's security review, since the *functional* behavior (the call succeeds, returns the right data) looks identical to the pre-migration in-process call — the security posture is not identical, and every newly-introduced service boundary is a new authorization decision point that needs its own explicit design (mTLS between services, the concrete mechanism, is covered in full in the later, dedicated `39-Service-Mesh` and `40-IAM` treatments; this module's obligation is recognizing that the boundary itself creates the requirement).

### 8.4 Blast-radius reduction as a security property of correct decomposition, not just an operational one
Database-per-service (§2.2) and business-capability decomposition (§2.1) jointly limit how far a single service's compromise can propagate — Intermediate Q9 already established this for database access specifically; the broader point is that a correctly-decomposed architecture is a **smaller, less severe security incident by construction** when any one service is compromised, compared to a monolith (where any compromise has access to the entire application's data and code) or a distributed monolith (where nominal service boundaries exist but shared databases/tight coupling mean a single compromise can still reach far beyond the nominally "compromised" service). This is a legitimate, additional argument for correct decomposition beyond the deployability/team-autonomy benefits already discussed — worth naming explicitly to a security-conscious interviewer, since it directly ties architectural correctness to a measurable security outcome (contained blast radius).

---

## 9. Scalability

### 9.1 Decomposition granularity at fleet scale — hundreds of services
The business-capability-vs-technical-layer decomposition principle (§2.1) doesn't change as an organization scales from a handful of services to hundreds, but the **cost of getting it wrong** scales non-linearly: a distributed-monolith symptom affecting three services in a 10-service estate is a team-level annoyance; the same symptom pattern replicated across dozens of service clusters in a 300-service estate (as Advanced Q10 describes) becomes an organization-wide velocity tax, since every one of those clusters independently reproduces the same coordinated-deployment friction. At this scale, decomposition correctness stops being a per-team design review question and becomes a **fleet-wide governance metric** (Advanced Q7's deployment-correlation tracking, Advanced Q10's org-wide remediation backlog) — the same detection technique, simply applied continuously and centrally rather than discovered reactively, team by team.

### 9.2 The Strangler Fig routing layer's own scalability ceiling
A routing layer designed for a modest, early-migration rule count (a handful of capability-specific routes) can itself become a scaling bottleneck as migration progresses and rule count grows into the dozens or hundreds (Advanced Q5's "routing configuration itself becoming hard to reason about" symptom, viewed here through a throughput/scalability lens rather than a maintainability one) — every additional routing rule adds evaluation cost to the request-path hot loop, and a routing layer implemented as a simple, linear rule-matching structure (fine at a dozen rules) can become a measurable per-request cost at hundreds of rules unless the implementation moves to a more scalable lookup structure (a trie/prefix-map keyed on path, or delegating to a purpose-built API Gateway product designed for this exact problem rather than a hand-rolled middleware, once rule count crosses a threshold where hand-rolled linear matching no longer scales acceptably).

### 9.3 Horizontal scaling independence — the actual payoff at fleet scale
The entire justification for correct, business-capability-aligned decomposition (§2.1) cashes out most visibly at scale: a correctly-decomposed Order service experiencing a Black-Friday-style traffic spike can be scaled independently (more instances, more compute) **without** needing to scale the Inventory or Payment services proportionally, since their load characteristics are genuinely decoupled — this is the scalability benefit a distributed monolith forfeits (Intermediate Q7), and it becomes dramatically more valuable, in both cost and operational terms, as fleet size and traffic-pattern diversity across services grows; at small scale, over-provisioning a handful of coupled "services" together is a tolerable inefficiency, but at hundreds-of-services scale, that same coupling means the organization is perpetually over-provisioning capacity for services that didn't need it, purely because decomposition failed to isolate genuinely independent scaling needs.

### 9.4 Event-driven fan-out at scale — the asynchronous path's own limits
§2.4 established that asynchronous, event-based communication decouples publisher and subscriber availability — at fleet scale, with potentially dozens of subscribers to a single, high-volume event type (an "OrderPlaced" event consumed by Inventory, Shipping, Analytics, Fraud, Notifications, and a growing list of future subscribers), the **publisher's fan-out cost** and the underlying broker's own partition/throughput scaling (the subject of the later, dedicated Kafka/RabbitMQ modules) become the new scaling constraint, replacing the synchronous call chain's compounding-availability constraint with a different one: a broker sized for five subscribers at migration time will not transparently absorb fifty subscribers added incrementally over years without deliberate broker-capacity and partitioning review — decoupling availability via events doesn't mean fan-out is free at unbounded scale, only that it fails differently (backlog growth, per 7.3) than a synchronous chain would (cascading unavailability).
