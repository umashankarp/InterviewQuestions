...
- **Choose the tier before the tool.** Business/Enterprise (no-train, minimal-retention, admin policy, audit logs) is the floor for any commercial codebase; consumer tiers are disqualifying for regulated source. Decide this first; the tool comparison is secondary.
- **Pin where inference runs.** Use Bedrock/Vertex region pinning for Claude Code where data residency is a requirement; document the actual data path for Copilot and confirm it against the requirement.
- **Sandbox the agentic tiers for real** (§2.4): working-tree-only filesystem, no production credentials, deny-by-default egress, no prod compute context. This is the highest-leverage control.
- **Distribute policy centrally and fail closed.** Enterprise-managed `settings.json` + hook bundle + MCP allowlist pushed from a control plane; if policy can't be fetched or verified, the assistant is disabled, not permissive.
- **Scope tasks small.** One well-defined change per agent session. Large vague tasks degrade under context compaction and produce unreviewable diffs.
- **Same gates, no bypass.** SAST, secret scan, dependency/license scan, four-eyes review, change-control ticket — identical for AI-assisted changes. Audit the pipeline for any rule keyed on "AI."
- **Reliable provenance.** Enforce the AI-assisted marker with a commit hook *and* a CI reconciliation that fails if it's missing (see §4, §14). Don't rely on the tool to self-report.
- **Allowlist MCP servers** (Module 167): a vetted, risk-tiered set, individual installation disabled at the policy layer, credentials via vault substitution never in context.
- **Archive transcript + config + model version** per AI-assisted commit as the audit record; add a behaviour canary against silent model drift.
- **Measure adoption and outcomes**, not just seats: acceptance rate, revert rate on AI-assisted commits, review time, and defect-escape rate compared to baseline — so the risk committee sees data, not vibes.
- **Human owns it.** Train engineers that an accepted suggestion is their code, their name, their accountability.

---

## 6. Anti-patterns

- **Consumer-tier on the real codebase.** Code potentially used for training; no admin controls; no audit. Non-starter in FinTech.
- **`bypassPermissions` / "always allow" as the default rollout setting.** Removes every gate at once for convenience; the blast radius is now "anything the shell can do."
- **Agent runs with the engineer's real cloud/prod credentials in the environment.** A redirected agent can now do what the engineer can do in production.
- **Treating "tests pass" as review.** Green CI proves covered behaviours; money-correctness and authz invariants are usually not covered (Module 178).
- **Waiving SAST/secret-scan for AI output** ("the model wouldn't do that") — it demonstrably does.
- **Glob-as-policy** (§4): assuming a pattern matches everything it "obviously" should, untested.
- **Letting engineers wire the assistant to arbitrary MCP servers.** Ungoverned third-party trust and data-egress decision made per-engineer (Module 167 §4 — the rug-pull).
- **Provenance the tool self-reports, unverified.** One code path that doesn't emit the marker and your audit trail silently has holes (§14).
- **Vendor concentration ignored.** The entire org's SDLC velocity now depends on one AI vendor's availability and pricing; no fallback, no exit plan (DORA-style operational-resilience gap).
- **Prompt-injection treated as hypothetical.** No egress control, no untrusted-content handling, agent pointed at repos containing third-party content and issue text.

---

## 7. Performance Engineering

- **Latency of completion** is the UX-critical number — sub-few-hundred-ms ghost text or engineers turn it off. This is a vendor property; your lever is network path (regional endpoint, no double-hop through a slow proxy) and not over-scanning in the local DLP agent (make the secret/PAN scan a fast regex+entropy pass, not a synchronous call to a remote DLP service on every keystroke-triggered request).
- **Agent-session cost and wall-clock** scale with the number of turns, which scales with task size and context growth. A 40-file vague task can be 10× the tokens and time of five scoped tasks that do the same work. Cost governance is per-session token budgets (Module 165 §2.5) and a turn cap; the org-level lever is training engineers to scope.
- **Context compaction** trades tokens for fidelity: past a threshold the agent reasons over a summary of its own work and quality drops. Keep sessions short enough to avoid it on critical tasks.
- **CI load** from the asynchronous coding agent: every issue-to-PR run consumes a sandbox runner and a full CI pass. At scale this is real spend and real queue contention — meter it, cap concurrency, and restrict which repos can trigger it.
- **Local DLP proxy** must not become the bottleneck: async audit emission (fire-and-forget to a queue, never blocking the model request), streaming pass-through (don't buffer the model's streamed response — Module 165 §2.3), and a fast-path for prompts with no secret-shaped content.
- **Benchmark the outcome, not the token count**: cost per *completed, merged* task is the honest metric — a cheap session that produces an unmergeable diff cost more than an expensive one that shipped.

---

## 8. Security

**Threat model — the assistant as an actor.**

| Threat | Vector | Mitigation |
|---|---|---|
| **Source/IP exfiltration** | Prompt context sent to model; injected instruction `curl`s source out | Enterprise no-train tier; region-pinned endpoint; deny-by-default egress from the agent sandbox; content exclusions on the most sensitive repos; repo-scoping (assistant sees only repos the engineer needs) |
| **Secrets in prompts / commits** | Engineer pastes a key; agent reads a `.env`; agent hardcodes a value | Local DLP scan (regex + entropy + Luhn for PANs) on every outbound prompt; pre-commit secret scanning; secret scanning in CI; no real secrets in the sandbox |
| **Prompt injection → redirected agent** | Poisoned README / comment / issue / web page / MCP response (Module 167 §2.3) | Untrusted-by-default handling of all tool output; approval gates on consequential actions; egress restriction; nothing valuable reachable from the sandbox; for the coding agent, attacker-influenceable issue text is assumed hostile |
| **Vulnerable generated code** | Model emits injectable query, weak crypto, missing authz check | SAST + manual security review, identical gates, no AI bypass; security-review focus on authz and money-correctness invariants tests miss |
| **Malicious / drifting MCP server** | Community server, or a vetted server's upstream "rug pull" adds a broad tool (Module 167 §2.3) | Central allowlist; capability-manifest drift detection; version pinning; risk-tiering (read-only vs code-writing vs deploying); vault-substituted credentials never in context |
| **License / copyright contamination** | Model reproduces public code verbatim | Copilot duplicate-detection filter on (enables the Copyright Commitment); independent license scan on added dependencies |
| **Privilege escalation via the agent** | Agent runs with engineer's prod/cloud creds | Sandbox has test/mock creds only; agent never has kubectl/deploy context for live systems |
| **Supply-chain: agent adds a dependency** | Model pulls in a package to solve a task | Dependency review gate applies to AI-added deps; SBOM updated; provenance shows the addition was AI-originated |
| **Audit-trail gaps** | A tool code path doesn't emit provenance; transcript not archived | Commit hook + CI reconciliation that *fails the build* on a missing marker (§14); append-only/WORM audit store |

**Regulatory weight.** SOX: named accountable human + segregation of duties + change record, unchanged. PCI-DSS: no PAN in prompts/logs/transcripts — the DLP scan and log redaction are in scope. Data residency / GDPR: documented data path, region pinning, DPA with the vendor. Operational resilience (DORA-adjacent): the AI vendor is now a critical third-party dependency — concentration risk, exit plan, and a degraded-mode-without-the-assistant plan are required artefacts.

---

## 9. Scalability

- **Rollout to thousands of engineers** is a policy-distribution and identity problem, not a compute one. The control plane (§12) pushes managed settings, hook bundles, and MCP allowlists to every managed workstation; SSO/SCIM governs seat provisioning and de-provisioning (a leaver loses assistant access with their other access).
- **Horizontal**: model inference scales at the vendor; your components (DLP proxy, MCP gateway, audit service) are stateless request handlers behind a load balancer — scale on request rate.
- **Audit store** is the write-heavy component: every prompt, tool call, and provenance record. Append-only, partitioned by date, tiered to cheap object storage with a WORM lock for the retention window; hot index only for the recent window. Same 7-year-vs-2%-hot calculus as Module 180 §12 — most of the volume is low-value and can live cold.
- **MCP gateway** scales with concurrent agent sessions × tool calls per session; cache capability manifests, and rate-limit per engineer to bound a runaway loop's fan-out.
- **Asynchronous coding-agent** capacity is CI-runner capacity: cap per-repo concurrency, restrict eligible repos, and meter spend — this is where costs scale non-linearly with adoption.
- **HA / DR**: the assistant is a productivity tool, not a transaction path — target graceful degradation (engineers work without it) rather than five-nines. But the *audit service* is on the compliance path and needs proper durability and DR. CAP-wise the audit store is CP (a lost provenance record is worse than a slow write); the assistant request path is AP (better to complete the request and emit the audit event asynchronously, reconciling later).
