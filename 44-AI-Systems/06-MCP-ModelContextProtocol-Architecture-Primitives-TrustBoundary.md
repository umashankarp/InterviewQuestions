...
**What:** **MCP (Model Context Protocol)** is an open, standardized protocol connecting AI applications (the **Host**, e.g., an IDE or chat client — which manages one or more **Client** instances, each a dedicated, 1:1 connection to a single **Server**) to external tools and data sources. A Server exposes three primitives: **Resources** (read-only, contextual data — analogous to a GET endpoint, no side effects), **Tools** (invokable functions with potential side effects — the protocol-standardized form of the function calling), and **Prompts** (reusable, server-defined prompt templates a user can explicitly invoke).

**Why:** MCP exists to solve exactly the N×M integration problem established for identity federation: without a standard protocol, every AI application (N of them) needing to integrate with every external tool/data source (M of them) requires a bespoke, N×M integration — MCP collapses this to N+M, exactly as the identity broker collapsed point-to-point federation, now applied to tool/context integration instead of identity trust relationships. **This module's central, genuinely new finding, testing A10's own prediction directly: MCP is not merely a familiar instance of an already-established pattern — it introduces a structurally new risk because its own design goal (frictionless, low-friction third-party server addition, "install this MCP server" as a one-click action) is in direct, unresolved tension with every trust-tier and least-privilege discipline Modules 163-166 established as necessary.**

**When:** MCP is warranted specifically for genuinely reusable, cross-application, or cross-team tool/context integrations where standardization's ecosystem-interoperability benefit outweighs the trust-governance overhead this module develops — not for a narrow, single-purpose, highly consequential internal tool, where the direct, custom-built, fully-governed function-calling integration remains the safer default.
...
*Why correct:* Matches.
*Common mistakes:* Failing to connect this to the identical pattern this course already established for identity federation.
...
*Follow-up:* Which primitive most directly corresponds to the function calling?

**B3. What is the structural tension this module identifies between MCP's design goal and the established authorization discipline?**
*Ideal Answer:* MCP is explicitly designed for frictionless, low-friction third-party server adoption; earlier analysis established that every new capability requires a formal, governed, least-privilege authorization review — these two goals are in direct, unresolved tension.
*Why correct:* Matches.
...
*Follow-up:* What concept does this most directly extend?

**B6. How does MCP's Resources-versus-Tools distinction relate to the authorization risk-tiering?**
*Ideal Answer:* It's a point of convergent design — MCP's own protocol-level primitive separation maps directly onto the benign-versus-consequential risk tiers, giving organizations a structural hook for applying that existing discipline.
*Why correct:* Matches.
...
*Follow-up:* What mechanism should sampling requests be subject to identically?

**B8. In the incident, was the individual relationship managers' original adoption decision unreasonable?**
...
**B10. Why is the independent-authorization backstop insufficient on its own against MCP's specific new risk?**
...
*Why correct:* Matches.
*Common mistakes:* Assuming the authorization discipline, correctly applied, fully closes this module's new risk without the additional, MCP-specific server-allowlist governance layer.
*Follow-up:* What specific control closes this upstream gap?

### Intermediate (10)

**I1. Design the governed MCP-server-adoption process that would have prevented the incident.**
*Ideal Answer:* A centralized, mandatory review process for any MCP server before it's added to any employee's Host application — reviewing the server's provenance/maintainer trustworthiness, its declared Tools' authorization scope against the risk tiers, and establishing a re-review trigger on any subsequent version change (directly closing the "rug pull" gap) — with individual, unreviewed installation structurally disabled at the Host-application-configuration level, not merely discouraged by policy.
*Why correct:* Matches the precise fix, including the structural (not merely policy-based) prevention mechanism.
...
**I3. Compare tool-description injection against RAG's indirect prompt injection — same mechanism, or genuinely distinct?**
...
*Follow-up:* Design an output-validation check (the fourth defense layer) specifically targeting tool-description-driven manipulation, distinct from a RAG-content-focused check.

**I4. Design the authorization-scope mapping applying the risk tiers to a specific MCP server's declared Resources and Tools.**
*Ideal Answer:* Every declared Resource defaults to the "benign" tier (read-only, lighter governance, no human-confirmation requirement) unless its specific data sensitivity warrants elevation (e.g., a Resource exposing highly confidential client data might still warrant access-control scoping even though it's read-only); every declared Tool is individually classified per the magnitude-calibrated framework (not merely "Tool = consequential" uniformly) — a narrow, low-magnitude Tool (e.g., "add a note to this client record") might remain benign-tier, while a broad or high-magnitude Tool (e.g., "bulk-export client records," exactly the incident) is explicitly consequential-tier, requiring human confirmation before any invocation.
*Why correct:* Correctly applies the magnitude-calibrated (not merely binary) risk-tiering to MCP's own primitive structure, matching the convergent-design finding with genuine, specific application rather than a vague restatement.
*Common mistakes:* Applying a uniform "all Tools are consequential, all Resources are benign" binary classification, missing the magnitude-calibration nuance that a Tool's actual risk depends on its specific scope, not merely its primitive category.
...
**I5. A server requests sampling to help it process a large dataset the server itself has direct access to. Design the authorization check this request should pass/.**
*Ideal Answer:* Treat the sampling request identically to a Tool invocation from the perspective: verify the requesting server is on the governed allowlist (I1); verify the specific sampling request's scope/purpose doesn't exceed what that server's own approved capability grant covers; apply the identical cost-governance interaction-level budgeting to the resulting LLM call, attributing its cost to the correct interaction/session rather than treating it as a cost-invisible, server-internal operation.
*Why correct:* Correctly applies both authorization and cost-governance scrutiny to the sampling request, matching the precise finding that inverted-control requests warrant identical, not exempted, governance.
...
*Common mistakes:* Treating this module's finding as a mere restatement of the trust-tiering, missing the specific, structural "decentralized adoption" dimension that makes it genuinely new.
*Follow-up:* Could an organization apply the trust-tiering discipline perfectly and still suffer the incident? Why?
...
**I8. Compare the governance cost of adopting MCP with proper allowlist/re-review governance (I1/I2) against building an equivalent set of integrations via the bespoke function-calling approach directly.**
*Ideal Answer:* MCP with proper governance still provides the genuine engineering-effort savings of the N+M architecture (reusing existing, community/vendor-built servers rather than building bespoke integrations for each) while adding the governance overhead this module establishes (allowlist review, drift detection) — a net positive specifically when the organization needs many, genuinely reusable integrations and can absorb the governance investment; a bespoke,-style approach avoids MCP's third-party-trust-surface risk entirely (since every function is built and reviewed in-house) but forfeits the engineering-effort reuse benefit, requiring the organization to build and maintain every integration itself — the correct choice, depends on whether the specific integration's reusability value across the organization outweighs the trust-governance investment MCP-with-proper-governance requires.
...
*Follow-up:* For which specific kind of integration would bespoke,-style building clearly be the better choice regardless of MCP's general availability?
...
*Ideal Answer:* Governance (allowlisting, drift detection) reduces but cannot fully eliminate the risk that a sufficiently sophisticated, patient attacker compromises a trusted, already-approved server's distribution channel in a way that produces a change subtle enough to evade the specific drift-detection signals (I2/I7) the organization has implemented — directly recurring this course's now-repeated finding (A5's claims-mapping-drift residual risk) that a governance fix reduces the *likelihood* of recurrence without making it structurally impossible, since the underlying limitation (trusting a third party's own security and distribution integrity) remains, however well-governed the organization's own review process is.
*Why correct:* Correctly identifies the residual, structural limitation even a well-implemented governance program cannot fully close, directly connecting to A5's identical honest-limitation-acknowledgment finding from an earlier module.
...
**I10. Synthesize whether this module confirms or refutes A10's prediction — did MCP return to a familiar risk instance, or introduce a genuinely new source?**
*Ideal Answer:* Genuinely new source, confirming the more consequential A10's two predicted possibilities: this module's central finding (the adoption-friction-versus-governance-rigor tension, and the rug-pull mechanism) is not merely a new instance of an already-established category (unlike, say, this module's Resources/Tools risk-tiering,, which genuinely IS a familiar-instance confirmation) — the specific mechanism of a protocol whose own design goal actively works against the governance discipline the rest of this domain has established is a structurally new tension this course's prior 166 modules never examined, since no prior domain's central technology was explicitly designed to optimize for low-friction adoption of third-party, potentially-untrusted capability in quite this specific way.
*Why correct:* Correctly resolves A10's open prediction with a precise, evidence-grounded answer, distinguishing which of this module's specific findings are familiar-instance confirmations versus the genuinely new contribution.
...
*Follow-up:* Given this genuinely new source identified, does the capstone need to specifically engineer a scenario exercising this exact tension, or has it already been sufficiently demonstrated in this module's own incident?
...
*Ideal Answer:* Overstated — MCP's standardization genuinely provides a well-specified *protocol* (message format, primitive structure, connection lifecycle), but says nothing about the trustworthiness of any *specific server's implementation* built against that protocol — a malicious or poorly-maintained server can be a fully protocol-compliant, standard-conforming MCP server while still exhibiting the rug-pull risk or the overly-broad-capability risk; standardization improves interoperability and reduces integration engineering effort, but provides zero inherent guarantee about any individual server's security, maintenance quality, or capability scope, which remain entirely a function of that specific server's own implementation and governance, independent of its protocol compliance.
...
**A5. Design a red-team exercise specifically targeting the tool-description injection vector, distinct from both the content-injection red-teaming and A5's multi-step-blast-radius red-teaming.**
*Ideal Answer:* Construct a test MCP server with deliberately crafted, adversarial tool descriptions (containing hidden instructions, misleading capability claims, or subtly-manipulative phrasing) and measure whether an agent connecting to it: (a) invokes the described tool with arguments consistent with the description's hidden, manipulative intent rather than the user's actual, legitimate request; (b) exhibits any behavioral change in unrelated, subsequent reasoning steps after having read the adversarial description, testing whether tool-description content can produce the same downstream-step redirection A5 tested for retrieved-content injection, now via the tool-metadata vector specifically.
*Why correct:* Correctly designs a red-team methodology targeting the specific, MCP-unique injection vector (tool-description metadata) while also testing for the downstream-propagation risk A5 established for a different vector, demonstrating appropriate methodology reuse alongside genuine, vector-specific new test design.
*Common mistakes:* Reusing or the existing red-team tests unmodified, without designing a test specifically targeting tool-description content as its own, distinct injection surface.
...
**A8. Synthesize this module's third-party-trust-boundary finding against the IAM-domain PAM/governance discipline. Is MCP-server governance simply PAM applied to a new asset class, or does it require a genuinely distinct governance model?**
*Ideal Answer:* Substantially the same underlying principle (least privilege, JIT-elevation-equivalent scoping, periodic re-certification, per the PAM framework) applied to a new asset class (MCP servers rather than human/service-account credentials) — but with one genuinely distinct wrinkle the original framework didn't need to address: the PAM assumes the *organization itself* controls provisioning and de-provisioning of the privileged asset (an employee's elevated access); MCP server governance must additionally account for the fact that the *asset's own capability can change unilaterally, from outside the organization's control*, at the third-party maintainer's own discretion (the rug pull) — a "privileged credential that can silently redefine its own privilege scope without the granting organization's knowledge or consent," a risk shape the original human/service-account-focused PAM framework never needed to consider, since a human's or service account's own permission scope is always organization-controlled, never externally, unilaterally redefinable by the credential itself.
*Why correct:* Correctly identifies both the strong structural parity with the PAM discipline and the one, precise, genuinely new wrinkle (externally, unilaterally redefinable privilege scope) this asset class introduces that the original framework didn't anticipate.
*Common mistakes:* Either declaring MCP governance identical to the PAM with no meaningful distinction, or treating it as an entirely unrelated, novel governance challenge, missing the precise "same core principle, one genuinely new wrinkle" answer this question requires.
*Follow-up:* Would the own break-glass emergency-access pattern (of that module) have any useful analogue for MCP server governance — what would "break-glass" mean in this context?
...
**E1. Q: An MCP server is proposed to give an internal AI assistant access to the core banking ledger and a market-data feed. What data-governance risks does routing regulated data through MCP introduce, and how do you control them?**
*Ideal Answer:* MCP makes it easy to expose data to an LLM Host, and *that* is the risk in a bank: regulated data (account numbers, PANs, PII, MNPI/market-sensitive data) flowing through the server into the model's context can leak (into prompts/logs/traces, into a model provider that trains on it, or across a data-residency boundary), and can breach need-to-know (the assistant assembling a picture no single user should see — the composition risk). Controls: (1) **data minimization at the server** — expose only the fields the task needs, tokenize/mask crown-jewel data (PAN, full account number) so raw values never enter the model, and apply the *user's own* entitlements so the server can't over-share; (2) **residency & provider terms** — ensure the model endpoint honors data-residency and a no-train/controlled-retention agreement, since the data leaves through the model, not just the server; (3) **PII/PCI-safe telemetry** — the assistant's logs/traces must redact, not capture, regulated values (correlation IDs, not PANs); (4) **read-only + risk-tiered Tools** — a ledger *read* server is different from one that can *post* entries; consequential Tools get authorization + human gates; (5) **MNPI/insider controls** — market-data access through an assistant must respect information barriers, or you've built an insider-trading surface; (6) full **audit** of what the assistant accessed for whom. The Principal framing: MCP doesn't change *what* regulated-data governance requires (minimization, entitlements, residency, redaction, need-to-know) — it adds a *new, easy pipe* that can bypass those controls if you don't apply them at the server and the model boundary; treat the MCP server as a regulated-data egress point and govern it exactly as one.
*Why correct:* Identifies leakage/residency/need-to-know/MNPI risks of routing regulated data through MCP and controls them with minimization/tokenization + entitlements + residency/no-train terms + redacted telemetry + read-only-vs-consequential tiering + audit.
*Common mistakes:* Exposing full records because "the model needs context"; ignoring the model provider as an egress/residency boundary; logging PANs into traces; no entitlement enforcement at the server; market-data access with no information-barrier consideration.
*Follow-up:* "How do you stop the assistant assembling a need-to-know violation across several individually-allowed reads?" / "Where does tokenization sit so raw PANs never reach the model?"

**E2. Q: The break-glass follow-up from A8: what would 'break-glass' emergency access mean for MCP-server governance in a regulated firm, and how do you keep it from becoming a backdoor?**
*Ideal Answer:* Break-glass for MCP = a pre-defined, tightly-controlled path to connect a *non-allowlisted* server (or grant a server elevated/consequential capability) during a genuine incident when the normal review cadence is too slow — the analogue of PAM emergency access. Keep it safe with the same discipline: (1) **time-boxed & auto-revoking** — the emergency connection/capability expires automatically (minutes/hours), never lingers; (2) **maker-checker to invoke** — a second authorized identity approves the break-glass, so no single person can wire in an ungoverned server; (3) **loud & audited** — every break-glass activation pages the security team and writes an immutable record (who, what server/capability, why, when), because the whole point is that it's rare and reviewed after the fact; (4) **least scope** — grant only the specific server/capability the incident needs, not blanket "disable the allowlist"; (5) **mandatory post-incident review** to either formally onboard the server through normal governance or remove it. The Principal framing: break-glass is a *controlled exception*, not an off-switch for governance — time-boxed, dual-approved, loudly-audited, least-scoped, and always followed by review — otherwise "emergency access" quietly becomes the ungoverned backdoor that MCP allowlisting existed to prevent.
*Why correct:* Maps PAM break-glass to MCP correctly — time-boxed auto-revoke + maker-checker + loud audit + least scope + mandatory post-review — and frames it as a controlled exception, not a governance bypass.
*Common mistakes:* A standing "emergency" bypass that never expires; single-person activation; unlogged; blanket allowlist disable; no post-incident onboarding/removal.
*Follow-up:* "What auto-revocation window is appropriate, and who approves the activation?" / "How do you ensure a break-glass server is either onboarded or removed after the incident?"

### AI Coding Assistants as MCP Clients — Claude Code & GitHub Copilot

**CC1. Q: Claude Code and GitHub Copilot are prominent MCP *clients* — engineers can connect them to MCP servers (GitHub, internal tickets, databases, custom servers) that read your code and systems. How do you govern which MCP servers your coding assistants may connect to in a bank, and why is this an MCP-governance problem, not just a tooling preference?**
*Ideal Answer:* Every MCP server a coding assistant connects to becomes an extension of that assistant's trust boundary *and* an indirect-injection channel (and the tool-description/retrieved-content injection vectors) — so an engineer casually wiring Claude Code or Copilot to a community MCP server is making an ungoverned third-party-trust and data-egress decision on the firm's behalf. Govern it exactly as this module prescribes for any MCP adoption: (1) a **centrally-enforced allowlist** of vetted servers, with individual/ad-hoc server installation disabled at the Host/config level (I1) — engineers connect only to approved servers, not arbitrary ones; (2) **risk-tier the servers** — a read-only docs server differs from one that can write code, open PRs, hit a database, or deploy; consequential-capability servers get stricter review and approval-gated actions; (3) **credentials via a vault/egress proxy**, never in the assistant's context or config in plaintext — the assistant should present an opaque placeholder, with the real token substituted at egress, so a prompt/history leak can't exfiltrate it; (4) **drift detection** on each server's capability manifest (I2) plus behavioral monitoring (A4) for source-level compromise; (5) treat **third-party/community MCP servers as untrusted supply chain** — review, pin versions, sandbox, and remember the rug-pull risk (A8's externally-redefinable privilege). The Principal framing: an AI coding assistant connected to MCP servers is a live instance of *exactly* this module's third-party-trust-boundary problem inside your own engineering org — the same allowlist + risk-tiering + vaulted-credential + drift/behavioral-monitoring governance applies, and "which MCP servers can our coding assistants use" is a security decision the platform owns, not a per-engineer convenience toggle.
*Why correct:* Frames coding-assistant MCP connections as this module's third-party-trust/injection problem, and applies centrally-enforced allowlist + risk-tiering + vaulted credentials + drift/behavioral monitoring + untrusted-supply-chain handling.
...
**Problem:** Implement the risk-tier classifier mapping MCP Tool declarations onto the magnitude-calibrated authorization tiers.
...
def classify_tool_risk(tool: ToolDeclaration, side_effect_keywords: set[str],
 ...
 # the incident: a "bulk export" Tool must NEVER be classified narrow,
...
class McpConnectionGate:
 ...
 f"version. Connection REFUSED pending mandatory re-review (the rug-pull defense)."
)
...
class SamplingGovernor:
 ...
 # Cost-governance treats this IDENTICALLY to a Tool invocation (/I5) —
...
**Trade-offs:** MCP's engineering-effort N+M benefit versus governance-effort cost that doesn't scale down commensurately; bespoke integration for narrow, highly-consequential tools versus MCP for genuinely reusable, cross-team integrations (I8).
...
**SOLID mapping:** SRP — each component handles exactly one governance concern; DIP — `SamplingGovernor` depends on injected `cost_governor` and `allowlist` abstractions, directly reusing/166's already-hardened cost-governance infrastructure rather than reimplementing it.
...
**Decision:** Should an organization adopt MCP broadly, as its standard integration approach, or maintain bespoke,-style integrations as the primary pattern with MCP reserved for specific, narrow cases?

**Option A — MCP as the universal default integration approach:** *Advantages:* Maximal N+M engineering-effort savings across every integration need. *Disadvantages:* Maximizes exposure to this module's own central risk (the trust-boundary tension) across the organization's entire integration surface, requiring the full governance program (A1) to be applied comprehensively, everywhere. *Risk:* High if governance isn't comprehensively, consistently applied; the governance-effort cost doesn't scale down with broader adoption.

**Option B — Bespoke,-style integration as the universal default, MCP never adopted:** *Advantages:* Avoids this module's third-party-trust-surface risk entirely. *Disadvantages:* Forfeits the genuine engineering-effort reuse benefit MCP's standardization provides, particularly costly for genuinely common, widely-needed integrations many organizations independently need to build from scratch.

**Option C — Risk-tiered: MCP for genuinely reusable, cross-team/cross-product integrations where the governance investment (A1) is justified by the integration's broad reuse value; bespoke-style integration for narrow, single-purpose, highly consequential tools where avoiding third-party trust surface entirely outweighs any reuse benefit (recommended):** Matches this course's now-fully-established risk-tiered-investment principle.
...
**Cross-team communication:** A1's full governance architecture requires coordination across the AI-systems team, the platform's existing IAM/authorization team, and — given the incident's business-line-specific origin — the specific business unit (relationship management) whose own operational needs drove the original, ungoverned adoption, reinforcing that AI-systems governance is never a purely technical, engineering-team-siloed concern.
...
**Cost optimization:** Option C's risk-tiered MCP-versus-bespoke adoption decision directly optimizes engineering investment by reserving MCP's real reuse benefit for cases genuinely warranting it, while avoiding its real governance cost for narrow, single-purpose integrations better served by direct, bespoke building.
