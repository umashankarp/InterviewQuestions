...
- **One feature definition, two stores.** Share the transformation code between offline and online; treat a feature as a governed, owned, versioned, lineage-tracked artefact — not a re-implemented snippet.
- **Point-in-time correctness** in every training join. If you can't prove a feature's value is as-of the label time, assume leakage.
- **Log the production feature vector for every scoring event** and run a **skew detector** comparing it to the offline recomputation. This is the only reliable training-serving-skew alarm.
- **Reproducible pipelines**: pipeline-as-code, hashed data snapshots, experiment tracking, pinned environments — enough that a validator can regenerate the model.
- **The registry carries governance metadata**, not just artefacts: validation status, tier, limitations, use restrictions, monitoring plan, approver.
- **Deploy in stages** — shadow → challenger → canary → prod — with rollback always available and a challenger for every material model.
- **Monitor all four drifts**; while labels lag, use **pre-registered proxy indicators with floors** (EPD, vintage curves, score PSI, approval rate, override rate) and act on sustained breaches — don't wait for the real labels.
- **Slice everything** — drift and performance concentrated in a segment is the real signal; aggregates hide it (the course-wide pattern).
- **Fairness testing is a gate**, not a metric — exclude prohibited attributes and their proxies; test disparate impact across protected classes.
- **Keep LLMs out of the regulated decision** where possible; where they inform one, wrap them in a validated deterministic surface + full evaluation (Module 184) + guardrails + human review + documentation.
- **The model inventory must be complete** — an unregistered model in production is the finding that matters most.
- **Engage validation early** — bring the independent validators in during design, not as a gate at the end, so "effective challenge" shapes the model instead of blocking it.

---

## 6. Anti-patterns

- **Different code for the same feature offline vs online** — the classic training-serving skew; great offline, disappointing prod.
- **Verifying feature parity only in steady state** — misses cold state, staleness, and restart windows (§4).
- **A systematically-wrong feature treated as a valid value** — worse than missing; no model defence.
- **Feature leakage from a non-point-in-time join** — the model learns from the future; offline metrics are a fantasy.
- **"We'll measure accuracy when the labels arrive"** for a credit model — the losses are booked by then; you need proxies with floors and the will to act on them (§14).
- **Aggregate-only drift monitoring** — a segment regression hides in the average (the recurring pattern).
- **An incomplete model inventory / shadow models** — the top MRM finding; a model nobody registered is a model nobody governs.
- **Treating validation as a rubber-stamp gate at the end** — "effective challenge" that can't get changes made isn't effective; engage early.
- **Prohibited attributes or their proxies in the feature set** — a fair-lending violation, not a modelling choice.
- **Trusting an LLM's post-hoc rationale as an explanation** — it's plausible, not faithful; not a defensible adverse-action reason.
- **Retraining on a schedule with no trigger discipline** — either you retrain when nothing changed (churn, revalidation cost) or you don't retrain when concept drift hit.
- **No challenger** — you have no evidence-based way to know a better model exists or that yours has decayed.

---

## 7. Performance Engineering

- **Online feature-serving latency** is inside the request budget. A fraud decision has ~10–50 ms total; feature retrieval must be single-digit ms — an in-memory / low-latency KV online store (Redis/DynamoDB/Cassandra/a purpose-built store), features denormalised per entity, batched multi-get, and a hard timeout with a defined fallback (score with what you have + a conservative adjustment) rather than blocking the authorisation.
- **Feature freshness vs cost** — a 30-day aggregate updated per-event is expensive; per-minute or per-hour may be fine. Set freshness SLAs per feature from how fast the signal actually moves.
- **Point-in-time joins at training scale** — an as-of join over years of history × millions of entities × thousands of features is a heavy Spark/warehouse job; partition by time, pre-aggregate, and materialise training sets rather than recomputing per experiment.
- **Backfill cost** — recomputing a new feature's history for training is often the dominant one-time cost of adding a feature; budget it.
- **Drift-monitoring pipeline** — PSI/KS on thousands of features × many segments × a cadence is real compute; sample, prioritise the top-importance features and the consequential segments, and run the full sweep less often.
- **Training pipeline throughput** — GBM training on tabular data is fast (minutes–hours); the time sink is feature computation and backfills, not the fit. Cache computed feature sets keyed on (definition version, entity set, time range).
- **Champion/challenger overhead** — scoring every transaction through N models multiplies serving cost; shadow a *sample* unless the full population is needed for a rare-event metric.
...
|---|---|---|
| **Prohibited-attribute / proxy leakage into a model** | A feature encodes race/sex/age or a close proxy (zip code, first name, certain merchant categories) | Feature governance: prohibited-attribute list + proxy review at feature onboarding; disparate-impact testing as a validation gate; feature lineage so a validator can trace what each feature is derived from |
| **Training-data poisoning** | An attacker (or a compromised upstream) injects labelled examples that teach a blind spot (transactions with pattern P are "not fraud") | Data provenance and anomaly detection on the training set; label-source controls; a challenger and outcomes analysis that would surface a systematic miss; restrict who can contribute labels |
| **Adversarial evasion** | Fraudsters probe the model and shape transactions to score below the threshold | Don't expose scores/thresholds; monitor for probing patterns; ensemble + frequent retrain + rules layer; treat the exact feature set and thresholds as sensitive |
| **Model / feature-logic theft** | The model artefact or the feature definitions (which encode fraud expertise) are exfiltrated | Treat models and feature repos as crown-jewel artefacts (Module 182 §A6) — access-controlled registry, egress control, signed/hashed, audit |
| **Feature store as an over-broad data-access surface** | The store aggregates data from many sources; a compromised consumer reads features it shouldn't (another business line's, or PII-derived) | Per-consumer feature-level access control; PII-derived features tagged and restricted; the online store is in data-classification scope |
| **Shadow / unregistered models** | A team ships a model outside the inventory; it's ungoverned, unvalidated, unmonitored | Automated discovery (scan serving infra, API gateways, notebooks-in-prod) reconciled against the inventory; block deploy paths that don't go through the registry |
| **Vendor-model opacity** | A hosted model can't be inspected; you can't validate its internals or training data | Validate the *use*, the guardrails, and the evaluation; require vendor attestations (no-train, data handling, testing); contractual notice of changes; a challenger you *can* inspect as a benchmark |

The framing: in a regulated firm the security-adjacent risks (fair lending, an incomplete inventory, an unvalidated model making decisions) are **compliance risks with regulatory teeth** — an MRA or a consent order — not just technical exposures.

---

## 9. Scalability

- **Feature store at scale** — millions of entities, thousands of features, years of history for point-in-time joins. Scale the offline store on a partitioned warehouse/lakehouse; scale the online store as a horizontally-partitioned low-latency KV store keyed by entity; separate the two so training backfills don't contend with serving.
- **Many models × many monitors** — 50+ models each with per-feature drift, per-segment performance, fairness, and proxy indicators is a monitoring-platform problem: a shared drift-monitoring service with per-model config, prioritised sampling, and a common alerting/triage flow, not 50 bespoke dashboards.
- **The MRM function is the bottleneck** — independent validation is skilled, slow work and there are more models than validators. Scale it with **materiality tiering** (deep validation only where the stakes justify it), **reusable validation components** (a standard fairness-test suite, a standard drift-monitoring template, a standard documentation format), **validation of the platform** (validate the feature store's point-in-time correctness once, centrally, so every model doesn't re-litigate it), and engaging validators early so rework is minimised.
- **Retraining orchestration** — dozens of models each with scheduled + trigger-based retrains, each running its pipeline, eval gate (Module 184), and (tier-dependent) revalidation — needs orchestration, quotas, and a queue, not per-team cron.
- **Cost governance** — feature compute/backfill, serving, monitoring, and retraining spend metered per model/team.
- **Consistency** — the model inventory and registry are **CP** (an ungoverned or unvalidated production model is the worst outcome); the online feature store trades some consistency for latency (last-write-wins, bounded staleness) but with **freshness SLAs and a coverage signal** so the model knows when a feature isn't trustworthy (§4).
