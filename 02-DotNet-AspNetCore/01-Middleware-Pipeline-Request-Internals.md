...
1. **Q: What does the middleware pipeline compile down to at startup — what is a `RequestDelegate` chain physically?** **A:** At `Build()`, ASP.NET Core folds the registered `Use` components in reverse order into a single composed `RequestDelegate`: each component is a function taking the *next* delegate and returning a new delegate wrapping it. The result is one nested closure chain built once per application, not per request — invoking a request is just calling the outermost delegate; there's no list iteration or dispatch table at request time.
2. **Q: Compare convention-based middleware classes with `IMiddleware` (factory-based) — what changes about lifetime and DI?** **A:** Convention-based middleware is constructed *once* (singleton-like) with constructor injection limited to singletons, and per-request dependencies must come as `InvokeAsync` method parameters (resolved from `RequestServices`). `IMiddleware` is resolved from DI *per request* via `IMiddlewareFactory`, so it can constructor-inject scoped services safely and be registered with any lifetime. Choosing convention-based and constructor-injecting a scoped service is the classic captive-dependency middleware bug.
3. **Q: How does short-circuiting interact with response-modifying middleware upstream — why can a short-circuit break response compression or logging?** **A:** Upstream middleware's "on the way out" code still runs when a downstream component short-circuits, but middleware that hooked response *start* (via `OnStarting`) or wraps the body stream only works if the short-circuiting writer plays by the rules: writing directly to the raw `Response.Body` bypasses a compression/buffering stream wrapper installed upstream, and setting headers after the first body write throws because headers are already flushed. Correct short-circuits set status/headers before writing and write through `HttpContext.Response.Body` as currently wrapped.
4. **Q: What are `HttpResponse.OnStarting` and `OnCompleted`, and what problem does each solve?** **A:** `OnStarting` registers a callback invoked just before the first response bytes flush — the last safe moment to set headers (ETags, timing headers, cookies computed late); `OnCompleted` runs after the response fully completes — safe for cleanup/telemetry that must not delay the client. They exist because middleware "on the way out" code runs *after* downstream wrote the body (headers frozen), so header mutation must be scheduled at start-time via callback instead.
5. **Q: Explain Kestrel's request processing model from socket to middleware — where do the thread(s) come from?** **A:** Kestrel's transport (Socket transport using `SocketAsyncEventArgs`/epoll-io_uring style async I/O) reads connection bytes into `System.IO.Pipelines` pipes; HTTP parsing happens on thread-pool threads processing pipe data, then the composed `RequestDelegate` executes on a thread-pool thread with async continuations resuming on any pool thread. There is no dedicated thread per connection or per request — which is why blocking in middleware directly attacks the shared pool (Module 9's starvation).
6. **Q: What is `IHttpContextAccessor`, what does it cost, and what's the modern guidance on using it?** **A:** It exposes `HttpContext.Current`-style ambient access backed by an `AsyncLocal<HttpContextHolder>` — imposing a small per-request cost (context flow through every async hop) and, more importantly, hiding a dependency that makes code untestable and unusable outside requests. Guidance: register it only where genuinely needed (logging enrichers, legacy adapters); prefer passing needed values (user, tenant, correlation ID) as explicit parameters or scoped context objects; never touch it from background threads where it silently returns null or a recycled context.
7. **Q: Why is `HttpContext` recycled between requests, and what bug class does that create for captured references?** **A:** Kestrel pools and reuses `HttpContext` and its feature objects to avoid per-request allocation; after the request ends the same object instance is reset and reused for an unrelated request. Any captured reference (in a fire-and-forget task, a static cache, a closure completing later) now observes *another request's* data — manifesting as cross-user data bleed, the nastiest bug class. Rule: copy needed values out before going async-void/background; never store `HttpContext` or its sub-objects beyond the request.
8. **Q: What is the feature collection (`HttpContext.Features`), and when would you use it over items/DI?** **A:** Features are per-connection/request capability interfaces (`IHttpResponseBodyFeature`, `IHttpUpgradeFeature`, `IHttpResetFeature`) the server and middleware use to expose/override low-level behaviors — the extensibility seam beneath the friendly `HttpContext` facade. Use them to detect capabilities (is response buffering available? can this connection upgrade to WebSockets?) or replace behavior (wrapping `IHttpResponseBodyFeature` to intercept body writes — how compression/caching middleware actually hook in); use `Items` for per-request app data and DI for services.
9. **Q: How does request buffering (`EnableBuffering`) actually work, and what are its limits under load?** **A:** `Request.EnableBuffering()` swaps the body stream for a `FileBufferingReadStream`: it buffers in memory up to a threshold (default 30KB) then spills to a temp file, making the body seekable for multiple readers (model binding after audit logging). Limits: large uploads consume disk and add I/O latency; unbounded concurrent buffered uploads can exhaust temp storage; and buffering defeats streaming semantics — so cap sizes (`MaxRequestBodySize`), buffer only routes that need re-reading, and never blanket-enable it fleet-wide.
10. **Q: Explain response buffering vs streaming trade-offs in middleware that must inspect/transform the response body.** **A:** To read the response, middleware replaces `Response.Body` with a `MemoryStream`, lets downstream write, inspects, then copies to the original stream — simple but O(response size) memory per request, breaks streaming/SSE/large downloads (client waits for full generation), and interferes with `Content-Length` vs chunked decisions. Alternatives: wrap the stream with a pass-through transformer (inspect while forwarding), use `IHttpResponseBodyFeature` replacement, or scope buffering to small JSON API routes only, with size caps and explicit exclusions for streaming endpoints.
11. **Q: What guarantees does the pipeline give about middleware exception propagation, and where do exceptions from `OnStarting` callbacks or after-response-start failures actually go?** **A:** Exceptions bubble up the delegate chain to earlier middleware (hence exception handlers register first). But once the response has started, an exception handler can't rewrite the status — ASP.NET Core aborts the connection (client sees a truncated response/reset), and exceptions in `OnStarting`/`OnCompleted` callbacks surface as server-level errors logged by Kestrel rather than flowing through the pipeline. This is why response-start is the point of no return, and why streamed endpoints need their own in-band error signaling.
12. **Q: How do `Map`, `MapWhen`, and `UseWhen` differ in pipeline topology — including what happens on the way back out?** **A:** `Map` branches on path prefix and *rejoins nothing* — the branch owns the rest of the request (and trims the matched prefix into `PathBase`). `MapWhen` branches on a predicate, also terminal. `UseWhen` inserts a conditional segment that *rejoins* the main pipeline afterward — the only one where downstream main-pipeline middleware still runs; its "way out" code also runs for matched requests. Choosing `MapWhen` where rejoining was needed silently skips the rest of the pipeline — a subtle misconfiguration.
13. **Q: Where do `UseRouting` and `UseEndpoints` (or top-level route registration) split responsibilities, and why does middleware *between* them exist as a pattern?** **A:** `UseRouting` matches the request to an endpoint and attaches it (`GetEndpoint()`) *without executing it*; `UseEndpoints`/the terminal endpoint middleware executes it. Middleware between them runs with full knowledge of the matched endpoint's metadata — authorization (reads `[Authorize]` metadata), CORS (reads policy metadata), rate limiting (reads policy attributes) — enabling metadata-driven cross-cutting behavior before the endpoint runs. That's why ordering violations there (auth before routing) break: the metadata isn't attached yet.
14. **Q: What is endpoint metadata, and how would custom middleware use it to implement per-endpoint behavior without attributes on the middleware itself?** **A:** Every endpoint carries an ordered metadata collection (attributes, policy objects, anything added via `.WithMetadata(...)`). Custom middleware placed after routing calls `context.GetEndpoint()?.Metadata.GetMetadata<MyPolicy>()` and adapts (skip auditing where `[SkipAudit]`, apply per-endpoint timeouts, choose serialization variants). This is the idiomatic alternative to route-string matching in middleware — declarative per-endpoint configuration with the pipeline reading it, exactly how `[AllowAnonymous]`, CORS, and output caching are implemented.
15. **Q: Explain graceful-shutdown mechanics in detail: what does the host do between SIGTERM and process exit, and where do middleware/in-flight requests fit?** **A:** On SIGTERM, `IHostApplicationLifetime.ApplicationStopping` fires; the server stops accepting new connections, in-flight requests get `HostOptions.ShutdownTimeout` (default 30s) to complete — during which the pipeline runs normally — then remaining requests are aborted (`RequestAborted` fires), hosted services' `StopAsync` run (also bounded), and the process exits. Production implications: shutdown timeout must exceed your longest legitimate request; Kubernetes `terminationGracePeriodSeconds` must exceed the host timeout plus preStop delays; and readiness must flip *before* SIGTERM-driven connection draining (preStop sleep pattern) so the load balancer stops sending traffic first.
16. **Q: What is `RequestAborted` actually connected to, and what are its reliability limits?** **A:** It's a `CancellationToken` fired when the client disconnects (TCP reset/FIN observed, HTTP/2 RST_STREAM) or the server aborts (shutdown, timeouts). Limits: detection depends on the server noticing the disconnect — with no pending read/write on the connection, abort may fire late or not until a write fails; proxies in between can mask client disconnects (LB holds its side open); and cancellation only helps if the whole downstream call chain propagates the token. Treat it as best-effort cost-saving, never as a correctness mechanism (don't rely on it to prevent double-effects).
17. **Q: How would you implement a request-timeout middleware correctly, and what does .NET 8's built-in request timeouts feature do that naive `CancellationTokenSource` wrapping misses?** **A:** Correct implementation links a timeout CTS with `RequestAborted`, flows the linked token downstream, catches `OperationCanceledException` distinguishing timeout from client abort, and returns 503/504 *only if the response hasn't started*. .NET 8's `UseRequestTimeouts` adds the missing pieces: per-endpoint policies via metadata (`[RequestTimeout]`), correct interplay with the response-started state, debugger-attached suspension, and standardized status handling — the edge cases (response already streaming, WebSockets exclusion, endpoint opt-out) that hand-rolled versions get wrong.
18. **Q: Describe how you'd propagate a correlation/tenant context through the pipeline into every downstream log line — compare `Items`, `AsyncLocal` scopes, and DI-scoped context objects.** **A:** `HttpContext.Items` is per-request but requires pulling `HttpContext` everywhere (visibility problem). Logger scopes (`ILogger.BeginScope`, backed by `AsyncLocal`) automatically enrich all log lines within the request's async flow — ideal for correlation IDs; set once in early middleware. A DI-scoped `RequestContext` object populated by middleware and constructor-injected is the cleanest for *business* code (typed, testable, explicit). Production stacks typically combine: middleware populates a scoped context object *and* opens a logging scope from it; `Items` remains for middleware-to-middleware handoff only.
19. **Q: What ordering and interaction rules govern `UseCors`, `UseOutputCache`/`UseResponseCaching`, `UseResponseCompression`, and `UseRateLimiter` — name two non-obvious interactions.** **A:** Canonical order: exception handler → HSTS/HTTPS redirect → forwarded headers (very early) → response compression → output caching → routing → CORS → rate limiter → auth → endpoint. Non-obvious interactions: (1) compression before caching means the cache may store compressed variants — the cache must vary by `Accept-Encoding` or serve wrong encodings; (2) rate limiting after auth enables per-user policies but lets unauthenticated floods reach the (expensive) auth middleware — many designs run a coarse IP limiter early *and* a per-user limiter after auth; (3) CORS must run before any middleware that could short-circuit without emitting CORS headers, or browser errors mask real responses.
20. **Q: A production API intermittently returns responses with mixed content from two different users. Walk the investigation priorities specific to pipeline/context misuse.** **A:** This is context-bleed; prioritize: (1) singleton services caching per-request state (a singleton holding "current user" set by middleware — two concurrent requests interleave); (2) captured `HttpContext`/`Response` in fire-and-forget or pooled objects completing after recycling (Q7); (3) `IHttpContextAccessor` used in code running outside the originating request's flow (queued work observing another request's context); (4) response-buffer pooling bugs — a pooled `MemoryStream`/`StringBuilder` reused without reset in custom middleware; (5) output cache keyed without user variance caching a personalized response. Reproduce with concurrent differentiated-user load; audit DI lifetimes (`ValidateScopes`), grep for `static` mutable state and un-awaited tasks touching context. The class is always "per-request data stored in a longer-than-request home."
