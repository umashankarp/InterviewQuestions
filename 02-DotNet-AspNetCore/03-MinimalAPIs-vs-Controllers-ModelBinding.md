...
**CPU/Startup**: Minimal APIs generally have **lower per-request CPU overhead and faster application startup** than Controllers, primarily because Controllers' action-invocation path involves more reflection-based dispatch and a richer, multi-stage filter pipeline to traverse per request — this is a genuine, measurable difference (frequently cited in Microsoft's own published TechEmpower-style benchmark comparisons), though its *practical* significance depends heavily on the specific service's actual bottleneck (§6's "measure before choosing purely on this basis" caution).

**Memory**: Both models ultimately allocate similarly for the core request/response cycle (both built on the same underlying `Endpoint`/`HttpContext` infrastructure, Module 9 §2.7) — the meaningful allocation difference is specifically in the **filter/action-invocation machinery's own overhead** (Controllers' richer pipeline allocates more per-request bookkeeping objects to support its additional stages) rather than in application-level data handling.

**Model binding cost**: Complex-type JSON body binding (either model) uses `System.Text.Json` internally (Module 3's `Span<T>`/UTF-8-based, already-optimized serializer) — this cost is essentially identical between the two models for equivalent DTOs; the performance delta between Controllers and Minimal APIs is concentrated in the surrounding filter/dispatch machinery, not in the actual data-binding/deserialization work itself.

**Benchmarking**: Microsoft's own published ASP.NET Core Minimal-API-vs-Controller benchmarks (and independently reproducible via a simple `bombardier`/`k6` load test comparing two equivalent endpoints) are the standard evidence base for this specific comparison — always cite/reproduce actual numbers for the *specific* scenario in question (payload size, endpoint complexity) rather than relying on generic, possibly-outdated "Minimal APIs are X% faster" folklore, directly the same measure-first principle recurring throughout this course.

**OpenAPI/Swagger generation cost**: `TypedResults`-based Minimal API endpoints generate OpenAPI metadata **without runtime reflection over `[ProducesResponseType]` attributes** (the type information is available directly from the strongly-typed return value at compile time) — a small but real startup/tooling-performance benefit over attribute-based metadata generation, worth mentioning as a concrete `TypedResults`-specific advantage beyond mere type-safety.

---

## 8. Security

- **The silent model-binding-inference regression (§4)** is itself a **security-adjacent** risk in some scenarios, not merely a functional bug — if a security-relevant filter parameter (e.g., an "includeDeleted"/"includeSensitive" flag intended to default to `false` and require explicit opt-in) silently binds incorrectly in the *opposite* direction (always `true` instead of always `false`, depending on the specific inference mismatch), the consequence could be an inadvertent over-broad data exposure rather than a merely-annoying missing-filter bug — worth explicitly considering the *direction* of a binding-inference mismatch's potential impact during any migration, not just noting "binding might change."
- **Over-permissive complex-type model binding** (binding an entire request body directly onto a domain/EF-Core entity type, rather than a dedicated DTO) is a classic **over-posting/mass-assignment vulnerability** — a client including unexpected extra JSON properties (e.g., `"isAdmin": true` on a user-registration payload bound directly to a `User` entity with an `IsAdmin` property) can silently set fields the API never intended to expose for client control. Mitigation: **always bind to a dedicated, narrowly-scoped DTO** (containing only the properties a client should legitimately be able to supply) and explicitly map to the domain entity in code, never bind directly to a rich domain/persistence entity type.
- **`[ApiController]`'s automatic model-state-validation 400 response** is a genuine, if indirect, security benefit — it structurally prevents invalid/malformed input from ever reaching action-method business logic, reducing the attack surface for injection/logic-confusion vulnerabilities stemming from unvalidated input reaching deeper code paths; Minimal APIs, lacking this automatic convention, place the burden of equivalent validation-enforcement explicitly on the team (a validation `IEndpointFilter`), and skipping it entirely (a real risk if a team assumes Minimal APIs "just work like Controllers" without realizing this specific convention doesn't carry over) is a genuine security-relevant gap to close deliberately.
- **OWASP relevance**: Directly maps to **A08 (Software and Data Integrity Failures)**/mass-assignment concerns for entity-bound model binding; **A04 (Insecure Design)** for missing input-validation enforcement in a Minimal API migration that assumed `[ApiController]`-equivalent behavior without actually implementing it.

---

## 9. Scalability

- **Horizontal scaling**: Both models scale identically at the infrastructure level (stateless replicas, Module 1/2/9's recurring theme) — the Controller-vs-Minimal-API choice affects per-replica CPU/memory efficiency (§7), which can modestly influence how many replicas are needed for a given throughput target, but doesn't change the fundamental horizontal-scaling architecture.
- **Vertical scaling**: Minimal APIs' lower per-request overhead means marginally better core-utilization efficiency under high concurrent load — a genuine, if usually secondary, factor in a cost-sensitive, very-high-throughput service's architecture choice.
- **Caching/Replication/Partitioning**: Not directly differentiated by the Controllers-vs-Minimal-APIs choice; response caching (`UseOutputCache`, Module 9 §9) applies identically to endpoints defined either way, since both compile to the same underlying `Endpoint` abstraction.
- **CAP theorem**: Not directly relevant to this module's topic.
- **HA/DR**: Minimal APIs' faster startup time (§7) is a genuine, concrete benefit for HA/DR scenarios involving frequent scale-out/scale-in (Kubernetes horizontal pod autoscaling reacting to load spikes) or fast-failover requirements — a new replica reaching "ready to serve traffic" state faster directly improves the system's ability to absorb sudden load increases or recover from replica failures with less service-level impact, a concrete, quantifiable factor worth weighing for services with these specific operational characteristics.
