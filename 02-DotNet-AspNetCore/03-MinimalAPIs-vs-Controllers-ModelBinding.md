...
1. **Q: How does a Minimal API endpoint delegate actually get invoked — what does `RequestDelegateFactory` do at startup?** **A:** `RequestDelegateFactory` inspects the lambda's signature via reflection (or, with Request Delegate Generator, at compile time), decides each parameter's binding source (route, query, header, body, DI, special types), and compiles a specialized `RequestDelegate` via expression trees that performs exactly those binding steps — no per-request reflection, no generic model-binding machinery. This is why Minimal APIs skip most MVC overhead: the binding plan is baked into compiled code per endpoint at startup.
2. **Q: What is the Request Delegate Generator (.NET 8), and why does Native AOT specifically need it?** **A:** It's a source generator producing the endpoint binding/invocation code at *compile time* instead of `RequestDelegateFactory`'s runtime expression compilation. Native AOT can't JIT-compile expression trees (interpreted fallback is slow) and reflection-based signature analysis is trimming-hostile — the generator emits statically analyzable code, making Minimal APIs AOT-compatible while controllers (deeply reflection-dependent) remain unsupported in AOT.
3. **Q: Walk through MVC's model-binding pipeline for a complex type — the roles of value providers, binder providers, and binders.** **A:** Value providers abstract raw sources (route values, query, form) into a uniform lookup; model binder *providers* run in order to select a binder for the target type (complex-object binder, collection binder, simple-type binder using TypeConverters); the chosen binder recursively binds properties — each property consulting value providers by name, with prefix composition for nesting (`Address.City`). Validation then runs over the bound graph. Every extensibility story (custom binders, custom value providers) plugs into one of these three seams.
4. **Q: How does `[FromBody]` deserialization differ between MVC and Minimal APIs when the body is invalid JSON vs. valid JSON with wrong shape?** **A:** MVC catches deserialization failures into `ModelState` errors (automatic 400 with field-level details under `[ApiController]`), treating both malformed JSON and type mismatches as model-state entries. Minimal APIs throw/handle `BadHttpRequestException` for malformed bodies producing a 400 (customizable), but shape mismatches follow System.Text.Json semantics — missing properties silently default unless `required`/nullability enforcement kicks in, and there's no ModelState dictionary; validation is your responsibility (filters, FluentValidation, or .NET's newer validation integration).
5. **Q: What are the exact binding-inference rules for a Minimal API parameter, in precedence order?** **A:** Explicit attributes win; then well-known types bind specially (`HttpContext`, `HttpRequest`, `CancellationToken`, `ClaimsPrincipal`, `Stream`/`PipeReader`); then a string/simple-convertible type matching a route parameter name binds from route, else from query; then types registered in DI bind from services (checked via `IServiceProviderIsService`); then complex types bind from the JSON body (GET/DELETE included, historically surprising); arrays of simple types bind from query (repeated keys); `IFormFile` from form. The DI check's involvement means *registering a type in DI changes an endpoint's HTTP contract* — the signature stays identical while the binding source silently moves.
6. **Q: Why is `[AsParameters]` more than sugar — what does it enable for API surface hygiene, and its constraints?** **A:** It explodes a struct/class's properties into individual bound parameters (each following normal inference/attributes), letting endpoints share standardized parameter sets (paging, sorting, filtering) as reusable types without forcing body binding — solving the "six query parameters copied across forty endpoints" duplication with compile-time-checked reuse. Constraints: the type needs an accessible constructor the factory can bind (primary constructors work), nested `[AsParameters]` isn't supported, and each property still needs an unambiguous binding source.
7. **Q: How does form binding and antiforgery interact in Minimal APIs (.NET 8+), and what's the security default worth knowing?** **A:** .NET 8 added `IFormCollection`/`[FromForm]`/`IFormFile` binding to Minimal APIs *with antiforgery validation required by default*: endpoints binding form data throw at startup unless antiforgery middleware is registered, and requests without valid tokens get 400 (opt out via `DisableAntiforgery()` for token-authenticated APIs). This encodes the security posture — browser-submitted forms need CSRF protection — into the framework default rather than developer memory, a deliberate contrast with the old "remember `[ValidateAntiForgeryToken]`" model.
8. **Q: Explain custom binding via `TryParse`/`BindAsync` static methods — how do they compare to MVC's `IModelBinder`?** **A:** Minimal APIs bind any type exposing `static bool TryParse(string, IFormatProvider, out T)` (for route/query strings) or `static ValueTask<T?> BindAsync(HttpContext, ParameterInfo)` (full-context binding — headers, multiple sources, async lookups). These are discoverable, type-owned, and AOT-friendly versus MVC's `IModelBinder` + provider registration, which is more powerful (participates in ModelState, prefix handling, metadata) but heavier ceremony. Pattern: strongly-typed IDs and value objects implement `TryParse` once and bind everywhere in both worlds (MVC also honors TypeConverters/TryParse in recent versions).
9. **Q: What happens with nullable vs non-nullable and optional vs required parameters in Minimal APIs — how do defaults, NRT annotations, and 400s interrelate?** **A:** A non-nullable parameter without default is required: missing route/query value → 400 automatically (`string name` from query must be present); nullable (`string? name`) or defaulted (`int page = 1`) parameters are optional, binding null/default when absent. For bodies, an empty body on a required complex parameter 400s while `Body? = null` allows it. NRT annotations thus *are* the API contract in Minimal APIs — flipping a `?` changes wire behavior, not just compiler analysis, which surprises teams treating nullability as documentation.
10. **Q: Compare filter capabilities: what can MVC action filters do that `IEndpointFilter` can't, and vice versa?** **A:** MVC filters offer five specialized stages with fine placement (resource filters pre-binding, exception filters, result filters around serialization) plus attribute-based per-controller targeting, ModelState integration, and framework-supplied context (action descriptors). `IEndpointFilter` is one unified around-invocation stage with typed access to bound arguments (`GetArgument<T>` — filters can *inspect and replace* strongly-typed parameters pre-handler, which MVC filters only do via untyped dictionaries), composability via `MapGroup` hierarchies, and short-circuiting by returning results. Endpoint filters can't intercept before binding or wrap serialization; resource-filter-style "skip binding entirely" scenarios have no Minimal API equivalent.
11. **Q: How do route groups (`MapGroup`) compose cross-cutting behavior, and what ordering semantics apply to nested group filters?** **A:** Groups apply prefixes, metadata, conventions, and filters to all contained endpoints; nesting composes (prefixes concatenate, metadata accumulates). Filter order: outer-group filters run before inner-group filters, which run before endpoint-specific filters — an onion from outermost group inward, with the handler at the center. This makes groups the Minimal API unit of API architecture: versioning (`/v1` group), tenancy, auth policies (`RequireAuthorization` on the group), and standardized validation attach once at the right layer instead of per endpoint.
12. **Q: What is the parameter-binding behavior difference for `CancellationToken`, and why does forgetting it matter more in streaming endpoints?** **A:** Both models bind `CancellationToken` to `HttpContext.RequestAborted` automatically when declared. Omitting it means downstream work runs to completion after client disconnects; for short queries that's wasted milliseconds, but for streaming endpoints (large file/SSE/long reports) an abandoned request continues consuming a connection, DB cursor, and thread-pool capacity for its full duration — multiplied by client-retry storms during incidents, this is a self-inflicted capacity attack. Declaring the token and flowing it through every awaited call is the fix; review streaming endpoints for it specifically.
13. **Q: Explain content negotiation differences: what does MVC's output formatter pipeline do that Minimal APIs deliberately dropped, and how do you handle XML or vendor media types in Minimal APIs?** **A:** MVC negotiates: it inspects `Accept`, selects among registered output formatters (JSON, XML, custom), honors `[Produces]`, and serializes accordingly. Minimal APIs write JSON via System.Text.Json, period — negotiation was dropped as rarely-needed complexity. Needing XML/vendor types in Minimal APIs means doing it manually (branch on `Accept` and call an XML serializer into the response stream, or a custom `IResult` type encapsulating negotiation) — at which point, if negotiation is a genuine product requirement across many endpoints, controllers are the pragmatic choice; bolting a negotiation framework onto Minimal APIs recreates MVC badly.
14. **Q: How does validation evolve across the stack — DataAnnotations in MVC, and what are the current Minimal API validation options including .NET 10-era built-ins?** **A:** MVC runs DataAnnotations + `IValidatableObject` automatically during binding into ModelState. Minimal APIs historically had *no* automatic validation — teams used endpoint filters invoking FluentValidation/DataAnnotations manually or libraries (MinimalApis.Extensions). The platform direction (validation support that walks bound parameters annotated with DataAnnotations, enabled via `AddValidation`-style APIs with source-generated validators in recent .NET) closes the gap AOT-safely. Interview-relevant point: know that "Minimal APIs validate automatically" was false for years — stating *how* validation is wired in your codebase shows real experience.
15. **Q: What are the model-binding security failure modes beyond over-posting — enumerate three with mitigations.** **A:** (1) Unbounded collection/dictionary binding: form/query binding to `List<T>` accepts thousands of indexed entries (`items[0]..items[9999]`), an allocation DoS — MVC caps via `MaxModelBindingCollectionSize`/`ValueCountLimit`; keep them sane. (2) Depth attacks: deeply nested JSON exhausts stack/CPU during deserialization — `JsonSerializerOptions.MaxDepth` (default 64) and request size limits are the guard. (3) Type-confusion via polymorphic binding: custom binders/converters that instantiate types named in the payload (`$type`-style) enable gadget attacks — never bind to open polymorphic hierarchies from untrusted input; use closed `[JsonDerivedType]` registration. Cross-cutting: request body size limits are the first line for all three.
16. **Q: A PUT endpoint intermittently receives a bound DTO with all-default properties despite clients sending correct JSON. What binding-level causes produce this?** **A:** Candidates: (1) content-type mismatch — client sent `text/plain`/missing header, MVC's `[FromBody]` would 415 but a permissive custom setup or manual deserialization defaulted; (2) case-sensitivity: custom `JsonSerializerOptions` replaced defaults losing `PropertyNameCaseInsensitive`, so camelCase payloads stop matching PascalCase properties — everything silently defaults; (3) a naming-policy change (global serializer options modified by another team/library — options are global mutable state); (4) the body was already consumed (logging middleware read without `EnableBuffering`/rewind) leaving an empty stream; (5) double serialization (stringified JSON — the body is a JSON *string* containing JSON). Diagnose by logging raw body + comparing serializer options at runtime.
17. **Q: How do `IResult`, `Results<T1,...,TN>`, and `TypedResults` interact with OpenAPI generation and testability?** **A:** `TypedResults.Ok(dto)` returns concrete types implementing `IEndpointMetadataProvider`, which self-describe status codes and schemas to OpenAPI generation at startup — the union type `Results<Ok<Order>, NotFound>` declares the complete response contract in the signature, compiler-checked (returning an undeclared result type won't compile). Testability: handler unit tests pattern-match the concrete result (`Assert.IsType<Ok<Order>>(result.Result)`) and inspect `.Value` directly — no HTTP round-trip or result-executor mocking, a genuine improvement over asserting on `IActionResult` internals.
18. **Q: Describe a robust API-versioning architecture for Minimal APIs using Asp.Versioning — where do groups, metadata, and binding interact?** **A:** `Asp.Versioning.Http` defines version sets; route groups per version (`MapGroup("/v{version:apiVersion}")` or header/media-type readers) associate endpoints with versions via metadata; OpenAPI generation produces per-version documents from that metadata. Binding interaction: DTO evolution across versions means distinct DTO types per version bound in their respective groups (never one DTO with version-conditional properties — inference/serialization can't express that), with mapping to shared domain handlers behind the endpoints. The group hierarchy keeps deprecation metadata (`.HasDeprecatedApiVersion`) and sunset headers declarative per version.
19. **Q: What's the migration architecture for moving a large controller API to Minimal APIs incrementally with zero contract drift — beyond side-by-side testing?** **A:** Run both models in one app (they share routing); migrate route-by-route behind contract locks: freeze the OpenAPI document per route as the contract artifact and diff generated specs (old vs new implementation) in CI — schema, status codes, nullability, parameter sources must match; snapshot-test serialized responses for representative payloads (naming policy, enum handling, date formats are where drift hides); port cross-cutting attributes to group filters with explicit equivalence notes (`[ApiController]` 400 behavior needs the validation filter replica); route traffic via the normal router (no proxy needed — same process), using endpoint metadata to tag migration status. The OpenAPI-diff gate is the load-bearing control: binding-inference differences (Q5) surface as spec diffs before any client notices.
20. **Q: As a Principal Engineer, a platform team wants to mandate Native AOT for all new HTTP services, which forces Minimal APIs. Evaluate the full implications through the binding/framework lens.** **A:** Gains: startup in tens of milliseconds and reduced memory (real money at high pod counts / scale-to-zero), trimmed binaries, and the discipline of source-generated binding/serialization (`JsonSerializerContext`) catching reflection dependencies at build. Costs: the ecosystem contract shrinks — no controllers, limited reflection-dependent libraries (some validation, mapping, mocking, and older SDKs break), source-generator ceremony per serialized type, slower inner-loop builds, and debugging differences; some teams will fight the constraint rather than benefit. Evaluation: mandate-by-default with an exception path is defensible only if the platform provides paved-road templates (serializer contexts, validation, OpenAPI, telemetry all pre-wired AOT-safe) and a compatibility checklist; otherwise make AOT opt-in for services with measured cold-start/density needs — a blanket mandate that ignores library reality externalizes platform ambition onto every product team's velocity.
