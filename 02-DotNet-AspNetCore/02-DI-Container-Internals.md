...
**CPU**: Constructor-parameter resolution and constructor selection (§2.4) happen via reflection-based mechanisms in the container's core resolution path — for `Transient`/frequently-re-resolved services in extremely hot code paths, this reflection overhead is measurable (though the container internally caches significant portions of the resolution plan per type after the first resolution, avoiding full reflection cost on every single call) — for the overwhelming majority of application code (resolved once per request, not thousands of times per second in a tight loop), this cost is negligible and not worth optimizing away.

**Memory**: `Singleton` services persist for the application's entire lifetime — any `Singleton` holding growing, unbounded internal state (a cache with no eviction policy, directly connecting to Module 1 §6's "static caches without eviction" anti-pattern) is a long-term memory-growth risk exactly analogous to that earlier discussion, now specifically anchored to the DI-lifetime mechanism that makes such state long-lived in the first place.

**Scope creation cost**: Creating a new DI scope (per request, or explicitly via `IServiceScopeFactory.CreateScope()`) has a small, generally negligible cost — but a background service creating a **new scope per very-high-frequency iteration** (e.g., a tight polling loop creating a scope every few milliseconds) can accumulate a measurable cost at sufficient frequency; batch/throttle scope creation appropriately for very high-frequency background work, exactly the same "measure before assuming it matters" discipline as every other performance topic in this course.

**`IHttpClientFactory`'s connection-pooling benefit**: Beyond DI-lifetime correctness, `IHttpClientFactory`-managed `HttpClient` instances correctly reuse and rotate underlying `SocketsHttpHandler` connection pools — directly preventing the well-documented "socket exhaustion from disposing `HttpClient` too frequently" and "DNS-change-blindness from never disposing `HttpClient` at all" dual failure modes, a genuinely important production-performance/reliability concern specifically tied to getting this particular DI-adjacent lifetime pattern right.

**Benchmarking**: BenchmarkDotNet comparing direct `new SomeService(dependencies...)` construction against container-resolved construction for a hot, frequently-resolved `Transient` service quantifies the reflection-based resolution overhead concretely — useful specifically when justifying (or ruling out) a manual object-pool/factory-pattern alternative for a proven-hot service-construction path, rather than assuming DI resolution cost matters without measurement.

---

## 8. Security

- **The multi-tenant cross-tenant-data-leakage captive-dependency incident (§4)** is itself a direct, severe security/compliance failure mode, not merely a correctness bug — worth explicitly framing captive-dependency prevention as a **security control**, not just a code-quality concern, when advocating for `ValidateOnBuild`/`ValidateScopes` to be universally enabled.
- **Service Locator anti-pattern (§6) as an audit/review obstacle**: a class that resolves its dependencies dynamically via `IServiceProvider.GetService<T>()` calls scattered through its method bodies (rather than declared constructor dependencies) makes it significantly harder for a security review to statically determine "what does this class actually depend on and potentially have access to" — a class's constructor signature is a natural, reviewable declaration of its capabilities/blast-radius; hiding that behind runtime service-location defeats this reviewability.
- **Eagerly-constructed `Singleton` services performing sensitive initialization work** (loading secrets/credentials at startup) should ensure that initialization failure (a missing/invalid secret) causes the **application to fail to start** cleanly and loudly, rather than silently proceeding with a partially-initialized, insecure default state — a `Singleton`'s constructor throwing during startup is the correct, safe failure mode for this class of concern.
- **OWASP relevance**: Cross-tenant data leakage from a captive-dependency bug (§4) is a severe instance of **A01 (Broken Access Control)** in a multi-tenant context — worth explicitly citing this OWASP category when making the business case for universal DI-validation enablement to a security-conscious audience.

---

## 9. Scalability

- **Horizontal scaling**: DI container state is entirely per-process — `Singleton` instances are independent per replica in a horizontally-scaled fleet (no cross-replica sharing), meaning any `Singleton`-held in-process cache/state is inherently per-replica, not globally consistent — directly connecting to Module 2/9's recurring "in-process state doesn't automatically scale across a fleet" theme; a `Singleton` cache needing fleet-wide consistency requires an externalized store (Redis), not just a container-lifetime choice.
- **Vertical/Server GC interaction**: `Singleton` services (long-lived, promoted to Gen 2 quickly, Module 1 §2.4) and `Scoped`/`Transient` services (short-lived, collected in Gen 0/1) map naturally onto the GC's generational model — a DI-lifetime choice is, among other things, implicitly also an allocation-lifetime choice, worth being aware of when reasoning about a service's GC-pressure profile (Module 1 §7) alongside its correctness requirements.
- **Caching/Replication/Partitioning**: `Singleton`-scoped in-memory caches are appropriate specifically for per-replica, best-effort, cache-hit-optional data (where each replica having a slightly different cache state is acceptable) — for cache consistency requirements spanning a fleet, an external distributed cache is required regardless of DI-lifetime choice, exactly Module 2 §9's earlier point, now anchored specifically to the `Singleton` lifetime as the in-process analog.
- **CAP theorem**: Not directly relevant to DI mechanics; the practical connection is that `Singleton`-lifetime, per-process caches are an implicit "AP-leaning, eventually (or never) consistent across replicas" choice by construction — worth explicitly naming this trade-off when a team reaches for `AddSingleton` for a cache without considering whether cross-replica consistency actually matters for that specific cache's use case.
- **HA/DR**: `IHostedService`/`BackgroundService` (Singleton-registered by the framework) instances are recreated fresh on every process restart (a new deployment, a crash-restart) — any in-memory state/progress tracking they maintain is lost across restarts unless explicitly persisted externally, a direct HA/DR design consideration for any background-processing service relying on DI-managed lifetime alone for its state.
