...
1. **Q: How does Microsoft.Extensions.DependencyInjection actually build resolvers — what happens between `BuildServiceProvider` and the first fast resolution?** **A:** The container compiles resolution logic per service: initial resolutions run through a slower reflection-based/call-site-interpreted path while, in the background, it generates optimized code (expression-tree compilation / IL emit) for that call site; subsequent resolutions hit the compiled delegate. This "tiered" strategy mirrors JIT tiering — fast startup without paying compilation for services resolved once, full speed for hot services — and explains why first-request latency includes container warm-up.
2. **Q: What is a call site in MEDI's internals, and why does understanding it explain both performance and error behavior?** **A:** For each registered service the container builds a call-site tree describing exactly how to construct it: constructor call sites with child call sites per parameter, constant call sites for instances, enumerable call sites for `IEnumerable<T>`, scope-validated wrappers, etc. Resolution walks/compiles this tree — which is why circular dependencies and missing registrations are detected during call-site construction (with full dependency-path error messages), and why deep graphs cost proportionally to their tree size until compiled.
3. **Q: Explain disposal ownership rules precisely: what does the container dispose, when, and what does it *not* own?** **A:** The provider/scope disposes `IDisposable`/`IAsyncDisposable` instances *it created*: scoped and transient disposables are tracked by their owning scope and disposed (reverse creation order) when the scope disposes; singletons dispose when the root provider disposes at shutdown. It does NOT dispose instances you registered pre-built (`AddSingleton(existingInstance)`) — you own those — and transients resolved *from the root provider* are tracked by the root, living (and accumulating!) until shutdown.
4. **Q: Why are transient `IDisposable` services resolved from the root provider a memory leak, and how do you catch this class of bug?** **A:** The root scope must track them for eventual disposal, so every resolution appends to the root's disposables list — an unbounded accumulation that behaves like a leak (objects never collected because the container references them). Catch it: `ValidateScopes` doesn't cover this (they're not scoped); use scope-per-operation discipline for anything resolving transients dynamically, watch for `SingletonUsingTransientDisposable` patterns in reviews, and diagnose in dumps by finding the root provider's disposables list holding thousands of entries.
5. **Q: What is `ActivatorUtilities.CreateInstance`, how does it differ from `GetRequiredService`, and where is it the right tool?** **A:** It constructs a type *not registered* in the container, satisfying constructor parameters from the provider plus optional explicit arguments — a hybrid of DI and manual construction. Right tool for: framework-style factories creating per-item handlers with runtime data (`CreateInstance<Handler>(sp, message)`), avoiding registering dozens of closed types. Caveats: it bypasses lifetime management (you own disposal), uses different constructor-selection rules (`ActivatorUtilitiesConstructor` attribute; prefers the constructor it can most fully satisfy), and repeated calls pay reflection cost unless you cache an `ObjectFactory` from `ActivatorUtilities.CreateFactory`.
6. **Q: How do keyed services (.NET 8) change multi-implementation patterns — and what were the pre-keyed alternatives?** **A:** `AddKeyedScoped<IPayment, StripePayment>("stripe")` + `[FromKeyedServices("stripe")]` (or `GetRequiredKeyedService`) select implementations by key at injection points — replacing the older patterns: resolving `IEnumerable<IPayment>` and filtering by a discriminator property, factory delegates (`Func<string, IPayment>` registrations), or named-options-driven factories. Keyed services centralize the mapping in registrations and keep constructors clean; the old enumerable/factory patterns remain better when selection logic is dynamic per call or needs data beyond a static key.
7. **Q: What are the semantics of overlapping registrations — `TryAdd*`, `Replace`, `Add` — and why do library authors need them?** **A:** `Add` always appends (last registration wins for single resolution; *all* appear in `IEnumerable<T>`); `TryAdd` adds only if the service type isn't registered (respecting app overrides); `TryAddEnumerable` adds only if that exact (service, implementation) pair is absent — preventing duplicate pipeline behaviors when an extension method is called twice. Library `AddMyLibrary()` methods must use `TryAdd*` so consumer registrations (before or after) override defaults predictably; using plain `Add` in a library creates order-sensitive, double-registration bugs.
8. **Q: Explain how `IEnumerable<T>` resolution ordering works and a real pattern that depends on it.** **A:** Implementations resolve in registration order, and this ordering is contractual — MVC filters, configuration `IConfigureOptions<T>` chains, and middleware-like pipelines (e.g., HttpClient message handlers, validation chains) execute in registration sequence. Patterns depending on it: decorator-ish pipelines built as ordered `IEnumerable<IStep>`, `IConfigureOptions<T>` then `IPostConfigureOptions<T>` layering, and "first matching handler wins" dispatchers. The corollary: registration order is part of your composition contract — refactoring that reorders `Add` calls can change behavior without any code diff in the services themselves.
9. **Q: How do you implement the decorator pattern in MEDI, which lacks first-class decoration — compare the approaches.** **A:** Options: (1) manual lambda registration — `AddScoped<IService>(sp => new CachingDecorator(ActivatorUtilities.CreateInstance<RealService>(sp), sp.GetRequiredService<ICache>()))` — explicit, no magic, but boilerplate per decoration; (2) Scrutor's `Decorate<IService, CachingDecorator>()` — rewrites existing registrations, clean but adds a dependency and registration-order sensitivity; (3) restructure to avoid decoration (pipeline/behavior patterns à la MediatR, or interception at a different seam like DelegatingHandlers for HTTP). For platform code, explicit lambda or source-generated composition beats container tricks for debuggability.
10. **Q: What's the lifetime story inside `IHttpClientFactory`'s handler pipeline — why is a `DelegatingHandler` registered as transient yet effectively pooled?** **A:** The factory builds handler *pipelines* cached per named client for `HandlerLifetime` (default 2 min): your transient `DelegatingHandler` registrations are resolved once per pipeline build from a dedicated scope, then that pipeline (and its handlers) is shared across every `HttpClient` handed out during the lifetime window. Consequences: handlers must be thread-safe (concurrent requests share them), must not capture per-request scoped services (they outlive requests — inject `IHttpContextAccessor` or resolve per-call via the request context instead), and their "transient" registration is misleading — treat them as pooled singletons.
11. **Q: Why can't scoped services flow into `IHostedService` constructors, and what's the full correct pattern for periodic scoped work including error/scope hygiene?** **A:** Hosted services are singletons constructed from the root scope before any request scopes exist; injecting scoped services either fails validation or silently captures a root-scoped instance forever. Correct pattern: inject `IServiceScopeFactory`; per iteration `await using var scope = factory.CreateAsyncScope()`, resolve dependencies inside, execute with try/catch (per-iteration failures logged, not host-fatal), and let scope disposal clean up per-iteration `DbContext` state — a fresh scope per tick, mirroring per-request semantics, so change-tracker growth and stale connections can't accumulate across iterations.
12. **Q: How does the Options infrastructure ride on DI — trace what `AddOptions<T>().Bind(...).Validate(...)` actually registers.** **A:** It registers: `IOptions<T>` as a singleton (`UnnamedOptionsManager` caching the computed value forever), `IOptionsSnapshot<T>` as scoped (recomputed per scope via `OptionsManager`), `IOptionsMonitor<T>` as singleton with change-token subscriptions, plus your lambdas as `IConfigureOptions<T>`/`IValidateOptions<T>` instances in an `IEnumerable` executed in order by `OptionsFactory<T>` when a value is first built. Understanding this explains everything downstream: why `IOptions<T>` never reloads (singleton cache), why snapshot in a singleton is a captive bug, and why multiple `Configure` calls layer in registration order.
13. **Q: What does scope-per-message look like in a message-consumer application, and what goes wrong with scope-per-consumer?** **A:** Correct: create a DI scope per message (or per batch with deliberate semantics), resolve handlers/DbContext inside, dispose after ack — giving request-equivalent isolation: fresh change tracker, per-message transaction, no state bleed between messages, disposal releasing connections promptly. Scope-per-consumer (one scope for the consumer's lifetime) makes `DbContext` grow unboundedly (tracked entities accumulate → memory + slow SaveChanges), one poison message's corrupted context state poisons subsequent messages, and scoped caches (like `IOptionsSnapshot`) never refresh. Frameworks (MassTransit, NServiceBus) do scope-per-message for you — hand-rolled consumers must replicate it.
14. **Q: Explain `IServiceProviderIsService` and `IServiceProviderFactory<TBuilder>` — who uses these extensibility points and why?** **A:** `IServiceProviderIsService` answers "would this type resolve?" without resolving — Minimal APIs use it to decide whether an endpoint delegate parameter binds from DI or from the request (the inference magic), and analyzers/framework code use it to fail fast. `IServiceProviderFactory<TBuilder>` lets a third-party container (Autofac, Lamar) replace MEDI at host-build time: the host populates `IServiceCollection`, hands it to the factory, and the returned provider serves the app — the seam enabling `builder.Host.UseServiceProviderFactory(new AutofacServiceProviderFactory())` to swap containers without frameworks noticing.
15. **Q: When is replacing MEDI with a third-party container still justified today, and what's the cost side of the ledger?** **A:** Justified for: property/method injection requirements (legacy codebases), assembly scanning with sophisticated conventions, first-class decorators/interceptors (AOP), per-tenant/child containers with overrides, and diagnostics tooling some containers provide. Costs: a second mental model layered on the MEDI abstractions every library targets, subtle semantic differences (disposal order, enumerable ordering, root-scope behavior) that bite during framework upgrades, slower adoption of new platform features (keyed services parity lag), and onboarding friction. Default stance: MEDI + targeted patterns (Scrutor for scanning/decoration) unless a hard requirement forces more.
16. **Q: How do DI scopes interact with `IAsyncDisposable` — what breaks with `using var scope = ...` when async disposables are involved?** **A:** Synchronous `Dispose()` on a scope containing services implementing *only* `IAsyncDisposable` throws (the container can't sync-dispose them safely); services implementing both get sync-disposed, silently skipping async cleanup paths (flushes). Use `CreateAsyncScope()` + `await using` — it returns `AsyncServiceScope` whose `DisposeAsync` handles both kinds properly. This became acute as EF Core's `DbContext` and streaming resources adopted `IAsyncDisposable`; any `CreateScope()` in async code is a review flag.
17. **Q: What are the concrete failure modes of resolving services during application startup (before the host runs) — the "root-provider-too-early" traps?** **A:** Building an intermediate `ServiceProvider` from a half-populated `IServiceCollection` (the classic `BuildServiceProvider()` inside `ConfigureServices`) creates a *second* container: singletons duplicate (two instances of "the" singleton — broken caches, double background connections), disposables in the temporary provider leak, and registrations added after the temp build are missing from it. Correct alternatives: `IConfigureOptions<T>` classes taking DI dependencies for config-time logic, deferred resolution in hosted-service `StartAsync`, or the options pattern's lazy composition — never an early `BuildServiceProvider` (now an analyzer warning ASP0000).
18. **Q: Design DI for a multi-tenant application where some services are per-tenant singletons — what are the viable architectures given MEDI has no child containers?** **A:** Options: (1) tenant-keyed caches inside singletons — a singleton `TenantServiceCache<T>` holding `ConcurrentDictionary<tenantId, T>` with per-tenant factories (simple, but manual lifetime/disposal per tenant and no per-tenant registration overrides); (2) keyed services with tenant IDs as keys (only for a small fixed tenant set — keys are static); (3) a scoped `ITenantContext` + scoped facades so per-request resolution picks tenant-appropriate config while heavyweight per-tenant resources (connection pools, clients) live in the keyed cache; (4) an actual child-container-capable third-party container (Autofac multitenant support) when per-tenant *registration* differences are real. Most estates need only (1)+(3); choosing (4) for hypothetical flexibility imports Q15's costs.
19. **Q: How would you diagnose a slow first request attributed to DI — what's measurable and what are the levers?** **A:** Measure with EventSource counters/`dotnet-trace` (the DI EventSource emits call-site build and compilation events) and startup profiling: distinguish container *build* (call-site validation at `BuildServiceProvider` with `ValidateOnBuild` — grows with registration count), first-resolution compilation per deep graph, and constructor work masquerading as DI cost (services doing I/O in constructors — the usual real culprit). Levers: keep constructors trivial (no I/O — move to `IHostedService` warmup), reduce graph depth/fan-out for hot paths, warm critical services during startup deliberately, and don't disable validation in production to "fix" what is actually constructor work.
20. **Q: As a Principal Engineer, define the DI composition-root governance you'd set for a large codebase — what rules prevent the container from becoming an unmaintainable magic layer?** **A:** Rules: registrations live in feature-owned extension methods (`AddOrdering()`) composed in one visible root — no scattered registrations or runtime mutation; libraries use `TryAdd*` conventions; lifetime choices documented at registration with the "why" for anything non-obvious; `ValidateOnBuild`/`ValidateScopes` on everywhere (including production); no service locator outside framework seams (analyzer-enforced); factories/decorators expressed explicitly rather than via container extensions that hide construction; keyed/named mechanisms used for closed sets only; and a CI test that builds the full provider and resolves every registered root type (catching wiring breaks at PR time). The principle: the object graph is architecture — it should be as reviewable as the code, and any registration a reviewer can't predict the effect of is too clever.
