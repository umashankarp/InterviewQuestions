...
1. **Q: Walk through what `UseAuthentication` does per request at the scheme-handler level — construction, `AuthenticateAsync`, and principal attachment.** **A:** The middleware resolves the default authenticate scheme, gets its handler from `IAuthenticationHandlerProvider` (handlers are constructed per request via DI), calls `InitializeAsync(scheme, context)` then `AuthenticateAsync()`, which returns an `AuthenticateResult` (success with ticket, none, or fail). On success it sets `HttpContext.User` to the ticket's principal. Crucially it authenticates only the *default* scheme — other schemes' identities attach later only if authorization policies name them, which is the root of multi-scheme confusion.
2. **Q: Explain the JWT validation pipeline inside `JwtBearerHandler` — what checks run, in what conceptual order, and which are configurable traps?** **A:** Token extraction (Authorization header or custom via events) → signature validation against keys from metadata (`IssuerSigningKeys`, fetched from the authority's JWKS endpoint and cached) → lifetime validation (`exp`/`nbf` with `ClockSkew`, default 5 minutes — a trap when teams expect crisp expiry) → issuer/audience validation (`ValidIssuer(s)`, `ValidAudience(s)` — disabling audience validation "to make it work" is a real vulnerability: any token from that issuer for *any* API becomes valid here) → claims principal construction (with claim-type mapping, see below). Each stage is toggleable in `TokenValidationParameters`, and most JWT security incidents trace to a disabled stage.
3. **Q: What is the claim-type mapping behavior of the JWT handler, and why does `sub` mysteriously become a different claim type?** **A:** The Microsoft JWT stack historically maps inbound claim names to SOAP-era ClaimTypes URIs (`sub` → `http://schemas.../nameidentifier`), via a static inbound-claim-type map — so code checking `User.FindFirst("sub")` finds nothing. Fixes: `JwtSecurityTokenHandler.DefaultInboundClaimTypeMap.Clear()`, set `MapInboundClaims = false`, or use `JsonWebTokenHandler` (the modern default) with mapping off — and standardize via `TokenValidationParameters.NameClaimType`/`RoleClaimType` so `User.Identity.Name` and role checks bind to the intended claims explicitly.
4. **Q: How does key rotation work for JWT validation — what does the handler do when a token arrives signed by an unknown `kid`?** **A:** The handler caches the authority's signing keys (from OIDC discovery/JWKS) per `ConfigurationManager` with automatic refresh intervals; on encountering an unknown `kid` it triggers a metadata refresh (bounded by refresh-interval throttling) and retries validation — enabling seamless rotation when the IdP publishes new keys ahead of use. Failure modes: IdPs that rotate abruptly without overlap cause fleet-wide 401 bursts until refresh; network egress blocks to the discovery endpoint break cold-started pods (they can't fetch keys at all — a hidden availability dependency on the IdP).
5. **Q: Compare cookie authentication's sliding expiration with JWT's fixed expiry + refresh tokens — what are the security/UX trade-offs each encodes?** **A:** Sliding cookies re-issue near-expiry on activity — great UX, but an active attacker with a stolen cookie stays authenticated indefinitely; absolute expiry caps that (use both: sliding within an absolute bound). JWTs are non-revocable until expiry, so short lifetimes (5–15 min) bound stolen-token damage while refresh tokens (long-lived, revocable, stored server-side and rotated on use with reuse detection) provide continuity — pushing revocation to the refresh boundary. The design axis is identical: how long can a stolen credential act, and where does the server regain veto power.
6. **Q: What is the authorization middleware actually evaluating — walk through policy → requirements → handlers → `AuthorizationResult` for an endpoint with `[Authorize(Policy = "X")]` plus a global fallback policy.** **A:** Authorization middleware reads the endpoint's `IAuthorizeData` metadata, combines policies (endpoint policy + default/fallback policies merge their requirements and scheme lists), authenticates any named schemes and merges principals, then `IAuthorizationService` runs *every* registered handler for each requirement (all requirements must succeed; any handler may succeed a requirement; `Fail()` vetoes even if another handler succeeded). `FallbackPolicy` applies to endpoints with *no* authorize metadata — the "secure by default" lever; `DefaultPolicy` applies to bare `[Authorize]`. Confusing the two is a common misconfiguration.
7. **Q: Design authorization for hierarchical resources (org → project → document) without loading the whole hierarchy per check — what are the architectural options?** **A:** Options: (1) denormalize ancestor IDs onto the resource (document row carries org/project IDs) so one load gives all levels for a requirement handler; (2) claims carry org/project role grants (kept coarse to bound token size) with resource checks comparing IDs — fails at fine grain (thousands of projects); (3) a dedicated authorization service/model (ReBAC à la Zanzibar — SpiceDB/OpenFGA) storing relationship tuples, queried per check with caching — right when sharing/inheritance semantics get rich; (4) precomputed effective-permission tables maintained by events — fast reads, eventual-consistency windows on grant changes. Progression: start with (1)+(2); adopt (3) when relationship complexity (shares, groups, inheritance overrides) makes handler code the bottleneck.
8. **Q: How should authorization behave in a BFF + downstream microservices architecture — where does each layer's responsibility begin and end?** **A:** The BFF/gateway enforces coarse authentication and route-level authorization (valid session, basic role gates) and exchanges the user context downstream (forwarding the JWT, or token exchange for audience-scoped tokens per service). Each downstream service *must* re-enforce its own authorization (its audience validation + resource checks) — never trusting that the gateway filtered correctly, because service-to-service paths, queues, and future callers bypass the gateway (zero-trust posture). Data-layer fine-grained checks (row ownership) live only in the owning service; duplicating them upstream creates drift where the gateway says yes and the service says no.
9. **Q: What is the confused-deputy risk in service-to-service auth, and how do token audiences + on-behalf-of flows mitigate it?** **A:** Service A, authorized to call B, gets tricked (or drifts) into performing a privileged action on B *for an unauthorized end user* — B sees only "A called me," losing the original actor's authorization context. Mitigations: audience-scoped tokens (a token for B can't be replayed at C — B validates `aud`); on-behalf-of/token-exchange flows preserving the end-user identity in the downstream token so B authorizes the *user*, not just the calling service; and separating service identity (workload auth — mTLS/SPIFFE, client credentials) from delegated user context, evaluating both ("A may call this endpoint" AND "the user may act on this resource").
10. **Q: Explain resource-based authorization via `IAuthorizationService.AuthorizeAsync(user, resource, policy)` — how do requirement handlers receive the resource, and how do you keep handlers testable?** **A:** The overload flows the resource object into `AuthorizationHandlerContext.Resource`; a typed handler (`AuthorizationHandler<OwnerRequirement, Document>`) receives it strongly typed and applies logic (`document.OwnerId == userId`) calling `context.Succeed(requirement)`. Testability: handlers are plain classes — construct a `ClaimsPrincipal` with test claims, an `AuthorizationHandlerContext` with the requirement + resource, invoke, assert succeeded/failed; no HTTP host needed. Keep handlers free of data access (pass the loaded resource in) so tests need no mocking beyond claim construction — loading belongs to the caller, deciding to the handler.
11. **Q: What are authentication scheme *forwarding* and the "smart scheme" pattern, and what problem do they solve in mixed cookie+JWT apps?** **A:** One logical default scheme forwards to the real scheme per request via `ForwardDefaultSelector` — e.g., inspect the request: Authorization header with Bearer → JWT scheme; else → cookie scheme. This solves apps serving both browser UI (cookies) and API clients (JWTs) from one host without per-endpoint scheme annotations everywhere: challenges also route sensibly (API gets 401, browser gets login redirect). Without forwarding, the default scheme handles everything, producing wrong challenges (HTML login redirects to API clients — breaking SPAs with opaque CORS errors).
12. **Q: How do you prevent authorization logic drift when checks appear in three places — endpoint policies, domain services, and database queries?** **A:** Accept that layered checks exist for different reasons (fast-fail vs. invariant vs. data filtering), but make them share one source of truth: a permission service/model (central policy definitions, claim constants, permission enums) that endpoint policies, domain guards, and query filters (EF global query filters using the same tenant/ownership predicates) all consume — never re-derive rules inline; add contract tests asserting the layers agree (a matrix test: for each role × action, endpoint response, domain check, and query visibility must be consistent); log authorization denials with the deciding layer to surface disagreement in production. Drift is inevitable when the rules live as scattered inline conditions — centralize the *rules*, distribute only the *enforcement points*.
13. **Q: What does proper step-up/recent-auth enforcement look like at the protocol level — which claims and parameters implement "you must have logged in within 5 minutes with MFA"?** **A:** The IdP stamps `auth_time` (when authentication occurred), `acr`/`amr` (context class/methods — MFA indicators); the API's policy requires `amr` contains `mfa` and `auth_time` within the freshness window, else returns a challenge indicating step-up (OIDC: re-authenticate with `prompt=login` and `acr_values` requesting MFA, or `max_age=300` forcing fresh auth). The mistake is implementing step-up purely client-side (client decides to re-auth) — the *API* must enforce the claim requirements server-side, or the step-up is decorative.
14. **Q: Explain external-login account linking (Google + Microsoft + local password for one user) — data model and the security pitfall in automatic email-based linking.** **A:** Model: a user aggregate owning multiple login rows (provider, provider-key) — ASP.NET Identity's `AspNetUserLogins` shape; sign-in resolves provider key → user, and linking adds a row after the user proves control of both. Pitfall: auto-linking by matching email trusts the *provider's* email claim — a provider that doesn't verify emails (or lets users edit them) lets an attacker create an account at that provider with the victim's email and inherit the victim's local account. Rule: link only via verified-email providers you allowlist, or require the user to authenticate with the existing account first (interactive linking), never silent email matching.
15. **Q: How does `IAuthorizationMiddlewareResultHandler` let you customize authorization outcomes, and give a production-grade use case beyond changing status codes.** **A:** It intercepts the final authorization result before default challenge/forbid handling, receiving the policy, result, and context. Production uses: emitting structured audit events for every denial (who, endpoint, failed requirements — feeding anomaly detection); returning RFC-compliant ProblemDetails with a machine-readable reason code and remediation hint (e.g., "step_up_required" driving client MFA flows) instead of a bare 403; and *soft-launch mode* — logging would-be denials while still allowing requests during a new policy's rollout (report-only authorization), which turns risky org-wide policy tightening into a measurable, staged change.
16. **Q: What are the mechanics and risks of long claim lists — token size, header limits, and the "groups claim explosion" problem?** **A:** Claims inflate JWTs (each AD group membership becomes a claim); tokens ride in headers with server limits (proxies/Kestrel commonly 8–32KB) — users in hundreds of groups hit 431s or silent truncation, and cookie-auth equivalents split into multiple cookies approaching browser limits. Mitigations: IdP-side group filtering (only app-relevant groups), the `groups` overage pattern (token carries a link/flag; app queries Graph for memberships), mapping groups → few app roles at token issuance, or shifting fine-grained data out of tokens entirely into the authorization service (claims say *who you are*; the PDP decides *what you can do*).
17. **Q: Compare policy-based authorization in-process vs. externalized policy engines (OPA/Cedar/SpiceDB) — what forces justify externalizing, and what's lost?** **A:** Externalize when: policies change faster than deployments (business-managed rules), must be uniform across polyglot services (one engine, many languages), require audit/versioning of policy-as-artifact, or express relationship models (ReBAC) poorly suited to imperative handlers. Lost: in-process speed (network hop per check — mitigated by sidecar/embedded evaluation and caching), compile-time type safety between code and policy (contract drifts caught at runtime), and debugging simplicity (a denial now spans two systems). Middle path: policy engine as library/sidecar with locally-evaluated bundles — central management, local evaluation latency; the decision hinges on who authors policy changes and how many runtimes must agree.
18. **Q: What is continuous access evaluation / token revocation reality — how do you build "suspend a user and cut access within a minute" on stateless JWTs?** **A:** Options ordered by invasiveness: shorten access-token lifetime so refresh becomes the enforcement point (suspension checked at refresh — worst-case = token lifetime); a distributed deny-list of revoked subjects/sessions checked by APIs (small — only revocations — cached with sub-minute TTL or pushed via pub/sub to instances; validates the 99% path statelessly, consults the list always); token binding to a session version claim (bump user's session version in a fast store; APIs compare claim vs store — one cache read per request); or full introspection (opaque tokens — every request hits the IdP; simplest semantics, heaviest dependency). Most estates land on short tokens + pushed revocation cache: honest about the trade-off between statelessness and revocation latency.
19. **Q: How do you test authorization comprehensively at the API level — design the test architecture that catches missing `[Authorize]`, policy gaps, and IDOR regressions.** **A:** Layers: (1) a startup convention test enumerating all endpoints, asserting each carries auth metadata or an explicit `[AllowAnonymous]` allowlist entry (fails CI when someone adds an unannotated endpoint — the missing-attribute class dies here, and `FallbackPolicy` backstops runtime); (2) policy unit tests on handlers (Q10); (3) matrix integration tests using `WebApplicationFactory` with a test auth handler minting arbitrary principals — for each endpoint × persona, assert expected status (401/403/200), catching policy misassignment; (4) IDOR tests: persona A creates a resource, persona B attempts each verb against it expecting 403/404 — templatized per resource type so new endpoints inherit the suite. The enumeration test is the highest-leverage control: it converts "did anyone forget?" into a build failure.
20. **Q: As a Principal Engineer, you inherit an estate with four homegrown auth patterns across 40 services (custom header trust, shared API keys, two JWT variants with different claim conventions). Sequence the consolidation.** **A:** First, inventory and risk-rank: map every service's authN/authZ mechanism, trust relationships, and exposure (internet-facing custom-header trust is a live incident — fix immediately with a tactical gateway enforcing real auth in front while internals migrate). Define the target: one IdP, standard token profile (documented claim conventions, audiences per service, lifetimes), shared platform libraries encapsulating validation + policy conventions, workload identity for service-to-service. Migrate strangler-style: gateway normalizes external auth first (blast-radius reduction before elegance), then service-by-service adoption of the platform library with dual-accept windows (old + new credentials) and telemetry proving old-path traffic reached zero before removal. Govern the end state: paved-road templates make the standard the path of least resistance, CI conventions (Q19's enumeration tests) prevent regression, and API-key issuance is sunset with dates, owners, and dashboards — consolidation succeeds by making the secure path easier than the legacy one, not by mandate alone.
