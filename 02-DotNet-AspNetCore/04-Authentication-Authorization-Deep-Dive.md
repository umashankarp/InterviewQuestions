...
**CPU**: Policy evaluation (running each requirement's handler(s)) adds a small, generally negligible per-request cost for simple claim/role checks — the real performance-relevant concern, per §4, is any I/O-bound work (database/external-service calls) inside authentication/claims-transformation/authorization handlers, which directly adds to the request's critical-path latency and, at scale, contends for shared downstream resources (database connection pools) exactly as any other request-path I/O would.

**Memory**: `ClaimsPrincipal`/`ClaimsIdentity` objects are ordinary, request-scoped heap allocations (Module 1) — not typically a meaningful allocation-pressure concern relative to other request-processing costs, unless claims-transformation logic is adding an unusually large number of claims per request.

**Caching claims-enrichment lookups**: Directly reusing Module 9 §9's "in-process cache is per-replica, not fleet-consistent" guidance — a short-TTL `IMemoryCache`-based cache inside `IClaimsTransformation` (§4's fix) is appropriate specifically because slight, bounded staleness in enrichment data is acceptable; for enrichment data requiring strict fleet-wide consistency, an external distributed cache (Redis) would be the correct choice instead, at the cost of a network round-trip (still far cheaper than a full database query on every request).

**JWT validation cost**: Cryptographic signature validation (for JWT bearer tokens) has a real, measurable per-request CPU cost (asymmetric-key signature verification specifically, more expensive than a symmetric-key HMAC check) — for extremely high-throughput services, this is worth profiling explicitly (BenchmarkDotNet) if authentication-layer CPU cost is suspected as a bottleneck, though for the vast majority of services this cost is negligible relative to downstream business-logic/database work.

**Benchmarking**: Load-testing the authentication/authorization path specifically (not just the overall endpoint) — isolating "how much of this request's total latency is attributable to auth" via distributed tracing spans — is the standard way to catch exactly the class of regression §4's incident represents, ideally as a standing part of a load-testing suite covering any authentication-layer change, not just a reactive investigation after a production incident.

---

## 8. Security

- **`IClaimsTransformation` cache staleness as a security-relevant trade-off, not just a performance one**: caching a "current permission level" claim (§4's fix) means a **revoked** permission/deactivated account might remain effectively active for up to the cache's TTL — for security-critical claims (account suspension, explicit permission revocation), this staleness window must be explicitly evaluated against the business's actual security requirements; a short TTL (seconds to low minutes) is usually an acceptable trade-off, but this should be a deliberate, documented decision, not an unexamined side effect of a performance fix.
- **`401` vs `403` as a deliberate information-disclosure decision** (§2.6): some security postures deliberately return `401` for both "not authenticated" and "authenticated but not authorized to know this resource exists" specifically to avoid confirming a protected resource's existence to an unauthenticated attacker (avoiding an enumeration/reconnaissance vector) — a legitimate, security-driven deviation from the "textbook" 401-vs-403 semantics, worth recognizing as an intentional choice rather than a "bug" if encountered in a security-conscious API design review.
- **Multi-scheme authentication ambiguity as an attack surface**: if a multi-scheme application doesn't explicitly scope which scheme(s) apply to which endpoints (§6), an attacker might attempt to authenticate via a **weaker** scheme (e.g., a legacy API-key scheme with weaker validation) to access an endpoint intended to require a **stronger** scheme (e.g., a properly-validated JWT with additional claims checks) — explicit, endpoint-level scheme scoping is a genuine security control, not merely a clarity improvement.
- **Claims injection/spoofing risk in custom scheme handlers**: any custom `IAuthenticationHandler` implementation (e.g., a custom API-key scheme) must rigorously validate the incoming credential **before** trusting any claims derived from it — a common mistake is trusting a claim value taken directly from an easily-forged request header without first validating it against a trusted, server-side source (directly analogous to Module 9 §8's forwarded-headers-trust-scoping lesson, here applied to authentication credentials instead of proxy headers).
- **OWASP relevance**: Directly maps to **A01 (Broken Access Control)** for resource-based authorization gaps (using role checks alone where ownership checks are actually needed) and multi-scheme ambiguity; **A07 (Identification and Authentication Failures)** for claims-injection/spoofing risks in custom scheme handlers.

---

## 9. Scalability

- **Horizontal scaling**: Cookie-based authentication requires either **sticky sessions** (routing a given client's requests consistently to the same replica, if the cookie's backing session state is stored in-process) or, far more commonly and correctly for horizontally-scaled deployments, **externalized session/data-protection key storage** (Redis-backed `IDataProtectionProvider`/distributed cache-backed session state) so any replica can validate/decrypt any client's cookie regardless of which replica originally issued it — JWT bearer tokens are inherently more horizontally-scaling-friendly by design (self-contained, stateless, validatable by any replica with the shared signing key/public key, no shared session store required at all).
- **Vertical scaling**: JWT signature validation's CPU cost (§7) scales with core count for a Server-GC-style highly-concurrent service; cookie-based session lookups (if backed by a distributed cache) instead scale with the cache infrastructure's own capacity, a different scaling dimension entirely.
- **Caching/Replication/Partitioning**: Claims-enrichment caching (§4/§7) is itself a caching-strategy decision directly inheriting Module 9 §9's per-replica-vs-distributed trade-off discussion — apply the same reasoning (is bounded staleness acceptable? is fleet-wide consistency required?) here specifically.
- **CAP theorem**: A distributed session/data-protection-key store backing cookie authentication introduces a genuine availability dependency — if that store is unreachable, no replica can validate any cookie-authenticated request, a direct instance of trading some availability for the consistency benefit of centralized, shared session state; JWT's stateless design sidesteps this specific dependency entirely (at the cost of the token-revocation difficulty discussed in a later dedicated OAuth2/JWT module).
- **HA/DR**: Data-protection key rotation/persistence (the mechanism underlying cookie encryption/validation) **must** be configured to persist and share keys across all replicas (via a shared key ring — Redis, Azure Blob Storage, a shared file system) for cookie-based authentication to survive a rolling deployment/replica restart correctly — a commonly-missed configuration step whose absence causes exactly the "users get logged out on every deployment" symptom (§14).
