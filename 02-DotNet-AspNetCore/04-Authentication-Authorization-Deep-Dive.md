...
[Authorize]
[HttpGet("/invoices/{id}")]
public async Task<Invoice> Get(int id) => await _db.Invoices.FindAsync(id);
```
**A:** Broken object-level authorization. The caller is authenticated and allowed to *use the endpoint*, but nothing checks they're allowed to see *this* invoice — change the ID and you get someone else's. It's the most common serious API flaw, because the check has to be written per handler, it's invisible in review (the endpoint looks protected), and functional tests written from the owner's perspective never catch it. Filter by the caller's scope in the query, or authorise the loaded resource.
*Follow-up: Should a failed object-level check return 403 or 404?*

**Q2. When does the framework return 401 versus 403?**
**A:** 401 is a *challenge* — not authenticated, or credentials not accepted, and the response tells you how to authenticate. 403 is a *forbid* — we know who you are and you're not permitted. Getting it wrong matters operationally: 401 for a permission failure makes clients loop through re-authentication forever, and 403 for an expired token stops them refreshing it. Use the authorization pipeline rather than hand-rolled checks and you get the right one automatically.
*Follow-up: A client is stuck in a redirect loop after their token expires. Which end is wrong?*

**Q3. What's the difference, and which matters more?**
```csharp
options.DefaultPolicy = new AuthorizationPolicyBuilder().RequireAuthenticatedUser().Build();
options.FallbackPolicy = new AuthorizationPolicyBuilder().RequireAuthenticatedUser().Build();
```
**A:** The default policy is what a bare `[Authorize]` means. The fallback applies to endpoints with *no* authorization metadata at all — so setting it makes the system deny-by-default, and a forgotten `[Authorize]` yields 401 rather than open access. The fallback matters far more, because it converts the most common authorization defect from a silent vulnerability into an obvious failure. I'd treat it as mandatory for any service handling non-public data.
...
**A:** A policy is a set of requirements; each requirement is evaluated by one or more handlers. Multiple *requirements* in a policy must all succeed — AND. Multiple *handlers* for the same requirement mean any one succeeding is enough — OR. That OR is deliberate and useful ("owner or admin"), and it's also why adding a handler can silently widen access. People get this backwards regularly, so it's worth being precise about which axis is which.
*Follow-up: How would you express "must be an owner AND must have completed MFA"?*

**Q5. What does `[AllowAnonymous]` do to your policies?**
**A:** Wins over all of them — controller-level, endpoint-level, and the fallback. So an accidentally-placed attribute silently exposes an endpoint every other control believes is protected. Because it overrides everything, it deserves specific review attention and, in a sensitive codebase, an automated check enumerating anonymous endpoints against an approved list. Treat it as an explicitly-registered exception rather than an ordinary attribute.
*Follow-up: How would you enumerate every anonymous endpoint at startup?*

**Q6. Why can't an attribute do object-level authorization?**
**A:** Attributes evaluate before the handler runs, so they know the caller and the endpoint but not the resource — it hasn't been loaded yet. `IAuthorizationService.AuthorizeAsync(user, resource, policy)` after loading lets a handler compare the caller against the resource's owner or tenant. Missing that layer is exactly the flaw in Q1, and it's structural: no amount of attribute configuration can express "this record belongs to you".
*Follow-up: Where in the handler should that check go relative to the load?*

**Q7. Users are randomly logged out after scaling to three replicas. Why?**
**A:** Data Protection keys. They encrypt and sign authentication cookies, and by default the key ring is stored locally and, in a container, ephemerally — so each instance has different keys and a cookie issued by one is rejected by another. Restarts invalidate everything too. The fix is a shared, persisted key ring, itself encrypted at rest with a managed key. It presents as random logouts correlated with deployments and load balancing, which is a confusing symptom for what's actually a configuration default.
*Follow-up: Where would you store the key ring, and how do you protect it?*

**Q8. What must be validated on an incoming JWT?**
**A:** Signature against the issuer's current keys, issuer, audience, expiry and not-before, and the algorithm — never trusting the token's own header to pick it. Skipping signature validation means anyone can mint tokens. Skipping *audience* validation means a token issued for a different service is accepted here, which is cross-service privilege escalation. Skipping expiry means a leaked token is valid forever. These get disabled during development to "make it work" and then ship, so the configuration deserves explicit review and a test asserting a tampered token is rejected.
*Follow-up: How do keys rotate without downtime, and what does your service need to do?*

**Q9. Why can a `ClaimsPrincipal` have multiple identities?**
**A:** Because a caller can be authenticated by more than one scheme at once — a cookie plus an API key, or an application identity plus a delegated user identity. Each `ClaimsIdentity` holds claims from one source. It matters when checking claims, because a naive lookup searches across all identities and may find a claim from a source you didn't intend to trust. In a system with multiple schemes, being explicit about which identity a claim came from is a real concern.
*Follow-up: How would you check a claim only from a specific scheme?*

**Q10. When do you need antiforgery tokens?**
**A:** When the browser attaches credentials automatically — i.e. cookies. A cookie-authenticated form post needs CSRF protection; an API authenticated with a bearer token in a header does not, because the browser won't attach that header cross-site. The nuance is `SameSite`, which mitigates much of the classic attack without being a complete replacement — and a service using cookies for some endpoints and bearer tokens for others needs both models understood rather than one blanket setting.
...
**Q1. A service serves both a browser SPA and machine-to-machine callers. How do you configure schemes?**
**A:** Register both — cookie or OIDC for interactive, JWT bearer for machines — and select per endpoint rather than relying on a single default, since the default determines what a bare `[Authorize]` means and getting it wrong either breaks one class of caller or silently accepts the wrong credential type. Policies then name the acceptable schemes explicitly. I'd also make challenge behaviour differ appropriately: a browser gets a redirect, an API client gets 401 with `WWW-Authenticate`. Mixing those produces the classic "API client receives an HTML login page".
*Follow-up: How do you make a single endpoint accept either scheme?*

**Q2. Roles, claims or policies?**
**A:** Roles are coarse groupings that work only while the set stays small — expressing fine-grained access as roles produces combinatorial explosion and roles named after individual features. Claims carry attributes about the caller. Policies express the *decision* and can combine claims, roles, resource state and external checks. Use policies as the unit code depends on, named for the operation (`CanApprovePayment`) rather than the identity, so the underlying mapping from roles or permissions can change without touching endpoint code. That indirection is what survives an access-model redesign.
...
**A:** Coarse decisions — may this caller invoke this operation at all — at the endpoint via policies. Fine, data-dependent decisions close to the data, in the application or domain layer, because that's the only place with the resource in hand. The critical point is that the endpoint is *not* the only entry path: a message consumer, a scheduled job or an internal call reaches the same operation without passing any attribute. So authorization that exists only at the controller is bypassable by design, and anything protecting data rather than an HTTP route must be enforced where the operation happens.
*Follow-up: How do you authorise an operation triggered by a queue message with no user?*

**Q4. How do you implement multi-tenant isolation so one missed check can't leak data?**
**A:** In depth, and never by relying on developers to remember a filter. Tenant identity comes from the authenticated principal, never the payload. A global query filter applies the tenant predicate automatically. The database enforces it too, via row-level security or per-tenant credentials, so an application bug alone isn't sufficient to cross the boundary. On top, automated tests that authenticate as tenant A and assert every repository method returns nothing for tenant B's identifiers. Isolation should be a property of the infrastructure with the application layer as a second line, not the only one.
*Follow-up: A reporting job legitimately needs cross-tenant access. How do you permit that safely?*

**Q5. How do you implement data-dependent permission checks without wrecking performance?**
**A:** Load the permission set once per request into the principal or a scoped context rather than querying per check — a handler hitting the database on every authorization evaluation turns one request into dozens of round trips. `IClaimsTransformation` is the natural place to enrich the principal, with caching keyed on the user and invalidated on permission change. The trade-off to be explicit about is staleness: cached permissions mean a revoked permission stays effective until expiry, which is a business decision about the acceptable revocation window rather than a technical detail to default silently.
*Follow-up: The business requires revocation within 60 seconds. How does that change the design?*

**Q6. What are the operational consequences of long-lived versus short-lived tokens?**
**A:** Long-lived tokens reduce load on the identity provider and simplify clients, but extend the window a stolen token is usable and make revocation effectively impossible without introspection or a deny list. Short-lived tokens with refresh give a real revocation window at the cost of more IdP traffic — which then becomes a critical dependency whose outage logs everyone out. The design decision is where you want the failure. I'd generally take short access tokens with a resilient IdP and cached signing keys, and be explicit that token lifetime is a security-versus-availability trade with a named owner rather than a default nobody chose.
*Follow-up: The IdP goes down. What should your service do with tokens that are still valid?*

**Q7. How do you test authorization meaningfully?**
**A:** Test the negatives, because the positive path is what everyone writes and what never fails. Integration tests asserting: unauthenticated gets 401; authenticated-but-unentitled gets 403; a caller from tenant A can't read tenant B's resource by ID; and every endpoint has an authorization policy attached. That last one is best done as a test enumerating the endpoint data source and asserting on metadata, since it catches the newly-added endpoint nobody remembered to protect. Unit tests of individual handlers are worth having but can't catch the failure that actually happens, which is a policy not being applied at all.
*Follow-up: How do you write that enumeration test so it fails helpfully rather than cryptically?*

**Q8. A user's permissions change but it only takes effect after they log out. Why, and what do you do?**
**A:** The permissions were baked into a token or cookie at sign-in and nothing re-evaluates them until reissue. Options: short token lifetimes so the window is bounded; a server-side check per request, reintroducing the lookup you were avoiding; validation events on the cookie handler re-checking a security stamp; or a revocation list at the gateway. Which is right depends on how quickly the business requires revocation to take effect — and that requirement should be stated explicitly, because teams usually discover they've accepted a 24-hour window only when someone asks during an incident.
*Follow-up: How does a security stamp work, and what does it cost per request?*

**Q9. How do you authorise service-to-service calls?**
**A:** Workload identity rather than shared secrets — each service gets its own credential (managed identity, mTLS certificate, or client-credentials token) so calls are attributable and revocable individually. Then decide explicitly whether the downstream authorises the *calling service*, the *original user*, or both, propagating user identity via token exchange or on-behalf-of where the downstream enforces user-level rules. The anti-pattern is a shared "internal" credential with broad rights plus an implicit assumption that anything inside the network is trusted, which fails completely once one service is compromised.
*Follow-up: How do you propagate the user's identity without letting a service impersonate arbitrary users?*

**Q10. "This request is being denied and I don't know why." How do you diagnose it?**
**A:** Authorization failures are deliberately uninformative to the caller, so diagnosis comes from the server side: log the evaluated policy, which requirements failed, and the principal's relevant claims — with the caveat that logging claims is itself a data-protection concern, so it needs to be scoped and access-controlled. The framework's authorization events and debug-level logging expose most of it. I'd also make the response carry a correlation ID so a user's report maps to a specific decision. Building this before you need it is the difference between a five-minute answer and a day of guessing.
...
**Q1. How would you design an authorization model that outlives its current org chart?**
**A:** Separate the three layers people conflate: identity (who you are), entitlements (what's been granted, managed by the business), and enforcement (what code checks). Application code depends on stable, operation-named permissions; the mapping from users and groups to those permissions lives in a system the business can change without a deployment. That indirection is what lets roles be reorganised, a business unit be onboarded, or an access review be conducted without touching services. I'd also insist entitlement changes are auditable and every enforcement point is discoverable, because in a regulated environment answering "who can approve a payment and why" is a requirement, not a nice-to-have.
*Follow-up: Where's the line between RBAC and ABAC, and what does ABAC cost operationally?*

**Q2. Centralised authorization service or in-process evaluation?**
**A:** Centralised gives consistency, one place for audit and change, and appeals to compliance — at the cost of a hot-path network dependency, availability coupling, and latency on every decision. In-process is fast and resilient but drifts, because each service ends up with its own interpretation. The pattern I favour is centralised *policy authoring and distribution* with local evaluation: policies and entitlement data pushed to services and evaluated in-process, with caching and a bounded staleness window. Hot path stays local, source of truth stays central, and it degrades to "last known good policy" rather than to an outage.
*Follow-up: How do you handle propagation delay when a permission is revoked for cause?*

**Q3. How do you enforce deny-by-default across dozens of services?**
**A:** Make the secure configuration the inherited default: a platform package setting the fallback policy, wiring the standard schemes, and configuring token validation correctly, so a service must actively opt out rather than actively opt in. Then verify rather than trust — a CI check enumerating endpoints and failing on any without an authorization policy or an approved anonymous exemption, plus periodic scanning of deployed services. Organisationally, anonymous endpoints require a recorded exception with an owner, turning an invisible risk into a reviewable list. Guidance alone fails here reliably, because the failure is an omission and omissions are exactly what review misses.
*Follow-up: A team disables the shared package's fallback policy to unblock a release. How do you find out?*

**Q4. How do you migrate authentication mechanisms with zero downtime?**
**A:** Run both in parallel: accept old and new credentials simultaneously, with telemetry attributing every authenticated request to its mechanism so you can see actual migration progress rather than guessing. Migrate clients in waves, keep a hard cut-off date with an owner, and make the old path disableable by configuration so it can be turned off quickly. What teams underestimate is long-lived sessions and cached tokens, which extend the tail far beyond the client migration — and the Data Protection key ring, which must be preserved across the transition or every existing session breaks on the first deploy. Plan the rollback explicitly, because auth incidents are severe.
*Follow-up: Telemetry shows 0.3% of traffic still on the old mechanism after six months. How do you close it out?*

**Q5. How do you make authorization decisions auditable to a regulator's standard?**
**A:** Every decision that matters — grants, denials on sensitive operations, privileged access use, every entitlement change — needs a durable, tamper-evident record with who, what, when, on which resource, under which policy version. That's distinct from application logging: defined retention, restricted access, completeness guarantees, so it can't be sampled or best-effort. Policy itself must be versioned so a past decision can be explained against the rules then in force, which is the requirement most systems fail. I'd give the audit path its own delivery guarantee — an outbox or append-only store — and treat failure to record as a first-class failure where the regulation demands it.
*Follow-up: Recording synchronously adds latency to every sensitive operation. How do you evaluate that trade?*

**Q6. What's your view on token size and claim bloat at scale?**
**A:** It's a real operational problem: every claim rides on every request, so a token stuffed with permissions costs bandwidth, can exceed header limits at proxies and gateways, and is expensive to validate and log. Past a few kilobytes you start hitting infrastructure limits that fail confusingly. The fix is to carry identity and a small set of stable claims in the token and resolve fine-grained permissions server-side with caching — which also improves revocation, since the token stops being the source of truth for entitlements. Set an explicit size budget and monitor it, because claim bloat accumulates one reasonable-sounding addition at a time.
*Follow-up: A team wants to add a per-tenant permission array to the token. Response?*

**Q7. How do you handle authorization in an event-driven system where operations have no user?**
**A:** Decide explicitly what authority a message carries. Options: propagate the original user's identity in the message so the consumer enforces user-level rules; give the consumer its own service identity and authorise it as a system actor; or treat authorisation as having happened at command acceptance and record that decision in the event. Each is defensible; the failure is not choosing, which produces consumers that either enforce nothing or reconstruct identity inconsistently. Be careful too that propagated identity is signed or otherwise trustworthy, since a message on an internal bus isn't automatically authentic — and long-lived messages can outlive the permissions they were issued under.
*Follow-up: A message sits in a DLQ for three days and is replayed. Whose permissions apply?*

**Q8. You have a day to assess an inherited service's auth posture. What do you check?**
**A:** In order: is there a fallback policy or is protection per-endpoint; what's on the anonymous-endpoint list; is token validation fully configured including audience and signature; is Data Protection shared and persisted; is there any object-level authorization or only endpoint-level; where does tenant identity come from; and are there hand-rolled checks bypassing the policy engine. Each has a fast concrete answer and each maps to a specific severe failure mode. I'd also grep for disabled validation flags and for `AllowAnonymous`, which surface the most common deliberate weakenings. That set gives a defensible risk picture quickly.
*Follow-up: You find object-level authorization missing across 80 endpoints. How do you sequence remediation?*

**Q9. Security requirements and delivery velocity genuinely conflict. How do you handle it?**
**A:** Move the cost into the platform rather than onto every team: if the secure path is also the easy path — a package configuring everything correctly, templates starting deny-by-default, test helpers making negative authorization tests trivial — the conflict largely disappears. Where a genuine conflict remains, frame it as a risk decision with a named owner and documented acceptance, because that's what it actually is. What I wouldn't do is grant blanket exceptions to hit a date, since auth exceptions have a way of becoming permanent and are exactly what shows up in a post-incident review.
*Follow-up: A director asks you to ship with a known authorization gap to hit a regulatory deadline. How do you handle it?*

**Q10. What does a mature authorization architecture look like at five years, and what usually goes wrong?**
**A:** Mature: deny-by-default everywhere; permissions named for operations and decoupled from org structure; entitlements managed outside code with audit; enforcement close to the data so non-HTTP paths are covered; automated verification that every entry point is protected. What goes wrong on the way is almost always accretion — roles added per feature until nobody can describe who can do what, authorization logic duplicated across services with subtle differences, and exceptions granted for migrations that never complete. The counter is treating the access model as a product with an owner and a periodic review rather than as something each team extends locally. Without that ownership it degrades regardless of how well it was designed.
