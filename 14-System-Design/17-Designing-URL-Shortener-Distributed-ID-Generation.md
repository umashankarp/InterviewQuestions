...
- **Derive the code length from the keyspace requirement, out loud** (§2.1). It converts an arbitrary-looking choice into arithmetic and takes fifteen seconds. *When not to:* if the interviewer supplies the length, don't re-derive it.
- **Use KGS block allocation for this specific problem** (§2.2C) — it removes per-write coordination while keeping codes short and dense. *When not to:* if you need time-sortable IDs or the ID must encode its origin, Snowflake is the right tool and the extra length is the price.
- **Emit analytics fire-and-forget, but count the failures** (§4). The latency must leave the critical path; the error signal must not.
- **Use a conditional write for any uniqueness constraint** (§2.7). Never check-then-act across a network boundary.
- **Default to 302, not 301** (§2.5), and be able to say why in terms of reversibility and analytics rather than HTTP semantics.
- **Check expiry in application code even when the store has TTL** (§2.8) — TTL deletion is eventual in every store that offers it, so trusting absence for *correctness* is a defect.
- **Make generated and custom-alias namespaces disjoint by construction**, not by lookup (§2.7).
- **Cache with request coalescing on miss.** *When not to:* if the miss rate is genuinely uniform and low, coalescing adds contention for no benefit — it pays off precisely when access is skewed, which here it is.

---

## 6. Anti-patterns

- **Truncated-hash IDs without collision handling.** *Why it fails:* the birthday bound makes collisions certain at scale (§2.2A) — at 6 billion records in a 57-billion space, `P ≈ 1.0`, not "unlikely." *Fix:* KGS blocks, or accept hashing with an explicit retry loop and state its cost.
- **Sharding the write path.** *Why it fails:* 40 writes/sec is three orders of magnitude below the threshold that justifies it; this is over-engineering that will be probed. *Fix:* compute the number first (§2.1) and let it eliminate the option.
- **Synchronous click writes.** *Why it fails:* triples redirect latency and couples the product's availability to analytics availability. *Fix:* async emit with bounded buffering and a drop counter.
- **301 for cost savings.** *Why it fails:* irreversibly destroys analytics and revocation for a mutable mapping (§2.5). *Fix:* 302 with a short edge TTL; if you need edge caching *and* analytics, ship the CDN's own logs (§4).
- **Check-then-act on custom aliases.** *Why it fails:* two concurrent claims both pass the check; the second silently overwrites the first. *Fix:* conditional write (§2.7).
- **Reusing deleted short codes.** *Why it fails:* a previously-shared link silently redirects somewhere new — a security issue, not a hygiene one. *Fix:* tombstone permanently; the keyspace can afford it.
- **Storing the URL as the partition key.** *Why it fails:* you look up by short code, never by URL, so this partitions on the wrong attribute and makes the only query you actually run a scan. *Fix:* `shortCode` is the partition key; if you need URL→code lookup for dedup, that's a separate GSI and a separate decision.
- **Treating cache hit rate in aggregate.** *Why it fails:* a 95% aggregate can hide a cold hot-key class; and it obscures that the datastore is now sized for 5% of load (§2.4). *Fix:* hit rate per key class, plus an explicit cache-loss survival plan.

---

## 7. Performance Engineering

**Where the time actually goes on a redirect.** Budget: 10ms p99 end-to-end at origin.

```
TLS termination + ALB routing        ~2 ms
Redis GET (same AZ)                  ~0.5 ms      ← the hot path, 95% of requests
Kinesis emit                          0 ms        ← must be off-path; if this shows up, it's a bug
Response serialization + return      ~0.5 ms
                                     ───────
Cache-hit path                       ~3 ms        p99 target easily met

Cache-miss path adds:
DynamoDB GetItem (same region)       ~5 ms
Redis SETEX                          ~0.5 ms
                                     ───────
Cache-miss path                      ~8.5 ms      still inside budget — good, because
                                                  it means a cache flush degrades
                                                  latency without breaching SLA
```

The design property worth naming: **the miss path is deliberately kept inside the SLA.** If the miss path were 50ms, a cache restart would breach the SLA for minutes. Because it's 8.5ms, cache loss is a throughput event (20× more DynamoDB load) rather than a latency event — and throughput is the easier of the two to absorb with provisioned capacity and shedding.

**Allocation and GC on the hot path.** At 12,000 req/sec, per-request allocation matters. The specific wins in .NET: parse the short code from the path with `ReadOnlySpan<char>` rather than `Substring`; use a pooled `ArrayPool<byte>` for the base62 decode buffer; avoid LINQ in the request path (each `Select`/`Where` allocates an enumerator and a closure). The base62 encoder should be allocation-free:

```csharp
// Stack-allocated, zero heap allocation for a value that is at most 11 chars.
public static string ToBase62(long value)
{
    const string Alphabet = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
    Span<char> buffer = stackalloc char[11];
    int i = 11;
    do { buffer[--i] = Alphabet[(int)(value % 62)]; value /= 62; } while (value > 0);
    return new string(buffer[i..]);            // the single unavoidable allocation
}
```

This matters not because 12,000/sec is a lot, but because Gen0 pressure translates directly into GC pauses that appear in p99 — the tail is where allocation shows up, not the mean.

**Connection pooling is the real ceiling.** Per Module 176 §A6, the constraint on a service like this is not CPU — it's the Redis and DynamoDB connection pools. A multiplexed Redis client (StackExchange.Redis uses a single multiplexed connection by design) avoids per-request connection acquisition; misconfiguring it as connection-per-request is the classic defect that produces high p99 at low CPU.

**Benchmarking.** Load-test to saturation and identify which resource saturates *first* — the point is to learn the failure order, not to confirm the SLA. Test specifically: (a) steady state at peak, (b) cold cache at peak, which is the actually-dangerous scenario, and (c) a single hot key at 30% of total traffic, which is §14's incident.

---

## 8. Security

A URL shortener is an unusually attractive attack surface, because its entire function is to **obscure a destination** — which is exactly what a phishing campaign wants.

**Open-redirect abuse is the primary threat.** Anyone can shorten a link to a credential-harvesting page and distribute it under your reputable domain. Mitigations, in order of value:

1. **Destination scanning at creation and re-scanning periodically.** Check against Google Safe Browsing / equivalent threat feeds. Periodic re-scan matters because the standard evasion is to shorten a *benign* page and swap its content after approval — a check performed only at creation is a point-in-time claim about a mutable resource.
2. **Interstitial warning for unverified destinations.** A one-click confirmation page showing the actual target destroys the obscurity the attacker wanted, at the cost of friction on legitimate links. Usually applied selectively — new accounts, unscanned destinations.
3. **Rate limiting on creation, per account and per IP** (Module 175's material applies directly). Bulk-creation is the tell for abuse.
4. **Block redirects to internal address space** — `127.0.0.0/8`, `10.0.0.0/8`, `169.254.169.254` (the cloud metadata endpoint), and `localhost`. This is SSRF prevention: if any part of your system *fetches* the destination (for scanning, title extraction, or preview generation), an attacker who shortens `http://169.254.169.254/latest/meta-data/iam/security-credentials/` and induces your scanner to fetch it has exfiltrated your instance credentials. Validate after DNS resolution, not before — `evil.com` resolving to `169.254.169.254` defeats a name-based blocklist, and re-resolution between check and fetch (DNS rebinding) defeats a naive one-time check.

**Enumeration.** Sequential codes (§2.2B, and to a lesser degree KGS blocks) make the corpus walkable: an attacker increments through the keyspace and harvests every link. For a public shortener that's a privacy problem — people shorten links they believe are unlisted. For an enterprise one it's data exfiltration, since shortened links routinely point at internal documents.

The fix is **not** to switch to a random ID scheme, which sacrifices density and the properties §2.2 selected for. Instead, decouple the *generated* ID from the *exposed* code by applying a keyed, format-preserving permutation:

```csharp
// A bijective permutation over the 62^6 space, keyed by a server-held secret.
// Sequential internal IDs → non-sequential external codes, with NO collisions
// (a bijection cannot collide) and NO extra storage.
public string Externalize(long internalId) => ToBase62(_permutation.Apply(internalId));
public long   Internalize(string code)     => _permutation.Invert(FromBase62(code));
```

A Feistel network over the ID space gives this cheaply. It preserves every property of dense sequential allocation while making codes unguessable — the *"and"* answer rather than the *"or"* answer, which is the kind of move that scores well.

**Access control.** Links to private resources need authorization at redirect time, not just possession of the code — "unguessable URL" is not an access-control mechanism (this is the classic IDOR/capability-URL confusion). If a link is enterprise-scoped, the redirect must check the caller's identity, which means the redirect path now has an auth dependency and a very different latency budget. Say so rather than assuming public links.

**PII in analytics.** Raw IP addresses are personal data under GDPR. Truncate to /24 (IPv4) or /48 (IPv6) at ingestion, before storage — truncating at query time means you stored it, which is the thing that requires a lawful basis. Retention limits belong in the storage lifecycle policy, not in a cleanup job.

**Secrets.** The permutation key above is a *durability-critical* secret: lose it and every existing short code becomes undecodable. It requires backup and a rotation strategy that keeps old keys available for decoding (key ID prefixed or inferred by generation range), which is a genuine design obligation, not an ops detail.

---

## 9. Scalability

**Read scaling is the whole problem and is nearly free**, because redirects are independent, cacheable, and read-only. Stateless redirect services scale horizontally without limit; the cache scales by sharding on the short code (which distributes perfectly, since codes are effectively uniform); DynamoDB scales by partition and the partition key is the code. There is no coordination anywhere on the read path, which is why this system can absorb enormous read growth cheaply.

**Write scaling is a non-issue at this scale** and would remain so at 100× — 4,000 writes/sec still sits at the edge of a single primary and comfortably within DynamoDB's per-partition limits given a uniform key.

**The genuine scaling risk is the hot key**, not aggregate volume. One viral link can consume a disproportionate share of a single cache shard and a single DynamoDB partition — DynamoDB's per-partition throughput ceiling (roughly 3,000 RCU) is a hard limit that aggregate provisioning does not relieve. Mitigations, in order:

1. **Edge caching handles it naturally** — a viral link is by definition highly cacheable, and CloudFront absorbs it entirely. This is the primary defence and it's automatic.
2. **Local in-process cache** on each redirect service node for the top-N codes, with a short TTL. A hot key served from process memory costs nothing and never touches Redis. This is a very small amount of code for a large amount of protection.
3. **Key replication** for identified hot keys — store `code#0`…`code#9` and read a random suffix, spreading one logical key across ten physical ones. Only worth it if the first two fail.

Note this is Module 175 §14's pattern precisely: aggregate cluster health stays green while one shard saturates, so **the metric must be per-shard, not fleet-wide** (§14 below).

**Multi-region.** Redirects are the ideal multi-region workload: read-only, cacheable, latency-sensitive, and tolerant of eventual consistency. Deploy redirect services and caches in every region, replicate the store (DynamoDB Global Tables), and route with latency-based DNS. The write path can stay single-region — 40 writes/sec does not justify multi-region write complexity, and a creation that takes 100ms extra for a distant user is unnoticeable, whereas a redirect that takes 100ms extra is the product being slow.

The consistency consequence to state: a link created in `eu-west-1` may not be resolvable in `us-east-1` for a second or two. For most links, irrelevant. For the "create then immediately test your own link" flow, visible and confusing — solve it with read-your-own-writes routing (Module 176 §I4) for the creating user's session specifically, not by making replication synchronous.

**CAP/PACELC positioning.** During a partition, choose **A** — serving a possibly-stale redirect is far better than serving an error, because a stale mapping is almost always still correct (mappings rarely change). In the else case, choose **L** — latency over consistency, aggressively, throughout. The one exception is revocation, which is a *consistency-critical* operation on an otherwise AP system: a revoked link that still resolves is a security failure. That asymmetry is the interesting part — the system is AP for reads and needs CP-like behaviour for one specific write, which is why revocation gets explicit cache purging (§2.8) rather than relying on TTL convergence.

**RPO/RTO.** RPO for the mapping store should be effectively zero — losing a created link means a distributed URL now 404s permanently and unrecoverably, with no way to even know what it pointed to. That argues for synchronous replication on the write path, which is affordable precisely because writes are rare. RPO for analytics can be minutes; losing some click events is a data-quality issue, not a correctness one. **Different RPOs for different data in the same system** is the mature position, and stating it is the same per-data-type discipline Module 37 §2.5 established for consistency.
