 ...
 **A:** Store registry entries with a short TTL (the expiration mechanism) refreshed via periodic heartbeat from each chat server for its currently-held connections — if a server crashes without cleanly deregistering, its entries simply expire naturally within the TTL window rather than persisting indefinitely as stale, incorrect routing information (directly avoiding a-style "orphaned state retained forever" risk, here bounded by TTL rather than requiring explicit cleanup); a message routed to an about-to-expire or already-expired stale entry fails the delivery attempt, triggering the message to be queued for offline-style delivery until the recipient's client reconnects and re-registers with a now-current, correct server mapping.
