...
**CPU:** Per-device encryption (§2.2) means the sender's **client device**, not the server, absorbs the cryptographic CPU cost — Double Ratchet encryption/decryption operations are individually cheap (sub-millisecond on modern mobile hardware), but a large group without the sender-key optimization multiplies this by device count, which is precisely why §2.4's optimization exists: it moves group-message CPU cost from O(members × devices) per message back to O(1) per message on the steady-state path, reserving the expensive multiplicative cost for the comparatively rare membership-change event.

**Memory:** Each device maintains Double Ratchet session state (chain keys, skipped-message-key caches for out-of-order delivery) per conversation partner device — for a user with thousands of contacts and group memberships, this is a real, bounded-but-nontrivial per-device memory footprint; skipped-key caches specifically must be bounded (a small maximum window) since an unbounded cache is both a memory-growth risk and, per §3.6, the surface for ratchet-desync-driven unbounded growth if a peer goes silent indefinitely without the session being torn down.

**GC/Allocations:** On the server, envelope routing is a high-throughput, low-per-message-cost path (§Step 1's estimation: 1,000,000 sends/s) — envelopes are opaque byte blobs, so the server does no cryptographic allocation at all; its allocation profile is dominated by ordinary message-routing/queueing objects, not cryptography, which is a direct, favorable consequence of the server never touching plaintext or performing encryption.

**Latency:** End-to-end delivery latency (p99 < 500 ms online, §Step 1) is dominated by network/routing hops (Module 03's transport layer), not cryptographic operation time — Double Ratchet operations add single-digit milliseconds at most; the latency-sensitive cryptographic operation is **prekey-bundle fetch plus X3DH** on first contact with a new device (§Step 2 walkthrough steps 1–3), which adds a network round trip before the first message of a new session can even be encrypted, worth explicitly calling out as the one place E2E setup cost is user-visible.

**Throughput:** Server-side throughput at 1,000,000 sends/s is achievable specifically because the server performs zero cryptography (§Step 1 finding 2) — this is the favorable, counter-intuitive point worth stating explicitly in an interview: an E2E-encrypted messenger's server infrastructure is *cheaper* per message than a plaintext messenger's would be, since content inspection, spam scanning, and rich-preview generation (§3.5) are all absent from the server's critical path by construction.

**Benchmarking:** Benchmark sender-key distribution cost (§Step 1's 2,500-pairwise-message figure for a 1,000-member group) against realistic group-churn rates, not just steady-state message volume — a group with low message volume but high membership churn can spend more aggregate client-side encryption cost on key redistribution than on actual messages, mirroring exactly the same "the expensive operation is the one that recurs on the wrong trigger" lesson as this course's other capacity-planning material.

**Caching:** Prekey bundles are explicitly *not* cached beyond single-use (§Step 2 API design: "each one-time prekey is returned at most once, ever") — this is a case where caching would directly undermine the security property (forward secrecy of the initial key exchange) the mechanism exists to provide, worth flagging as a place where the normal "cache for performance" instinct is actively wrong.

---

## 8. Security

*(This module is security-and-cryptography-centric by nature; §2, §3.4, and §Step 3 already carry substantial security depth. This section summarizes the OWASP/threat-model framing specifically, without repeating that material.)*

**Threats:** The dominant threat is not the cryptographic primitives (Signal Protocol is extensively peer-reviewed, §2.1) but **device-list integrity** (§Step 3 §3.4) — a compromised or legally-compelled server adding a device it controls to a target's device list, causing senders to unknowingly encrypt to an attacker-controlled device. Secondary threats: prekey exhaustion attacks (§Step 3 §3.2, an attacker cheaply draining a target's one-time-prekey bucket to force weaker fallback sessions); metadata analysis (who-talks-to-whom, unprotected by content encryption, §2's honest limitation); and a compromised device exposing all conversations that device (not other devices of the same user) participates in.

**Mitigations:** Key transparency (§Step 3 §3.4) — publishing device-list changes to an append-only, auditable log — is the structurally strongest mitigation for the device-list threat, converting an undetectable compromise into a detectable one. Safety-number verification (§10 Advanced Q9) mitigates active MITM against initial key exchange specifically, at the cost of requiring inconvenient, rarely-performed manual user action. Rate-limiting prekey-bundle fetches per requester (§Step 3 §3.2) mitigates the prekey-exhaustion attack.

**OWASP mapping:** Broken Authentication/Session-Management maps to device-linking (§Step 3 §3.3) — a flawed linking flow that allowed server-mediated key injection would be a direct authentication-bypass vulnerability at the cryptographic-identity level, categorically more severe than an ordinary session-token vulnerability since it compromises the confidentiality guarantee itself, not just account access. Security Logging and Monitoring Failures maps directly to §Step 4's standing dashboard requirement — "the count of servers or services that have any access path to plaintext, whose correct value is zero" — since a silent regression here would be invisible without deliberate, continuous monitoring.

**AuthN/AuthZ:** Device identity *is* the authentication mechanism for message decryption (§3.1's "the device, not the user, is the cryptographic identity") — there is no separate application-level authorization layer that matters for content confidentiality, though ordinary account-level authentication (who may register/link a device at all) remains a conventional, necessary complementary control.

**Secrets:** Private keys never leave the device that generated them, by protocol design — there is no server-side secret whose compromise exposes message content, which is the defining property distinguishing this system's secrets-management posture from almost every other system in this course (where a server-held secret's compromise is the primary threat scenario).

**Encryption:** Double Ratchet (per-message symmetric keys, forward secrecy, post-compromise security) for 1:1 and sender-key sessions (shared symmetric key, rotated on membership change) for groups — both detailed in §2 and §Step 3; media is client-encrypted with a random per-file key transmitted inside the E2E message envelope (§Step 2 components, Media Service).

---

## 9. Scalability

**Horizontal scaling:** The server-side routing/storage tier scales horizontally and near-linearly, precisely because it performs no cryptography and holds no long-lived per-conversation state beyond routing metadata (§Step 1 finding 2) — this is the system's most favorable scaling property, a direct dividend of the E2E design.

**Vertical scaling:** Not a meaningful lever anywhere in this architecture — the design is throughput-bound by envelope count and storage churn, both of which scale via horizontal partitioning (below), not per-node capacity.

**Caching as scaling:** Deliberately limited — prekeys are single-use by design (§7), and envelopes are short-lived, deleted-on-ACK (§Step 2 data model) rather than cached/retained, since retention here is "a liability, not a feature." This system's scalability story is almost entirely about *not* accumulating state, the inverse of the caching-as-scaling-lever pattern common elsewhere in this course.

**Replication/Partitioning:** Envelopes partitioned by recipient device (§Step 2 store selection) — a natural, evenly-distributing shard key at 2 billion users' worth of devices. Prekey/device stores require strongly-consistent partitioning specifically because prekey consumption must be exactly-once (§Step 2's "this is precisely why prekeys look like a cache and are not").

**Load balancing:** Standard connection-tier load balancing (Module 03) for routing; no load-balancing concern is specific to the E2E/multi-device layer itself, since it adds no compute-heavy server-side work to balance.

**High Availability:** The routing/storage tier's HA story is conventional (Module 03). The *cryptographically* consequential availability question is different: session state lives entirely on devices, so a server outage cannot corrupt or lose cryptographic state, only delay delivery — a notably strong property compared to systems where the server holds authoritative state whose loss during an outage is catastrophic.

**Disaster Recovery:** The device-list audit log (§Step 2 data model, append-only) and the prekey/device registry are the state genuinely worth protecting with DR rigor — envelope loss is recoverable (senders can be prompted to resend via delivery-receipt gaps), but device-list history loss would undermine the evidentiary basis §3.4's key-transparency mitigation depends on.

**CAP theorem:** The routing tier favors availability (deliver when possible, queue when not) — content confidentiality is unaffected by a partition, since it depends on cryptography, not on server consistency. The prekey-consumption path, by contrast, must favor consistency over availability during a partition (§Step 2's "an eventually-consistent store hands the same prekey to two senders") — the same split-CAP-posture pattern this course applies repeatedly to different data classes within one system, here mapped onto ordinary-message-routing-availability versus prekey-consumption-consistency.
