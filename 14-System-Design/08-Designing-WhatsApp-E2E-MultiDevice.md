...
**Sender-side encryption cost as a genuinely new bottleneck class**: unlike every other system in this course's arc where the server/infrastructure absorbs the bulk of processing cost, per-device encryption (§2.2) shifts real computational cost onto the **sending client device** itself — a large group message (before the sender-key optimization) could require the sender's phone to perform dozens/hundreds of encryption operations before the message even leaves the device, a latency and battery-cost concern specific to end-to-end-encrypted messaging that has no direct analog in Module 39's server-mediated (not E2E) chat architecture. **Sender-key distribution cost amortization**: the sender-key mechanism (§2.4) trades an occasional, more expensive "distribute the key to every current member's every device" operation (at group-join/membership-change time) for a cheap, constant-cost per-message operation thereafter — directly the same amortized-cost reasoning as Module 33 §2.1's `List<T>` growth-doubling strategy, now applied to cryptographic key distribution instead of array resizing. **Offline multi-device delivery backlog size**: since delivery status must now be tracked per-device (§2.5), a user with several devices, one of which stays offline for an extended period (an old, rarely-used linked device), accumulates a growing backlog specifically for that device — directly Module 26's Streams backlog-monitoring discipline, now needing per-device granularity rather than per-user.

## 8. Security
**Forward secrecy and post-compromise security as the Double Ratchet's core value**: even if a device's current encryption key is somehow compromised, forward secrecy ensures past messages remain unreadable (the keys used for them have already been discarded/rotated), and post-compromise security ensures the protocol "heals" — future messages become secure again once new key material is exchanged, even without the user necessarily realizing a compromise occurred — this is a fundamentally stronger guarantee than a static, unchanging encryption key would provide, directly justifying the added protocol complexity over a naive "just encrypt with a fixed key" approach. **The out-of-band device-linking QR code as the actual security boundary**: Module 39 §8's general "authenticate the connection" concern is insufficient here — the QR-code-scanning step is what prevents the **server itself** (a compromised, coerced, or malicious server operator) from being able to inject a fraudulent "new device" into a user's account undetected — recognizing the server as a potential adversary in this specific threat model (not merely an untrusted network) is the key mental shift E2E-encryption system design requires beyond ordinary Module 12-style server-mediated authentication. **Metadata privacy as a separate, still-unsolved-by-E2E-encryption concern**: E2E encryption protects **message content**, but the server, as the relay, inherently still observes **metadata** (who messages whom, when, how often, message sizes) — a genuinely honest system-design answer should explicitly acknowledge this distinction (content privacy vs. metadata privacy are different guarantees) rather than implying E2E encryption alone makes all communication patterns invisible to the platform operator.

## 9. Scalability
**Group size as a direct multiplier on sender-key redistribution cost, not just message-delivery fan-out**: unlike Module 38's celebrity-account fan-out concern (bounded by follower count alone), a group chat's scalability here is bounded by **membership-change frequency** as much as raw size — a large, frequently-churning group (members constantly joining/leaving) requires frequent sender-key rotation (§2.4), each rotation itself an O(members × devices-per-member) operation, potentially making membership churn (not steady-state messaging) the actual scaling bottleneck for very large, highly-dynamic groups. **Per-device state as a genuinely new scaling dimension beyond Module 39's per-user state**: the connection registry (Module 39 §2.2) and delivery-backlog tracking (§2.5) must now scale with **total device count**, not total user count — a platform where the average user has 2-3 linked devices effectively has 2-3x the connection/delivery-tracking state Module 39's single-device-per-user model would have assumed, a direct, quantifiable capacity-planning adjustment (Module 37 §2.2) required specifically because of the multi-device requirement.
