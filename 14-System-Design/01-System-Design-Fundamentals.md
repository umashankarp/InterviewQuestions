...
**Cache-aside** (lazy loading): application checks the cache first, falls back to the database on a miss, populating the cache afterward — the most common pattern, directly/§Hard exercise's stampede-resistant caching implementation. **Write-through**: writes go to the cache and the database synchronously together — keeps the cache always consistent with the database, at the cost of added write latency. **Write-behind** (write-back): writes go to the cache immediately, asynchronously flushed to the database later — lowest write latency, but risks data loss if the cache fails before flushing (directly the same durability-vs-latency trade-off as/26's write-concern discussions, now at the caching-layer). Cache invalidation strategy (TTL-based, event-driven via a message queue, or explicit invalidation on write) must be chosen deliberately — "cache invalidation is one of the two hard problems in computer science" is a cliché precisely because getting it wrong (serving stale data indefinitely, or thrashing the cache with unnecessary invalidations) is a common, real failure mode.
...
graph TB
 ...
 App1 --> Cache["Distributed Cache (Redis,-26)"]
...
- Treating CAP theorem as a one-time, system-wide decision rather than recognizing it applies per-operation/per-data-type (directly DynamoDB's per-request consistency parameter,, demonstrates this granularity is achievable and often necessary).
...
4. **Q: Why is sharding described as "the hardest-to-reverse" scaling lever, and why should it be a later, not first, resort?** **A:** Changing a shard key after data is distributed requires substantial data migration/redistribution (/27's recurring warning); the earlier, cheaper levers (vertical scaling, read replicas, caching) should be exhausted first since they're far less operationally risky and disruptive to reverse or adjust.
 ...
 **A:** Load balancer overhead: ~2ms (network-layer routing, minimal processing); authentication check: ~10ms (a cached/fast claims check, per the lesson about NOT putting an uncached database call here); cache lookup: ~5ms (Redis,, sub-millisecond typically, budgeted generously); remaining ~183ms budgeted for the cache-miss database path (20's query-optimization target) — explicitly reserving the *majority* of the budget for the slowest, least-predictable component (the database) rather than distributing evenly, since database query time is both the largest and most variable component; this budget should be **validated under load** (not just calculated on paper) via the load-testing discipline this course has repeatedly emphasized (§Advanced Q7).
...
*(This module's entire structure IS the system-design section — the production example, the worked exercises, and the extensive cross-referencing throughout/// collectively constitute the deep-dive system-design content typically split across separate sections in other modules.)*
