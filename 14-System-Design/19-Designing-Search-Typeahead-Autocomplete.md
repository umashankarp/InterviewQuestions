...
- **Define the analysis chain once and apply it to both indexing and querying through the same code path** (§2.2). *Why:* asymmetry produces zero results with no error and no explanation. *When not to:* deliberate asymmetry is occasionally correct — index-time n-grams with exact-term queries — but it must be explicit and documented, never accidental.
- **Treat typeahead and search as separate systems** with separate indexes, budgets, and freshness requirements (§1). *When not to:* a tiny corpus (a few thousand entities) can serve both from one structure; below that scale the separation is overhead.
- **Precompute top-K completions at every FST node** (§2.4) — it makes prefix latency independent of subtree size, which is what makes the budget achievable.
- **Start posting-list intersection with the shortest list, and use skip lists** (§2.1). This is usually the engine's job, but knowing it is what lets you diagnose a slow query.
- **Keep stopwords; handle their low value in ranking** (§2.2). Removal is unfixable at query time and breaks real queries.
- **Use index aliasing for all reindexing** (§2.5) — zero-downtime, instantly reversible. A system that cannot reindex routinely accumulates decisions it cannot revisit.
- **Prefer fewer, larger shards** (§2.6). Every shard increases tail exposure; over-sharding for "scale" makes latency worse.
- **Return partial results on deadline rather than waiting for every shard** (§2.6). Correct for search specifically, because relevance is approximate — and wrong for most other domains in this folder.
- **Measure with interleaving, not A/B, for ranking changes** (§2.8) — 10–100× more sensitive because it removes between-user variance.
- **Version and test any configuration that changes query semantics** (§4). Synonyms, analyzers, and boosts are logic.

---

## 6. Anti-patterns

- **Asymmetric analysis between index and query.** *Why it fails:* zero results, no error, no diagnostic. *Fix:* one definition, one code path (§2.2).
- **Running prefix matching over the document index for typeahead.** *Why it fails:* cannot meet a 50ms budget, and typeahead should search *queries and entities*, not documents. *Fix:* a separate FST with precomputed top-K (§2.4).
- **Treating Elasticsearch's 1-second refresh as a requirement.** *Why it fails:* pays continuous query and merge cost for freshness nobody asked for. *Fix:* derive the refresh interval from the actual staleness tolerance (§2.5).
- **Over-sharding for scale.** *Why it fails:* increases tail-amplification exposure; adding shards does not add query throughput — replicas do. *Fix:* size shards for index volume, add replicas for QPS (§2.6).
- **Removing stopwords.** *Why it fails:* breaks phrase queries and legitimate short-term searches irrecoverably. *Fix:* keep them, downweight in ranking.
- **Relying on popularity alone for ranking.** *Why it fails:* a self-reinforcing loop that new items cannot break into. *Fix:* deliberate exploration budget (§2.3).
- **Zero-result rate as the only matching metric.** *Why it fails:* directionally blind — it cannot see over-matching, which is §4's incident. *Fix:* also monitor result-set size distribution.
- **A relevance test set that is never refreshed.** *Why it fails:* it measures a corpus and query distribution that no longer exist, so it passes while quality degrades. *Fix:* rebuild from recent logs on a schedule, with an owner (§2.8).
- **Replacing lexical search with vector search.** *Why it fails:* vector retrieval is poor at exact identifiers — part numbers, ISINs, error codes — which are often the highest-intent queries. *Fix:* hybrid retrieval with RRF (§2.7).
- **Hydrating full documents from the index.** *Why it fails:* the index is optimized for matching, not for storing display data; storing large fields bloats it and slows every query. *Fix:* store only what ranking needs; fetch display fields from the primary store by ID.

---

## 7. Performance Engineering

**Where query time goes**, for a 12-shard cluster with a 200ms p99 budget:

```
Query understanding (spell, synonyms, entities)   ~5 ms
Coordinator fan-out + network                     ~2 ms
Per-shard: term lookup + posting intersection    ~10 ms   } in parallel
Per-shard: BM25 scoring of candidates            ~15 ms   } across shards
Per-shard: local top-K heap                       ~2 ms   }
Gather + global merge                             ~3 ms
Re-rank top 100 (model inference)                ~25 ms
Hydrate display fields (batch by ID)             ~15 ms
                                                 ───────
                                                  ~77 ms typical
  Tail: whichever shard is slowest dominates (§2.6) — the p99 is not
  the sum of p99s, it's the sum plus the max, and the max is what moves.
```

**The dominant lever is candidate-set size.** Scoring cost is linear in candidates, so a query matching 2M documents costs 2,000× one matching 1,000. Mitigations: aggressive filtering *before* scoring (structured filters are cheap and cut the candidate set dramatically — this is why query understanding pays for itself), and **WAND / block-max WAND**, which uses per-block maximum scores to skip blocks that provably cannot enter the top K. Block-max WAND commonly delivers 3–10× on multi-term queries and is the single most important query-time optimization in modern lexical search.

**Memory is the real constraint, not CPU.** Search performance is dominated by whether the index fits in page cache. A cluster where the working set exceeds RAM does disk I/O per query and latency degrades by an order of magnitude — and the transition is a cliff, not a slope. This makes **index size a latency parameter**, which reframes several decisions: storing positions doubles or triples size (so phrase support has a latency cost), storing large display fields in the index is actively harmful, and `doc_values` for sorting/aggregation are columnar and disk-friendly by design, whereas in-memory field data is not.

**JVM GC is a specific hazard** for JVM-based engines. Large heaps produce long pauses that appear directly in the tail, and a GC pause on one shard becomes that query's latency for every user hitting it. The standard guidance — heap at or under ~31GB to keep compressed object pointers, and leave the majority of RAM to the OS page cache rather than the heap — is counter-intuitive (more heap feels better) and is exactly the opposite of what teams do by default.

**Typeahead has an entirely different profile.** With a memory-mapped FST there is essentially no computation — traverse and read. The costs are network round trip and, critically, **QPS**: at 5–15 requests per search session, typeahead is often the highest-QPS endpoint in the product. The optimizations are therefore client-side: **debounce** (wait ~100ms after the last keystroke; typing at speed should not generate a request per character), **cancel in-flight requests** when a new keystroke arrives, and cache prefix results client-side since `ca` → `car` → `card` are related and the client already has the earlier answers.

**Benchmarking.** Replay *real* query logs, never synthetic queries — real distributions are heavily skewed with a long tail of expensive multi-term queries that synthetic tests miss entirely. Test specifically: the 99th-percentile-cost query, cold page cache (which is what a node restart produces), and behaviour during a merge, since merges compete with queries for I/O and are when tail latency is worst.

---

## 8. Security

**Entitlement filtering is the dominant concern**, and it is architecturally awkward in a way that makes it a frequent source of leaks. Search must return only documents the caller may see, and there are two approaches with very different properties:

**Filter at query time** — add the caller's permitted-scope as a filter clause. Correct, always current, and the cost grows with permission complexity: a user with access to 10,000 individual documents produces an enormous filter. This is the right default.

**Filter after retrieval** — retrieve top-K then remove unauthorized results. **Almost always wrong**, and worth being able to explain why: if 8 of the top 10 are filtered out, the user sees 2 results and pagination breaks in a way that leaks information — the *count* of results the user cannot see is itself disclosure, and in a financial context "how many instruments exist matching this criterion" can be material.

The pattern that scales: index a **permission token set** on each document (the roles/groups that may see it) and filter on the caller's token set, which is a cheap terms filter rather than a document enumeration. The obligation this creates is that **permission changes require reindexing the affected documents**, so revocation is not instant — and that window must be a stated policy, not an accident. For genuinely sensitive corpora, the resolution is separate indices per security domain, which makes cross-domain leakage structurally impossible rather than filter-dependent — the same *make it unrepresentable* preference as Modules 177 §E7 and 178.

**Query logs are sensitive data.** What people search reveals intent, and in aggregate it reveals strategy: at a financial firm, the queries a desk runs disclose positions they are researching. Query logs therefore need the same access controls as the data itself, retention limits, and care in analytics — and "we log all queries for relevance tuning" is a statement with compliance consequences that must be made deliberately.

**Injection and resource exhaustion.** Exposing a raw query DSL to users is the search equivalent of SQL injection — Elasticsearch's `script` fields and some query types permit arbitrary computation. Never pass user input into a query DSL structure; build the query programmatically from parsed input. Separately, some queries are *legitimately* catastrophic: deep pagination (`from: 100000` requires every shard to produce 100,000 results and the coordinator to merge them), leading-wildcard queries (`*ing` cannot use the term index and scans the dictionary), and enormous boolean clauses. These need hard limits — max clauses, max pagination depth (with `search_after` cursors as the correct alternative), and query timeouts — because a single user can otherwise take down the cluster without malice.

**Typeahead leaks through suggestions.** A suggester built from query logs will surface other users' queries, which can expose confidential terms — a deal codename typed by one user appearing as a suggestion to another is a real and serious incident class. Mitigations: only include queries issued by a minimum number of *distinct* users (a k-anonymity threshold), exclude anything matching sensitive patterns, and scope suggestions by entitlement domain. This is not hypothetical and it is easy to ship accidentally, because the default behaviour of "build suggestions from logs" produces it.

---

## 9. Scalability

**The two scaling axes are independent and conflating them is the standard error.** Index size scales by **sharding**; query throughput scales by **replicas**. Adding shards to a QPS problem makes it worse (more fan-out, worse tail); adding replicas to an index-size problem does nothing.

**Shard sizing.** The practical guidance is tens of gigabytes per shard — large enough that shard count stays low (limiting tail exposure, §2.6), small enough that recovery and rebalancing complete in reasonable time. Shard count in Elasticsearch is fixed at creation, so this is a decision made once and changed only by reindexing — which is why routine reindexing capability (§2.5) is what makes the decision reversible.

**Time-based indices for append-mostly corpora.** Logs, news, and market commentary are naturally partitioned by time, and this is transformative: queries with a time filter touch only relevant indices, old indices become read-only (so they merge to a single optimal segment and can move to cheaper storage), and deletion is dropping an index rather than deleting documents — which matters because document deletion in a segment-based index is a *tombstone* that continues consuming space until a merge reclaims it. Retention becomes free instead of expensive.

**Hot-warm-cold tiering** follows from that: recent indices on fast nodes with more replicas, older ones on cheaper nodes with fewer, oldest as searchable snapshots on object storage. This is where large search deployments make their cost savings, and it is only available if the corpus is time-partitioned.

**The routing optimization.** If a query can always be scoped to one tenant, routing that tenant's documents to a single shard converts a scatter-gather into a **single-shard query** — eliminating fan-out and tail amplification entirely. This is the strongest available latency win for multi-tenant search, and it costs a hot-shard risk for large tenants, mitigated by giving the biggest tenants dedicated indices. The general principle is the same one Module 178 §9 reached for ledgers: **choose a partition boundary that queries do not cross, and the distributed problem disappears.**

**CAP/PACELC.** Search is comfortably **AP**: a slightly stale or partial result set is acceptable, and refusing to serve results is worse than serving imperfect ones. In the else case, choose **L** — latency over consistency, aggressively. This is one of the few systems in this folder where that is the right answer without qualification, and the reason is §2.6's: there is no exact answer for a partial result to be wrong *about*.

**RPO/RTO.** The search index is a **derived** artifact — the source of truth is the primary database. That makes RPO effectively irrelevant: a lost index is rebuildable. What matters instead is **rebuild time**, which is the real recovery metric and is routinely unmeasured. A cluster that takes 40 hours to reindex has a 40-hour RTO regardless of its replica configuration, and teams discover this during an incident. Rebuild time should be measured, tracked as it grows with the corpus, and treated as the number that must stay within the availability requirement.

**Multi-region** is straightforward for search precisely because the index is derived: replicate the *indexing* pipeline to each region and let each build its own index, rather than replicating the index itself. Regions may transiently disagree, which is acceptable for search. This is much easier than the equivalent problem for a ledger, and saying why — derived state has no authoritative copy to keep consistent — demonstrates the distinction.
