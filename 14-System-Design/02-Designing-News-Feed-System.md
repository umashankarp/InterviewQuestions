...
**Fan-out latency vs read latency, the core trade-off quantified**: fan-out-on-write pays O(followers) write cost at post time but O(1) (a single precomputed-list read) at feed-read time; fan-out-on-read pays O(1) at post time but O(following-count) scatter-gather cost at every single feed read — given that feeds are read far more frequently than posts are created (a strong non-functional assumption worth explicitly stating and, per Module 37 §2.1, verifying), the aggregate system-wide cost of pure pull is typically far higher than pure push **for the majority of normal-follower-count accounts**, which is precisely the quantitative argument for why push is the right default and pull is reserved specifically for the celebrity-account exception (§2.4) where push's cost inverts catastrophically. **Precomputed feed storage and its own scaling limits**: a Redis sorted-set-per-user feed cache (Module 25 §2.1's ZSet structure, naturally suited to timestamp-ordered feed entries) has a practical per-user size limit (trimming old entries via `ZREMRANGEBYRANK`, directly Module 25 §7's "bound the structure's growth" discipline) — an unbounded, ever-growing per-user feed cache reproduces Module 23 §4's unbounded-embedded-array incident in a different structural guise, requiring the same deliberate bounding discipline. **Ranking latency budget**: if ranking is ML-model-based (a later, more advanced topic beyond this module's scope), it adds a real, must-be-budgeted latency cost on top of candidate gathering — directly Module 37 §7's latency-budget-allocation discipline, requiring the ranking step's specific latency contribution to be measured and bounded, not assumed to be negligible.

## 8. Security
**Follower-graph privacy and access control**: a user's follow/follower list, and the visibility of a private account's posts, requires resource-based authorization (Module 12 §2.4) applied per post — a feed-generation service must verify, for every candidate post surfaced (whether via push or pull), that the requesting user is actually authorized to see it (respecting blocks, private-account restrictions), not merely that a follow relationship nominally exists — a naive fan-out-on-write implementation that pushes a post into a follower's feed cache **before** checking whether that follower has since been blocked, or whether the account has since gone private, can leak content the current authorization state should prevent, a genuine BOLA-adjacent risk (Module 16 §2.1) specific to this system's precomputed-cache architecture. **Rate limiting post creation**: given fan-out's write-amplification cost (§2.2/§7), post-creation itself is exactly the kind of expensive, resource-consuming operation Module 16's rate-limiting discipline should protect — an attacker (or a buggy client) posting at a high rate from a high-follower-count account could trigger a deliberate or accidental denial-of-service against the fan-out infrastructure, directly the same algorithmic/infrastructure-complexity-attack-vector concern raised in Module 33 §8/Module 35 §8, now at the feed-fan-out-system level. **Content moderation as an architectural concern**: any feed system handling user-generated content needs an explicit point in the architecture (typically at post-creation time, before or concurrent with fan-out) for content-moderation checks — deferring this entirely to a post-hoc review process means potentially-harmful content propagates through the fan-out pipeline to millions of followers before any moderation action occurs, a genuine trust-and-safety architectural gap worth proactively raising in a design discussion even if not explicitly asked about.

## 9. Scalability
**The celebrity problem as the canonical illustration of Module 37 §9's scaling-ladder discipline**: this entire module's central lesson (§4) is a direct, concrete instance of "a design correctly scaled for its original assumptions can require architectural evolution as those assumptions change with growth" — the hybrid model (§2.4) is itself an instance of climbing the scaling ladder progressively (starting with the simpler push model, evolving to a hybrid specifically once the pull-for-celebrities need is demonstrated, not preemptively built for a scale that doesn't yet exist). **Sharding the precomputed feed cache**: as user count grows, the Redis-based feed cache itself needs sharding (Module 25 §2.5's Cluster hash-slot mechanism) — sharding by user ID (the natural key every feed read/write already uses) directly parallels Module 27's partition-key-design discipline, with the same high-cardinality, evenly-distributed-access-pattern reasoning applying identically at this system's caching layer. **Geographic distribution for global scale**: for a globally-distributed user base, feed-read latency benefits from regional caching/CDN-adjacent techniques (Module 37 §7's CDN discussion) — but the underlying follow-graph and post data still need a coherent, likely-AP-leaning (Module 37 §2.5) cross-region consistency model, since "does my feed show a post my friend made 30 seconds ago from a different continent" is a use case that can tolerate the eventual-consistency window a globally-distributed system's cross-region replication inherently introduces (directly Module 37 §Advanced Q4's regional-affinity reasoning, here applied to feed data specifically rather than a shopping cart).
