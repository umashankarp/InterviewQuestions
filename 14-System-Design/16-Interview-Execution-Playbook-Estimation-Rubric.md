...
- **Run the fixed requirements script every time (§2.1).** Improvising the opening burns budget you need at minute 30. *When not to:* if the interviewer opens with a fully-specified prompt including numbers, skip to confirming and move on — running the script against a prompt that already answered it wastes the same budget from the other direction.
- **Compute only numbers that eliminate an option, and say what they eliminated.** *When not to:* a pure "deep dive on one component" round may not need estimation at all; don't perform the ritual if the prompt doesn't call for it.
- **State the partition key and the query it makes expensive, in the same breath.** This single habit is a disproportionately strong signal because so few candidates do it.
- **Announce your time budget at 0:00 and enforce it audibly.** Cutting yourself off reads as senior; being cut off reads as junior.
- **End every deep dive with the failure mode and its detection.** *When not to:* if you're at 43 minutes, a rushed failure list is worse than a clean close on the current point.
- **Ask "which part should I spend our time on?" early.** The interviewer usually has a target; letting them name it converts a guessing game into a directed exercise.
- **Keep a "things I'm deliberately not doing" list visible on the whiteboard.** It converts omissions — which look like gaps — into decisions, which look like judgment.

---

## 6. Anti-patterns

- **Box-drawing at minute one.** Feels productive, guarantees you design an unscoped problem. *Why it fails:* the design has no forcing constraints, so every component is unjustifiable when probed. *Fix:* nothing gets drawn until the out-of-scope sentence has been said aloud.
- **Estimation theatre.** Computing QPS, storage, and bandwidth, then never referencing them again. *Why it fails:* it demonstrates arithmetic, not judgment, and consumes five minutes. *Fix:* every number must be followed by "therefore…".
- **Resume-driven architecture.** Reaching for Kafka/Kubernetes/CQRS because you know them well. *Why it fails:* interviewers at this bar specifically probe for over-engineering, and "we might need it later" is a known-bad answer. *Fix:* name the simpler option you rejected and the specific requirement that rejected it.
- **The vocabulary diagram.** Boxes labelled with technologies, no arrows carrying semantics, no data model. *Why it fails:* it collapses instantly under one "what's inside that box?" *Fix:* for each box be able to state its input, its output, its state, and how it fails.
- **Answering a deep-dive invitation by re-describing the high-level design.** *Why it fails:* it reads as evasion, and it burns the scoring segment. *Fix:* when a question is asked about component X, the next 4 minutes must be strictly inside X.
- **Designing only the happy path.** *Why it fails:* at Staff+, failure design *is* the design; a happy-path-only answer caps at senior regardless of elegance. *Fix:* the failure-mode habit from §2.5.
- **Capitulating on probes.** Changing the answer at the first raised eyebrow. *Why it fails:* it reveals the original choice had no reasoning. *Fix:* distinguish probe from correction (§2.7) before responding.
- **Silent thinking.** Long pauses with no narration. *Why it fails:* an unobservable thought process cannot be scored; the interviewer must assume the worst. *Fix:* narrate the branch you're evaluating, even incompletely.

---

## 7. Performance Engineering

The performance content of an interview answer is mostly about **knowing which resource is the constraint**, and candidates routinely misidentify it.

**The four constraint classes, and their tells:**

| Constraint | Tell in the requirements | Design consequence |
|---|---|---|
| **CPU-bound** | Encoding, encryption, compression, pricing/risk math, ML inference | Horizontal scale works linearly; caching does nothing; batch size matters |
| **Memory-bound** | Large working sets, in-memory indexes, per-connection state | Vertical scale hits a wall; partitioning is mandatory, not optional |
| **I/O-bound** | Most CRUD systems, anything database-fronted | Async everywhere; connection pool sizing dominates; caching is the big lever |
| **Network-bound** | Media, market data, replication, cross-region | Placement and compression dominate; CDN/edge is the lever |

The commonest mistake is treating an I/O-bound system as CPU-bound and proposing "more app servers" for a problem where the database connection pool is the actual ceiling. Adding app servers to a pool-constrained system makes it **worse** — more contenders for the same finite pool, deeper queueing, higher p99.

**Latency budgeting, spoken.** Given a 200ms p99 target: allocate, don't distribute evenly. Reserve the majority for the slowest and most variable component (almost always the uncached database path), and note that the budget must be *validated under load*, because p99 under concurrency is dominated by queueing, not by service time. Little's Law is the tool: `L = λW` — a service handling 1,000 req/s with 50ms mean latency has ~50 requests in flight, which is what sizes the thread pool and the connection pool. A candidate who reaches for Little's Law unprompted is signalling clearly.

**The tail-latency point worth making.** If a request fans out to 100 shards and each has a 1% chance of a slow response, the probability that *at least one* is slow is `1 − 0.99^100 ≈ 63%`. The p99 of the whole becomes roughly the p99.99 of the parts. This single arithmetic fact justifies hedged requests, and dropping it into a fan-out design is a reliable Staff signal. (Developed fully in `16-Distributed-Systems/06-TailLatency-HedgedRequests-TailAtScale.md`.)

---

## 8. Security

Security is where the FinTech panel diverges most sharply from a generic product-company loop, and where candidates most often lose points by omission rather than error. At a bank or payments firm, an otherwise-excellent design that never mentions authorization boundaries, audit, or data residency reads as *someone who has not worked in a regulated environment* — an immediate concern for a Staff+ hire.

**The four things to mention unprompted, in ~60 seconds total, on any design touching regulated data:**

1. **AuthN vs. AuthZ boundary.** Where is identity established, and where is *entitlement* enforced? The common failure is enforcing authorization only at the gateway, leaving internal service-to-service calls implicitly trusted — which means any compromised or buggy internal service can read anything. Say "authorization is enforced at the data-access layer, not only at the edge."
2. **The audit trail as a first-class component.** In a SOX/PCI-scoped system, "who saw what, when" is a functional requirement with its own retention, immutability, and query needs — not a logging afterthought. Audit records must be append-only and must survive the deletion of the thing they describe.
3. **Data classification driving the topology.** PII/PCI/MNPI determine what may cross a region boundary, what may be logged, and what must be tokenized. "Card data never enters our systems; we hold a network token" removes an entire compliance scope and is exactly the kind of scope-reduction move a Principal is expected to make.
4. **Secrets and key rotation as design, not ops.** Where do keys live, who can read them, and what breaks during rotation? A design that cannot rotate a key without downtime has a defect, not an operational inconvenience.

**The OWASP items that actually surface in system design** (as opposed to code review): broken access control at the *object* level (IDOR — user A requests `/accounts/{B}` and the handler trusts the path parameter); SSRF where a service fetches a user-supplied URL; and injection surfaces at any layer where a raw query path bypasses the ORM — the exact defect class Module 132's tenant-leak incident documents, where a `UNION ALL` branch omitted the manual tenant filter that an interceptor was assumed to guarantee.

**Rate limiting as a security control, not just a capacity control.** Pre-authentication endpoints are CPU-amplification targets — a bcrypt verify is expensive *by design*, which makes an unlimited login endpoint a denial-of-service surface. Naming this connects to Module 175 §8 and reads as someone who has actually operated an exposed system.

---

## 9. Scalability

**The scaling ladder, in mandatory order.** Interviewers probe whether you reach for the expensive rung first:

```
1. Vertical scale          — cheapest, fastest, boring. Exhaust it. A modern box
                             is enormous; "it won't scale" is usually unmeasured.
2. Read replicas           — solves read-heavy, introduces replica lag, and thus
                             read-your-own-writes as a NEW correctness problem.
3. Caching                 — huge for read-heavy, near-useless for write-heavy.
                             Adds a second source of truth and invalidation risk.
4. Async / queue offload   — removes work from the request path. Converts a
                             latency problem into a consistency/lag problem.
5. Horizontal shard        — near-unbounded, hardest to reverse. Resharding is a
                             migration project, not a config change.
6. Multi-region            — solves geography and DR; every consistency question
                             you had now has a 100ms floor under it.
```

Each rung buys throughput and **costs a new correctness problem**. Naming the cost is the signal. "We'll add read replicas" is rung 2 with no cost named; "we'll add read replicas, which means the write path now needs read-your-own-writes handling — I'd route a user's reads to the primary for a few seconds after their own write, keyed by a session token" is rung 2 with the cost named and solved.

**On sharding specifically:** the shard key is chosen for *distribution* and *query locality* simultaneously, and those pull against each other. Hot-partition risk is the failure — a celebrity user, a single hot instrument at market open, one whale merchant. The mitigations to have ready: composite keys with a bounded random suffix (spreads the hot key across N sub-partitions at the cost of a scatter on read), separate handling for the known-hot minority (the celebrity path differs from the normal path), and consistent hashing with virtual nodes to keep rebalancing incremental.

**HA/DR vocabulary you must use correctly**, because misusing it is a visible tell:

- **RPO** = how much data you can afford to lose (drives replication mode: sync = RPO 0 and a latency cost; async = RPO measured in seconds and no latency cost).
- **RTO** = how long you can be down (drives whether standby is warm, hot, or active-active).
- Active-passive is cheaper and has a failover window with real risk (the failover path is the least-tested code you own). Active-active removes the window and forces you to solve conflicting concurrent writes across regions — for a ledger, that is a genuinely hard problem, not a configuration.

**CAP, correctly stated.** CAP applies *during a partition*, and it applies **per-operation**, not per-system. The mature version is PACELC: during a **P**artition choose **A**vailability or **C**onsistency; **E**lse (normal operation) choose **L**atency or **C**onsistency. The "else" half is what actually governs daily behaviour and is where most real designs live — and most candidates never mention it. (Full treatment: `16-Distributed-Systems/03-PACELC-Consistency-Models-SplitBrain.md`.)
