...
**Startup latency (time-to-first-frame) as a distinct, critical metric**: unlike a typical API's request latency, video streaming has a specific, user-perceived "how long until playback starts" metric, directly influenced by manifest-fetch latency, initial segment size, and CDN edge proximity — this should be explicitly measured and budgeted (Module 37 §7) as its own, separate concern from steady-state streaming throughput. **Transcoding cost and prioritization**: transcoding is genuinely compute/GPU-intensive at scale — prioritizing which renditions to generate first (§4's fix: cheapest/lowest-resolution first) and potentially deferring rarely-requested renditions (a very old, rarely-viewed video's 4K rendition might be generated on-demand only if actually requested, rather than upfront for every upload) directly trades storage/compute cost against availability latency, a genuine, deliberate trade-off. **CDN cache-hit-rate optimization**: since the CDN is the primary serving path (§2.4), cache-hit rate (not database query performance) is the dominant performance lever — ensuring popular content is pre-warmed/replicated to CDN edges proactively (for anticipated high-demand content, e.g., a scheduled premiere) versus relying purely on reactive, first-request cache population directly affects the viewer experience for a video's very first viewers in a new region.

## 8. Security
**Signed URLs for access-controlled content**: for private/restricted videos, CDN-served content still needs authorization enforcement — since the CDN itself typically can't evaluate the platform's own authorization logic (Module 12), the standard pattern is a **signed URL** (a time-limited, cryptographically-signed URL generated by the origin after the origin verifies authorization, which the CDN can validate structurally — signature and expiration — without needing to understand the platform's authorization model itself) directly extending Module 31's Proxy-pattern "control access" concept to the CDN-delivery context specifically. **Content moderation for uploaded video**: directly Module 38 §8's content-moderation discussion, now for a much richer media type — automated content-scanning (copyright detection, policy-violation detection) typically runs as part of the transcoding pipeline (§2.2) itself, another independent job in the same fan-out structure, potentially gating a video's public availability until moderation clears it. **DRM for premium/licensed content**: for content requiring digital-rights-management protection (licensed movies, premium subscriber content), the streaming architecture must integrate encryption/license-server validation into the adaptive-bitrate playback flow — a genuinely more complex requirement than ordinary content delivery, worth explicitly flagging as a distinct, additional architectural layer if the specific platform's content includes DRM-protected material.

## 9. Scalability
**Storage tiering by access frequency**: most view volume concentrates on recently-uploaded/currently-popular videos (a power-law distribution directly analogous to Module 38's follower-count skew, here applied to video popularity instead) — a mature platform tiers storage accordingly (hot, frequently-accessed renditions on fast, CDN-adjacent storage; cold, rarely-viewed old content on cheaper, higher-latency archival storage, regenerating rarely-requested high-resolution renditions on-demand rather than storing them indefinitely, per §7's cost/latency trade-off). **Horizontal scaling of the transcoding worker fleet**: since transcoding jobs are independent (§2.2/§4's fix) and stateless per-job, the worker fleet scales identically to any other stateless, queue-consuming fleet (Module 37 §9's default, Module 38 §Advanced Q4's burst-absorption pattern) — auto-scaling the worker count based on queue depth directly. **Global CDN distribution as the actual horizontal-scaling mechanism for the read path**: unlike a typical system where the application/database tier is the horizontal-scaling focus, a video platform's read-path scaling is almost entirely a function of CDN edge-node distribution and cache-hit rate (§7) — the origin/database tier serves a comparatively tiny fraction of actual byte volume, making CDN architecture the dominant scalability lever for this specific system class, a genuinely different scaling story than nearly every other system this course has covered.
