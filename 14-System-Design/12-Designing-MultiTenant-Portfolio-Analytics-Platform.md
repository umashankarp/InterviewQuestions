...
## 5. Best Practices
- Layer isolation so no single mechanism is load-bearing — application, data-access, and storage-level enforcement together.
- Make absent tenant context an error, never an unscoped query; default deny.
- Resolve tenant identity from authenticated credentials, never from a request parameter a caller could set.
- Test isolation by seeding two tenants and asserting no foreign rows leak, across every query path.
- Allocate compute per tenant with fair-share scheduling rather than first-come-first-served.
- Verify onboarding migrations by reproducing the client's own reported figures, not by trusting the load succeeded.

## 6. Anti-patterns
- A single isolation mechanism with exception paths that are invisible where edits happen (the incident).
- Resolving tenant from a request-supplied identifier rather than from credentials — a direct IDOR.
- Shared compute with no per-tenant quotas, so a legitimate large job degrades every other tenant.
- Per-tenant code branches instead of configuration and extension points.
- Support staff with blanket access to tenant data as the diagnostic strategy.
- Onboarding treated as provisioning rather than a verified data migration.

---

## 7. Performance Engineering

**CPU/Memory:** The analytics workload resembles the — compute-bound, parallelizable — but with the added constraint that resource consumption must be *attributable* per tenant, both for fair-share scheduling and for billing. Instrument at the tenant level from the start; retrofitting attribution is difficult.

**Latency:** Interactive analytics has a genuine UX threshold — beyond a few seconds users context-switch. This differs from the batch orientation and pushes toward pre-computation of common views.
...
**Benchmarking:** Benchmark under multi-tenant contention, not single-tenant load. A platform that performs well in isolation and poorly under realistic tenant mix has been benchmarked wrong (this is exactly §Intermediate Q2's isolation test, applied to a heavier workload).

**Caching:** Cache keys must include tenant identity — a cache is another place the isolation boundary can be crossed, and a cross-tenant cache hit is a leak with the same consequence as a query leak (§Intermediate Q5's warning, with higher stakes here).
...
**Mitigations:** the layered enforcement; per-tenant encryption keys so a storage-level compromise does not yield all tenants' data; and careful treatment of any cross-tenant aggregate (peer benchmarking is a valuable product feature and an inference risk — it must be sufficiently aggregated that individual contributions cannot be reverse-engineered).

**OWASP mapping:** Broken Object-Level Authorization is the central risk, at tenant granularity. the incident is precisely a BOLA failure originating in query construction rather than in an authorization check.
...
**Secrets:** Per-tenant credentials and encryption keys, managed, with key rotation that does not require coordinated tenant downtime.
...
**Horizontal scaling:** By tenant partition, with tiered capacity reflecting tenant size heterogeneity.
...
**Caching:** Per-tenant cache namespaces; pre-computed common views per tenant to meet interactive latency.
...
**Load balancing:** Requests route by tenant to their assigned partition; the fair-share scheduler governs compute allocation within contention.
