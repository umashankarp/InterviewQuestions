...
- **A consistent snapshot.** Nothing moved during the run. the incident is exactly what happens when this is lost and not replaced: the snapshot-pinning discipline is the intraday reconstruction of what batch provided implicitly.
...
Directly the Parallel Run and the migration reconciliation: each converted job runs alongside its batch predecessor, and their outputs are compared. The batch is authoritative until reconciliation demonstrates equivalence over a representative period.
...
Some jobs should stay batch, and identifying them early prevents wasted effort: genuinely period-scoped computations (month-end accounting close), jobs whose inputs only change daily (a vendor file delivered once), and jobs where regulatory definition requires a point-in-time computation.

The "old systems never die" warning applies with a twist: here, *some* of the old system correctly survives, so the discipline is distinguishing "deliberately retained batch" from "not yet migrated batch" — and tracking the latter with a deadline while exempting the former explicitly, so the residual batch estate does not become an undifferentiated backlog nobody can reason about.
...
## 5. Best Practices
- Migrate upstream-first along the dependency DAG; converting a consumer ahead of its inputs delivers no freshness.
- Define exit criteria as demonstrated scenario coverage, not elapsed reconciliation time, with synthetic injection for rare events.
- Map what runs before and after a job in the batch sequence before extracting it — ordering encodes dependencies that code does not.
- Compare reconciliation at the batch's cadence (intraday value as of cut-off), not at an arbitrary intraday moment.
- Make freshness explicit per value so hybrid-period consumers can reason about mixed staleness.
- Distinguish deliberately-retained batch from not-yet-migrated batch, tracking only the latter.

## 6. Anti-patterns
- Cutting over on elapsed-time evidence without scenario coverage (the incident).
- Migrating the business-visible consumer first, producing intraday capability fed by stale inputs.
- Treating the hybrid period as a brief transition rather than the state the firm operates in for years.
- Assuming a job's dependencies are visible in its code when the batch encoded them in sequence ordering.
- Letting engineering silently choose semantics for computations whose meaning changes intraday.
- An undifferentiated "remaining batch" backlog conflating what should stay with what has not yet moved.
...
**Throughput:** Sized for peak event rate (market open, volatility) rather than total daily volume. Batch capacity planning is a volume-over-window calculation; intraday is a rate calculation, and the peak-to-mean ratio (the 50–100×) means naive translation under-provisions severely.
...
**Benchmarking:** Benchmark the hybrid state, not just the target state. The hybrid runs longest and carries the awkward interactions, and a benchmark of the clean end-state measures a configuration the firm will not occupy for years.

**Caching:** Batch could cache freely within a run because inputs were frozen; intraday cache invalidation must follow the dependency graph, and an incorrect graph now produces stale outputs continuously rather than being reset by the next night's run.
...
**Caching:** the dependency-graph-driven invalidation.

**Replication/Partitioning:** Intraday state needs replication batch did not — batch could be rerun from inputs, whereas intraday state is continuously updated and its loss means rebuilding from event history (the replay), which takes time the intraday cadence does not have.
...
**CAP theorem:** Batch was trivially consistent (a quiet world). Intraday forces per-output choices: the limits engine remains CP; analytics views can be AP; the reporting is CP. The migration is where these choices must be made explicitly for outputs that never needed a choice before, and defaulting silently is how a CP requirement becomes an AP implementation.
 ...
 **A:** A consistent snapshot (nothing changes during the run), a total order of computation, a natural completion signal, and a repair window.
 ...
 **Follow-ups:** "Which one caused the incident?" (Loss of the consistent snapshot — snapshot pinning is its intraday reconstruction.)

2. **Q: Why migrate upstream-first rather than starting with the business-visible consumer?**
 **A:** An intraday job fed by batch inputs cannot be fresher than its stalest input, so converting a consumer first delivers no value while consuming budget.
 ...
 **Follow-ups:** "Why is this counterintuitive?" (Business pressure comes from downstream, so the visible consumer is what stakeholders ask for first,.)

3. **Q: Why is the hybrid period a design target rather than a transition?**
 **A:** For a large estate it lasts years — it is where the firm operates for most of the migration, so mixed-freshness consumption must be designed for rather than endured.
 ...
 **Follow-ups:** "What makes mixed freshness dangerous?" (A view mixing intraday positions with overnight FX is misleading in a way users cannot easily detect,.)

4. **Q: What went wrong in the incident?**
 **A:** Corporate-action handling lived in a separate batch step between the migrated job and its consumer; the intraday service never implemented it, and six weeks of reconciliation could not reveal this because no corporate action occurred in that window.
 ...
 **Follow-ups:** "What replaced the exit criteria?" (Demonstrated scenario coverage rather than elapsed time, with synthetic injection for rare events,.)

5. **Q: Why must reconciliation compare at the batch's cadence?**
 **A:** The intraday job produces many values daily and the batch one; comparing an arbitrary intraday value against the batch's end-of-day figure diverges legitimately and buries real differences in noise.
 ...
 **Follow-ups:** "What else must reconciliation do?" (Classify legitimate divergence rather than treating it as failure, or it becomes noise and stops being read,.)

6. **Q: Give an example of a computation whose meaning changes intraday.**
 **A:** P&L attribution — "P&L for the day" is well-defined, but "P&L so far today" requires deciding the start point and how intraday position changes are attributed, which is a business definition.
 ...
 **Follow-ups:** "What is the failure mode?" (A number that looks like its batch predecessor but answers a subtly different question,.)

7. **Q: Which jobs should deliberately remain batch?**
 **A:** Genuinely period-scoped computations (month-end close), jobs whose inputs change only daily (a once-daily vendor file), and jobs where regulatory definition requires point-in-time computation.
 ...
 **Follow-ups:** "Why track the distinction explicitly?" (So retained batch is not confused with un-migrated batch in an undifferentiated backlog,.)

8. **Q: Why does intraday typically increase total compute?**
 **A:** Incremental recomputation repeats work batch did once; the gain is latency, not efficiency.
 ...
 **A:** Per-output staleness monitoring — intraday has no single completion moment, so freshness must be tracked per output rather than as one job status.
 ...
 **Follow-ups:** "Why is there no completion signal?" (Nothing is ever 'done' — there is no moment when everything is current,.)

10. **Q: Why does intraday have no maintenance window?**
 **A:** Batch had a natural quiet period; intraday runs continuously, so deployments and schema changes must happen against a live system — an operational capability the organization may not have.
...
1. **Q: Explain precisely why the reconciliation could not have detected the gap.**
 ...
 **Follow-ups:** "What is the general principle?" (Elapsed-time evidence measures time, not coverage — in a domain with episodic events these are weakly correlated,.)

2. **Q: Design the scenario inventory that replaced the time-based exit criteria.**
 ...
 **Follow-ups:** "How do you know the inventory is complete?" (You do not with certainty — which is why the third fix, mapping the batch sequence's surrounding steps, matters: it derives the inventory from what the batch actually did rather than from imagination.)

3. **Q: Why were the batch's dependencies invisible in the migrated job's code?**
 **A:** Batch encoded them in **job ordering** — position roll-forward ran before corporate actions, which ran before valuation — so the contract was the schedule, not an interface. Reading the job's code reveals nothing about what must run before or after it.
 ...
 **Follow-ups:** "What is the practical countermeasure?" (Explicitly map the surrounding sequence and each step's contribution before extraction — the third fix.)
 ...
 **Follow-ups:** "Why does the oldest bound it?" (A view is only as current as its stalest input — the same reasoning as the migration ordering.)

5. **Q: Why is benchmarking the hybrid state more important than benchmarking the target state?**
 **A:** The hybrid is where the firm operates for years, and it carries interactions the clean end-state does not — batch jobs reading intraday stores mid-update, intraday jobs waiting on batch outputs. The end state is simpler and arrives last.
 ...
 **A:** Batch could cache freely because inputs were frozen for the run and the next run started clean. Intraday must invalidate along the dependency graph continuously, and an incorrect graph produces persistently stale outputs rather than errors reset overnight.
 ...
 **Follow-ups:** "Which module established this graph risk?" (a missing edge means a value silently fails to recompute.)

7. **Q: Why do new intraday services often inherit over-broad authorization?**
 **A:** Batch jobs commonly run under broad service accounts nobody has re-examined; replicating that breadth into new services propagates an old weakness into new infrastructure, and the migration is the natural point to correct it.
 ...
 **A:** Batch state could be rebuilt by rerunning the batch; intraday state is continuously updated, and rebuilding from event history takes time the freshness commitment does not permit.
 ...
 **A:** Two separate registers: deliberately-retained batch (with the reason — period-scoped, daily inputs, regulatory point-in-time) and not-yet-migrated batch (with an owner and target date). Conflating them produces a backlog nobody can reason about and lets un-migrated work hide among legitimately-retained jobs.
 ...
 **Follow-ups:** "Which prior module warned about this?" (the "old systems never die" — with the twist that here some correctly survive,.)

10. **Q: Synthesize how this migration affects each of Modules 129–133.**
 **A:** gains genuine intraday risk but requires snapshot pinning to replace batch's implicit consistency. the snapshots become the consistency mechanism the estate depends on rather than one consumer's convenience. is largely unaffected — it was already event-driven, and is in fact the event source that makes the migration possible. gains intraday analytics but inherits the hybrid period's mixed-freshness problem in a multi-tenant setting. becomes feasible for intraday regimes but loses batch's natural completeness boundary, so its completeness reconciliation (there) needs an intraday-appropriate definition.
 ...
 **Follow-ups:** "Which is most affected?" (losing the batch boundary means 'everything for the day' no longer has a natural definition.)
 ...
 **Follow-ups:** "Why is (4) valuable given (1)–(3)?" (Because the scenario inventory cannot be proven complete — shadow running provides detection for scenarios nobody thought to enumerate, which is precisely the failure class represents.)

2. **Q: A team proposes converting the risk engine first because PMs are demanding intraday risk. Evaluate.**
 **A:** It delivers the appearance of progress and none of the value: risk computed continuously from positions that update nightly is exactly as stale as before, and PMs will observe that immediately. It also consumes the migration's early credibility on a phase that cannot demonstrate benefit. The correct response is to sequence upstream-first and manage the expectation — explaining that positions must move first, with a dated commitment for when risk follows. If political pressure makes upstream-first untenable, the honest alternative is a narrow vertical slice (one asset class, positions through risk) that delivers genuine intraday risk for a subset, rather than a horizontal layer that delivers none.
 ...
 **A:** It forfeits the comparison basis exactly when it is most valuable — the post-cutover period, when the intraday path is authoritative for the first time and its untested scenarios begin occurring naturally. the incident surfaced eleven days after cutover; with the batch still computing in shadow it would have been detected by divergence rather than by incorrect downstream numbers. The cost of running it is compute and the discipline to keep it correct; the benefit is detection during precisely the highest-risk window (Advanced Q1's fix 4).
 ...
 **Follow-ups:** "What should a consumer do on breach?" (Consumer-specific: the limits engine refuses (CP), an analytics view displays a staleness warning (AP) — the same per-consumer posture split,.)
 ...
 **Common mistakes:** Reimplementing from code reading, which reproduces what the code does without revealing what it is for — and misses behaviours arising from interaction with the surrounding sequence.
 ...
 **A:** Describe the controls: parallel running with reconciliation against the authoritative batch, scenario-coverage-based exit criteria rather than elapsed time (Advanced Q1), shadow running post-cutover (Advanced Q3), and the own independent completeness reconciliation continuing throughout. Then disclose the incident, its detection, remediation, and the resulting change to exit criteria — because the regulator will assess the firm's control environment more on how it responded to a failure than on whether one occurred, and proactive disclosure of a remediated issue is materially better received than its later discovery (§Expert Q6's posture).
 ...
 **A:** The claim is "the intraday implementation is equivalent to the batch it replaces." Its declared basis is sustained reconciliation agreement. Earlier analysis showed why that is insufficient: agreement over a period demonstrates equivalence *for the cases that occurred*, and in a domain with episodic events the unobserved cases are exactly where implementations differ — because they are the cases the new implementation's author never encountered. What distinguishes this capstone from the prior five is that the gap is not in the system's own logic but in **the evidence used to authorize the change**: the reconciliation was correct, the implementations genuinely agreed, and the conclusion drawn from that agreement was still wrong.
 ...
 **Follow-ups:** "Where else does this reasoning error appear?" (Anywhere a period of clean operation is taken as evidence of correctness — including, in this run, the six-week-equivalent framings and any 'it's been running fine' argument.)
 ...
 **Follow-ups:** "What does percentage-complete specifically distort?" (It rewards converting many small jobs over the upstream-first sequencing that actually delivers freshness,.)

9. **Q: How should the migration handle a job that must change semantics rather than merely cadence?**
 ...
 **Common mistakes:** Governing the technical migration without the evidence-standard and sequencing disciplines, which is where the failure actually originated.
...
### Easy — As-Of Propagation Through a Derived Value (Intermediate Q4)
...
### Medium — Scenario Inventory Gate (Intermediate Q2)
