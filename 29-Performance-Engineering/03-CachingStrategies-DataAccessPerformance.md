...
## 5. Best Practices
- Cache data whose read/write ratio and staleness tolerance genuinely justify the invalidation burden — never as a uniform, blanket default (Intermediate Q8 below).
- Always include every parameter affecting a cached value's content in its key, and include the requesting principal's identity in the key for any principal-scoped data (Intermediate Q2/Q3).
- Protect every hot-key TTL expiry with a stampede mitigation (lock or stale-while-revalidate) before it's needed in production, not after the first incident.
- Route post-write reads to the primary (or a confirmed-caught-up replica) for any flow with a genuine read-your-own-writes requirement.
- Prefer keyset/cursor pagination for any result set expected to grow deep; reserve offset pagination for small, shallow, page-number-navigable UIs only.
- Size connection pools against the database's actual configured connection ceiling, multiplied across every horizontally-scaled instance — not against a single instance's own comfort level.
- Layer a targeted, explicit invalidation path on top of a TTL default for the specific cases where staleness is genuinely unacceptable (the production example), rather than shortening the global TTL for everyone.

## 6. Anti-patterns
- "Cache everything" applied uniformly, ignoring each data type's actual access pattern and staleness sensitivity.
- A cache key missing a parameter (tenant ID, requesting principal, API version) that affects the cached content, causing cross-context collision.
- Relying on a short TTL alone as a substitute for genuine, event-driven invalidation when staleness is genuinely regulatory- or safety-sensitive (the emergency-block case above).
- Offset-based pagination on a result set expected to grow into the tens of thousands of rows.
- Sizing each application instance's connection pool in isolation, without accounting for the aggregate demand across the full horizontally-scaled fleet.
- Assuming a read replica is always caught up, with no read-your-own-writes handling for flows that need it.
- Restarting a cache cluster and routing full production traffic to it immediately, with no warm-up/pre-fill step.

---

## 7. Performance Engineering

**CPU:** Compression of cached values trades CPU (compress on write, decompress on read) for memory/network footprint — worthwhile only when the cache is memory- or network-bound rather than CPU-bound, and only for values large enough that compression overhead doesn't exceed the size benefit; measure both sides before applying it.

**Memory:** Cache eviction policy (LRU/LFU) determines what survives under memory pressure — LRU evicts by recency, LFU by total frequency; a bursty, one-off access pattern to unrelated keys can evict a consistently popular key under LRU that LFU would have protected.

**GC/Allocations:** In .NET, a cache-aside implementation that deserializes a full object graph on every hit (rather than caching a pre-serialized, ready-to-write payload) reintroduces allocation and GC pressure the cache was meant to avoid — cache the response-ready serialized form when the consuming path doesn't need to mutate it.

**Latency:** The dominant latency lever is hit rate, not per-operation cache speed — a 99% hit rate against a 1ms cache and a 50ms origin yields an effective average latency near 1.5ms; a drop to 90% hit rate (still "mostly working") more than triples the effective average, a non-linear sensitivity worth stating explicitly to stakeholders evaluating "is the cache still healthy."

**Throughput:** Connection-pool exhaustion, not raw query cost, is frequently the actual throughput ceiling under concurrency — profile pool-wait time specifically, not just query execution time, before concluding "the database is the bottleneck."

**Benchmarking:** Load-test the cache layer itself at realistic key cardinality, not just request volume — a high-cardinality, low-repeat workload stresses eviction machinery very differently than a low-cardinality, highly-repeated one at identical request-per-second numbers.

---

## 8. Security

**Threats:** Cross-principal data leakage via an under-scoped cache key (a cache entry meant for one tenant/user served to another because the key omitted the principal's identity) — a caching-layer recreation of broken object-level authorization (OWASP API1:2023) rather than a missing authorization check. Cache poisoning — an attacker able to influence what gets written into a shared cache (e.g., via a manipulated request that becomes part of the cache key or value) can cause the cache to serve manipulated data to other users. Timing side-channels — a cache hit and a cache miss have measurably different response times, which can leak information about whether a given key (e.g., a specific username, in a "does this account exist" check) is present, even when the response body itself is identical.

**Mitigations:** Always include the authenticated principal's identity in the cache key for principal-scoped data, and never skip the authorization check on a cache hit — a cache existing does not imply the requester is authorized to see it; re-verify authorization on every request regardless of cache status. Validate and canonicalize any user-controllable input before it becomes part of a cache key, exactly as for any other injection surface. For genuinely sensitive existence checks (account enumeration), apply constant-time response handling or uniform caching behavior regardless of hit/miss to avoid a timing-based oracle.

**OWASP mapping:** API1:2023 Broken Object Level Authorization (cross-principal cache leakage); API8:2023 Security Misconfiguration (an overly permissive cache-key scheme or a shared cache without tenant isolation).

**AuthN/AuthZ:** Authorization must be evaluated fresh on every request; a cache stores *data*, never an authorization *decision*, unless that decision is itself scoped and invalidated exactly as rigorously as the underlying data it depends on.

**Secrets:** Never cache credentials, API keys, or session tokens in a general-purpose data cache without the same encryption-at-rest and access-control rigor applied to a dedicated secrets store — a general cache is typically not audited or access-controlled to that standard.

**Encryption:** Encrypt cache traffic in transit (TLS to Redis/Memcached) for any cache carrying PII or financial data, and consider encryption at rest for a cache node's persistence/snapshot files if the cache is configured to persist to disk.

---

## 9. Scalability

**Horizontal:** A distributed cache cluster (Redis Cluster, a sharded Memcached ring) scales capacity and throughput by partitioning keys across nodes via consistent hashing — adding nodes redistributes a bounded fraction of keys rather than the whole keyspace, but any resharding event still produces a transient burst of cache misses for redistributed keys, worth planning for (a gradual, rate-limited resharding rather than an instantaneous cutover).

**Vertical:** A single larger cache node raises capacity per node but doesn't address the single-node throughput ceiling or the blast radius of that node's failure — horizontal partitioning is generally preferred once a single node's capacity/throughput becomes the binding constraint.

**HA/DR:** Cache high availability (Redis Sentinel/Cluster replica failover) protects against a single node's failure but does not protect against a cache *cluster-wide* outage — the application must degrade gracefully (falling through to the origin, with the stampede-protection and rate-limiting mechanisms above bounding the resulting origin load) rather than hard-failing when the cache is entirely unavailable.

**CAP trade-off:** A distributed cache overwhelmingly favors availability with eventual consistency under partition — a deliberate, appropriate choice given caching's inherently derived, disposable nature, distinct from a system of record's typical strong-consistency requirement.

**Multi-region:** Region-local caches, colocated with each region's application tier for low read latency, backed by a cross-region invalidation-propagation mechanism (a pub/sub broadcast) when the underlying source of truth changes in any region — an explicit, bounded cross-region propagation delay accepted as the trade-off for regional read latency.
