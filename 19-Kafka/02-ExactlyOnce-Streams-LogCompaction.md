...
## 5. Best Practices
- Understand exactly-once's actual scope precisely: it covers Kafka-to-Kafka consume-transform-produce cycles, never external side effects, which always require independent idempotency handling regardless of Kafka's own transactional guarantees.
...
- Choose log compaction explicitly for any topic serving as a current-state-per-key changelog (the corrected fix); never assume a sufficiently long time-based retention window is an adequate substitute.
...
- Choosing time-based retention for a topic that's actually serving as a current-state-per-key changelog, risking silent, delayed-manifesting data loss for infrequently-updated keys.
...
- Treating a Kafka Streams application's internal state-store rebuild as a rare, ignorable edge case rather than a routine operational event (triggered by any restart) that must be tested against, exactly as the incident illustrates.

---

## 7. Performance Engineering

**CPU/Memory**: enabling `enable.idempotence=true` adds per-message sequence-number bookkeeping (Module 54 §2.1) with negligible CPU cost, but transactions (2.2) add real, measurable overhead — every `BeginTransaction`/`CommitTransaction` pair is a round-trip to the transaction coordinator, and committing too frequently (one transaction per single record) can materially cap throughput; the standard mitigation is committing a transaction per **batch** of consumed/produced records rather than per record, amortizing the coordinator round-trip cost across many records (Module 54 §Advanced Q7). Kafka Streams' RocksDB-backed state stores add JVM off-heap memory pressure proportional to state-store size — a large windowed aggregation (a wide time window over high-cardinality keys) can consume gigabytes of off-heap memory per instance, a frequently underestimated capacity-planning input.

**Latency**: `processing.guarantee=exactly_once_v2` adds latency beyond `at_least_once` — the transactional commit adds a coordinator round-trip on Kafka Streams' internal commit interval (`commit.interval.ms`, defaulting higher under EOS than at-least-once specifically to amortize this cost) — a real, measurable trade-off between processing latency and the correctness EOS provides, which is why EOS should be enabled deliberately per-topology based on actual duplicate-sensitivity, not as an unconditional default.

**Throughput**: log compaction itself is a background I/O cost — the compaction thread reads and rewrites log segments to remove superseded records, competing for disk I/O and CPU with the broker's normal produce/fetch request handling; a compacted topic with a very high update rate per key (2.6's "dirty ratio" concern, §Intermediate Q8) can create sustained background I/O pressure distinct from, and additive to, a standard topic's steady-state I/O profile.

**Scalability**: Kafka Streams state-store scaling is bounded by the same partition-count ceiling as any consumer group (Module 54 §2.4) — a stateful aggregation topology's maximum parallelism is fixed by the input topic's partition count, meaning under-provisioning partitions on a stream's **source** topic silently caps the entire topology's eventual scale-out ceiling, discovered only once traffic growth demands more instances than partitions can support.

**Benchmarking**: benchmark a compacted topic's steady-state size and compaction lag under a realistic, sustained update rate per key, not a cold-start/empty topic — §4's own incident shows compacted-topic behavior only manifests certain failure modes (state-store rebuild against genuinely aged, sparse data) under conditions a fresh benchmark environment doesn't naturally produce.

**Caching**: Kafka Streams' RocksDB state stores are themselves a local cache/materialized-view layer backed by a durable, compacted changelog topic (2.5's underlying mechanism) — treating the changelog topic's retention/compaction correctness as directly equivalent to the state store's own correctness (as the incident's team implicitly did) is the specific caching-layer mistake this module's Production Example documents in full.

## 8. Security

**Threats**: a transactional producer's `transactional.id` is a powerful credential — any principal able to produce under a given `transactional.id` can fence out (abort) another producer instance's in-flight transaction using that same ID, a legitimate mechanism for zombie-producer fencing that becomes a denial-of-service vector if `transactional.id` values aren't scoped per-authorized-principal via ACLs.

**Mitigations**: per-`transactional.id`-prefix ACLs (`kafka-acls.sh --transactional-id`) restricting which authenticated principal may use which transactional-ID namespace, exactly mirroring per-topic ACL scoping (Module 54 §8) applied to the transactional-coordinator surface specifically; SASL/mTLS broker authentication (Module 54 §8) remains the baseline prerequisite this all builds on.

**AuthN/AuthZ**: a Kafka Streams or ksqlDB application needs `Read` on its source topics, `Write` on its output/changelog topics, and — for EOS — `Write` on the internal transaction-coordinator-managed `__transaction_state` topic; a common misconfiguration is granting broad `Write` ACLs on all topics matching a wildcard prefix rather than the specific set actually required, violating least-privilege for no operational benefit.

**OWASP mapping**: broken access control (an unscoped `transactional.id` ACL allowing one service to fence another's legitimate transactions); security misconfiguration (ksqlDB's own REST query interface, Module 54-adjacent, left without its own access-control layer despite sitting behind properly-ACL'd Kafka topics); sensitive data exposure (a compacted topic's tombstone metadata or retained latest-value-per-key state persisting sensitive data indefinitely if encryption at rest isn't applied).

**Secrets**: transactional-producer credentials and any external-API keys referenced inside a Kafka Streams topology (e.g., the risk-scoring model client from Module 54's Advanced Q4 fraud-detection example) belong in a managed secrets store, never embedded in topology configuration.

**PII/PCI considerations**: a compacted `customer-profiles`-style topic (§4) that retains the latest value per key **indefinitely** is a materially different data-retention posture from a standard, time-bounded topic — a data-residency or right-to-erasure requirement (e.g., GDPR-style deletion) must be satisfied via the tombstone mechanism (2.5) explicitly and verifiably, not merely assumed satisfied because "old data eventually ages out," which is precisely false for a compacted topic by design.

## 9. Scalability

**Horizontal scaling**: a Kafka Streams application scales out by adding instances up to the source topic's partition count (§7), with state-store partitions automatically redistributed across instances as a consequence of the standard consumer-group rebalance mechanism (Module 54 §2.4/2.5) — no manual state-migration step is required, since each instance rebuilds its assigned state-store partitions from the corresponding changelog-topic partitions during rebalance.

**State-store scaling at real volume**: for state significantly larger than a single instance's practical local-disk/RAM budget, the state must be partitioned finely enough (via the source topic's partition count) that each instance's assigned shard fits comfortably — this is a capacity-planning input on partition count that's independent of, and additional to, the parallelism-driven partition-count sizing discussed in Module 54 §12; a topology with heavy per-key state (e.g., a long-window aggregation) may need materially more partitions than pure throughput parallelism alone would suggest.

**Cross-cluster replication**: MirrorMaker 2 (Module 54 §9) replicating a compacted topic across regions preserves the compaction semantics on the destination cluster (the mirrored topic is itself configured with `cleanup.policy=compact`), but replication lag (Module 54 §9) means a DR region's compacted "current state" view can lag the primary's true current state by the same measurable delay — a DR failover runbook for a Kafka Streams application backed by a compacted changelog must account for this lag explicitly when reasoning about how stale a post-failover state-store rebuild's data might be.

**Load balancing**: ksqlDB queries distribute across the ksqlDB cluster's own nodes independently of the underlying Kafka partition assignment — a persistent query's actual processing parallelism is still bounded by the same partition-count ceiling as any Kafka Streams topology underneath it, since ksqlDB is a SQL layer over Kafka Streams (2.4), not a separately-scaling execution engine.

**High availability**: a Kafka Streams application's standby replicas (`num.standby.replicas > 0`) maintain a warm, continuously-updated copy of a state-store partition on a separate instance, enabling near-instant failover on instance loss instead of a cold rebuild from the changelog topic — a direct, configurable trade-off between steady-state resource cost (double state-store storage/compute) and failover RTO, worth enabling for any topology where a cold-rebuild RTO (proportional to changelog-topic/state-store size) would breach an operational SLA.

**Disaster recovery**: RPO for Kafka Streams state depends on both the source topic's own replication guarantees (Module 54 §9) and, for cross-region DR specifically, MirrorMaker 2's replication lag for both the source and the internal changelog topics — a complete DR posture statement must name both, not just the primary data topic's replication lag alone.
