 ...
 **Common mistake:** "Value types are always on the stack" — false; depends on the container.
...
These are the CLR/GC questions that actually get asked in Principal/Staff loops at trading, payments, and market-data firms — where GC behavior is a *latency-SLA and correctness* concern, not a curiosity.

**FT1. Q: A shared object pool is proposed to eliminate GC on the hot path of a multi-tenant payments platform. As the Principal reviewing the design, what do you probe for before approving it?**
**A:** Pooling trades GC pressure for *state-lifecycle risk*, and in a multi-tenant financial system the top risk is **state leakage across reuses** — a pooled buffer/`StringBuilder`/DTO returned without being fully reset can carry one tenant's PAN, account number, or balance into the next tenant's response. That's not a performance bug, it's a **PCI-DSS/data-isolation incident**. I probe for: (1) a mandatory, *tested* reset step on return (prefer `IResettable` / `ObjectPool<T>` with a policy over hand-rolled reset that drifts); (2) what happens on exception mid-use — is the object still returned to the pool in a half-populated state (`try/finally` return is itself the bug)? (3) pool sizing and the failure mode under burst (does `Rent` allocate a fresh instance, or block?); (4) whether pooled objects hold references that keep large graphs alive in Gen 2; (5) measurement proving the pool is warranted at all (allocation rate on the path, not a hunch).
**Why correct:** It reframes pooling as a correctness/compliance decision, which is exactly the altitude a Principal is expected to operate at — the GC win is secondary to not leaking cross-tenant data.
**Common mistakes:** Treating pooling as a pure performance optimization; forgetting the exception-path return; hand-rolling reset logic that silently omits a new field added six months later.
**Follow-ups:** "How would you *test* for state leakage across pool reuses?" (property-based/fuzz test asserting a rented object is always in a known-clean state) / "When is `ArrayPool<byte>.Shared` dangerous?" (returning without clearing when the buffer held secrets; not clearing means the next renter sees stale bytes up to the rented length).

**FT2. Q: A market-data normalization service consuming 2M messages/sec periodically stalls ~20ms during Gen 2 GCs and falls behind, dropping ticks. Rank your remediation options by risk and impact.**
**A:** Root strategy is to *take the hot path off the GC's radar*, cheapest/safest first: (1) **Eliminate hot-path allocations** — parse directly from the network buffer with `Span<T>`/`ReadOnlySequence<byte>`, represent messages as `struct`s in pre-allocated arrays/ring buffers, avoid LINQ/boxing/`string` on ingest (highest impact, moderate effort, low risk). (2) **`ArrayPool`/pooled buffers** for unavoidable transient arrays. (3) **`GCLatencyMode.SustainedLowLatency`** during market hours to suppress full blocking Gen 2s — buys headroom but only defers the problem if allocation continues. (4) **Server GC + background GC + DATAS**, tuned heap hard limit. (5) **Split the process** — isolate the latency-critical ingest loop from allocation-heavy downstream work (enrichment, persistence) into separate processes/pods so the ingest process's heap stays tiny and its Gen 2 collections are trivial. (6) **NativeAOT** to remove JIT warm-up variance at process start (matters at market open / failover). I would *not* lead with `GC.Collect` scheduling — it's a symptom-masker here. The real fix is almost always #1: if the ingest path allocates near-zero, Gen 2 pressure disappears.
**Why correct:** Ranks by risk and addresses root cause (allocation rate) rather than reaching for GC-mode knobs that only reshape *when* the pause happens.
**Common mistakes:** Jumping straight to `GC.Collect` or `SustainedLowLatency` without cutting allocations; assuming Server GC alone fixes tail-latency (it improves throughput, not necessarily worst-case pause).
**Follow-ups:** "How do you prove ingest is allocation-free?" (`dotnet-counters` Allocation Rate ≈ 0 on the hot path, ETW `GCAllocationTick`) / "Why does splitting the process help even with the same total allocation?" (a smaller heap → cheaper mark phase → shorter Gen 2 pause in the latency-critical process).

**FT3. Q: A regulator asks you to demonstrate that your risk-calculation service produces *reproducible* results for audit. Does the CLR's JIT/tiered-compilation/PGO nondeterminism threaten that? Give the precise answer.**
**A:** JIT tiering, PGO, and OSR change *when and how fast* code runs, never *what value it computes* — Tier 0 and Tier 1 native code are semantically equivalent, so reproducibility of results is not threatened by the compilation pipeline. The real subtlety is **floating-point**: results can differ across CPU architectures, across `x86` (historical x87 80-bit intermediates) vs SSE/AVX codegen, and with FMA (fused-multiply-add) contraction the JIT may or may not emit, causing last-bit differences. For bit-for-bit auditability I'd: (1) pin the compute to a fixed instruction-set baseline / avoid arch-dependent FMA where exactness is required, (2) prefer `decimal` for monetary values (exact base-10, no representational error — non-negotiable for ledger/settlement amounts) and reserve `double` for statistical/risk models where a documented tolerance is acceptable, (3) capture inputs + a deterministic calc version so a result can be *recomputed*, not just stored. The honest regulator-facing answer: *the runtime does not make correct code nondeterministic; floating-point representation and money-type choice are where reproducibility is actually won or lost.*
**Why correct:** Separates a common myth (JIT nondeterminism changes results) from the genuine reproducibility risks (FP semantics, `double` vs `decimal`), which is exactly the distinction a Principal in regulated finance must hold.
**Common mistakes:** Claiming the JIT makes results nondeterministic; using `double`/`float` for money; conflating "deterministic timing" (which the CLR does *not* guarantee) with "deterministic results" (which it does, for a fixed FP environment).
**Follow-ups:** "Why `decimal` and not `double` for a ledger balance?" (base-2 `double` can't represent 0.1 exactly → accumulating rounding error over millions of postings → reconciliation breaks) / "How does NativeAOT affect this?" (removes JIT-tier timing variance but the FP/arch considerations are identical).
