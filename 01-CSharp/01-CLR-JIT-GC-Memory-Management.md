...
**CPU**: JIT Tier 1 recompilation and PGO devirtualization directly reduce CPU cost of virtual/interface calls in hot paths. Profile with `dotnet-trace` + PerfView/`speedscope` flame graphs to find JIT vs GC vs app-code CPU split.

**Memory**: Track working set (`dotnet-counters` → `GC Heap Size`, `% Time in GC`, `Gen 0/1/2 Size`, `LOH Size`). Rule of thumb: allocation *rate* (bytes/sec) matters more than heap *size* for pause-time impact — a high alloc rate means frequent Gen 0 collections even if the heap stays small.

**GC**: Key counters — `# Gen 0/1/2 Collections`, `% Time in GC`, `Allocation Rate`. A healthy service: Gen 0 collections frequent and cheap, Gen 2 collections rare. Red flag: Gen 2 collections happening every few seconds under steady load.

**Allocations**: Minimize via `Span<T>`, pooling, avoiding LINQ in hot loops (creates iterator + delegate allocations), struct-based value types for small short-lived data, `[SkipLocalsInit]` for perf-critical stackalloc buffers (avoid zero-init cost) when safe.

**Latency vs Throughput**: Server GC = throughput-optimized (bigger pauses, less frequent). Workstation + Concurrent = latency-optimized. `SustainedLowLatency` trades some throughput/memory for minimal pause spikes — used in trading systems.

**Benchmarking**: Always use **BenchmarkDotNet** for micro-benchmarks — never hand-roll `Stopwatch` loops (JIT warm-up, GC noise, and CPU frequency scaling will lie to you). Always benchmark Release builds, server-class hardware analog, with `[MemoryDiagnoser]` attached to see allocations/op.

**Caching**: In-process (`IMemoryCache`) reduces GC-visible allocation vs re-fetching/re-parsing, but grows Gen 2 footprint — always bound size. Distributed (Redis) avoids heap growth entirely at the cost of network latency + serialization allocation.

---

## 8. Security

- **Deserialization of untrusted data** (`BinaryFormatter`, unrestricted `System.Text.Json`/`Newtonsoft` polymorphic type resolution) can lead to RCE — `BinaryFormatter` is obsolete/removed in modern .NET specifically because of this (CVE history). Never enable `TypeNameHandling.All` in Newtonsoft.Json for user-controlled input.
- **`unsafe` code and P/Invoke** bypass CLR memory/type safety — any buffer-length mistake reintroduces classic C-style memory corruption (buffer overrun, use-after-free via mismanaged `GCHandle`/pinning). Restrict `unsafe` blocks to audited, narrow, well-tested code paths; never expose raw pointer arithmetic to input-derived lengths without bounds checks.
- **GC-triggered timing side-channels**: in genuinely high-security contexts (crypto key handling), GC compaction can *move* sensitive data in memory without zeroing the old location immediately, and `string` immutability means secrets held as `string` linger in memory until collected (can't be securely wiped). Mitigation: `SecureString` (deprecated guidance now favors `Span<byte>`/`ReadOnlySpan<byte>` with **explicit zeroing** and pinned buffers) for secrets that must be scrubbed deterministically, and avoid `string` for passwords/keys.
- **Reflection/dynamic code loading (`Assembly.Load`, `Reflection.Emit`)** from untrusted sources is a major attack surface — full trust CLR has no sandboxing since .NET Core removed AppDomain-based sandboxing; isolation now must be at the OS/container/process level, not in-process.
- **OWASP relevance**: A03 (Injection) via unsafe deserialization; A08 (Software/Data Integrity failures) via loading unsigned/untrusted assemblies; A06 (Vulnerable components) — keep runtime patched, since JIT/GC bugs (rare but real) are CVE-tracked in the runtime itself.

---

## 9. Scalability

- **Horizontal scaling**: Each pod/instance has its own independent CLR + GC heap — no shared-memory GC coordination needed, which is why .NET services horizontally scale cleanly. But **each replica must be sized individually** for Server GC's per-core heap allocation — under-provisioned CPU limits with Server GC on causes heap-count mismatches (fixed by DATAS in .NET 8+, or explicit `GCHeapCount`).
- **Vertical scaling**: More cores directly benefits Server GC throughput (more parallel GC threads) up to a point; more RAM lets Gen 2/LOH grow before triggering collections — but very large heaps (dozens of GB) increase full-GC pause duration even with background/concurrent GC, since final compaction phases still need brief stop-the-world work.
- **Caching/Replication/Partitioning**: Out of scope for the *runtime* itself, but GC behavior directly informs cache sizing decisions — e.g., putting a 2GB in-process `IMemoryCache` inside a pod competes directly with the GC's working set, increasing Gen 2 pause times; a dedicated Redis cache instead keeps the CLR heap small and predictable.
- **CAP theorem**: Not directly a CLR/GC concept, but relevant when principal engineers must explain why *stateless, horizontally-scaled .NET services with externalized state (Redis/SQL)* are preferred over *sticky, stateful in-process caching* — it's simultaneously a CAP/architecture decision and a GC-heap-management decision.
- **HA/DR**: GC pauses (even background-GC's short blocking phases) can occasionally trip aggressive health-check timeouts — tune Kubernetes liveness probe timeouts to tolerate expected p99.9 GC pause duration, or you'll see pods killed and restarted *because* of GC, compounding load on remaining pods (cascading failure pattern).
