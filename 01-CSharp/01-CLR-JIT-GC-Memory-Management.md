...
1. **Q: What is a GC safe point, and why can't a thread be suspended for GC at literally any instruction?** **A:** The GC can only walk a thread's stack accurately when the thread is at a point where the JIT has recorded which registers/stack slots hold live object references (GC info). Threads are suspended at safe points — method calls, loop back-edges, allocation sites — because between them the JIT may hold references in registers in ways the GC info doesn't describe. Fully-interruptible vs partially-interruptible code bodies differ exactly in how densely this info is recorded.
2. **Q: What is the Pinned Object Heap (POH), and what fragmentation problem does it solve?** **A:** Pinning an object (via `fixed` or `GCHandle.Pinned`) prevents the compacting GC from moving it, and pinned objects scattered through Gen 0/ephemeral segments create holes that fragment the heap and defeat compaction. The POH (.NET 5+) is a dedicated non-compacted heap where long-lived pinned buffers (e.g., socket buffers allocated via `GC.AllocateArray(pinned: true)`) live, keeping the main heap freely compactable.
3. **Q: How did .NET 7's move from GC segments to regions change heap behavior?** **A:** Segments were large contiguous per-generation memory blocks; regions are many smaller (~4MB) uniformly-sized units that any generation can own. Regions let the GC return freed memory to the OS at finer granularity, rebalance memory between generations and heaps more flexibly, and make features like DATAS practical — the observable effect is a working set that tracks actual live data more tightly.
4. **Q: Explain the finalization queue and f-reachable queue, and how an object gets "resurrected."** **A:** Finalizable objects are registered on the finalization queue at allocation; when one becomes unreachable, the GC moves its reference to the f-reachable queue, which makes it reachable again (rooted by the queue) until the dedicated finalizer thread runs its finalizer — hence finalizable objects survive at least one extra collection. Resurrection happens if the finalizer stores `this` into a reachable location, making a "dead" object live again — legal but almost always a design smell.
5. **Q: What are the four main `GCHandle` types and a legitimate use for each?** **A:** `Weak` (tracks an object without keeping it alive; caches), `WeakTrackResurrection` (also observes resurrected state; diagnostics), `Normal` (roots an object without a managed reference, e.g., handed to native code as a context), and `Pinned` (roots and prevents movement, for interop buffers). Leaked `Normal`/`Pinned` handles are a classic "managed memory leak from native interop" root cause.
6. **Q: When is `WeakReference<T>` the right tool, and what's the classic mistake with it?** **A:** It's right for caches/lookup tables where you want to reuse an object if it happens to still be alive but never extend its lifetime — e.g., an event-aggregator that shouldn't cause lapsed-listener leaks. The classic mistake is using it as a "cheap cache" without a real eviction policy: Gen 0/1 collections clear weak references so aggressively that hit rates collapse, so a real cache (with size/TTL policy) usually beats a `WeakReference` cache.
7. **Q: What does `GC.AddMemoryPressure` actually do, and when should you call it?** **A:** It tells the GC that a small managed object holds a large unmanaged allocation (e.g., a 100MB native bitmap behind a 24-byte wrapper), so the GC weighs collection urgency using the true cost, collecting sooner than the tiny managed size alone would justify. Call it (paired with `RemoveMemoryPressure` on release) in wrappers around large unmanaged resources; better yet, prefer `SafeHandle` + deterministic disposal so pressure accounting is a backstop, not the primary lifetime mechanism.
8. **Q: What is an allocation context (TLAB-style allocation), and why are Gen 0 allocations nearly free?** **A:** Each thread gets a private slice of the Gen 0 nursery (its allocation context); allocating is just a pointer bump plus a limit check, with no locks and no free-list search. Only when the context is exhausted does the thread take a slower path to get a new one, and only when the ephemeral budget is exhausted does a GC trigger — which is why allocation *rate* (which burns budget and triggers GCs) matters more than individual allocation cost.
9. **Q: Compare ReadyToRun (R2R) with plain tiered JIT — what does R2R buy and what does it cost?** **A:** R2R ships ahead-of-time-compiled native code alongside IL, so startup skips Tier-0 JIT for precompiled methods — faster cold start, lower first-request latency. Costs: larger binaries, and R2R code is compiled with versioning-resilient (slightly pessimized) code, so tiered compilation still recompiles hot methods at Tier 1 to claw back peak throughput; it also can't use runtime-observed type profiles until re-JIT happens.
10. **Q: What is On-Stack Replacement (OSR) and what gap does it close in tiered compilation?** **A:** Without OSR, a long-running loop inside a method stuck at Tier 0 would never benefit from Tier-1 recompilation, because tier-up historically happened only on method *entry*. OSR lets the runtime transition an actively-executing method mid-loop to optimized code by patching the live stack frame — closing the "hot loop in a method called once" hole (e.g., `Main`'s processing loop in a console ETL job).
11. **Q: Why can a large struct be a performance *pessimization* compared to a class, despite avoiding heap allocation?** **A:** Structs are copied by value on assignment, parameter passing, and return; a 128-byte struct passed down three call levels copies 384 bytes and defeats register allocation, while a class reference is one pointer copy. Guidance: keep structs small (≈16–24 bytes), pass large ones by `in`/`ref readonly`, and remember defensive copies on non-`readonly` struct members can silently multiply copying.
12. **Q: What is string interning, and why is calling `string.Intern` on user data dangerous?** **A:** The intern pool deduplicates identical string instances (compile-time literals are interned automatically) so equal strings share one object. Interning arbitrary runtime/user-supplied strings pins them in the pool for the process lifetime — an unbounded, uncollectable memory growth vector (effectively a leak an attacker can feed) — so interning is only appropriate for a small, closed vocabulary of repeated values, and a `ConcurrentDictionary`-based private pool is usually safer.
13. **Q: How do you distinguish heap fragmentation from genuine live-object growth in a production dump?** **A:** In `dotnet-dump`/WinDbg, compare committed heap size against the sum of live object sizes (`dumpheap -stat`, `gcheapstat`, `eeheap -gc`): a large "free" object total interleaved through segments indicates fragmentation (classically LOH pinning or historical LOH non-compaction), while live-size growth points to a real leak or footprint increase. Fragmentation fixes target pinning patterns/POH/LOH compaction; leak fixes need `gcroot` on the growing types.
14. **Q: What does `GC.TryStartNoGCRegion` do, and what are its sharp edges?** **A:** It pre-commits enough memory to satisfy a specified allocation budget and suppresses GC entirely until you exit the region (or exceed the budget) — used for latency-critical windows like processing a market-data burst. Sharp edges: exceeding the budget triggers a GC anyway (or throws on entry if it can't reserve), the region is process-wide (Server GC: all heaps), and forgetting `EndNoGCRegion` in a `finally` leaves the runtime in a pathological no-collect state.
15. **Q: How does the GC decide Gen 0's budget, and why does a bigger Gen 0 sometimes *improve* latency?** **A:** The budget is tuned dynamically from survival rates and cache-size heuristics: a larger Gen 0 means fewer, slightly longer Gen 0 pauses, and — crucially — more objects die before the next collection (higher infant mortality realized), reducing promotion and downstream Gen 1/2 pressure. That's why raising `GCGen0Size` (or letting Server GC's larger budgets apply) can cut total GC time even though individual pauses grow slightly.
16. **Q: Which GC metrics from `dotnet-counters` would you alert on for a latency-sensitive API, and what does each indicate?** **A:** `% Time in GC` (overall GC tax — sustained >10–15% means allocation-rate or promotion problems), Gen 2 and LOH collection counts/rates (frequent full blocking GCs are the p99 killer), allocation rate (MB/s — the upstream cause), Gen 2/LOH/POH sizes (growth trends → leak vs steady-state), and pause-duration percentiles via EventPipe GC events. Correlating pause timestamps against p99 spikes distinguishes "GC-caused" tail latency from other causes before any tuning.
17. **Q: What's the difference between `GCLatencyMode.SustainedLowLatency` and `LowLatency`, and when is either legitimate?** **A:** `LowLatency` suppresses blocking Gen 2 collections and is meant only for *short* critical windows (its contract assumes you exit quickly; memory can balloon). `SustainedLowLatency` is a durable mode that avoids blocking full GCs where possible (favoring background GC) at the cost of higher steady-state memory — legitimate for trading/telemetry ingestion services; both are useless (and risky) as blanket "make it faster" switches if allocation rate is the real problem.
18. **Q: A container has 2 vCPUs but Server GC creates far fewer heaps than on the 32-core build machine. Explain the mechanism and its operational consequence.** **A:** Server GC creates one heap + dedicated GC thread pair per *effective* CPU, and the runtime reads cgroup CPU limits (`--cpus`) to compute that count, not the host's core count. Consequence: memory footprint and GC parallelism differ per environment — a service tuned by benchmarks on the build machine behaves differently under Kubernetes CPU limits, so GC sizing must be validated against the actual limited container, and CPU limits changes silently change GC topology.
19. **Q: Why does `GC.Collect()` in production code usually make things worse, and what are the rare defensible uses?** **A:** Forced full collections promote survivors prematurely (aging Gen 0/1 objects into Gen 2, growing the expensive generation), wreck the self-tuning budget heuristics, and induce stop-the-world pauses on demand. Defensible: after a known massive one-time drop (level-load boundary in a game, tenant unload), before a `TryStartNoGCRegion`, or in tests validating finalization — always with `GCCollectionMode.Optimized`/documented justification, never as a periodic "memory hygiene" timer.
20. **Q: Walk through your end-to-end triage when a .NET pod's RSS grows unbounded but `GC.GetTotalMemory` stays flat.** **A:** Flat managed heap + growing RSS means the growth is outside the GC heap: suspects are native leaks from interop/libraries (e.g., unclosed native handles, a leaking native dependency), unmanaged buffers (unreturned `NativeMemory`/`Marshal.AllocHGlobal`), thread stacks (thread leak — check thread count), loader/assembly leaks (repeated collectible `AssemblyLoadContext` loads that never unload), or heap *commit* not yet returned to the OS (regions mitigate this). Confirm with `dotnet-counters` (working set vs GC heap size), then a native-aware dump (`dotnet-dump` + `maddress`/`eeheap`, or `umdh`/`heaptrack` equivalents), checking handle and thread counts alongside.
