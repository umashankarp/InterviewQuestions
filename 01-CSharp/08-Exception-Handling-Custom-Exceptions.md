...
1. **Q: What actually happens, runtime-mechanically, when an exception is thrown — why is the *throw* expensive rather than the try?** **A:** Entering a `try` block is essentially free in .NET (the JIT records EH clauses as metadata ranges; no runtime setup per try). Throwing allocates the exception object, captures the stack trace (walking and materializing frames — the dominant cost), then runs the two-pass EH process: pass one searches up the stack evaluating filters to find a handler; pass two unwinds, running `finally` blocks. Cost scales with stack depth and rethrow count — microseconds each, which only matters when exceptions become control flow at high frequency.
2. **Q: How do `finally` blocks interact with exceptions thrown *inside them*, and what's the masking hazard?** **A:** An exception thrown in a `finally` during unwinding *replaces* the in-flight exception — the original vanishes (unlike Java's suppressed-exception attachment), so a cleanup failure (`Dispose` throwing) masks the real cause, and diagnostics show the symptom, not the disease. Discipline: `finally`/`Dispose` bodies must be non-throwing (guard or swallow-with-log inside them); when both matter, capture the primary first and wrap both into an `AggregateException` explicitly.
3. **Q: Explain `Exception.Data`, `HResult`, and `Source` — which are worth using in modern code and which are legacy?** **A:** `Data` (an `IDictionary`) is genuinely useful for attaching diagnostic context (order ID, request path) *when you can't change the exception type* — enrichment middleware and catch-augment-rethrow patterns use it, and structured loggers can emit it; beware non-serializable keys/values. `HResult` matters only at COM/interop boundaries and for some Marshal-based dispatch; `Source` is a free-text assembly name few tools rely on. Prefer typed properties on custom exceptions over all three when you own the type.
4. **Q: What's the right pattern for enriching an exception with context without losing the original — compare wrap-and-rethrow vs `Data` augmentation vs logging-and-`throw;`.** **A:** Wrap (`throw new OrderProcessingException("Order 42 failed", ex)`) when crossing an abstraction boundary where the inner type is an implementation detail — preserves everything via `InnerException` but changes the caught type (breaking upstream catch clauses is a contract change). `ex.Data["orderId"]=42; throw;` preserves type and stack while adding context — best mid-layer. Log-and-`throw;` risks duplicate logging at every layer; the rule is log where *handled* (once), enrich where context exists.
5. **Q: How does exception handling flow through `async` code — where does the state machine put exceptions, and what changes with `await` vs `.Wait()`?** **A:** The state machine catches everything escaping the body and stores it on the returned Task (via `SetException`); `await` rethrows the *first* exception via `ExceptionDispatchInfo` — preserving the original stack and appending the resumption stack, without wrapping. `.Wait()`/`.Result` instead throw `AggregateException` wrapping all stored exceptions. This asymmetry is why blanket advice differs: `catch (HttpRequestException)` works around `await` but silently misses the aggregate-wrapped one around `.Result`.
6. **Q: When `Task.WhenAll` observes multiple failures, `await` throws only the first — what are the patterns to observe all of them, and when does it matter?** **A:** Patterns: keep the `Task` reference and inspect `task.Exception.InnerExceptions` in a catch; or collect per-task results via wrappers that capture their own outcomes (`Task.WhenAll(tasks.Select(Wrap))` returning result objects); .NET 9 adds `Task.WhenEach` for as-completed handling. It matters when each branch's failure needs distinct handling/compensation (parallel writes to independent systems) or when swallowed secondary failures hide systemic issues (2 of 3 downstreams failing looks like 1).
7. **Q: Explain `ExceptionDispatchInfo.Capture(...).Throw()` — what does it preserve that `throw ex;` doesn't, and name two legitimate call sites.** **A:** It captures the exception *with its original stack trace* and rethrows later from a different context, appending (not replacing) stack information — `throw ex;` resets the trace to the rethrow site. Legitimate sites: marshaling an exception across threads (background worker to caller — how `await` itself rethrows), and deferred rethrow patterns (store the failure, run cleanup/retries, rethrow the original if unrecoverable). It's the sanctioned "rethrow from elsewhere" tool; `throw;` only works inside the original catch.
8. **Q: What is the global unhandled-exception surface in a modern ASP.NET Core service — enumerate the distinct channels and what each catches.** **A:** (1) Exception-handling middleware (`UseExceptionHandler`/`IExceptionHandler` in .NET 8+) — request-pipeline exceptions, converting to ProblemDetails responses; (2) `AppDomain.CurrentDomain.UnhandledException` — exceptions escaping non-pool threads, process-fatal, log-of-last-resort; (3) `TaskScheduler.UnobservedTaskException` — faulted, never-awaited tasks at finalization (non-fatal but signals fire-and-forget bugs); (4) `BackgroundService.ExecuteAsync` escapes — which since .NET 6 *stop the host* by default (`HostOptions.BackgroundServiceExceptionBehavior`); (5) Kestrel connection-level exceptions. A production service needs deliberate handling at all five, not just the first.
9. **Q: Why did .NET 6 change `BackgroundService` failure behavior to stop the host, and what does a resilient worker look like?** **A:** Previously an escaping exception silently killed just that service — the app kept running minus a critical worker (silent partial failure, the worst mode). Now the default halts the host so orchestrators restart a visibly-failed pod. Resilient pattern: a top-level loop inside `ExecuteAsync` catching per-iteration exceptions (log, backoff, continue), distinguishing poison-item skips from systemic failures (crash after N consecutive failures — fail fast into the orchestrator's restart/backoff), and honoring the stop token so shutdown remains graceful.
10. **Q: Design the exception-to-HTTP mapping layer for an API: where do domain exceptions become status codes, and what leaks must the design prevent?** **A:** One boundary component (exception middleware/`IExceptionHandler`) owns the mapping: domain exceptions carrying an error classification map to 400/404/409/422 with ProblemDetails (type, title, traceId, safe details); everything unrecognized maps to 500 with *no* internal detail (no messages, stack traces, or SQL — those go to logs correlated by traceId). Prevent: exception messages written for developers leaking to clients (treat `Message` as internal unless the type is explicitly marked client-safe), catch-map logic duplicated per controller (drift), and mapping-by-string-matching on messages (brittle) instead of by type/property.
11. **Q: What is the "vexing exceptions" taxonomy (boneheaded/vexing/exogenous/fatal), and how does it prescribe catch policy?** **A:** Boneheaded (`NullReference`, `IndexOutOfRange`) are bugs — never catch, fix code. Vexing are artifacts of bad API design (`int.Parse` on user input) — avoid by using non-throwing alternatives (`TryParse`). Exogenous (file deleted, network drop) are unavoidable external realities — catch precisely, at the layer that can respond. Fatal (`OutOfMemory`, `StackOverflow`, engine failures) — don't catch meaningfully; let the process die and restart clean. The taxonomy converts "should I catch?" from taste into classification.
12. **Q: Explain exception behavior across `IDisposable` composition: `using` A, B, C where B's `Dispose` throws during an exception unwind. What runs, what's lost?** **A:** Nested `using`s desugar to nested try/finally: C disposes first, then B, then A. If the body threw and B's `Dispose` throws during unwind, C already disposed, **A still disposes** (each finally is independent), but the original body exception is replaced by B's (the masking hazard) — callers now see the cleanup failure. With `await using`, same ordering with async disposal. Design consequence: `Dispose` must be idempotent and non-throwing; resource wrappers that can fail on close (flushing streams) should expose explicit `Flush/Close` for the failable part so `Dispose` is best-effort.
13. **Q: How should retry logic classify exceptions when the failure is wrapped (e.g., `HttpRequestException` inside your `PaymentGatewayException`)? Design the classification contract.** **A:** Classification must not string-match or depend on wrap depth: define an interface/property on your exception hierarchy (`ITransientFailure` marker or `FailureKind` enum property) set at wrap time by the code that *knows* the cause (the gateway client, which saw the timeout vs the 400), and have retry policy check the outermost exception's declared classification — never unwrap-and-guess. For third-party exceptions entering your boundary, classify immediately at the adapter (map `SocketException`/5xx/timeouts → transient; 4xx/validation → permanent) so ambiguity never propagates inward.
14. **Q: Why shouldn't tooling parse `Exception.StackTrace` strings, and what should fingerprinting/grouping use instead?** **A:** Stack trace formatting and frame availability differ across runtime versions, OSes, AOT (which may lack frames or show stubs), tiering (inlined frames vanish; `[StackTraceHidden]` hides more), and async (state-machine frames rewritten for readability). `Exception.StackTrace` is diagnostic text, not a contract: parsing it for dedup/fingerprinting breaks across upgrades. Fingerprint instead on exception type + top user-code frame from the `System.Diagnostics.StackTrace` API, or use the telemetry platform's symbolicated grouping.
15. **Q: When is catching `OutOfMemoryException` ever legitimate, and what's the difference between "an allocation failed" and "the process is doomed"?** **A:** A single enormous failed allocation (a 2GB array request) can throw OOM while the process remains perfectly healthy — catching *that specific, anticipated* case (rejecting an oversized upload with a 413) is legitimate at the specific call site. Systemic OOM (heap exhaustion) means arbitrary subsequent allocations — including those in your catch/logging — will fail unpredictably; no reliable recovery exists, so process-level fail-fast (+ orchestrator restart) is correct. The distinction is locality: caught adjacent to a known-huge allocation = defensible; global OOM handling = theater.
16. **Q: Explain `Environment.FailFast` — what it skips, and two scenarios where it's the *right* call.** **A:** It terminates immediately: no exception propagation, no `finally` blocks, no finalizers — writing the message (and optional exception) to crash telemetry/event log and producing a dump per configuration. Right when: (1) state-corruption detection — an invariant breach (security check bypassed, critical data structure corrupt) where running *any* cleanup code could persist or act on corrupted state; (2) unrecoverable startup/config failure where throwing risks a half-alive host retrying forever. It's the "stop before we make it worse" tool; using it for ordinary errors destroys graceful shutdown.
17. **Q: How do exception filters (`when`) enable "catch only if I can handle it" designs — give an advanced pattern beyond logging.** **A:** Because filters run *before* unwinding and can inspect anything, they enable: retry-budget-aware catching (`catch (TransientException) when (attempt < max && budget.TryConsume())` — unhandlable failures pass by untouched, preserving original crash state for dumps); tenant/feature-flag-scoped handling (`when (flags.GracefulDegradationEnabled)`); and severity routing (`catch (Exception e) when (Classify(e) == Recoverable)`). The superpower is *declining* to catch: a failed filter leaves the first-pass search continuing as if the catch didn't exist, keeping the eventual dump's stack intact at the throw point.
18. **Q: What discipline prevents "exception-driven development" in service integration — where a team's normal control flow becomes try/catch pyramids?** **A:** Push expected outcomes into return types: adapters translate downstream failures into result objects (`Result<T, FailureKind>` or discriminated outcomes) at the boundary, so business code branches on values, reserving exceptions for genuinely exceptional infrastructure failure; enforce via review heuristics ("a catch block inside a domain service is a smell; catches belong to adapters and boundaries") and by measuring thrown-exception rates in telemetry (`dotnet-counters` exception count) with budgets. The distinction to teach: exceptions are for *contract violations and infrastructure*, not for the documented unhappy paths of a remote API.
19. **Q: How do you keep exception hierarchies from decaying over years — enumerate the design-review rules you'd enforce for a shared platform library.** **A:** Rules: every new exception type must answer "what does a catcher *do differently* with this type?" (no per-feature types with identical handling — collapse into one type + `ErrorCode` property); hierarchies stay shallow (base per bounded context, one level of meaningful leaves); constructors always chain `innerException`; types carry typed context properties with non-throwing accessors; message text is developer-facing and culture-invariant; classification (transient/permanent, client-safe/internal) is a first-class property, not tribal knowledge; and obsolete types get `[Obsolete]` + mapping shims for one major version, never silent removal (catch clauses are a binary contract).
20. **Q: Your service shows p99 latency spikes correlating with bursts of first-chance exceptions, but error rates are near zero. Reconstruct the investigation and likely findings.** **A:** Near-zero *unhandled* errors + heavy first-chance activity means caught exceptions as control flow: profile with EventPipe (`dotnet-trace` exception events / `dotnet-counters` exception rate) to find throwing call sites; typical culprits — `int.Parse`/format probing over dirty data (swap to `TryParse`), EF `FirstAsync` + catch as existence checks, cache-miss-as-exception patterns, validation-by-exception in a hot loop, or a polling loop catching timeouts every tick. Each throw costs stack capture (deep async stacks make it worse), and in bursts this flattens p99. Fix by converting the hot expected paths to Try/result patterns and keep an exception-rate SLO to catch regressions.
