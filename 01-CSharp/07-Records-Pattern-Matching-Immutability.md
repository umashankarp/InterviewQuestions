 ...
 **A:** A `record` can only inherit from another `record` (not from a plain `class`, and vice versa — a plain `class` cannot inherit from a `record`) — this restriction exists specifically because the generated equality/`with`/`EqualityContract` machinery (/) assumes a consistent, record-aware inheritance chain; mixing a mutable, non-value-equality-based base class into a record hierarchy would make it impossible for the compiler to correctly generate a sound `Equals` (it wouldn't know whether/how to incorporate the base class's own mutable state into value-based comparison) — the restriction is a deliberate design choice preventing an entire category of "the compiler generated equality logic that doesn't actually make sense" bugs, rather than an arbitrary limitation.
 ...
 **A:** Push back on the conclusion while validating the underlying concern: the shallow-copy gotcha is real and worth respecting, but it's a **solved, well-understood problem** (immutable collection member types, an analyzer rule) — abandoning records entirely for an event-sourcing system trades away substantial, directly-relevant benefits (automatic, correct value equality for event comparison/deduplication, concise `with`-based event-versioning/migration code, `EqualityContract`-correct behavior across an event-type hierarchy) in exchange for avoiding a gotcha that a small amount of disciplined type selection already prevents. The better recommendation: adopt records **with** the immutable-collection-member discipline and the supporting analyzer rule as a package — "records have a gotcha we need to guard against" is a reason to add a guardrail, not a reason to revert to hand-written mutable classes that would need to solve the exact same underlying "keep historical event data genuinely unchanged" problem manually, almost certainly with more code and more opportunities for a *different*, less-well-understood mistake (/5's recurring theme: don't abandon a well-designed language feature because of one gotcha that has a known, narrow fix — apply the fix).

### Expert (FinTech Principal Panel)

1. **Q: Design a tamper-evident, append-only audit ledger where each entry is a record. How do records' immutability and value equality help, where do they *not* give you tamper-evidence, and what must you add?**
 **A:** Records give you the right *shape*: a `sealed record LedgerEntry(long Seq, DateTimeOffset At, string Account, decimal Amount, string PrevHash, string Hash)` is value-immutable (with `init` members + immutable collection fields) so an entry can't be accidentally mutated in memory, and value equality lets you compare/deduplicate entries. But immutability in the CLR is **not** tamper-evidence — a `record` is only immutable to code that respects `init`; someone with database access can still rewrite a persisted row. Tamper-evidence must come from a **hash chain**: each entry stores a cryptographic hash over its own canonical serialization *plus the previous entry's hash*, so altering any historical entry invalidates every subsequent hash. Add: a deterministic canonical serialization (field order and `decimal` formatting fixed — don't rely on record `ToString`, which isn't a stability contract), periodic signed checkpoints (anchor a hash to a WORM store / external timestamp authority), and verification jobs. The Principal point: records prevent *accidental* mutation in the process; cryptographic chaining + append-only storage + signing prevent *malicious* mutation — you need both, and conflating "immutable type" with "immutable record of truth" is a classic audit finding.
 **Why correct:** Uses records for in-memory immutability, correctly states their limit, and supplies the hash-chain/canonicalization/signing that actually deliver tamper-evidence.
 **Common mistakes:** Believing an immutable `record` equals a tamper-proof audit trail; hashing over `ToString` (unstable); mutable collection fields breaking the immutability assumption.
 **Follow-ups:** "Why can't you rely on `record` `ToString` for the canonical form?" / "Where does WORM storage / an external timestamping authority fit?" / "How does this relate to the Event Sourcing module's event store?"

2. **Q: In an event-sourced settlement system, you must evolve `TradeExecuted` (add a `Venue` field, rename another) while replaying years of historical events. How do records and `with`-expressions help, and what's the trap?**
 **A:** Events are immutable facts — records are the natural fit (value equality for dedup, concise construction). Schema evolution is handled by **upcasting**: keep old event versions as their own record types (`TradeExecutedV1`, `TradeExecutedV2`), and an upcaster transforms `V1 → V2` on read, where `with` makes it concise (`v1 with { Venue = "UNKNOWN" }`-style mapping into the new shape with sensible defaults for fields that didn't exist). The trap: **never mutate or reinterpret already-persisted events in place**, and never assume a single evolving record type can deserialize every historical version — that silently corrupts history or throws on old data. Also beware the shallow-copy gotcha inside upcasters that carry mutable collections forward. Keep serialization tolerant (ignore-unknown / explicit version tags) so a V1 blob never accidentally binds to V3's contract. Principal framing: records make each *version* clean and immutable; disciplined versioned types + upcasters make *evolution* safe — the immutability of the past is the whole point of event sourcing, and records enforce it in-memory while your storage/serialization strategy enforces it on disk.
 **Why correct:** Correct upcasting model (versioned record types + `with`), and names the cardinal event-sourcing rule (never rewrite history) plus the serialization-tolerance requirement.
 **Common mistakes:** One mutable event type edited over time; upcasting by mutating old events; assuming `with` deep-copies mutable members.
 **Follow-ups:** "How do you store the event version discriminator?" / "How do snapshots interact with upcasting?"

3. **Q: A low-latency pricing engine shares reference-data objects (instrument definitions, curves) across many threads. Argue for immutable records here on both a correctness and a performance basis, including the snapshot-swap pattern.**
 **A:** Immutable records are ideal for shared reference data because **immutable objects are inherently thread-safe** — no locks, no `volatile`, no defensive copies needed for reads, so the hot pricing threads read curve/instrument data with zero synchronization overhead (locks on shared read paths are a classic latency killer). Updates use **atomic snapshot swap**: build a new immutable snapshot off-path, then publish it with a single reference assignment (`Volatile.Write` / `Interlocked.Exchange` of the root reference); in-flight readers keep using the old snapshot (still fully valid because it's immutable) and the GC reclaims it once no thread references it (the snapshot-swap). Records make this ergonomic: value semantics, `with` for deriving the next snapshot cheaply, and value equality for change detection/dedup. Caveats a Principal raises: the shallow-copy gotcha means every field of the snapshot must itself be immutable (`ImmutableArray<T>`, nested records) or the "immutable snapshot" is a lie; and 2× memory briefly during a swap is the accepted cost versus the alternative (in-place mutation under a lock on the hottest read path, which trades determinism and latency for memory you didn't need to save). This is the same reasoning as the RTB immutable-snapshot design, expressed through records.
 **Why correct:** Grounds the choice in lock-free reads (correctness + latency) and the atomic snapshot-swap update pattern, with the immutability-all-the-way-down caveat.
 **Common mistakes:** Locking shared read paths in a latency-critical engine; a "snapshot" with mutable fields; mutating shared reference data in place.
 **Follow-ups:** "Why is a single reference assignment a safe publish point, and what memory-ordering guarantee do you need?" / "How do you avoid a torn read of the snapshot root?"
