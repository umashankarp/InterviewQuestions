...
var a = new Order(1, new List<string> { "x" });
var b = new Order(1, new List<string> { "x" });
Console.WriteLine(a == b);
```
**A:** `False`. The generated equality compares each field, and `List<string>` uses reference equality — two different lists are never equal regardless of contents. This is the single most common record surprise, and it's why a record with a collection member silently never gets a cache hit when used as a key. Hold immutable collection types with structural equality, or write `Equals`/`GetHashCode` explicitly.
*Follow-up: What if it were a `string[]` instead of a `List<string>`?*

**Q2. What does `with` actually do?**
**A:** Copies every field from the original into a new instance, then applies the changes — a *shallow* copy. That's the trap: if the record holds a reference to a mutable object, the copy shares it, so mutating through one is visible through the other. People reach for `with` expecting clone safety and get aliasing. The discipline is to make records hold only immutable members, which makes shallow copying genuinely safe.
*Follow-up: How would you deep-copy a record that must hold a mutable collection?*

**Q3. Is this object immutable?**
```csharp
record Basket { public init List<Item> Items { get; init; } }
```
**A:** No. `init` stops the *property* being reassigned after construction — it says nothing about the object it points at. That `List<Item>` is fully mutable through its own API, so anyone holding the basket can add and remove items. Shallow versus deep immutability is the distinction, and confusing them is the most common misunderstanding here. Real immutability means every reachable member is itself immutable.
*Follow-up: How would you enforce deep immutability for a type used as a cache key?*

**Q4. `record class` or `record struct` — how do you choose?**
**A:** `record class` gives reference semantics with value equality: heap-allocated, passed by reference, can be null. `record struct` gives value semantics throughout: copied on assignment, no allocation as a local, never null. Struct for small immutable values used in high volume — money, coordinates, identifiers — where allocation matters and copying is cheap. Class for anything larger, anything nullable, anything polymorphic. Size matters because a large struct is copied on every pass, which can cost more than the allocation you avoided.
*Follow-up: Why should a `record struct` almost always be `readonly`?*

**Q5. What breaks when you use a mutable object as a dictionary key?**
**A:** The hash is computed at insertion and used to place the entry in a bucket. If the object then mutates in a way that changes its hash, the entry sits in the wrong bucket — present in the dictionary and unreachable by lookup, and possibly findable by a *different* key that now hashes there. With records this is easy to do accidentally, because the generated hash includes every field, so one mutable field makes the key unstable. Keys must be immutable for as long as they're keys.
*Follow-up: What happens with a record containing a `List<T>` used as a key — and why is that worse than it looks?*

**Q6. Why is a derived record never equal to its base?**
**A:** The generated equality compares `EqualityContract` — effectively the runtime type — before comparing fields. So a `Derived` with identical base field values is not equal to a `Base`. That's the correct, least-surprising behaviour, and it's why records solve the classic asymmetric-equality problem that hand-written `Equals` overrides usually get wrong. Worth knowing the mechanism, because people assume it's a `GetType()` comparison and it isn't quite.
*Follow-up: Would you allow record inheritance in a domain model at all?*

**Q7. What does the compiler check about a `switch` expression's exhaustiveness?**
**A:** It warns when the patterns don't provably cover every input, so adding a new enum value or derived type surfaces as a warning at every switch that no longer covers it — turning "find everywhere that handles this" from a search problem into a build problem. That's genuinely valuable. It's also why a catch-all discard arm is a mixed blessing: it silences the warning, so a new case is silently handled by the fallback. I'd use a discard arm that *throws* for genuinely unexpected input rather than one returning a default.
*Follow-up: How do you get exhaustiveness checking over a closed hierarchy of classes rather than an enum?*

**Q8. When is `record` the wrong choice for a domain type?**
**A:** When the type has *identity* rather than value — an entity whose two instances with identical field values are still different things, like two customers who happen to share a name. Value equality on an entity is actively wrong: it makes distinct entities compare equal, which corrupts sets, dictionaries and ORM change tracking. Records are also a poor fit for types with behaviour-heavy invariants enforced across mutation. Clean split: records for value objects and DTOs, ordinary classes with identity equality for entities.
*Follow-up: How would you enforce that split so it doesn't erode as the codebase grows?*

**Q9. `IReadOnlyList<T>` versus `ImmutableArray<T>` — what's the difference?**
**A:** `IReadOnlyList<T>` is a read-only *view*: it stops the holder mutating through that interface, but if it's backed by a `List<T>` someone else still references, the contents can change underneath — including mid-enumeration. `ImmutableArray<T>` is a *guarantee*: the data can't change, so it's safe to share across threads and cache without defensive copying. The distinction matters most at API boundaries, where returning `IReadOnlyList<T>` over your internal list communicates a promise you aren't actually keeping.
*Follow-up: What's the cost difference between `ImmutableList<T>` and `ImmutableArray<T>`?*

**Q10. Why does immutability simplify concurrency?**
**A:** There's no state transition to synchronise — any number of threads can read the same instance with no lock, no memory-barrier reasoning, and no risk of seeing a half-updated object. Updates become a new instance published with a single atomic reference assignment, which is a far smaller surface than fine-grained locking. The cost is an allocation per update, so it fits read-heavy data — configuration, snapshots, lookup tables — better than high-frequency mutation.
*Follow-up: What do you have to be careful about when publishing by atomic reference swap?*

---

## 3. Intermediate (10 Q&A)

**Q1. A record used as a cache key never gets a hit despite apparently equal values. Diagnose it.**
**A:** Almost certainly a reference-typed member without value equality — most often a `List<T>` or array, whose `Equals` is reference equality, so two records with identical contents compare unequal and hash differently. Could also be a member whose type overrides `Equals` but not `GetHashCode`, or a `float`/`double` where computed values differ in the last bit. Confirm by comparing member by member and checking `GetHashCode` directly. Fix is immutable collection types with structural equality, or explicit `Equals`/`GetHashCode` — and the general lesson is that a record's generated equality is only as good as its members'.
*Follow-up: You need a record with a collection member and structural equality. Options and costs?*

**Q2. What are the serialisation implications of records and `init` members?**
**A:** Most serialisers handle records via the constructor or `init` setters, but the details bite. `System.Text.Json` needs a parameterless constructor or a matching parameterised one; positional records depend on parameter *names* matching JSON property names; `required` members change what a deserialiser must supply. So renaming a positional parameter is a wire-format breaking change that looks like a harmless refactor — that's the specific trap. For anything on a contract boundary I'd pin the mapping with explicit attributes and cover it with round-trip tests, so a rename fails a test rather than a consumer.
*Follow-up: How do you evolve a positional record's contract without breaking existing consumers?*

**Q3. When does pattern matching become a design smell?**
**A:** When a `switch` over derived types appears in several places, each adding behaviour that belongs on the types themselves — that's polymorphism reimplemented as a dispatch table, and every new type means finding and updating every switch. Pattern matching is right when the behaviour belongs to the *consumer* rather than the type (rendering, serialisation, translation between models, anti-corruption layers) or when you're matching over types you don't own. Rule of thumb: one switch is a legitimate visitor-like boundary; the same switch shape in three places means the behaviour should move onto the hierarchy.
*Follow-up: The types come from a third-party library so you can't add methods. Then what?*

**Q4. How would you model a closed set of states so the compiler helps you handle all of them?**
**A:** An abstract base with a private constructor and nested sealed derived records gives a closed hierarchy the compiler can reason about — a `switch` expression warns when a case is unhandled, and each state can carry its own data, which an enum can't. Enums are fine when states carry no data, but any integer can be cast in, so they're not genuinely closed. For domain states with associated data — a payment that's `Pending`, `Settled(DateTime)`, `Failed(reason)` — the sealed hierarchy makes illegal combinations unrepresentable rather than merely discouraged.
*Follow-up: What do you lose versus an enum, especially at persistence boundaries?*

**Q5. What's the performance profile of the immutable collections?**
**A:** `ImmutableArray<T>` is a thin wrapper over an array — reads as fast as an array, but every mutation copies the whole thing, so it's ideal for read-mostly data and terrible for incremental building. `ImmutableList<T>` uses a balanced tree, so mutation is O(log n) with structural sharing, but reads and enumeration are considerably slower and allocate more. Getting it wrong shows up as either quadratic behaviour building an array item by item, or surprising read cost in a hot loop. Build with a mutable builder and freeze at the end.
*Follow-up: How do you build a large `ImmutableArray<T>` without quadratic copying?*

**Q6. How would you introduce immutability into an existing mutable domain model?**
**A:** From the leaves inward: convert small value objects first — money, date ranges, identifiers, addresses — since they're self-contained and low-risk, and their immutability then constrains the aggregates above them. Aggregates and entities usually stay mutable, because their whole purpose is identity with changing state, so I wouldn't force records on them. The genuinely hard part is code relying on shared-instance mutation for a side effect — that aliasing has to be found and made explicit before immutability can be imposed, and it's usually where the real bugs already are.
*Follow-up: You find code relying on shared mutation for cache invalidation. How do you unwind it?*

**Q7. What does `required` add, and how does it interact with serialisation?**
**A:** It forces the caller to set a member during initialisation, giving constructor-like guarantees without writing a constructor — useful for types with many members that look optional and aren't. It closes the gap where `init` allowed construction in an invalid state. The interaction to watch: serialisers and reflection-based factories must be able to satisfy it, so a type with `required` members may need attributes to stay deserialisable — and adding `required` to an existing public type is a breaking change for every caller. Good default for new DTOs, careful change for existing ones.
*Follow-up: `required` or a parameterised constructor — when would you still prefer the constructor?*

**Q8. Where does the snapshot-and-swap pattern break down?**
**A:** It works beautifully when configuration or lookup data lives in an immutable object behind a single field: readers take the reference and use it lock-free, a writer builds a new instance and swaps atomically. It breaks when updates are frequent enough that rebuilding is expensive; when readers need a *consistent* view across two separately-swapped objects; and when a reader holds an old snapshot long enough that acting on stale data is a correctness problem rather than a freshness one. All three are design questions to answer explicitly rather than assume away.
*Follow-up: Two related snapshots must be updated together. How do you make that atomic for readers?*

**Q9. How would you review a PR converting a set of DTOs to records?**
**A:** Check what equality now means — records introduce value equality, so anything relying on reference equality (identity comparisons, dictionary usage, change tracking) may silently change behaviour. Check members for mutable reference types, which break both equality and the safety of `with`. Check contract stability, since positional parameter names become serialised names and a rename is a wire break. And ask whether these are genuinely values rather than entities, because "convert everything to records" is a common and mistaken sweep. If all that's clean, it's usually a good change that removes a lot of boilerplate.
*Follow-up: The PR also changes a type used as a dictionary key. What specifically do you check?*

**Q10. When would you use a property pattern over an ordinary `if`?**
**A:** When it reads as an assertion about shape and collapses several conditions — `order is { Status: Active, Customer.Tier: Premium, Total: > 1000 }` is clearer than the equivalent chain, and it null-checks the path for free. It becomes *less* readable when the pattern grows deep or mixes many unrelated conditions, at which point a named predicate method communicates intent better. The value isn't brevity, it's that the intent becomes declarative and the compiler can reason about completeness in a switch.
*Follow-up: Where does a property pattern silently differ from the `if` you'd have written by hand?*

---

## 4. Expert / Architect (10 Q&A)

**Q1. How do you decide, at architecture level, where value semantics belong?**
**A:** Follow the domain rather than the language feature. Values are defined entirely by their attributes and interchangeable when equal — money, quantities, date ranges, identifiers. Entities have a lifecycle and a distinct identity that persists through change. Getting it wrong does concrete damage: value semantics on an entity breaks tracking and set operations, and identity semantics on a value forces field-by-field comparison everywhere. I'd settle it at the modelling stage, encode it in the type system, and make it visible in the codebase structure so the distinction outlives the people who made it.
*Follow-up: Money is the classic value object, but conversion and rounding are behaviour. Where does that live?*

**Q2. What's your position on immutability as a default across a service codebase?**
**A:** Immutable by default at the boundaries and for values; mutable where mutation is the point. Immutable DTOs, events, configuration and value objects eliminate whole classes of aliasing and concurrency bug at essentially no cost, so that's the standard. Forcing it on entities and aggregates produces awkward rebuild-the-world code that fights both the ORM and the domain, so I wouldn't mandate it. The judgement to communicate is that immutability is a tool for eliminating shared-mutable-state bugs — applying it where there's no sharing and no concurrency buys nothing and costs readability and allocation.
*Follow-up: A team wants to go fully immutable including aggregates, functional-core style. Would you approve?*

**Q3. How do records affect API and event contract evolution?**
**A:** They make some changes look safer than they are. Adding a member to a positional record changes the constructor signature and the deconstruction arity — source- and binary-breaking. Renaming a positional parameter breaks the serialised contract. Adding `required` breaks every caller. Meanwhile value equality means two events previously distinct by reference may now compare equal, which can change deduplication behaviour subtly. For published contracts I'd fix the shape explicitly with attributes, keep positional records for internal use and named-property records with `init` for external ones, and cover the wire format with round-trip contract tests.
*Follow-up: How would you version an event contract expressed as a record?*

**Q4. What are the memory implications of a heavily record-based design at scale?**
**A:** Every `with` is an allocation, so a design transforming objects through many stages produces one object per stage per item — fine at request scale, potentially significant in a high-throughput pipeline. `record struct` avoids the allocation but introduces copying, which dominates once the struct exceeds a couple of machine words and is passed frequently. Immutable collections add their own overhead, particularly `ImmutableList<T>` in read-heavy loops. My guidance: record-based transformation is the right default and its cost is invisible in a normal service, but a high-throughput data path should be measured specifically, because that's exactly where "just use `with`" produces gen0 churn worth eliminating.
*Follow-up: A pipeline does six `with` transformations per item at 50k items/sec. How do you approach it?*

**Q5. Pattern matching versus polymorphism — what guidance would you set for a large team?**
**A:** Polymorphism where the behaviour belongs to the type and the set of types is open, because adding a type shouldn't require editing existing code. Pattern matching where the behaviour belongs to the consumer or the type set is genuinely closed — translation layers, serialisation, rendering, mapping between bounded contexts, and types you don't own. The guidance names the smell rather than the rule: the same switch shape in more than two places means the behaviour wants to move onto the hierarchy. I'd also require switches over closed hierarchies to avoid a silent default arm, so adding a case produces a compiler warning — that single convention is what makes the approach safe over years.
*Follow-up: A switch has grown to thirty arms across three files. What's your refactoring approach?*

**Q6. How does immutability change your caching and concurrency approach at scale?**
**A:** It removes the hardest question in caching — what happens when a cached object is mutated by one consumer and observed by another. With immutable entries, sharing is free, no defensive copying on read, and entries can be handed to any number of threads safely. That makes lock-free read paths practical and lets you reason about staleness purely in terms of *when* a snapshot was taken rather than what state it might be in. The cost moves to invalidation and allocation on update, which are much better-understood problems. "Cache only immutable objects" is a rule I'd hold firmly, because a shared mutable cached object is a bug class that reproduces only under load.
*Follow-up: A cached object is expensive to rebuild and one field changes frequently. How do you handle that?*

**Q7. How would you migrate a widely-used mutable DTO to an immutable record across many services?**
**A:** Not in one step. Introduce the immutable type alongside, add conversions both ways, migrate consumers incrementally with telemetry showing who's still on the old type, then deprecate and remove. The dangerous discovery is consumers who *relied* on mutating the DTO after receiving it — usually to patch a field or carry extra state — and each of those is a small design conversation rather than a mechanical change. I'd find them by making the old setters obsolete so the compiler produces the list, and set a deprecation date with an owner, because these migrations otherwise stall permanently at the last few consumers.
*Follow-up: One consumer mutates the DTO to carry a correlation ID through their pipeline. What do you tell them?*

**Q8. Where does "make illegal states unrepresentable" stop being worth it?**
**A:** When the encoding becomes more complex than the rule it enforces, when the type gymnastics obscure the domain from the people who own it, or when the state legitimately comes from outside the system and must be validated at runtime anyway. Records, `required`, closed hierarchies and exhaustive switches let you encode a lot of domain rules so invalid combinations fail at compile time — genuinely valuable for stable, central invariants like a payment that can't be both refunded and pending. My rule is to encode invariants that are stable and central and validate the rest at the boundary, because over-encoding produces a model only its author can change.
*Follow-up: Where exactly do you put validation for state arriving from an external system?*

**Q9. How do these features interact with an ORM?**
**A:** Uncomfortably in places, because ORMs are built around identity, change tracking and mutation while records are built around value equality and replacement. Value objects map well — owned types or converters handle immutable records cleanly. Making an *entity* a record fights the tracker, since two distinct rows with identical values now compare equal, and updating means replacing rather than mutating. `init`-only members are usually workable via constructors or backing fields but add friction. My guidance: records for value objects and read models, ordinary classes for tracked entities, and an explicit mapping layer if the domain and persistence models want different shapes — which in a system large enough to ask, they usually do.
*Follow-up: The team wants one set of types for domain, persistence and API. When is that acceptable?*

**Q10. How would you introduce these features to a team on an older mutable codebase without creating inconsistency?**
**A:** Start where the payoff is unambiguous and the blast radius small: new DTOs and value objects as records, new state machines as closed hierarchies with exhaustive switches. Write down the split — values versus entities — so the choice is a rule rather than individual preference, because the worst outcome is a codebase where identical concepts are modelled three ways depending on who wrote them. Explicitly *don't* mandate retrofitting existing types, since a mechanical sweep changes equality semantics across the codebase and the regressions are silent. And seed it with one well-reviewed exemplar per pattern, because teams copy the nearest example far more reliably than they read guidance.
*Follow-up: Six months in, half the codebase uses each style. Converge or accept the split?*
