...
**CPU**: `record`-generated `Equals`/`GetHashCode` perform direct field-by-field comparison/hashing (via `HashCode.Combine`) — fast, comparable to a well-hand-written equality implementation, and for `record struct` specifically, meaningfully faster than the reflection-based default `ValueType.Equals` that an ordinary (non-record) `struct` falls back to if you don't hand-write your own.

**Memory**: `record class` instances are ordinary heap-allocated reference types (Module 1) — `with` expressions allocate a new instance per call (the shallow-copy `<Clone>$()` plus any changed `init` assignments), a real, deliberate allocation cost paying for the immutability/non-destructive-mutation guarantee; `record struct` avoids this for small, frequently-copied value types, trading struct-copy cost (Module 3's large-struct-copy caution applies here too) for zero heap allocation.

**Pattern matching**: Type patterns (`is Customer c`) compile to an efficient `isinst`/type-check IL instruction, no reflection involved — comparable in cost to a hand-written `as` + null-check. Property/positional patterns compile to a sequence of ordinary property-access/`Deconstruct`-call + comparison instructions — the JIT can often optimize/reorder these checks similarly to an equivalent hand-written `if` chain; there's no meaningful inherent performance penalty for using pattern matching over equivalent hand-written conditional logic.

**Immutable collections**: `ImmutableList<T>`'s O(log n) operations (a persistent, tree-based structure enabling structural sharing between "versions") trade higher per-operation cost for the correctness guarantee that unlocks safe record/`with` usage (§4) — profile before assuming this matters; for records mutated/copied infrequently (most domain/business-object usage), the cost is negligible relative to the correctness benefit; for genuinely hot, high-frequency collection-mutation code, a `List<T>` wrapped in explicit defensive-copy discipline (or simply not modeled as an immutable record in the first place) may be the better-profiled choice.

**Benchmarking**: BenchmarkDotNet comparing `record struct`'s generated `Equals` against a plain (non-record) `struct`'s default `ValueType.Equals` on the same field layout is a concrete, illustrative way to demonstrate the reflection-based-default-equality cost this module's §2.1 flags — a genuinely worthwhile exercise for a team still using plain structs with hand-rolled or (worse) entirely default `Equals`.

---

## 8. Security

- **`ToString()` auto-generated by records includes every property's value by default** — for a record containing sensitive data (passwords, tokens, PII), the generated `ToString()` (invoked implicitly by logging frameworks, string interpolation, exception messages) can leak that data into logs/traces far more easily than a hand-written class where `ToString()` wasn't overridden at all (defaulting instead to the unhelpful-but-harmless `Namespace.TypeName`). Mitigation: override `ToString()` explicitly to redact sensitive properties for any record containing secrets/PII, or mark such properties to be excluded (there's no built-in record attribute for this as of C# 13 — this requires a manual override or a custom source generator).
- **`with` expressions and shallow-copy semantics (§2.3/§4) as a data-integrity risk**, not merely a correctness one: an immutable-looking audit-log or event-sourcing record (Module 35, a later module) that's actually only shallowly immutable due to a mutable member could have its "immutable historical record" silently altered after the fact via a shared mutable reference — a genuine data-integrity/non-repudiation concern in domains (finance, compliance) where the *whole point* of an immutable record type is a guarantee that historical data cannot change.
- **Deserialization into records from untrusted input** carries the same general concerns as any deserialization (Module 1 §8) — records don't introduce new risk here beyond ordinary type/library-specific deserialization safety practices, but the `init`-only property model means some serializers require specific support/configuration to populate `init` setters correctly, worth verifying for any given serialization library rather than assuming.
- **OWASP relevance**: A09 (Security Logging and Monitoring Failures)-adjacent, specifically inverted — records can cause *excessive*, unintentional sensitive-data logging via their convenient default `ToString()`, a distinct but related concern to under-logging; general data-integrity concerns (not a named OWASP category directly) for compliance-relevant "immutable" record types that are only shallowly so.

---

## 9. Scalability

- **Horizontal/vertical scaling**: Not directly a scaling-mechanism concern; the relevant connection is that immutable, value-equality-comparing records are naturally **safe to share across threads without synchronization** (Module 2's concurrency concerns) — a genuine, real benefit for horizontally-scaled, highly-concurrent service code that frequently passes DTOs/domain events between threads/async continuations, since there's no risk of one thread's mutation affecting another's view of "the same" data (as long as the shallow-copy gotcha, §4, has been correctly closed via immutable member types).
- **Caching/Replication/Partitioning**: Value-equality-based records are naturally well-suited as cache keys (correct `GetHashCode`/`Equals` for use in `Dictionary<TKey,TValue>`/`ConcurrentDictionary<TKey,TValue>`) without needing hand-written equality overrides — directly useful for any caching-layer key type built from records instead of hand-rolled tuple-like classes.
- **CAP theorem**: Not directly applicable to records/pattern matching mechanics; the practical connection is that immutable, well-defined record-based event/message payloads are the natural data-carrying unit for the distributed-systems patterns covered in later modules (Event Sourcing, CQRS, Outbox) — a domain event should almost always be modeled as an immutable record precisely because its historical, already-occurred nature demands the "this cannot change after the fact" guarantee records (correctly implemented, per §4/§8) provide.
- **HA/DR**: Immutable event/message records replayed during disaster recovery (event sourcing rebuild, message-queue redelivery) depend on genuinely-immutable historical data to reconstruct state correctly — the shallow-copy gotcha (§4/§8), if present in an event-sourced system's event records, is a direct threat to DR correctness (a "historical" event that can actually still be mutated undermines the entire replay-to-rebuild-state guarantee such systems rely on).
