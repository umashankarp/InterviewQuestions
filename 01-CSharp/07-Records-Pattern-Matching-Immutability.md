...
1. **Q: Walk through what the compiler actually synthesizes for `record Person(string Name, int Age)` — enumerate the members.** **A:** A constructor taking the positional parameters; public init-only auto-properties per parameter; a `Deconstruct(out string, out int)`; value-based `Equals(Person)`, `Equals(object)`, `GetHashCode`, and `==`/`!=` operators; `EqualityContract` (virtual, returns the runtime type); a protected copy constructor (used by `with`); a synthesized `PrintMembers` + `ToString`; and `IEquatable<Person>` implementation. Any of these you write manually suppresses the synthesized version — the customization escape hatch.
2. **Q: How does `with` work mechanically, and why does it require a record (or record struct) rather than any class?** **A:** `with` compiles to a call to the record's synthesized (or user-written) copy constructor producing a clone, then applies the specified property setters (which is why targets must be `init` or settable). Ordinary classes lack the compiler-known clone member (`<Clone>$`), so the compiler can't guarantee a correct copy — C# ties `with` to types that opt into the copy contract (records, record structs, and anonymous types).
3. **Q: Why does record equality include `EqualityContract` runtime-type checking rather than just comparing properties, and what surprising inequality does this produce?** **A:** Without it, a `Person` and a derived `Employee` with identical `Person`-level fields would compare equal asymmetrically (violating equality's contract) — so records compare `EqualityContract` (the actual runtime type) first. Surprise: two instances with byte-identical property values but different concrete record types (base vs derived, or two siblings) are never equal — including a base-typed variable holding a derived instance compared to a genuine base instance.
4. **Q: When should you override a record's synthesized `GetHashCode`/`Equals`, and what's the consistency trap when you customize only one?** **A:** Override when semantic identity differs from structural: case-insensitive key fields, excluding cache/timestamp fields, or reference-heavy members whose default (reference) equality is wrong or whose hashing is expensive. Trap: customizing `Equals` without `GetHashCode` (or vice versa) breaks the "equal objects hash equal" invariant — records synthesized both from the same fields, and your override must maintain the pairing manually or dictionary/set behavior corrupts silently.
5. **Q: What's the collection-member problem with record equality — why do two records with equal `List<int>` contents compare unequal, and what are the design options?** **A:** Synthesized equality calls `EqualityComparer<T>.Default` per member; for `List<T>` that's reference equality, so structurally-equal lists don't match. Options: use `ImmutableArray<T>`(still reference-ish semantics — careful) or custom-compare by overriding `Equals`/`GetHashCode` with sequence comparison; wrap collections in a value-semantic type (e.g., a `ValueList<T>` implementing structural equality); or accept reference semantics and document that records-with-collections aren't value-comparable — the key is making a deliberate choice, since the default silently surprises.
6. **Q: Explain `readonly record struct` vs `record struct` vs `record class` — what changes across the three?** **A:** `record class` (default): heap reference type, value equality, `with` via copy constructor. `record struct`: value type, *mutable by default* (positional properties are `get/set`, a departure from record class's init-only), synthesized value equality that beats default struct reflection-based equality. `readonly record struct`: all instance fields/properties immutable (init-only positional), compiler enforces no mutating members — the go-to for small immutable value objects (money, coordinates, IDs) with zero allocation and fast equality.
7. **Q: Why is the default struct `Equals` slow, and how much does `record struct`'s synthesized equality actually change?** **A:** `ValueType.Equals` may fall back to reflection over fields (unless all fields are bittable and it can memcmp), boxing along the way, and default `GetHashCode` is notoriously weak (may use only the first field). `record struct` synthesizes field-by-field `EqualityComparer<T>.Default` comparisons and a proper combined hash — typically an order of magnitude faster and allocation-free, which is why plain structs used as dictionary keys should either implement `IEquatable<T>` manually or just be record structs.
8. **Q: How do records serialize with System.Text.Json — what works out of the box and where are the sharp edges?** **A:** STJ supports records natively: it uses the parameterized constructor for deserialization (matching JSON names to constructor parameters case-insensitively) and properties for serialization. Sharp edges: constructor-parameter renames break deserialization contracts (parameter names *are* the contract); `[JsonPropertyName]` must go on the positional parameter (flows to the property); polymorphic record hierarchies need `[JsonDerivedType]` configuration; and `init` setters work for object-initializer deserialization but required/nullability interplay (`required`, default parameter values) needs testing against real payloads.
9. **Q: What does the `required` modifier (C# 11) add beyond positional record parameters, and when combine them?** **A:** `required` forces object-initializer assignment at compile time without forcing constructor position — enabling many-property records with named initialization (`new Config { Endpoint = ..., Timeout = ... }`) that still can't be constructed incompletely. Combine: positional parameters for the 2–4 identity-defining members, `required init` properties for additional mandatory configuration — avoiding 10-parameter positional constructors while keeping compile-time completeness checking (with `[SetsRequiredMembers]` on constructors that fulfill them).
10. **Q: Explain list patterns (C# 11) with a realistic parsing use, including slice capture.** **A:** `if (args is ["--config", var path, .. var rest])` matches a sequence's shape positionally: literal first element, captured second, and `.. var rest` capturing the remainder (as an array/slice for countable/sliceable types). Realistic uses: CLI argument dispatch, protocol token streams, and CSV-ish row shape validation — replacing index bounds checks + element comparisons with one declarative, exhaustiveness-checkable expression. Requires the type expose `Length`/`Count` and an indexer (and `Range` indexer for slice capture).
11. **Q: How does the compiler decide a switch expression is exhaustive, and why do sealed record hierarchies still require a discard arm?** **A:** Exhaustiveness reasons over the *static type's* value space: for reference-type hierarchies the compiler doesn't treat "sealed base + known derived set" as closed (C# has no compiler-recognized closed hierarchies yet — unlike F#/Kotlin/Rust unions), so matching all derived types still warns without a `_`/base arm (null alone also needs handling). Idiom: add a final arm throwing `UnreachableException` (or `SwitchExpressionException`) — making the "can't happen" explicit, safe against future derived types, and silencing CS8509 honestly rather than with a silent default.
12. **Q: What's the pattern-matching lowering story — does a big property-pattern switch compile to efficient code or cascaded type checks?** **A:** The compiler builds a decision DAG: shared tests (type checks, null checks, property loads) are evaluated once and branches share sub-tests, constant string/numeric switches lower to hash-based jump tables or binary search where beneficial — generally as good as or better than hand-written if-cascades, since the compiler dedupes tests you'd repeat. Caveats: `when` guards constrain reordering (evaluated in order), and patterns calling property getters mean user code runs during matching — property getters with side effects break the "tests are pure" assumption the DAG optimizations rest on.
13. **Q: Why should pattern-matched property getters be pure, and what bug appears when they're not?** **A:** The compiler may evaluate a getter once and reuse the result across arms, or reorder/share tests in the decision DAG — a getter with side effects (logging, lazy mutation, counters) executes an implementation-defined number of times, and a getter returning different values across calls (e.g., `DateTime.Now`-derived) can make matching internally inconsistent (matched arm's guard saw a different value than the pattern test). Treat patterns as declarative predicates: any observable behavior inside them is undefined-ish territory.
14. **Q: Model a state machine (order lifecycle) with records + switch expressions such that illegal transitions are unrepresentable. Sketch the approach.** **A:** Represent each state as its own record carrying only that state's valid data (`Draft(items)`, `Submitted(items, at)`, `Paid(items, at, paymentId)`, `Shipped(..., tracking)`); transitions are static functions `Submitted Submit(Draft d)` — the type signature makes `Ship(Draft)` uncompilable, and data needed only after payment (paymentId) doesn't exist on earlier states (no nullable placeholder fields). A switch expression over the state union handles behavior per state with compiler-assisted coverage; persistence maps states to a discriminator column/document type. The payoff: invalid-state bugs move from runtime validation to compile errors.
15. **Q: How do records serve DDD value objects, and where do they fall short requiring manual work?** **A:** Records give value equality, immutability, and concise declaration — the value-object trifecta — free. Short-falls: no built-in *validation* (a `record Email(string Value)` accepts garbage; you need a private constructor + static factory `Create` returning a result, or a validating primary constructor that throws), no normalization by default (case folding, trimming — must be done before/instead of raw positional assignment), collection members break equality (Q5), and ORMs need configuration (EF value-object mapping via owned types/conversions, with records' constructor binding support varying by version). Records are the 80% syntax; the domain rigor is still yours.
16. **Q: Explain the interaction of records with EF Core entities — why are records usually wrong for entities but right for owned values?** **A:** Entities have *identity* semantics (two loads of Order 42 are "the same" regardless of field values) — record value equality actively contradicts this, breaking identity-map assumptions, change tracking comparisons, and making proxies/lazy-loading awkward; entities also mutate over their lifecycle, fighting init-only members. Owned value objects (Address, Money) are identity-less and immutable — exactly record semantics; map them as owned types or via converters, replacing whole values on change (which EF tracks as a member replacement). Rule: records for values, classes for entities.
17. **Q: What is nondestructive mutation's cost profile — when does heavy `with` usage become a GC problem, and what are the mitigations?** **A:** Each `with` clones the full record (all fields copied, new allocation) — fine for small records at request rates, but N-mutations-per-item pipelines (folding 10 updates over a 30-field record per event × 100k events/sec) allocate garbage proportional to size × mutation count. Mitigations: batch changes into one `with` (single clone for multiple property updates), use a mutable builder for hot multi-step construction then freeze into the record once, keep records small/nested so `with` on a leaf doesn't copy the world (though nesting means multiple `with` levels), or use `record struct` where lifetime permits stack allocation.
18. **Q: How do you evolve a widely-consumed record contract without breaking binary/source compatibility — what changes are safe?** **A:** Safe-ish: adding `init` (non-required) properties with defaults (source-safe; binary-safe for object-initializer users, but *changes synthesized equality/ToString/deconstruct* — semantic break for anyone comparing or deconstructing); adding non-positional members. Breaking: adding/reordering/renaming positional parameters (constructor and `Deconstruct` signature breaks; JSON constructor-name contracts break), changing property types, and adding properties when consumers persist/compare records (equality now differs across versions). For public API records, treat the positional list as frozen after v1, prefer additive `required`/`init` properties with explicit versioning notes, and document that equality semantics evolve with membership.
19. **Q: Compare modeling "one of several outcomes" with a sealed record hierarchy vs. OneOf/Either library types vs. exceptions — decision criteria?** **A:** Sealed record hierarchy: best when outcomes carry rich distinct data, are domain-meaningful, and handled at multiple sites — switch exhaustiveness (with the discard-arm caveat) plus named types aid discoverability; costs a few type declarations. `OneOf<T1,T2>`/Either: terse for ad-hoc two-way results, but generic-position types read poorly (`OneOf<Order, ValidationErrors, NotFound>` at every signature), and pattern support is library-mediated. Exceptions: correct for *exceptional* violations (bugs, infrastructure failure), wrong for expected branches (declined payments) — they erase outcomes from signatures and cost stack captures. Rubric: expected+data-carrying → records; exceptional → exceptions; quick internal plumbing → OneOf sparingly.
20. **Q: As a Principal Engineer, you're setting org guidance on records adoption. What are your rules, and what misuse patterns do you explicitly warn against?** **A:** Rules: default to records for DTOs/messages/events/value objects/config snapshots (immutability + equality are contract-appropriate); `readonly record struct` for small high-volume values; classes for entities, services, and anything with identity or mutable lifecycle; sealed hierarchies + exhaustive switches for closed outcome sets. Warnings: records-with-mutable-collection members (broken equality + shallow `with`), records as EF entities, "record because it's shorter" on stateful services (equality/`with`/`ToString` semantics all wrong), `ToString` PII leakage into logs (synthesized ToString prints every member — require overrides on records holding sensitive data), and equality-membership drift when someone adds a property without realizing they changed dictionary-key semantics. Adoption succeeds when teams understand records are a *semantic contract* (value-ness), not syntax sugar.
