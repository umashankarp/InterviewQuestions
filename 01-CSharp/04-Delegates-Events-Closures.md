 ...
 **A:** Push back on: (a) a shared **static** event bus is effectively a global mutable singleton with all the lapsed-listener leak risk multiplied across every module that ever subscribes, with no natural lifecycle boundary forcing anyone to think about unsubscription; (b) the multicast-return-value hazard (§Advanced Q3) and exception-abort-on-first-throw behavior make raw events a poor fit for cross-module communication where isolation and predictable semantics matter more than in a single well-owned class; (c) it creates implicit, hard-to-trace coupling between modules (any module can silently subscribe to any other's events, with no compile-time-visible dependency graph, unlike explicit interface-based dependencies or DI-resolved handler collections) — a maintainability and architecture-governance concern as the system grows, since "who listens to this event" becomes effectively undiscoverable without a full-codebase search. Recommend: a proper in-process mediator/domain-event dispatch mechanism (Expert Q2) with DI-registered handlers (visible, testable, individually lifecycle-managed by the container rather than an ever-growing static invocation list), reserving raw C# `event`/delegate usage for what it's actually well-suited to — narrow, well-owned, single-class-scoped notification APIs (e.g., a UI control's `Click` event, or a tightly-scoped domain object's own lifecycle hooks), not as the backbone of an entire application's module-to-module communication architecture.

### FinTech Principal Panel — High-Frequency Question

**FT1. Q: A service raises a synchronous C# `event PaymentReceived` and three handlers (ledger posting, fraud scoring, customer notification) subscribe. A junior engineer says "once the event fires, the payment is recorded." As the interviewer, what's wrong with that statement, and what do you require for a real trade/payment lifecycle?**
**A:** An in-process C# event is **at-most-once, in-memory, non-durable, and synchronous-on-the-caller's-thread** — none of which are safe assumptions for a financial fact. Concretely: (1) if the process crashes after the payment row commits but before/while the event's handlers run, those handlers simply *never run* — the ledger posting is silently lost with no retry, no record that it was owed. (2) A throwing handler aborts the rest of the invocation list unless manually isolated — fraud scoring throwing could skip customer notification. (3) The handlers run on the caller's thread inside the request, coupling the payment's latency to the slowest handler. So "the event fired" proves nothing about whether the ledger was posted. What I require for a trade/payment lifecycle: **persist the fact and the intent-to-notify in the same DB transaction** (the **Outbox pattern**), then a separate relay publishes to a durable broker with **at-least-once** delivery and **idempotent** consumers — so a crash at any point is recoverable by replay, ordering/isolation is explicit, and each consumer (ledger, fraud, notification) can fail and retry independently without losing the event. The C# event is fine for a UI hover; it is categorically the wrong primitive for "money moved."
**Why correct:** Names the four real deficiencies (durability, delivery-once, exception isolation, latency coupling) and lands on the industry-standard fix (transactional outbox + durable broker + idempotent at-least-once consumers).
**Common mistakes:** Treating "event raised" as "side effects committed"; adding a `try/catch` per handler and thinking that solved durability (it didn't — a crash still loses the work); putting the broker publish *outside* the DB transaction (dual-write problem — the reason Outbox exists).
**Follow-ups:** "Why can't you just publish to the broker directly inside the handler?" (dual-write: DB commit and broker publish aren't atomic — one can succeed while the other fails) / "Why must the consumers be idempotent if the broker is at-least-once?" / "Where does this connect to the Saga and Event Sourcing modules?"
