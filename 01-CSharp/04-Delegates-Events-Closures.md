...
**Q1. What does a delegate hold at runtime?**
**A:** A target object reference, a method pointer, and an invocation list. The target reference is the part people forget — a delegate keeps the object whose method it points at alive for as long as the delegate lives. That single fact explains most delegate-related memory leaks. For a static method the target is null, which is why static-method delegates don't have this problem.
*Follow-up: What's the return value when you invoke a multicast delegate with three subscribers?*

**Q2. Why doesn't this unsubscribe?**
```csharp
publisher.DataReady += () => Handle();
// later...
publisher.DataReady -= () => Handle();
```
**A:** Two separately-written lambdas are two different delegate instances, even with identical source text. `-=` compares by target and method, finds no match, and silently removes nothing — no error. You have to keep a reference to the exact delegate you subscribed with and unsubscribe using that. This is one of the most common causes of a subscription leak.
*Follow-up: Same trap with an async event handler — what does that look like?*

**Q3. Which direction does the reference go in an event subscription, and why does it matter?**
**A:** The *publisher* holds a reference to the *subscriber*, through the delegate's target. That's the opposite of what most people picture. So when a long-lived singleton exposes an event and a short-lived per-request object subscribes, that request object — and its entire object graph — stays alive for the singleton's lifetime. Nothing in the code looks wrong; the leak is in the lifetime asymmetry.
*Follow-up: How would you confirm that specific cause in a memory dump?*

**Q4. What does the compiler generate for a lambda that captures a local?**
**A:** A display class — a generated class with a field per captured variable — and it rewrites the method so the variable lives in that class rather than on the stack. The lambda becomes a method on the display class, and the delegate's target is the display class instance. That's why capture is by *reference*: both the outer method and the lambda read and write the same field. It's also why a capturing lambda allocates — a display class plus a delegate.
*Follow-up: What's different for a lambda that captures nothing?*

**Q5. What does the `event` keyword actually add over a public delegate field?**
**A:** It restricts external code to `+=` and `-=` only. A public delegate field lets any consumer invoke it, replace the whole invocation list with `=`, or clear it — all of which break the publisher's contract. Declaring it as an event generates `add`/`remove` accessors and keeps invocation private to the declaring type. Otherwise it's the same delegate field; the difference is purely about who may do what.
*Follow-up: When would you write explicit `add`/`remove` accessors?*
...
```
— versus the more common `_handlers?.Invoke(this, args)`.
**A:** With `?.Invoke`, one subscriber throwing means the remaining subscribers never run — and the publisher usually has no idea. That's the fragility of multicast invocation: one badly-behaved handler silently disables everyone registered after it. If independent delivery matters, enumerate `GetInvocationList()` and invoke each inside its own try/catch, deciding explicitly whether to log or collect the failures.
*Follow-up: Where would you rather that isolation live — the publisher or every handler?*

**Q7. Why is `Handler?.Invoke(this, args)` the recommended pattern?**
**A:** It compiles to taking a local copy of the delegate and then invoking it, which closes a race: between checking for null and invoking, another thread could unsubscribe the last handler and make the field null. The snapshot removes that window. What it doesn't solve is that a handler unsubscribed a microsecond ago may still be invoked from the snapshot — handlers have to tolerate that.
*Follow-up: So is event invocation thread-safe? What isn't it protecting?*

**Q8. Why are `async void` event handlers so common, and what do they cost?**
**A:** Because `EventHandler` returns `void`, so an async handler has no other shape. The cost is that the publisher can't await it — the handler returns at its first suspension point and the publisher carries on as if it were done — and any exception goes to the ambient context rather than being catchable, which usually means a process crash. If a handler genuinely needs async work, either the event should be a delegate returning `Task` that the publisher awaits, or the handler wraps its whole body in try/catch.
*Follow-up: How would you design an event-like notification that supports async handlers properly?*

**Q9. Why is a non-capturing lambda cheaper than a capturing one?**
**A:** It has no state, so the compiler creates the delegate once and caches it in a static field — same instance every call. A capturing lambda has to allocate a fresh display class holding the current values plus a fresh delegate each time, because each invocation's captured state differs. In a hot loop that difference is the whole cost, and it's why modern APIs increasingly offer overloads taking an explicit state argument so callers can avoid capturing.
*Follow-up: How would you rewrite `list.Where(x => x.Id == localId)` to avoid the capture?*

**Q10. Delegate call versus virtual call — what's the runtime difference?**
**A:** A virtual call resolves through the type's method table using the object's concrete type. A delegate call is an indirect call through a stored function pointer plus a target reference, so it generally can't be devirtualised or inlined. The JIT can sometimes inline through a delegate when the target is provably constant, but assume a delegate costs slightly more than a virtual call and considerably more than a direct one. Irrelevant almost everywhere; relevant in an extremely hot inner loop.
*Follow-up: When would you use a generic struct constraint instead of a delegate to get the call inlined?*

---

## 3. Intermediate (10 Q&A)

**Q1. Dumps show view models rooted by a singleton's event. What are your options?**
**A:** Symmetric lifetime management is the straightforward fix — subscribe in init, unsubscribe in `Dispose`, and make sure the subscriber actually gets disposed. That relies on discipline code review won't reliably enforce. A weak-event pattern removes the reliance by having the publisher hold a weak reference, at the cost of complexity and slower invocation. The third option is inverting the relationship: instead of subscribing to a singleton, receive notifications through something whose lifetime matches yours. I'd prefer the third where the architecture allows it, because it removes the failure mode rather than managing it.
*Follow-up: What's the downside of weak events when the subscription is the only remaining reference?*

**Q2. Why does this print 10 ten times?**
```csharp
for (int i = 0; i < 10; i++)
    tasks.Add(Task.Run(() => Console.WriteLine(i)));
```
**A:** The lambda captures the *variable* `i`, not its value, and a `for` loop has one `i` shared across all iterations. By the time the tasks run, the loop has finished and `i` is 10. `foreach` was changed in C# 5 so the iteration variable is fresh each time, but `for` still behaves this way. Fix is an explicit per-iteration local copied inside the loop body.
*Follow-up: Why is this combination with `Task.Run` especially confusing to debug?*

**Q3. How do you spot an accidental `this` capture, and why does it matter more than capturing a local?**
**A:** It happens whenever a lambda touches an instance field or calls an instance method — the compiler captures `this`, not the field, so the delegate roots the entire containing object and everything it references. That matters far more than capturing a small local because the retained graph can be a whole controller or view model with its dependencies attached. You spot it by reading what the lambda touches; you confirm it in a dump where the display class field is the containing type. Mitigation is copying the needed value into a local first.
*Follow-up: A lambda registered with a static cache captures `this`. What's the resulting lifetime?*

**Q4. Event, injected callback, or in-process mediator — how do you choose?**
**A:** An `event` fits genuinely optional, multi-subscriber notification where the publisher doesn't care if anyone's listening. An injected callback or small interface fits a mandatory single collaborator, and is far better for testability and for making the dependency visible in the constructor. A mediator fits decoupled dispatch with cross-cutting behaviour, at the cost of indirection that makes call graphs hard to follow. My default in a service codebase is the injected interface, because events hide dependencies from the constructor and therefore from anyone reading the class.
*Follow-up: A team has adopted a mediator for everything internal. What symptoms would make you push back?*

**Q5. Code raises an in-process event and assumes handlers have completed when it returns. What's wrong?**
**A:** For synchronous handlers it happens to be true, which is exactly what makes it dangerous — it works until someone adds an `async void` handler, at which point the publisher continues after that handler's first `await` and the ordering assumption breaks silently. It's also fragile because handlers run on the publisher's thread, so a slow one blocks the publisher, and an exception skips the rest. If completion actually matters, this shouldn't be an event — it should be an awaited call or an explicit pipeline.
*Follow-up: How do you refactor an existing event with ten subscribers where three now need to be async?*

**Q6. Delegate allocation is showing up in a hot-path profile. What do you do?**
**A:** In order: hoist the delegate out of the loop if the captured state doesn't change; use an API overload that accepts explicit state so nothing is captured; mark the lambda `static` so accidental capture becomes a compile error; and in genuinely extreme cases replace the delegate with a generic struct implementing an interface so the JIT specialises and inlines. I wouldn't go past the first two without a profile proving delegate allocation is material — a display class and a delegate dying in gen0 usually cost nothing.
*Follow-up: What does marking a lambda `static` actually guarantee, and why is it useful even where perf doesn't matter?*

**Q7. How do you make event invocation resilient when the subscribers are plugins or third-party code?**
**A:** Enumerate `GetInvocationList()` and invoke each handler in its own try/catch, logging failures with enough identity to name the offending subscriber so one plugin can't silence the others. Add a timeout or watchdog if handlers can block, since a hung subscriber otherwise hangs the publisher, and document that handlers must be fast and non-blocking. If the subscribers are genuinely untrusted, in-process events are the wrong boundary altogether — you can't defend against a plugin that allocates a gigabyte or spins a core, so that needs process isolation or a queue.
*Follow-up: You add per-handler catches and now failures are invisible because they're only logged. How do you surface them?*

**Q8. How would you unit test a class that raises events, and what usually gets missed?**
**A:** Subscribe a test handler, exercise the operation, assert on the arguments and invocation count — that part is easy. What gets missed is the negative and lifecycle cases: that unsubscribing actually stops delivery, that no event fires on the failure path, that raising with no subscribers doesn't throw, and above all that the subscriber is unsubscribed on disposal, which is the leak everyone ships. I'd also assert that a throwing handler doesn't prevent the rest from running, if the publisher claims that isolation.
*Follow-up: How would you write an automated test that fails when a subscription leak is introduced?*

**Q9. When is a weak-event pattern right, and when is it a smell?**
**A:** Right when the publisher's lifetime is genuinely unbounded relative to subscribers and unsubscription can't be reliably enforced — classic in UI frameworks where the platform owns object lifetimes. A smell in a service codebase, where it usually means ownership is unclear and someone is patching over it: weak references make lifetimes non-deterministic, so objects disappear at times the collector chooses and bugs become load-dependent and unreproducible. I'd rather fix the ownership so subscriptions are scoped, and treat weak events as an exception with a comment explaining why that wasn't possible.
*Follow-up: A cache uses weak references so entries "clean themselves up". What will the team observe in production?*

**Q10. What's wrong with this DI registration?**
```csharp
services.AddSingleton<IReportBuilder>(sp => {
    var ctx = sp.GetRequiredService<AppDbContext>();   // scoped
    return new ReportBuilder(ctx);
});
```
**A:** The factory lambda captures a scoped `DbContext` into a singleton, so one request's context becomes application-lifetime state — stale data, growing change tracker, and cross-request bleed. It's insidious because the captured dependency doesn't appear in any constructor, so the container's own scope validation can't see it. Rule I'd enforce: singleton registrations capture nothing but the provider, and resolve scoped services from a scope created per operation.
*Follow-up: How would you detect a captured scoped dependency before production?*

---

## 4. Expert / Architect (10 Q&A)

**Q1. A team proposes an in-process event bus to decouple modules in a modular monolith. How do you evaluate it?**
**A:** Ask what property they actually want, because "decoupling" conflates three things: compile-time independence, runtime independence, and team independence. An in-process bus gives the first and none of the others — handlers still run in the same process, on the publisher's thread by default, sharing its failure domain, transaction and memory, so a slow or failing handler is still the publisher's problem. It also destroys call-graph navigability, which is a genuine long-term cost in a large codebase. I'd support it where the notification is genuinely fire-and-forget and cross-module, insist events represent facts rather than commands, and require anything needing durability, retry or ordering to go through a real broker with an outbox.
...
**A:** Attack the pattern first, because prevention beats detection here: prefer scoped DI-managed collaborators over long-lived publishers, and make static and singleton events an explicitly reviewed exception, since that's the shape that leaks. Where the pattern must exist, standardise a subscription-token type that's `IDisposable` and unsubscribes on dispose, so the lifetime is expressed in the type system rather than in a comment. For detection, a soak test in CI running a realistic workload and asserting on heap growth and instance counts catches this class before release. And make the diagnosis reproducible — a runbook showing how `!dumpheap` plus `!gcroot` identifies it turns a two-day investigation into twenty minutes.
*Follow-up: The soak test is slow and gets skipped on PRs. Where do you run it instead?*

**Q3. Callbacks and events are being used as extension points in a shared platform library. What API guidance do you set?**
**A:** Prefer explicit interfaces over events for anything the platform *depends on*, because an interface makes the contract discoverable, versionable and mockable while an event makes it optional and invisible. Where callbacks are right: take an explicit state parameter so consumers aren't forced into closures on hot paths, return `Task` where async is plausible so it isn't a breaking change later, and accept a `CancellationToken`. Specify the threading contract explicitly — which thread it runs on, whether it may block, whether reentrancy is permitted — because unspecified threading contracts are what generate support tickets forever. And treat every callback as untrusted: the platform isolates failures rather than assuming well-behaved consumers.
*Follow-up: You need to add a parameter to an existing public event's args type. Options without breaking consumers?*

**Q4. How do you reason about threading and reentrancy for events in a concurrent service?**
**A:** The default contract is unforgiving: handlers run synchronously on whatever thread raised the event, concurrently if it's raised concurrently, and possibly reentrantly if a handler causes the same event to be raised again. So handler code must be thread-safe and must not assume it's called one at a time — an assumption almost never written down and almost always made. If a handler takes a lock the publisher also holds, that's a lock-ordering deadlock waiting for load. My guidance is to specify the contract in the publisher's documentation, keep handlers short and non-blocking, and where serialisation is genuinely needed use an explicit queue with a single consumer rather than locks inside handlers.
*Follow-up: Where do you put the boundary — "handler must be thread-safe" or "publisher serialises delivery"?*

**Q5. You're migrating a system with heavy in-process event usage to a distributed architecture. What breaks?**
**A:** Every implicit guarantee that in-process events gave for free: synchronous ordering, same-transaction visibility, exactly-one delivery, and the ability to observe a handler's exception. Across a network you get at-least-once delivery, out-of-order arrival, partial failure and no shared transaction — so handlers that were idempotent by accident become the design problem. My approach is to first make the in-process events *look* distributed: explicit event contracts, idempotent handlers, no reliance on ordering or on the publisher's transaction. Only then swap the transport. Doing it the other way round produces a distributed system with monolith assumptions baked in, which is the most expensive class of failure to unwind.
*Follow-up: Which of those in-process guarantees would you try hardest to preserve, and how?*

**Q6. How do you make event-driven code inside a process observable?**
**A:** Instrument the dispatch point rather than the handlers, since that's the one place you control: emit an activity per raise tagged with the event type and subscriber count, and record per-handler duration and outcome when invoking the invocation list individually. Without that, an event-heavy codebase is a black box during an incident — you see the request slow down and can't see which subscriber caused it. I'd also expose subscriber counts as a metric, because a subscription leak shows up there long before it shows up as memory pressure. General principle: any indirection you add, you must also add observability for, or you've traded debuggability for decoupling.
*Follow-up: How do you keep that instrumentation from dominating the cost of a high-frequency event?*

**Q7. What's your position on domain events raised from aggregates?**
**A:** Good modelling device, wrong mechanism if implemented as C# `event` members. The right pattern is for the aggregate to *record* events into a collection that the application layer collects and dispatches after the transaction commits — that keeps the domain free of dispatch concerns, makes the events testable as data, and crucially stops handlers running inside the aggregate's transaction where a failure would corrupt the unit of work. Raising real C# events from an aggregate couples the domain to subscribers, makes ordering implicit, and invites handlers to do I/O mid-transaction. The record-versus-dispatch distinction is one of the more reliable signals of whether someone has actually operated a DDD codebase.
*Follow-up: A handler needs to run in the same transaction for consistency. How do you do that without coupling the aggregate?*

**Q8. How do closures interact with DI lifetimes, and what failure does that produce?**
**A:** Any lambda registered with the container — a factory delegate, an options callback, a filter predicate — captures whatever it references, and if the registration is singleton, that captured state becomes singleton-lived. The classic failure is a factory capturing a scoped service or a request value, silently promoting it to application lifetime: stale or cross-request values, or a disposed `DbContext` used by a later request. It's insidious because the captured dependency appears in no constructor, so the container's lifetime validation can't see it. I'd ban capturing anything but the provider in singleton registrations and review composition-root lambdas specifically.
*Follow-up: How would you write an architecture test that catches that?*

**Q9. A shared internal library exposes a static event that many services subscribe to. Assessment?**
**A:** That's the worst combination available: unbounded publisher lifetime, invisible global coupling, no scoping per tenant or request, subscription leaks with no natural unsubscribe point, and — in a library used by many services — a failure mode that reproduces differently in each consumer. It also makes testing awful, since state leaks between tests and ordering becomes significant. I'd treat it as a defect rather than a design and plan a migration: introduce an injectable instance-scoped notifier with the same shape, dual-publish during transition, migrate consumers with telemetry showing who's still on the static path, then remove it. The organisational point is that a shared library's mistakes are amortised across the estate, so its API deserves stricter review than any single service's.
*Follow-up: One consumer can't migrate for two quarters. How do you stop the deprecation stalling forever?*

**Q10. What separates an excellent answer from an adequate one on delegates and events?**
**A:** An adequate answer knows the syntax and that events can leak. An excellent one reasons about **reference direction and lifetime asymmetry** as the root cause — publisher holds subscriber — and therefore predicts where leaks will occur before seeing them. It knows capture is by reference and that `this` capture roots whole graphs; it treats the threading and reentrancy contract as something to state explicitly; it recognises multicast invocation's failure isolation problem; and it distinguishes when an event is the right coupling mechanism from when an injected interface or a durable message is. The distinguishing quality is treating these as *lifetime and coupling* constructs rather than as syntax.
*Follow-up: Given that, what's the first thing you'd check when you see `+=` in a code review?*
