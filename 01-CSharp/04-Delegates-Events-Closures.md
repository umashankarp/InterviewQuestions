...
1. **Q: What does a delegate instance actually contain at runtime, and what happens on `Invoke`?** **A:** A delegate object holds a target object reference (`_target`, null for static methods), a method pointer (`_methodPtr`, possibly through a stub), and for multicast, an invocation list array. `Invoke` is JIT-compiled into a call through the stored pointer with the target as `this` — modern runtimes make this comparable to an interface dispatch, not reflection; the reflection-like cost only appears with `DynamicInvoke`.
2. **Q: Why is `DynamicInvoke` orders of magnitude slower than `Invoke`, and when is it ever acceptable?** **A:** `DynamicInvoke` boxes arguments into an `object[]`, performs runtime signature validation and binding, and unboxes the return — reflection-style machinery per call — versus `Invoke`'s direct JIT-compiled call. Acceptable only in genuinely dynamic plumbing (test harnesses, scripting bridges, generic event forwarding) where the signature isn't statically known and call frequency is low; in hot paths, convert once to a typed delegate via `Delegate.CreateDelegate`/expression compilation and cache it.
3. **Q: Explain what `Delegate.CreateDelegate` over a `MethodInfo` gives you compared to calling `MethodInfo.Invoke`, and the caching pattern that exploits it.** **A:** `CreateDelegate` performs binding/validation once and returns a typed delegate whose subsequent calls run at normal delegate speed, while `MethodInfo.Invoke` pays boxing and validation on every call. The standard pattern: reflection discovers methods at startup (e.g., handler registration by convention), creates typed delegates (often via an open instance delegate `Func<TTarget,TArg,TResult>`), caches them in a dictionary — reflection cost at startup, zero reflection at runtime.
4. **Q: What is an open instance delegate, and what advanced scenario does it enable?** **A:** A delegate bound to an instance *method* but no instance — the target becomes the delegate's first parameter (`Func<MyType, int>` over `MyType.SomeMethod`). It enables high-performance dispatch frameworks (serializers, mappers, mediators) to cache one delegate per method and invoke it against any instance, instead of creating per-instance closed delegates or using reflection per call.
5. **Q: How do lambdas convert to expression trees versus delegates, and why can't every lambda become an `Expression<T>`?** **A:** Assigning a lambda to `Expression<Func<...>>` makes the compiler emit code that *builds a data structure* describing the lambda instead of compiling its body. Only expression-bodied lambdas convert — statement bodies, `async` lambdas, and (historically) many newer C# constructs (e.g., `?.`, tuples in some positions, pattern matching) aren't supported in expression trees, which is why LINQ providers lag language evolution and why complex predicates sometimes must become `Func<T,bool>` (client-side) instead.
6. **Q: What's the allocation profile difference between `static` lambdas, non-capturing lambdas, instance-capturing lambdas, and local-capturing lambdas?** **A:** Non-capturing (and `static`) lambdas are cached in a static field — one allocation ever. Capturing only `this` allocates a delegate per creation (target = existing instance) but no display class. Capturing locals allocates a display class *plus* the delegate; and if the same local is captured by multiple lambdas in one scope, they share one display class — meaning one long-lived lambda can pin every captured variable in that shared class, the classic "closure keeps a huge object alive" leak.
7. **Q: Explain how a closure in a loop that captures the loop variable *and* an expensive local creates an unexpected retention bug even with `foreach`.** **A:** Because display classes are per-scope, a lambda capturing both the (per-iteration) `foreach` variable and a method-level local shares a display class chain: storing that lambda long-term (event subscription, cached callback) roots not just the iteration value but every other captured variable in the enclosing scopes — e.g., a 200MB buffer local mentioned in another lambda in the same method. Fix: extract the lambda creation into a helper method whose parameters are exactly the needed values, cutting the capture set explicitly.
8. **Q: Why does subscribing an anonymous lambda to an event make correct unsubscription impossible, and what are the three practical alternatives?** **A:** `-=` removes by delegate equality (same method + same target); a fresh lambda expression creates a *new* delegate instance each evaluation, and two lambdas — even textually identical — with different display-class instances never compare equal, so `-=` silently removes nothing. Alternatives: store the delegate in a field and unsubscribe that exact instance; use a named method (equality then works, for static/same-target cases); or use a subscription-token pattern (`IDisposable` returned from a `Subscribe` method) that removes by token, sidestepping delegate equality entirely.
9. **Q: What is the "handler re-entrancy" hazard when raising events, and how do you defend against it?** **A:** A handler can, synchronously inside the raise, subscribe/unsubscribe handlers (mutating the list — safe in C# because invocation snapshots the immutable multicast list, but the *newly removed* handler still runs this raise, surprising authors), raise the same event again (unbounded recursion), or mutate the state that triggered the event (infinite change-notification loops). Defenses: document re-entrancy semantics, guard with an `isRaising` flag or dispatch queue for state-change events, and keep handlers side-effect-light; UI frameworks and observable-collection implementations all carry these guards internally.
10. **Q: Compare `EventHandler<T>`/CLS event pattern with exposing `Action<T>` — why does the BCL convention still matter?** **A:** The `(object sender, TEventArgs e)` shape gives handlers the source instance (critical when one handler observes many publishers), supports args-class evolution (add properties without breaking subscribers), and is what tooling/analyzers/WeakEvent infrastructures expect. `Action<T>` is terser and fine for internal callbacks, but loses sender identity, breaks on payload evolution (each new parameter is a signature break), and surprises maintainers expecting the standard pattern on public APIs.
11. **Q: When wrapping a C# event as an `IObservable<T>` (Rx) or `IAsyncEnumerable<T>`, what semantic mismatches must you handle?** **A:** Events are hot (fire whether or not anyone listens; subscribers miss prior events), have no completion or error channel, and are push-synchronous — while `IObservable<T>` models completion/error explicitly and `IAsyncEnumerable<T>` is pull-based needing buffering/backpressure decisions for a push source (unbounded buffer? drop? block the raiser?). A correct adapter (like `Observable.FromEventPattern` or a `Channel<T>` bridge) must choose and document the buffering/loss policy and guarantee unsubscription on disposal — the naive adapter leaks the subscription and silently drops or unboundedly buffers under pressure.
12. **Q: What ordering and consistency guarantees do multicast delegates give, and why shouldn't business logic depend on handler order?** **A:** Handlers run synchronously in subscription order, and that order is technically reliable — but depending on it couples correctness to an implicit, unenforceable registration sequence (DI registration order, module load order), which changes with refactoring and breaks silently. If B must run after A, model the dependency explicitly (pipeline/ordered-handler list with declared priorities, or A raising a second event) rather than encoding it in subscribe timing.
13. **Q: How does raising an event from multiple threads interact with handler execution — what does the snapshot guarantee, and what doesn't it guarantee?** **A:** Reading the delegate field (into a local / via `?.Invoke`) atomically snapshots an immutable invocation list, so the raiser never sees a torn list and concurrent `+=`/`-=` are individually safe. It does *not* guarantee: handlers won't run concurrently with themselves (two threads raising = same handler executing in parallel — handlers must be thread-safe or the publisher must serialize), that an unsubscribed handler won't run "after" unsubscription (the snapshot race), or any memory-visibility ordering of state the handlers read beyond what the handlers synchronize themselves.
14. **Q: Design a `WeakEventManager`-style mechanism: what problem does it solve and what's the core implementation trick?** **A:** It solves the lapsed-listener leak structurally: the publisher holds `WeakReference`s to subscriber targets instead of strong delegate references, so a forgotten unsubscribe can't keep the subscriber alive. Core trick: store (WeakReference to target, MethodInfo/open delegate) pairs — *not* the closed delegate, which strongly roots the target — and on raise, dereference each weak target, invoke via the open delegate for live ones, and prune dead entries. Cost: slower raises, GC-nondeterministic delivery cutoff, and it can't work for closures (the display class is the target and nothing else roots it — it would be collected immediately).
15. **Q: Why do closures over the loop of an `async` pipeline (e.g., `tasks.Add(ProcessAsync(() => item))`) produce subtler bugs than the classic `for` capture, and what discipline prevents them?** **A:** The delegate executes arbitrarily later on other threads, so capture bugs manifest as intermittent wrong-data races rather than deterministic "always last value" behavior — and captured mutable state (a shared `List`, a `DbContext`, `this._currentItem`) is now accessed concurrently, adding thread-safety violations on top of staleness. Discipline: materialize per-iteration values into locals before lambda creation, pass state as explicit method/task parameters instead of capturing, and treat any mutable capture crossing an async boundary as a code-review flag.
16. **Q: What are the observable differences between invoking handlers via `Invoke()` versus iterating `GetInvocationList()` with per-handler try/catch — beyond exception isolation?** **A:** Per-handler iteration also lets you: continue after a failure and aggregate exceptions (vs. multicast's stop-at-first-throw), observe/log each handler's identity and duration (per-subscriber telemetry), run handlers with per-handler timeouts or on captured contexts, filter handlers (e.g., skip ones whose target implements a disabled marker), and collect every return value instead of the last. The cost is an array allocation per raise and manually reimplementing the null-race-safe snapshot pattern.
17. **Q: Explain how event subscriptions interact with DI lifetimes — the singleton-publisher/scoped-subscriber trap.** **A:** A scoped or transient service subscribing to a singleton's event creates a strong reference from a process-lifetime object to a per-request object: the subscriber (and its object graph — `DbContext`, request buffers) leaks for the app's lifetime, and worse, the "dead" subscriber keeps *handling events* against disposed dependencies (`ObjectDisposedException`s or writes to a disposed context). Rules: only singletons may subscribe to singleton events; scoped consumers should use a mediator/message-bus with scope-aware dispatch, or subscribe via `IDisposable` registration that the scope's disposal deterministically tears down.
18. **Q: How would you make a high-frequency event raise allocation-free, and what standard patterns leak allocations accidentally?** **A:** Keep the delegate cached (no per-raise lambda creation), pass state through the event-args generic parameter rather than capturing (no display class per raise), reuse or pool `EventArgs` objects for extreme rates (with documented "don't store the args" contract, as `SocketAsyncEventArgs` does), and avoid `GetInvocationList` (array per call) on the hot path. Accidental allocations: interpolated strings in a raise-side log, `?.Invoke(this, new EventArgs(...))` per tick with a heavy args class, and LINQ over the invocation list.
19. **Q: When replacing events with `Channel<T>` for an in-process producer/consumer pathway, what semantics change — enumerate the trade?** **A:** Gained: backpressure (bounded channel blocks/drops instead of unbounded synchronous fan-out), async consumption decoupled from the producer's thread, single-consumer ordering guarantees, natural shutdown semantics (writer completion), and no lapsed-listener leaks. Lost: multicast (a channel item is consumed once — fan-out needs explicit multiplexing), synchronous "handler completes before raise returns" semantics some callers rely on, and automatic per-subscriber lifecycle (subscribe/unsubscribe becomes topology you manage). Events are a synchronous broadcast primitive; channels are an async queue — swapping them changes contract, not just mechanism.
20. **Q: As a Principal Engineer, define the decision rubric you'd publish for "C# event vs. callback interface vs. mediator vs. message queue" for in-process communication.** **A:** C# events: optional, broadcast, fire-and-forget notifications where publisher and subscribers share a lifetime scope and no ordering/error contract is needed (UI, tooling, framework hooks). Callback interface (or injected delegate): exactly-one collaborator, required behavior, testable and explicit — dependencies, not notifications. Mediator/in-process bus: many-to-many domain events needing DI-scope-aware dispatch, pipeline behaviors, and later externalization potential. Message queue/channel: work handoff needing buffering, backpressure, retry, or lifetime decoupling. The rubric's spine is: is it a *notification* (events), a *dependency* (interface), a *domain fact* (mediator), or *work* (queue) — misclassifying work as notification is the root of most event-abuse architectures.
