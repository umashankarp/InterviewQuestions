...
**CPU**: `async`/`await` itself adds small but real per-suspension overhead (state machine `MoveNext` dispatch, `ExecutionContext` capture/restore) — negligible for I/O-bound code (dwarfed by actual I/O latency), but can matter in extremely hot, rarely-actually-suspending code paths (favor `ValueTask` + synchronous-completion fast paths there).

**Memory**: State machine boxing (only on genuine suspension) + continuation delegate allocations + `ExecutionContext`/`SynchronizationContext` capture objects are the main allocation sources. `ConfigureAwait(false)` and `ValueTask` are the two primary allocation-reduction levers.

**Threading**: The shared CLR ThreadPool is a finite, contended resource across an entire process — one team's sync-over-async bug throttles every other feature sharing that pool (§4). Monitor `ThreadPool Queue Length` and `ThreadPool Thread Count` as first-class production signals, exactly like GC counters from Module 1.

**Latency vs Throughput**: `async`/`await` primarily buys **throughput** (more concurrent in-flight requests per thread) not lower **per-request latency** (a single request isn't inherently faster async vs sync — it can even be marginally slower per-call due to state machine overhead) — a frequently mis-stated trade-off in interviews. The latency win shows up indirectly: under load, sync code's thread exhaustion causes queueing delay that async code avoids.

**Benchmarking**: BenchmarkDotNet with `[MemoryDiagnoser]` to compare `Task` vs `ValueTask` allocation counts on synchronous vs asynchronous completion paths; use load-testing tools (k6, NBomber, `dotnet-counters` during load) to observe thread-pool behavior under realistic concurrency — micro-benchmarks alone won't reveal thread-pool starvation, which is fundamentally a load/contention phenomenon.

**Caching**: Async cache-lookup APIs (`IDistributedCache.GetAsync`) are a canonical `ValueTask` use case — cache hits complete synchronously (avoid `Task` allocation), cache misses genuinely await the backing store.

---

## 8. Security

- **Unbounded fire-and-forget async work** can be leveraged for resource-exhaustion DoS if triggered by user input without concurrency limits (e.g., a "send verification email" fire-and-forget triggered per request with no throttling) — always bound concurrent background work (semaphores, bounded channels, queue-based work distribution) when triggered by untrusted/high-volume input.
- **`CancellationToken` propagation gaps** are a subtle security-adjacent issue: if a client disconnects/cancels but the server-side chain doesn't propagate `HttpContext.RequestAborted` down to DB/external calls, orphaned work keeps consuming resources — enables cheap amplification-style resource exhaustion (many cancelled-but-still-running downstream calls).
- **`ExecutionContext`/`AsyncLocal` carrying security principal/claims**: ensure custom `AsyncLocal`-based context propagation (e.g., a custom "current tenant" or "current user" ambient context) is set/cleared correctly across all async boundaries — a bug here can leak one request's security context into another's continuation under thread-pool reuse if implemented incorrectly (extremely subtle, high-severity class of bug — always prefer explicit parameter passing over ambient async context for security-sensitive data unless a framework requires the ambient pattern).
- **OWASP relevance**: A04 (Insecure Design) if fire-and-forget/background task patterns lack rate limiting; A05 (Security Misconfiguration)-adjacent if thread-pool exhaustion from async misuse is exploitable as a low-cost availability attack (a slow endpoint with a sync-over-async bug can become a de facto DoS vector under moderate concurrent load, even without malicious intent).

---

## 9. Scalability

- **Horizontal scaling**: Async I/O is what makes a single .NET process capable of handling thousands of concurrent in-flight requests on a small thread pool — directly increasing the useful concurrency each replica can sustain before needing to scale out, improving cost-efficiency of horizontal scaling (fewer replicas needed for the same concurrent I/O-bound load).
- **Vertical scaling**: More cores help thread-pool capacity for genuinely CPU-bound offloaded work (`Task.Run`), but do **not** help I/O-bound async throughput nearly as much — I/O-bound scalability is bounded by downstream systems (DB connections, external API rate limits) and thread-pool queue health, not core count.
- **Caching/Replication/Partitioning**: `ValueTask`-based async cache APIs, `IAsyncEnumerable` for paginated/streamed replication reads — same runtime principles apply across data-layer scalability patterns covered in later data-store modules.
- **CAP theorem tie-in**: Async I/O is what makes it *practical* to fan out to multiple replicas/partitions concurrently (`Task.WhenAll` across shards) within a latency budget — sequential blocking calls to N partitions would multiply latency by N; concurrent async calls keep it closer to O(1) (bounded by the slowest single call).
- **HA/DR**: `CancellationToken`-aware async chains enable graceful shutdown (drain in-flight requests within a timeout during a rolling deploy) — a core requirement for zero-downtime deployments in any HA setup.
