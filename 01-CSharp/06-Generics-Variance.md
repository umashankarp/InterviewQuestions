...
1. **Q: What is a "shared generic dictionary" lookup in the CLR, and when does shared generic code pay this hidden cost?** **A:** Reference-type instantiations share one native code body, so when that code needs a `T`-specific artifact at runtime (a `typeof(T)`, `new T[n]`, a call to another generic, a static-field access), it can't hardcode it — it consults a per-instantiation dictionary (reached via the method table or an instantiation-context parameter) to fetch the right handle. Value-type instantiations get specialized code with everything inlined, which is one reason `List<int>` operations can outperform structurally identical `List<string>` paths that hit dictionary lookups.
2. **Q: Why does calling an interface method on an unconstrained `T` differ in cost from calling it on `T : IComparable<T>`, at the generated-code level?** **A:** With the constraint, the compiler emits a `constrained.` callvirt: for value types the JIT can devirtualize and call the method directly on the unboxed value (often inlining it); unconstrained code must box the value into an interface reference first — an allocation per call. This is why generic algorithms with proper constraints are allocation-free over structs while `(IComparable)obj` casts are not.
3. **Q: Explain how `EqualityComparer<T>.Default` achieves zero-allocation equality for value types and why hand-rolled `==`-style generic comparisons can't.** **A:** `EqualityComparer<T>.Default` is resolved per closed type: for a `T` implementing `IEquatable<T>` it dispatches to `Equals(T)` directly (no boxing, devirtualized in specialized value-type code, and the JIT now often inlines the Default comparer entirely). Generic code writing `x.Equals(y)` on unconstrained `T` boxes; `object.Equals(x, y)` boxes both — so the idiom for generic containers is always `EqualityComparer<T>.Default.Equals` (and `Comparer<T>.Default` for ordering).
4. **Q: What's the difference between `default(T)`, `default`, and `T?` inside generic code, and the nullability subtlety with unconstrained `T`?** **A:** `default(T)`/`default` yields null for reference types and zero-initialized for value types. For unconstrained `T`, `T?` does *not* mean `Nullable<T>` — it means "defaultable T" (annotated as maybe-null for reference types, plain `T` for value types), which is why `FirstOrDefault` returns `T?` in annotations but still returns `0` (not null) for `int`. `Nullable<T>` semantics require `where T : struct`; misunderstanding this leads to wrong "is it null?" checks in generic helpers.
5. **Q: Why can't you write `where T : Enum`-style numeric code before C# 7.3/generic math, and how do C# 11's static abstract members solve the general problem?** **A:** Pre-7.3 you couldn't even constrain to `System.Enum`/`Delegate`; and even then, operators like `+` are static methods, which interfaces couldn't declare — so generic `Sum<T>` had no way to say "T supports +". Static abstract interface members let interfaces declare static operators (`INumber<T>` : `IAdditionOperators<T,T,T>`…), resolved per closed type at JIT time with full specialization for structs — giving generic math the performance of hand-written `int` code without per-type overloads.
6. **Q: How does the JIT devirtualize and inline generic interface calls for struct type arguments, and what design pattern exploits this deliberately?** **A:** In specialized value-type instantiations the JIT knows `T`'s exact type, so a constrained interface call resolves to a concrete method — devirtualizable and inlinable, disappearing entirely for trivial bodies. The "struct strategy" pattern exploits this: accept a `TStrategy : struct, IStrategy` type parameter and call its methods in a hot loop — each strategy compiles to its own fully-inlined loop body (how `SortHelper`, hashing, and `TensorPrimitives`-style code achieve zero-cost abstraction), at the cost of code-size growth per instantiation.
7. **Q: What is generic code bloat, when is it a real problem, and what are the mitigation patterns?** **A:** Every value-type instantiation generates its own native code body; a generic-heavy library closed over many struct types multiplies code size — mostly irrelevant for JIT server apps, but material for NativeAOT binaries and memory-constrained targets where every instantiation is precompiled. Mitigations: hoist type-independent logic into non-generic shared helpers (the `List<T>` → non-generic `Array.Resize` pattern), constrain generic surface area to the thin typed layer, use reference-type abstractions where specialization buys nothing, and audit AOT size reports for instantiation explosions.
8. **Q: Why is `Nullable<T>` special-cased by the runtime, and what generic behaviors look anomalous because of it?** **A:** The CLR gives `Nullable<T>` unique treatment: boxing a null `Nullable<T>` produces a null reference (not a boxed struct), boxing a non-null one boxes the underlying `T` (so a boxed `int?` *is* a boxed `int`), and `is`/unbox conversions work both directions. Consequences in generics: `typeof(T)` inside `T = int?` reports `Nullable<int>`, but `GetType()` on a boxed value never shows it; `where T : struct` excludes `Nullable<T>`; and generic code storing boxed values can't distinguish "was `int?`" from "was `int`".
9. **Q: Explain covariance's broken corner: array covariance. Why does `object[] arr = new string[1]` compile, and what does it cost even when correct?** **A:** Array covariance predates generics (Java compatibility-era design): `string[]` is assignable to `object[]`, so the type system permits writes that only runtime checks can catch — `arr[0] = 42` throws `ArrayTypeMismatchException` at runtime, not compile time. Cost: every store into an array of a non-sealed reference type performs a runtime type check (covariance check), which generic `List<T>`/spans avoid — one reason `Span<T>` is invariant and a micro-reason typed collections beat object arrays.
10. **Q: Why is `Span<T>` invariant even though `ReadOnlySpan<T>` supports some covariance-like conversions?** **A:** `Span<T>` is readable *and* writable, so `Span<string>` → `Span<object>` would allow writing an arbitrary `object` into a `string[]` — unsound without per-write checks the span's raw-memory performance contract forbids. `ReadOnlySpan<T>` removes the write channel, so the runtime permits `ReadOnlySpan<Derived>` → `ReadOnlySpan<Base>` conversions for reference types (implemented via `CastUp`) — the same "out position only" logic as interface covariance, applied at the memory-view level.
11. **Q: How do generic constraints participate in overload resolution vs. how do they not — explain the "constraints are not part of the signature" rule and its practical workaround.** **A:** Overload resolution picks a method by parameters first and only *then* checks constraints — two same-signature methods differing only by constraints are a redeclaration error, and an unsatisfied constraint on the chosen best method is an error, not a fallback to the next candidate. Workaround: differentiate signatures structurally (marker parameter types, extension methods over different receiver types like `IEnumerable<T>` vs `T[]`), or in modern C# lean on static abstract interface constraints where distinct interfaces make parameter types genuinely different.
12. **Q: What are the rules for calling a generic method with inference across inheritance — why does `Process(derivedList)` sometimes infer `T = DerivedList` instead of the intended interface?** **A:** Type inference works from the compile-time types of arguments to the best exact match — it infers the most specific type satisfying the parameter shape, not the interface you had in mind; a `List<Derived>` argument against `IEnumerable<T>` infers `T = Derived`, but against a plain `T` parameter infers `T = List<Derived>`. When the inferred `T` then fails a constraint or binds the "wrong" overload, the fixes are explicit type arguments, changing the parameter to the intended abstraction, or casting the argument — and API designers should shape parameters so natural inference lands on the intended type.
13. **Q: Explain self-referencing ("CRTP-style") constraints — `where T : IComparable<T>` or `class Entity<T> where T : Entity<T>` — what they buy and their costs.** **A:** They let a base type refer to the *derived* type statically: fluent builders returning `T` (not the base), `IEquatable<T>` avoiding boxing, static abstract factories (`static abstract T Create()`), and per-derived-type static state. Costs: the pattern doesn't actually *prove* `T` is the current class (you can write `class B : Entity<C>`), it's viral through the hierarchy's generic signatures, confuses consumers, and blocks heterogeneous collections (`List<Entity<?>>` isn't expressible) — so it's justified in frameworks/primitives, rarely in ordinary domain models.
14. **Q: How do static fields and static constructors behave across generic instantiations, and what's a bug and a technique this enables?** **A:** Each closed type (`Cache<int>`, `Cache<string>`) has independent static fields and runs its own static constructor on first use. Bug: a "global" registry declared as a static field in a generic class silently shards per type argument (registrations vanish depending on `T`). Technique: deliberate per-type caching — `TypeCache<T>.Value` computed once per `T` with zero-lookup access (faster than `Dictionary<Type, V>`), used widely for reflection caches, serializer plans, and `EqualityComparer<T>.Default` itself.
15. **Q: What happens when generic virtual methods meet value types — why are `virtual void M<T>()` calls the most expensive dispatch in the CLR?** **A:** A generic *virtual* method can't be resolved by a fixed vtable slot per instantiation (unbounded instantiations × virtual dispatch), so the runtime resolves them through a slower path: a generic-dictionary-backed lookup (JIT "virtual generic method" resolution with a runtime cache). Guidance: avoid generic virtual methods on hot paths — restructure as a generic *interface* on the type (constrained dispatch), a non-generic virtual taking an abstraction, or visitor-style double dispatch designed around the limitation.
16. **Q: How does reflection over generics differ between generic type *definitions* and closed types — `typeof(List<>)` vs `typeof(List<int>)` — and what's the standard runtime-composition pattern?** **A:** `typeof(List<>)` is the open definition (uninstantiable, `ContainsGenericParameters == true`); `MakeGenericType(typeof(int))` closes it. The standard pattern for "I have a `Type` at runtime and a generic algorithm": close the generic method/type via `MakeGenericMethod`, then cache a compiled delegate per closed type (via `CreateDelegate` or expression compilation) so reflection cost is paid once — this is how serializers, mappers, and DI containers bridge runtime `Type`s into compile-time generic code. NativeAOT caveat: `MakeGenericType` over value types requires the instantiation to exist statically or be rooted, or it fails at runtime.
17. **Q: Why does the DI container registration `services.AddScoped(typeof(IRepository<>), typeof(Repository<>))` work, and what constraint-related failure can it hide?** **A:** Open-generic registration stores the definitions; on resolution of `IRepository<Order>`, the container closes `Repository<>` over `Order` via `MakeGenericType`. Hidden failure: if `Repository<T>` declares constraints (`where T : class, IEntity`) that a requested argument violates, `MakeGenericType` throws at *resolution* time — a runtime composition-root error no compiler saw. Containers vary in whether they check constraints to skip non-matching registrations (Microsoft DI does constraint-match open generics) — a behavior worth verifying when multiple open-generic implementations coexist.
18. **Q: What's the interaction between variance and delegates stored in invariant structures — e.g., why can't `List<Func<Animal>>` accept a `Func<Cat>` you're holding, and what are the resolution options?** **A:** `Func<out T>` makes `Func<Cat>` assignable *to* `Func<Animal>` — but `List<Func<Animal>>` is invariant in its element type only in the sense that it stores exactly `Func<Animal>`; adding a `Func<Cat>` works via the variance conversion at the point of add (an implicit reference conversion exists). The real trap is the reverse and with interfaces lacking variance annotations, or value-type arguments (variance never applies to value types — `Func<int>` has no relation to `Func<object>`). Options: convert explicitly (wrap in a lambda `() => (Animal)catFunc()` — allocates and changes identity), redeclare storage over the variant-compatible type, or make the custom interface variant where sound.
19. **Q: Design guidance: when should a public API's generic parameter live on the *type* vs. on the *method*, and what does getting it wrong cost consumers?** **A:** Type-level when the parameter is part of the object's identity/state (a `Repository<TEntity>` holds `TEntity`-shaped state; different closings are different objects, DI-registrable per entity). Method-level when it's per-call and inferable from arguments (a `Map<TSource,TDest>(TSource s)` on a non-generic mapper) — inference makes call sites clean and one instance serves all types. Wrong placements cost: type-level-when-per-call forces consumers to hold N instances and defeats inference (`new Helper<Foo>().Do(foo)`); method-level-when-stateful forces re-passing context and prevents per-type registration/caching, pushing state into dictionaries keyed by `Type` at runtime.
20. **Q: As a Principal Engineer, you're reviewing a shared "generic abstractions" library where nearly every interface has 3–5 type parameters with elaborate constraint webs (`IHandler<TIn, TOut, TContext, TError> where TContext : IContext<TIn>...`). What's your evaluation framework?** **A:** Assess whether the generics are load-bearing (do call sites genuinely vary along each axis, with compile-time safety or performance specialization actually exploited?) or speculative (single closing in practice — generics as ceremony). Check the consumer experience: can a mid-level engineer implement/mock these without a wiki page; do error messages on mismatch remain comprehensible; does inference work at call sites or must every usage spell out 4 arguments? Check evolution cost: every added parameter is viral across all implementers — versioning becomes near-impossible. Typical verdict: collapse rarely-varying parameters into concrete types or context objects, keep generics where variance/specialization/type-safety earns its complexity, and require a working consumer-code sample in the design review before any multi-parameter generic abstraction ships.
