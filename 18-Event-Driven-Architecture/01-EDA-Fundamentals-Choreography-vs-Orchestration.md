...
Event-Carried State Transfer's larger payload size (§2.2) has a real, measurable serialization/network/storage cost compared to Event Notification's minimal payload — at very high event-volume/throughput systems, this becomes a genuine capacity-planning input (broker storage, network bandwidth), a concrete trade-off against the availability-decoupling benefit fat events provide, worth explicitly measuring rather than assuming payload size is a negligible concern regardless of scale. An orchestrator (§2.5) sitting in the critical path of every workflow instance must itself be horizontally scalable (typically via a persisted, resumable state-machine model, so any orchestrator instance can pick up any in-flight workflow rather than requiring workflow-instance affinity to a specific orchestrator process) to avoid becoming a throughput bottleneck as workflow volume grows.

## 8. Security
Event-Carried State Transfer payloads (§2.2) can inadvertently propagate sensitive data (customer PII embedded in an order event) to every subscriber, including ones that don't actually need that specific data — apply data-minimization discipline explicitly when designing fat-event schemas (include only the fields subscribers genuinely need, not simply "the entire source record" for convenience), directly Module 28's security-domain data-minimization principle (later dedicated module) applied to event-schema design specifically. An orchestrator holding centralized visibility/control over an entire multi-service workflow (§2.5) is a natural target for authorization scrutiny — compromising the orchestrator's credentials could allow an attacker to trigger or manipulate an entire business workflow's steps, a materially larger blast radius than compromising any single choreographed service's independent, narrow reaction logic.

## 9. Scalability
Topics with many independent subscribers (§2.7) scale each subscriber's processing independently — a slow Analytics-service subscriber falling behind processing `OrderPlaced` events doesn't block or slow down the Inventory-service subscriber's independent processing of the same event stream (assuming the broker supports independent per-subscriber offset tracking, a Kafka-specific mechanism covered in depth in Module 53/the dedicated Kafka module). Choreographed systems scale participating-service count more easily than orchestrated ones (adding a new independent reaction requires no change to any existing service, §2.6), while orchestrated systems scale workflow throughput more predictably (the orchestrator's own horizontal scaling, §7, directly governs overall workflow throughput, a single, well-understood scaling lever compared to choreography's harder-to-reason-about aggregate throughput across many independently-scaling services).
