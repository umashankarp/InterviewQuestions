...
## 5. Best Practices
- Layer contract tests, replay-based tests, and chaos experiments on top of unit tests — none substitutes for another.
- Make consumer assumptions that go beyond declared schema fields (like a dedup key's scope) explicit, registered contract terms, not silent implementation details.
- Inject the actual failure condition in chaos experiments (a killed broker, real injected lag), never a fabricated symptom.
- Use captured real production event sequences for replay-based testing rather than only hand-written scenarios, to surface edge-case density hand-written tests miss.
- Test choreographed flows via the same liveness monitoring production depends on, verifying the monitoring itself would catch a stall.
- Retain production-side verification (reconciliation, canaries, DLQ monitoring) regardless of test coverage, since no pre-production environment fully reproduces production's real conditions.

## 6. Anti-patterns
- Relying on mocked integration tests alone for a system whose real failures are timing-, duplication-, and ordering-dependent.
- Treating schema-registry compatibility enforcement as sufficient contract verification, missing undeclared consumer-side assumptions.
- Chaos experiments that inject a fabricated error response rather than the actual underlying condition.
- Testing choreographed flows only for the happy path, with no verification that stall-detection monitoring actually works.
- Treating a fully green pre-production test suite as proof a production verification layer (reconciliation, canaries) is no longer needed.
- Hand-writing test scenarios exclusively, without ever replaying real historical production traffic through a candidate change.
...
**Throughput:** Chaos experiments injecting load (a simulated catch-up burst, the shape) must be run against infrastructure sized to match production's actual downstream capacity, or the experiment either under- or over-states real risk.

**Scalability:** As the number of consumer teams grows, a shared contract registry scales far better than any pairwise integration-test arrangement between every producer and every consumer, which grows combinatorially.

**Benchmarking:** Benchmark the chaos experiment's *detection* path (does the alert fire, does the dashboard show the expected signal) with the same rigor as the injected condition itself — and both show that the injected condition alone teaches little if the monitoring response isn't also being validated.
...
**Mitigations:** Anonymize or tokenize sensitive fields in captured replay fixtures before they leave production's security boundary; scope chaos experiments with explicit blast-radius limits (a single non-critical consumer group, a canary region) and an automatic abort condition, per the canary-gate discipline applied to chaos testing specifically.
...
**Secrets:** Standard; chaos-experiment tooling credentials scoped narrowly to only the infrastructure the experiment is authorized to affect.
...
**Caching:** — versioned, cached replay fixtures as durable regression assets.

**Replication/Partitioning:** A chaos experiment testing cross-region behavior requires its own multi-region test infrastructure, mirroring production's actual topology rather than a single-region approximation that cannot exercise replication-lag conditions at all.
...
**Disaster Recovery:** DR drills are this domain's most consequential chaos experiment category; the "inject the condition, not the symptom" principle applies most sharply here, per the finding that a clean-stop drill tests the wrong condition.

**CAP theorem:** Chaos experiments are the primary tool for empirically verifying which side of a CAP trade-off a system actually exhibits under a real partition, rather than relying on the documented, intended design — the split-brain was exactly a case where the intended (undocumented) behavior and actual behavior diverged, and a chaos experiment injecting a real partition would have surfaced it before production did.
